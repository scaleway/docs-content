...
2. Create a new text file called `scaleway.tf` in your favorite text editor, for example `nano`: 
    ...
    $ nano scaleway.tf 
    ```
3. Add the following content to the file to deploy a Development DEV1-L instance running the Ubuntu Focal Fossa (20.04 LTS) base image in the `fr-par-1` zone. Replace `<SCW_ACCESS_KEY>` and `<SCW_SECRET_KEY>` with your own [API key information](/identity-and-access-management/iam/how-to/manage-api-keys/)  and `<SCW_DEFAULT_PROJECT_ID>` with your own Project ID:
    ```json
      terraform {
        required_providers {
          scaleway = {
