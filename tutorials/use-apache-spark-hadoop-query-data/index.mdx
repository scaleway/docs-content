...
excerpt: |
  In this tutorial we are going to use several technologies to install an Apache Spark cluster, upload data on Scaleway's S3 and query the data stored on the S3 directly from spark using the Hadoop connector.
totalTime: PT35M
steps:
...
permalink: docs/tutorials/use-apache-spark-hadoop-query-data/
keywords: cloud tutorials, cloud documentations, cloud server, cloud hosting, ubuntu server, debian server, linux server, hadoop, spark, s3, sql, terraform, ansible
dates:
