...
To achive this, we use, besides a [Serverless Function](https://www.scaleway.com/en/betas/#serverlessfunctions) the following elements:
...
- NodeJS or Python3 (with pip) installed on your local comoyter
...
1. Install the NodeJS `pg` dependency:
    ...
    npm i pg
    ```
2. Create a `handler.js` file at the root of your folder using `touch`, then open it in a text editor, for example `nano`: 
...
4. [Create the function](/compute/functions/how-to/create-a-function/) in the Scaleway console, then [upload your zipped package](/compute/functions/how-to/package-function-in-zip/#how-to-upload-your-zip-file-to-the-scaleway-console) and add the environment variables in the **Advanced** section of the function.
    ...
    Alternatively use the [serverless framework](LINK) to create the function and upload your code automatically.
...
Python libraries to connect to PostgreSQL requires postgresql Lpoinux library. [You will need to use Docker to generate the dependencies.](/compute/functions/how-to/package-function-in-zip/#specific-libraries-(with-needs-for-specific-c-compiled-code)
