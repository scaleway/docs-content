...
- **[Request-level conditions](/iam/reference-content/understanding-request-level-conditions)**, based on the IP address, user agent, or time of the request.
- **[Resource-level conditions](/iam/reference-content/understanding-resource-level-conditions)**, based on attributes of the resource being accessed, such as its name, ID, or locality. These are only available for a limited set of products — see [Products supporting resource-level conditions](/iam/reference-content/supported-products-resource-level/) for details.
...
This page covers the concepts and CEL syntax common to both types of conditions. For the variables, functions, and examples specific to each type, refer to the dedicated pages.
...
- Request-level variables (`request.ip`, `request.time`, `request.user_agent`) are documented on [Request-level conditions](/iam/reference-content/understanding-request-level-conditions).
- Resource-level variables (`resource.name`, `resource.id`, `resource.locality`) are documented on [Resource-level conditions](/iam/reference-content/understanding-resource-level-conditions).
