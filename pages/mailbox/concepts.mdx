...
Alias addresses allow you to receive emails sent to multiple addresses in the same mailbox. Emails sent to an alias are delivered to the main mailbox, and you can only consult these emails from the main mailbox.

For example, if your main email address is `john.doe@yourdomain.com`, you can create the alias `john-newsletter@yourdomain.com` to use when you subscribe to newsletters. You can then filter and delete emails sent to this alias without impacting your main address.
...
Domain Name System (DNS) is a name management system for computing devices connected to a network, be it public (internet) or private. It translates text-based [domain names](#domain-name) to numerical IP addresses or other services such as emails.
