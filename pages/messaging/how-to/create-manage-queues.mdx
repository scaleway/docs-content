...
2. Click **Create queue**. The Queue creation wizard displays.
    ...
    <Lightbox src={image} alt="In the Scaleway console, the Create a Queue screen displays. The different sections of the screen are described in the steps below." />
...
3. Click the queue you wish to modify. The queue's **Settings** page displays:
    ...
    <Lightbox src={image2} alt="In the Scaleway console, the queue's settings displays includes a Queue information panel (showing its type, number of messages, region, creation date, URL and ARN), a Queue configuration panel (with visibility timeout, message retention period, max message size, content based deduplication and dead-letter queue settings), and a Delete queue panel (with a button to delete the queue)." />
