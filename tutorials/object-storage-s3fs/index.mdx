...
tags: object-storage s3fs fuse s3fs-fuse mount-bucket mount
steps:
...
Start by installing the dependencies of `s3fs-fuse` by executing the following commands, depending on your operating system:
  ...
  On macOS Big Sur you need to add permissions to FUSE. Go to the `Settings > Security & Privacy > General` tab to allow the extension.
