...
### How many peering connectors can I create?

Each Organization can have up to 512 peering connectors. Contact [Scaleway Support](https://console.scaleway.com/support/create/) to request more quotas.

### What are the limitations for Scaleway VPC Peering?

Refer to the [dedicated documentation](/vpc-peering/reference-content/features-limitations/#limitations) for comprehensive information on the limitations for VPC Peering.

## Pricing and billing

### How is VPC Peering billed?

Billing does not start until two matching, compatible connectors are detected and enter a `Peered` state. Unmatched or incompatible connectors in an `Orphan` or `Conflict` state are not billed.
...
[Learn more about troubleshooting VPC peering connector statuses](/vpc-peering/reference-content/statuses/).
