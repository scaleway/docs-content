...
- [Key Manager permission sets](/iam/policies-permissions/permission-sets/#key-manager) for every [principal](/iam/concepts/glossary/#principal) that reads or writes objects in the bucket: 
...
Object Storage permission sets alone are not sufficient to read (`ObjectStorageReadOnly`, `ObjectStorageObjectsRead`) and write (`ObjectStorageObjectsWrite`, `ObjectStorageObjectsDelete`) objects encrypted with SSE-KMS. For example, when an [application](/iam/concepts/glossary/#application) has only `ObjectStorageFullAccess` and no Key Manager permission set, it receives an "access denied" error when it calls `PutObject` on a bucket that has default encryption enabled, even when the KEK and the bucket are in the same Project.
</Message>
