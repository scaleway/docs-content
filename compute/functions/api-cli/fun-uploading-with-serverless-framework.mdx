...
ES-Modules are encouraged to use since it's more efficient and easier to setup and debug.
...
- `commonjs` is used by default
- `commonjs` allows you to use `require()` but no `import/export`
- `module` allows you to use `import/export` syntax but disables `require()`
