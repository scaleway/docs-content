...
Scaleway savings plan is a commitment-based discount program that provides more cost-effective billing compared to the pay-as-you-go model. This plan merges the benefits of discounted rates with the adaptability of pay-as-you-go billing.
...
Your Organization is eligible to purchase a savings plan if:
    ...
    - It does not have any outstanding invoices
...
At Scaleway, the discounts are applied in the following order:
    ...
    - Savings plans and discounts (vouchers or credits) - Any applicable vouchers or credits are used and applied after the savings plan discount.
...
Scaleway does not make refunds for savings plans that have already been invoiced.

### Taxes and currency

The total cost displayed for the savings plan does not include applicable taxes. Taxes are displayed in a dedicated section of the invoice, under the summary of your Projects' total consumption.

<Lightbox image={image2} size="medium" alt="Sample invoice displaying separate rows for Untaxed total costs and Total taxes." />

Currently, Scaleway invoices all customers in euros (EUR). If your primary currency is different, our Payment Service Providers (PSPs) apply the prevailing exchange rate.

## How to purchase a savings plan

Follow the instructions in the [How to purchase a savings plan](/billing/how-to/purchase-savings-plan/) documentation to do so.
...
The **Compute savings plan** can be used with the following resources, simultaneously and across all regions:
    ...
    - Instances
      - Cost Optimized
      - Development (excluding STARDUST)
      - General Purpose (excluding Gen3 (BASIC3-X and STANDARD3-X) and COPARM)
      - Production Optimized (excluding POP-WIN)
      - Specialized (excluding Gen3 (COMPUTE3-X and MEMORY3-X))
      - Workload Optimized (excluding Gen3 (MEMORY3-X))

The following resources are **not covered** by the savings plan discount:

    - Instances Gen3 (BASIC3-X, STANDARD3-X, COMPUTE3-X, MEMORY3-X), STARDUST, COPARM, START1, X64, B300, H100, RENDER, L4, L40S, POP-WIN
