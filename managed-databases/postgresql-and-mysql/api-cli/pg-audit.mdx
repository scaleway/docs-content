...
<Tabs id="pgauditlog">
 ...
 <TabsTab label="For a specific database">
   ...
   2. Run the following command to set `pgaudit.log` for the database. Replace `<database>` with the name of your database, and `<class>` with one or more of the statement class values listed above.
      ```
...
The primary function of the principal audit role is to oversee and manage the auditing process. The auditing process is generally handled by an independent individual or company, which will use the audit logs to perform their audit.
...
To set up session audit logging of all statements in all databases and roles, `pgaudit.log` to `ALL` in the advanced settings tab of your Database Instance dashboard in the Scaleway console.
