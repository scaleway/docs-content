...
| `email_queued_at` | Email reception date in by TEM proxy or API                                                          |
...
| `email_next_try`   | Next time we will try to send the email to the recipient server  |
...
```json
{
    ...
    "email_from": string,
	"email_to": string,
	"email_headers": [{"name": string, "value": string}],
}
```
## email_dropped
```json
