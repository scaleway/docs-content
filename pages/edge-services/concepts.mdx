...
You can also use a [Serverless Container](/serverless-containers/concepts/) (or [Serverless Function](/serverless-functions/concepts/)) as an Edge Services backend. This feature is not available in the Scaleway console. See [How to deploy a Serverless Container behind Edge Services](/serverless-containers/how-to/deploy-container-behind-edge-services/) for instructions using the CLI or Terraform.
...
The destination host must be associated with the Load Balancer (or its backend servers). You can set only one host per pipeline. If your Load Balancer is in front of multiple hosts, you can create a separate Edge Services pipeline for each. Each host therefore gets its own Edge Services endpoint and cache.
...
You can customize the endpoint with a user-defined subdomain, replacing the standardized endpoint with the subdomain of a domain you already own — e.g., `http://my-own-domain.com`. Using customized endpoints requires an associated [certificate](#certificate), and [CNAME record](#cname-record).
...
You can create an Edge Services pipeline for each of your Object Storage buckets or Load Balancers. You can enable and disable caching, WAF, and other optional parts of the pipeline at any time. Customizing the endpoint is also optional.
