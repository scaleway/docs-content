...
The customer creates an InterLink using their own dedicated physical connection, where they get sole use of a single InterLink port on Scaleway's router, with the entire bandwidth of that port. This connection and its bandwidth can then be used to provision several InterLinks for themselves.
