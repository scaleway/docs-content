...
- **Max simultaneous**: Defines the maximum number of simultaneous requests (for HTTP) or simultaneous connections (for TCP) to a backend. This is particularly relevant when using the [First available](#balancing-method) balancing method. The default value is 3, the minimum value is 1, and the maximum value depends on the Load Balancer type.
- **Queue timeout**: Defines the maximum length of time (in ms) to queue a request/connection for a given backend if [stickiness](#sticky-sessions) is enabled. The default value is 5 000, the minimum value is 1 and the maximum value is 2 147 483 647.
...
- **Retry policy**: Select whether the Load Balancer should try again on the same server (default) or redispatch and try to different servers
