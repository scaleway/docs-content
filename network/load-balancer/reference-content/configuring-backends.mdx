...
You can create frontends and backends at the same time as creating the Load Balancer itself, or create an "empty" Load Balancer and create/add frontends and backends later. 
...
Activating TLS encryption encrypts connections between the Load Balancer and the backend server(s). Note that the backend server should have its own SSL/TLS certificate.
...
Note that the same Verify Certificate setting you select here will also be used for health checks, TODO what should this be changed to? if you choose HTTPS for your [health check method](#choosing-a-health-check-method).
...
- **Tunnel timeout**: Defines the maximum allowed tunnel inactivity time (in ms) between a client and a backend server after a Websocket is established. This value takes precedence over client and server timeout in determining when to close the connection. [TODO: min/max values]
- **Server timeout**: Defines the maximum allowed time (in ms) that a backend server has to process a request. [TODO: min/max values]
- **Connection timeout**: Defines the maximum allowed time (in ms) to establish a connection between a client and a backend server. [TODO: min/max values]
...
- **Limit backend load**: When this toggle is deactivated, the default values for backend protection apply. Activate the toggle to modify the following values:
- **Max simultaneous**: Defines the maximum number of simultaneous requests (for HTTP) or simultaneous connections (for TCP) to a backend. This is particularly relevant when using the [First available](#balancing-method) balancing method.
- **Queue timeout**: Defines the maximum length of time to queue a request/connection for a given backend if [stickiness](#sticky-sessions) is enabled.
...
- **Retry policy**: Select whether the Load Balancer should try again on the same server (default) or redispatch and try to different server
- **Max. retries**: Defines the maximum number of times to retry establishing a connection after the first attempt. A value of 3 for example means that an initial attempt is made, and if that fails then the Load Balancer will try three more times to establish a connection.

### S3 failover

Activating the S3 failover feature allows you to redirect users to a static website hosted on Scaleway Object Storage, in the case that none of your Load Balancer’s backends are available. This website could be a simple, single webpage or else something much more complex: you build it according to your own requirements. 
