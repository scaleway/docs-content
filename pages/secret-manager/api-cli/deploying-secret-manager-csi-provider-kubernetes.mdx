...
| `defaultRegion` | [required] The [region](/secret-manager/concepts/#region) of your secrets | none |
| `objects` | [required] A YAML-formatted list that defines each secret to mount, specified either by its [secret ID](#access-a-secret-by-secret-id) or its [secret path](#access-a-secret-by-secret-path) | none |
...
| Parameter    | Description            | Default |
|--------------|------------------------|---------|
| `secretID`   | The UUID of the secret | none    |
| `revision`   | [optional] The revision to access, e.g., `latest_enabled`, `1`, `2`, etc. | `latest_enabled` |
...
| Parameter    | Description            | Default |
|--------------|------------------------|---------|
| `projectID` | [optional] The project ID containing the secret  | `defaultProjectID` or `defaultOrganizationID` |
| `secretPath` | The absolute path to the secret folder | none |
| `secretName` | The name of the secret | none |
| `revision` | [optional] The revision to access, e.g., `latest_enabled`, `1`, `2`, etc. | `latest_enabled` |
| `targetPath` | [optional] The relative path where the secret will be mounted | `secretPath/secretName` |
