...
Using a Docker image for your AI project allows you to create and deploy an isolated environment for your project, which can be easily transferred between machines.

All [Scaleway GPU Instances](https://www.scaleway.com/en/gpu-instances/) come with prebuilt AI Docker images, using an environment based on Anaconda or Pipenv (depending on the GPU OS image).

As mentioned earlier, Docker images are portable and you can also run images provided by other sources and use them with the GPU (for instance Docker images from Nvidia, Google, etc.). You could also choose to build your own Docker images.
