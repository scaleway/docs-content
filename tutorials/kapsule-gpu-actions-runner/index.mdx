...
In this tutorial, we will walk you through the process of setting up a GitHub Actions self-hosted runner using Scaleway Kapsule (Kubernetes) with an NVIDIA CUDA image. By leveraging powerful GPU instances, you can accelerate your CI/CD workflows, especially for tasks that require intensive computation, such as machine learning model training, data analysis, and high-performance computing applications.
...
- Installing NVIDIA device plugins
...
4. Configure the **Private Network** for secure, isolated communication:
    ...
    * Or, create a new Private Network to attach to the cluster.
...
1. Deploy the NVIDIA device plugin to manage GPU resources:
   ```sh
...
You have successfully set up a GitHub Actions self-hosted runner using Scaleway Kapsule with an NVIDIA CUDA image. This setup allows you to leverage the power of GPUs for your CI/CD workflows.