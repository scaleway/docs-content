---
title: How to use the RPN VPN with your Dedibox Servers
description: This page explains how use the RPN VPN with your Dedibox Servers
dates:
  validation: 2021-10-07
  posted: 2021-10-07
---

The RPN VPN service allows you to access machines inside your RPN from an external connection or a from a Dedibox server having no RPN interface.
It is based on [OpenVPN](/tutorials/install-openvpn/) and you can add the RPN VPN to your RPNv1 group like any other server.

The key benefits of the RPN VPN service allow you to:

* Connect yourself from anywhere in your totally secure and private network    
* Connect servers not having a RPN interface to the RPN

<Message type="requirement">

* You have an account and are logged into the [Dedibox Console](https://console.online.net)
* You have a RPN compatible Dedibox dedicated server
* You have installed a OpenVPN client on your local computer

</Message>


## How to download the RPN VPN configuration

1. Click on **RPN** in the menu on top of the page, then click **RPN VPN** to display a summary of your RPN VPN services.

    <Lightbox src="scaleway-rpn-vpn-01.png" alt="" />

2. Click **Configure** next to the RPN VPN service you want to configure. The RPN VPN information displays.

    <Lightbox src="scaleway-rpn-vpn-02.png" alt="" />

    <Message type="tip">

    If you do not have any RPN VPN service yet, you can order it from this page by clicking **Order RPN VPN**.

    </Message>

3. Download the certificate files for your RPN VPN. To display the password associated with your RPN VPN, click **Show Password**.

    <Lightbox src="scaleway-rpn-vpn-03.png" alt="" />

To use your RPN VPN in your RPNv1 group, you have to add it to your group as a RPN server. For more information follow our [RPN documentation](/dedibox-network/rpn/quickstart/).

## How to configure the RPN VPN client

<Message type="requirement">

To complete the following steps, you need to have the following infomation available on the client:
    * IP of the Online.net VPN server
    * CA Certificate
    * Client Certificate
    * `.key` Certificate
    * RPN VPN Password

</Message>

### How to configure the a Linux client

1.  Launch the installation of [OpenVPN](/tutorials/install-openvpn/) using the following command:

    ```
    root@mydedibox:~# apt install openvpn
    ```

2. Edit the client configuration file `/etc/openvpn/client.conf` in a text editor of your choice. For example `nano`: 

    ```
    root@mydedibox:~# nano /etc/openvpn/client.conf
    ```
    
    The configuration file loooks like the following example:

    ```
    client
    dev tun
    ;proto tcp
    proto udp
    float
    remote <IP of the RPN VPN server> 1194
    nobind
    persist-key
    persist-tun
    ca <ca.crt>
    cert <cert.crt>
    key <cert.key>
    askpass <password file>
    ns-cert-type server
    comp-lzo
    verb 3
    ```

    Set the following values in the configuration file:

    * `<IP of the RPN VPN server>`: IP of the RPN VPN server
    * `<ca.crt>`: Path to ca.crt
    * `<cert.crt>`: Path to the client .crt
    * `<cert.key>`: Path to the client .key
    * `<password file>`: Path to the file containing your password

    Then save the file and exit the text editor.

3. To autostart the VPN during the boot process of the server, edit the file `/etc/default/openvpn` and insert the following line:

    ```
    AUTOSTART="client"
    ```

    This line allows to autostart [OpenVPN](/tutorials/install-openvpn/) with the config file located at /etc/openvpn/client.conf

After the connection to RPN VPN server has been established, you may check if you can see a new interface, using your RPN VPN IP:

    ```
    root@mydedibox:/etc/default# ifconfig tun0
    tun0      Link encap:UNSPEC  HWaddr 00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00  
            inet addr:10.90.250.25  P-t-P:10.90.250.26  Mask:255.255.255.255
    ```

You can also check if the routes for the RPN network have been set:

    ```
    root@mydedibox:/etc/default# route -n | grep 10.90
    10.90.0.0       10.90.250.26    255.255.0.0     UG    0      0        0 tun0
    10.90.250.1     10.90.250.26    255.255.255.255 UGH   0      0        0 tun0
    10.90.250.26    0.0.0.0         255.255.255.255 UH    0      0        0 tun0
    ```

Now you can [configure your server](/dedibox-network/rpn/quickstart/) in your RPN group.

<Navigation title="See Also">
  <PreviousButton to="/dedibox-network/rpn/how-to/configure-rpnv2/">How to configure the RPNv2</PreviousButton>
  <NextButton to="/dedibox-network/rpn/how-to/configure-rpnv1-gateway/">How to configure the RPNv1 gateway</NextButton>
</Navigation>
