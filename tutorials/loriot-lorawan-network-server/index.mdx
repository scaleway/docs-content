---
title: Connecting Loriot LoRaWAN&copy; network server to your IoT Hub
description: This page explains how to configure your Loriot backend to have your LoRaWAN&copy; devices send messages to IoT Hub
tags: Scaleway IoT Hub LoRaWAN&copy;
excerpt: |
  LoRaWAN&copy; is a low-power long-range radio protocol to use when 3G/4G/5G is not required. Loriot is a LoRaWAN&copy; network server provider and is able to connect to IoT Hub to bring your LoRaWAN&copy; devices' data to the cloud.
totalTime: PT30M
image: /assets/images/docs/scaleway.png
keywords: iot, smart, connected, embedded, devices, hubs, mqtt, network, Sigfox, lora, lorawan, bridge
categories:
  - iot-hub
dates:
...
## LoRaWAN&copy; and Loriot Overview

LoRaWAN&copy; is a low-power, low-bandwidth, long-range radio protocol used by many devices to send their data over the air. This protocol is specifically designed for battery powered devices with little to no installation cost.

LoRaWAN&copy; devices broadcast their data to reachable LoRaWAN&copy; gateways, which forward the messages on the LoRaWAN&copy; operator network. The operator de-duplicates the messages from multiple gateways and hand over the message to the "LoRaWAN&copy; application". In our scenario, the "LoRaWAN&copy; application" is Scaleway IoT Hub.

[Loriot](https://loriot.io) is a LoRaWAN&copy; network server provider on which to connect your LoRaWAN&copy; gateways and build your private LoRaWAN&copy; network.

In this tutorial we are going to:

- Create a IoT Hub to receive/send messages from/to LoRaWAN&copy; devices
...
- Add another Device and connect it as a subscriber to receive LoRaWAN&copy; messages

---

## Create an IoT Hub

First, we need to create a Hub. Its role will be to distribute messages from your LoRaWAN&copy; devices to the intended targets. Please follow [this procedure](/iot/iot-hub/how-to/create-hub/) to create an IoT Hub if you don't have one already. The hub's name and plan does not matter for this tutorial.
...
- Now subscribe to the LoRaWAN&copy; devices messages:
...
- Your subscription now appears in the `Subscriptions` block and the messages from your LoRaWAN&copy; devices will start showing in the `Messages` block.
...
That's it! Your LoRaWAN&copy; devices data is now backed by a production-grade dashboard!
