---
meta:
  title: Connecting Messaging and Queing SQS or SNS to the AWS-CLI
  description: This page explains how to connect Scaleway Messaging and Queing SQS or SNS to the AWS-CLI
content:
  h1: Connecting Messaging and Queing SQS or SNS to the AWS-CLI
  paragraph: This page explains how to connect Scaleway Messaging and Queing SQS or SNS to the AWS-CLI
...
This guide shows you how to install the AWS-CLI and configure it for connection to Scaleway Messaging and Queuing SQS or SNS.
...
1. Create a file named `~/.aws/config` by running the following command:
    ...
    ```
2. Open the `~/.aws/config` file you just created in a text editor and edit it as follows (the SNS or SQS endpoint URL can be found in the console on the **Overview** page for the relevant protocol)
    ```
    [plugins]
    endpoint = awscli_plugin_endpoint
    [default]
    sns =
      endpoint_url = https://sns.mnq.fr-par.scw.cloud
    sqs =
...
3. Generate a credentials file using the following command:
    ...
    aws configure
    ```
4. Open the `~/.aws/credentials` file you just created, and add the access key and secret key you saved when you generated your [SQS or SQS credentials](/serverless/messaging/how-to/create-credentials/) as follows:
    ```
    [default]
