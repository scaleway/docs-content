---
meta:
  title: Backup your Kapsule Cluster on Object Stroage with Velero
  description: In this tutorial you will learn to configure Velero to Backup your Kubernetes Kapsule Cluster on Object Storage.
content:
  h1: Backup your Kapsule Cluster on Object Stroage with Velero
  paragraph: In this tutorial you will learn to configure Velero to Backup your Kubernetes Kapsule Cluster on Object Storage.
...
3. Initialize Verlero. Make sure to replace `VELERO-BUCKET-NAME` with the name of the Object Storage bucket you want to use. Edit the backup location if your bucket is in a zone different than `fr-par`. 
    ```
    velero install \
...
1. Create a backup of all namespcaes in the cluster:
    ```
...
For more information about the creation of backups using Velero, refer to the [Velero backup reference documentation](https://velero.io/docs/v1.11/backup-reference/).
...
2. Check the status of your restore: 
    ...
    ```
    When the `Phase` status changed to `Completed` your backup has been restored successfully:
    ```
    Phase:                       Completed
    ```
