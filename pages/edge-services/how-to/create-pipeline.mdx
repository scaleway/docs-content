---
title: Create an Edge Services pipeline
description: Learn how to create an Edge Services pipeline for a Scaleway backend to enable caching, WAF, and secure content delivery at the edge.
dates:
...
- Customize your backend endpoint using a subdomain of your own domain
...
<Message type="tip">
You can also use Serverless Containers and Serverless Functions as Edge Services backends. This feature is not available in the Scaleway console. See [Deploy a Serverless Container behind Edge Services](/serverless-containers/how-to/deploy-container-behind-edge-services/) for instructions using the CLI or Terraform.
</Message>

## Create an Edge Services pipeline

1. In the [Scaleway console](https://console.scaleway.com) side menu, go to **Edge Services**. The **Edge Services** page opens.

2. Click **Create pipeline**. The pipeline creation wizard appears.
...
3. Under **Configure backend**, select the **Backend type** for this pipeline, then configure the backend depending on the type you selected:
    ...
    <Tabs>
        ...
        - Select the **Load Balancer** from the dropdown. The Edge Services pipeline will connect to this Load Balancer when requesting content.
        - Select a **frontend** associated with the Load Balancer from the dropdown. The Edge Services pipeline will connect to the Load Balancer on this port when requesting content. Scaleway recommends that you select an HTTPS port, e.g., `443`. Note that if you are creating multiple pipelines for the same Load Balancer, each one must be configured for a different frontend.
        ...
        <Message type="tip">
        If you are setting up an Edge Services pipeline for Kubernetes Load Balancers, see [Help for Kubernetes Load Balancers](#help-for-kubernetes-load-balancers).

        Note that you cannot use a [private Load Balancer](/load-balancer/concepts/#private-load-balancer) as an Edge Services backend. Load Balancers must have a public IP address to be compatible with Edge Services. You can use ACLs to filter traffic toward public Load Balancers via the [API](https://www.scaleway.com/en/developers/api/load-balancer/zoned/acls/).
        </Message>
...
6. Check the cost summary for the pipeline, notably whether it falls within the limits of your current [subscription plan](/edge-services/reference-content/understanding-pricing/), then click **Create Edge Services pipeline**.
    ...
    You are returned to the **Pipelines** tab, where the newly created pipeline now appears.


    <Message type="important">
    For Object Storage backends, your bucket [visibility](/object-storage/concepts/#visibility) can be set to **private**, but any objects within it that you want to expose via Edge Services must be set to [public](/object-storage/how-to/manage-object-visibility/). However, if you are using Edge Services with bucket website, objects can remain private.
    </Message>

## Access your bucket via Edge Services
...
The endpoints shown above are available as standard. However, with Edge Services, you can also choose to configure a **custom domain** from which your bucket can be accessed. Read more about this in [Configure a custom domain](/edge-services/how-to/configure-custom-domain/).
