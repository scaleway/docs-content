...
When your system does fails to boot or boots incorrectly after the installation of a  new kernel version, you can use the [rescue mode](/compute/instances/concepts/#rescue-mode) to boot into the previous working kernel.

For example, the instance may boot with the root filesystem in `read-only` mode:
...
4. Update the `grub` boatloader, so the change of the kernel is taken into account:
    ```
    # update-grub
...
5. Switch back the Instance's `boot_type` to `local` and reboot the Instance:
    ```
    scw update {Instance_ID} boot-type=local
    scw reboot {Instance_ID}
    ```
