...
* Startup probe (optional, not yet available in the Scaleway console): verifies that your application has successfully started. Until this probe succeeds, the liveness probe is not executed. This is useful for applications with a long startup time.
* Liveness probe: verifies that your application is ready to receive traffic. This probe will check the health of your application. If it fails repeatedly, the container is considered in error. (the container will not be restarted, but the traffic will not be routed to it).

Both probe types share the same configuration options:
- The **probe type** defines the type of check to perform:
    - The **TCP** probe will check if the TCP connection of your Serverless Container is opened on the specified port
    - The **HTTP** probe allows you to define a custom path to your Serverless Container. It can be useful to check if all requirements are met, such as the connection to the database, etc.
- The **failure threshold** corresponds to the maximum number of checks that can fail before declaring the Serverless Container in error. Defaults to 30 for liveness probe.
- The **interval** corresponds to the duration in seconds between every check. Defaults to 10 seconds for liveness probe.
- The **timeout** corresponds to the maximum amount of time to wait for a check to complete. If the check does not complete within this time, it is considered failed. Defaults to 1 second for liveness probe.
- The **path** (HTTP probe type only) corresponds to the endpoint to call your Serverless Container (example: `/health`).
...
the container will enter the `error` state after **300 seconds (5 minutes)** of consecutive failed health checks.
