...
You can monitor resource usage in real time from the [Cockpit Load Balancer dashboard](/cockpit/how-to/access-grafana-and-managed-dashboards/). Scaleway also provides managed alerts in Cockpit to notify you when a limit is approaching. See [How to configure the alert manager](/cockpit/how-to/enable-alert-manager/) for more information.

## Performance limitations by size

The following limits vary depending on the Load Balancer (LB) size. You can also find them in the console when [creating a new Load Balancer](/load-balancer/how-to/create-load-balancer/). 

| LB size | Bandwidth | Max simultaneous connections | Buffer size limit\* |
| :------ | :-------- | :--------------------------- | :------------------ |
| LB-S    | 200 Mbps  | 20,000                       | 16 KB               |
| LB-M    | 500 Mbps  | 50,000                       | 32 KB               |
| LB-L    | 1 Gbps    | 160,000                      | 64 KB               |
| LB-XL   | 4 Gbps    | 3,000,000                    | 128 KB              |

\*The buffer size that the Load Balancer allocates for each connection when processing requests/responses. This limit may be reached for example when the Load Balancer is running HTTP [protocol](/load-balancer/concepts/#protocol) for the frontend/backend and the header size exceeds this limit.
