...
2. Use `growpart` to increase the partition size of your block storage volume (here `/dev/sdb1`):
    ```
    growpart /dev/sdX 1
    ```
...
3. Increase the partition size of the volume using `resize2fs`. Replace `/dev/sdX` with the name of your block volume in the following command:
    ```
    resize2fs -p /dev/sdX1
    ```
4. Check the new block volume size using `lsblk`:
    ```
    root@scw-distracted-keldysh:~# lsblk
    NAME    MAJ:MIN RM   SIZE RO TYPE MOUNTPOINTS
    loop0     7:0    0  63.3M  1 loop /snap/core20/1852
