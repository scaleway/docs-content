...
You can interact with text models using several ways:
...
- **Structured outputs (schema mode)** (deterministic/structured):
   ...
   - Structured outputs is newer and has been implemented by OpenAI in September 2023 to enable stricter, schema-based response formatting.

<Message type="note">
    - All LLMs on the Scaleway library support **JSON mode** and **Structured outputs**, however, quality of results will vary in the schemaless JSON mode.
    - JSON mode: It is important to explicitly ask the model to generate JSON output either in system prompt or user prompt. To prevent infinite generations, you are encouraged to ask the model for short JSON objects.
...
Bu using [Pydantic](https://docs.pydantic.dev/latest/concepts/models/), users can define the schema as a Python class and enforce the model to return results adhering to this schema.
