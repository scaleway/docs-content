...
<Message type="note">
If you are going to use this base code with the Responses API, note that for now you must use `gpt-oss-120b` as the only supported model.
</Message>
...
<Tabs id="structured-output_manual">
    <TabsTab label="Chat Completions API">
    ```python
    extract = client.chat.completions.create(
        messages=[
...
<Tabs id="structured-output_schemaless">
    <TabsTab label="Chat Completions API">
    ```python
    extract = client.chat.completions.create(
        messages=[
