...
meta:
  ...
  description: This page shows you how to setup NetBox, a web application designed to help manage and document computer networks, on a Scaleway Elements Instance using the managed PostgreSQL database service.
content:
  h1: Setting up NetBox with a managed PostgreSQL database
  paragraph: This page shows you how to setup NetBox, a web application designed to help manage and document computer networks, on a Scaleway Elements Instance using the managed PostgreSQL database service.
...
- You have a [Instance](https://www.scaleway.com/en/virtual-instances/) running Ubuntu Focal Fossa (20.04 LTS)
...
4. Download the [latest release](https://github.com/netbox-community/netbox/releases/latest) of NetBox. At the time of writing this tutorial it is version 3.0.10 and unpack it into the `/opt` directory:
  ```
  wget https://github.com/netbox-community/netbox/archive/refs/tags/v3.0.10.tar.gz && tar -xzf v3.0.10.tar.gz -C /opt
  ```

5. Create a symlink from the actual netbox directory to `/opt/netbox`:
  ```
  ln -s /opt/netbox-3.0.10/ /opt/netbox
  ```
