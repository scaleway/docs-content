...
2. Create a `tutorial.conf` file with the following content:
    ...
    Where:
    - `db-uri` must use credentials with an [application](/iam/principals/create-application/) having **ServerlessSQLDatabaseDataReadWrite** permissions (not **ServerlessSQLDatabaseReadWrite** or **ServerlessSQLDatabaseFullAccess**)
