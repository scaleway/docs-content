...
6. Click **Create Read Replica**. A list of your Read Replicas displays.
    <Message type="important">
    Read Replicas have the same node types as the main node. For example, if your main node is of the DOCDB-GP-S type, so is your Read Replica. The Replica also inherits the main node's configuration. The data of your main node is fully replicated in the Read Replica.
    </Message>
