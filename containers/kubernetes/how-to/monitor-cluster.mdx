...
4. Click **Kubernetes Cluster Overview** in the list of dashboards. A status overview of your cluster displays. This dashboard allows you to monitor multiple components of your Kubernetes cluster: control-plane, nodes, managed resources and the cluster system applications.
...
The cluster overview dashboard offers real-time monitoring capabilities for the `kube-apiserver` within your Kubernetes cluster. Serving as a crucial component on the control plane, the `kube-apiserver` acts as the gateway to the Kubernetes API. This grants users the primary means to interact with the cluster.

In cases where your clusters are large, containing numerous resources such as nodes, pods, and custom resources (CRDs), or when experiencing a large number of simultaneous requests to the apiserver from controllers like argocd and velero, the CPU and memory utilization can surge. As a result, the apiserver's performance may deteriorate significantly, leading to sluggishness or even unresponsiveness, as evidenced by `kubectl` returning an error such as `"EOF"`.
