...
After migration, these resources will be managed under [Storage](/block/quickstart/) instead of Compute.

During migration, unified snapshots will be converted into [Block Storage Low Latency](/block/) snapshots and removed from Compute. If a unified snapshot is part of an image, it will be replaced by the corresponding Block Storage Low Latency snapshot.
...
- An [Instance](/instances/how-to/create-an-instance/) using [Block Storage volumes](/block/how-to/create-a-volume/)
...
To learn more about managing your migrated Block Storage volumes and snapshots from the Scaleway console, refer to the [Block Storage Quickstart Guide](/block/quickstart/). Additionally, you can explore advanced features using the [Scaleway Block Storage API](https://www.scaleway.com/en/developers/api/block/).
