...
- `Glacier`: your data is archived and must be restored before being used. The Glacier storage class is only available in `fr-par` and `nl-ams` [regions](/object-storage/concepts/#region-and-availability-zone). Refer to our [Cold storage FAQ](/faq/objectstorage/#cold-storage) for more information.
