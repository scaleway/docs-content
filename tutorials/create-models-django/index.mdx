...
Now that your environment (`my-django`) is set up, we can create a new application. The difference between an application and a project is that an app is a Web application that does something. A project is a collection of configuration and apps for a particular website. A project can contain multiple apps. An app can be in multiple projects.
...
5. Run `makemigrations` to indicate Django that you've made some changes to your models
    ...
    <Message type="note">
      You can read the migration for your new model if you like; it is the file `polls/migrations/0001_initial.py`
    </Message>
...
Now that we have the questions on the admin page, we need to configure the multiple choice response.
