...
2. Update the configuration cell of the file with your API access key and secret key, and optionally your bucket endpoint, as shown below:

    ```json
    %%configure -f { "name": "My Spark", "conf":{
    "spark.hadoop.fs.s3a.access.key": "[your-api-access-key]",
    "spark.hadoop.fs.s3a.secret.key": "[your-api-access-key]",
    "spark.hadoop.fs.s3a.endpoint": "[object-storage-bucket-endpoint]" } }
    ```
...
3. Run the command below to start processing the data contained in your Object Storage bucket:
