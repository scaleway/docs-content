...
Now the applet is set up, we need to find our webhook key, head to your [webhook settings](https://ifttt.com/maker_webhooks/settings), you will find there a URL like `https://maker.ifttt.com/use/<key>`. Write down the key part, we will use it later.
...
In your [Scaleway console](https://console.scaleway.com), head to the `Devices` tab of your IoT Hub. From there, to add a new Device, give it any name and pick the `Allow insecure connection` option (you do not want to do that in production, but this is fine for this tutorial's purpose). Click the `Add` button, and, finally, the `Close` button.
