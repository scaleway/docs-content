...
The `serverless.yml` file determines the configuration of a namespace that contains one or more containers with the same runtime. In the following example, we use one container.
...
provider:
  name: scaleway
  runtime: python37, node14, node16, go118
