---
title: Account
description: Find troubleshooting resources for resolving issues with Scaleway accounts.
dates:
  posted: 2023-10-05
productIcon: AccountExperienceProductIcon
---

<ProductHeader
  productName="Account troubleshooting"
  productLogo="accountExperience"
  description="Discover how to solve the most common issues in Scaleway account management."
/>

## Featured Pages

<Grid>
    <Card
      title="Account FAQ"
      description="General info on Scaleway accounts."
      url="/account/faq/"
      label="See more"
    />
    <Card
      title="How to manage your personal data"
      description="Manage personal data under GDPR."
      url="/account/profile-preferences/manage-my-personal-data-gdpr/"
      label="See more"
    />
    <Card
      title="How to recover a lost password"
      description="Recover a lost password for your account."
      url="/account/login-credentials/recover-a-lost-password/"
      label="See more"
    />
</Grid>
<Grid>
    <Card
      title="How to report abuse incidents"
      description="Report abuse incidents related to Scaleway accounts."
      url="/account/support/report-abuse/"
      label="See more"
    />
    <Card
      title="Subscribing to Scaleway status updates"
      description="Learn how to subscribe to Scaleway status updates"
      url="/account/support/scaleway-status-updates/"
      label="See more"
    />
</Grid>

## Account troubleshooting pages

<LinksList>
  - [I cannot close my account and erase personal data](/account/troubleshooting/cannot-erase-account)
  - [I cannot log in to my account](/account/troubleshooting/cannot-log-into-my-account)
  - [I cannot use my passkey as a passwordless login](/account/troubleshooting/cannot-use-passkey)
  - [I see a 'Quotas exceeded' error message](/account/troubleshooting/quotas-exceeded-error-message)
  - [I can't verify my identity](/account/troubleshooting/cannot-verify-identity)
  - [My account requires a manual review or further review](/account/troubleshooting/manual-further-review)
  - [I can't validate my phone number](/account/troubleshooting/cannot-validate-phone-number)
</LinksList>