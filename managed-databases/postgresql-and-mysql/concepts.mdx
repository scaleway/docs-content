...
Refer to the [Block Storage Low Latency documentation section](/block/concepts/) to learn more about this volume type.
...
<Message type="note">
  HA standby nodes are not accessible to users unless the main node becomes unavailable and the standby takes over. If you wish to run queries on a read-only node, you can use [Read Replicas](/postgresql-and-mysql/how-to/create-read-replica/).
</Message>
...
You can select the volume type upon Database Instance creation, and you can [change the type](/postgresql-and-mysql/how-to/change-volume-type/) anytime.