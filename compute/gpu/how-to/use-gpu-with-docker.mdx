...
We recommend that you map volumes from your GPU Instance to your Docker containers, so that your work is not lost when you exit the Docker container. For increased data redundancy, we also recommend storing the directories you map to your Docker containers on [Block Storage](/block/quickstart/).
...
You can also map [Block Storage](https://www.scaleway.com/en/block-storage/) volumes into your containers. Block Storage is fully backed by SSDs. These three-time replicated, high-speed drives allow up to 5,000 IOPS. Once [attached](/block/how-to/attach-a-volume/) and [mounted](/block/api-cli/managing-a-volume/#mounting-and-using-a-block-storage-volume) in the host OS of the GPU Instance, you can map the volume like a local volume, as we did above.

<Message type="tip">
  [Block Storage](/block/quickstart/) volumes are independent of your GPU Instance and provide three-time replicated storage. It is recommended to use Block Storage for storing your datasets, training logs, model source code, etc.
</Message>
