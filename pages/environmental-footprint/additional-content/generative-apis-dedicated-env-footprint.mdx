...
For Genrative APIS products (Dedicated deployment & Serverless), the following elements are taken into account when calculating the Generative API - Serverless footprint:
...
- **Generative API offering resources**: The amount of compute used (measured in Tokens and translated into GPU inference time) during your API requests.
...
- **Usage Impact**: Energy consumption of GPU nodes during request processing (inference), Power Usage Effectiveness (PUE) specific to the datacenter hosting the model (e.g., DC5) and the nergy mix (carbon intensity) of the country supplying the electricity (location-based).
- **Operational Impact** Impact related to the cross-cutting services required to ensure the API's operation, such as the network, model storage (object storage), and serverless control plane components.
...
## Calculation aspects
To accurately allocate the GPU server's impact to your requests, Scaleway relies on performance benchmarks specific to each GPU model and architecture (e.g., Llama 3.1 on NVIDIA H100).
For each model, we evaluate: 
- Input speed: The number of tokens processed per second during the prompt.
