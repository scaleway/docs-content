...
Below you can find an example of a pod configuration with affinity set to target the `beta.kubernetes.io/arch=arm64` label, which is present by default on Scaleway ARM nodes:
...
In this example, the pod's affinity is configured to be scheduled on nodes that have the label `beta.kubernetes.io/arch` with the value `arm64`.
