...
3. Create a new user. This is necessary because some of the commands we will be running later in this tutorial should not be carried out as root. Here, we call the new user `mariecurie`, but you could call them `erpnext` or whatever you like. Enter a password for them when prompted. You can leave the user information at default values if you wish.
...
8. SSH back into your instance with the following command:
  ```
  ssh root@<remote_machine_ip>
  ```
...
2. Initialize the Frappe framework using the following command. Remember to replace `/home/mariecurie` with the proper home path of your user:
  ...
  This step may take a while. Note that the script is able to recover from warnings such as `Command not being executed in bench directory` and `xxx is not a valid editable requirement`.
