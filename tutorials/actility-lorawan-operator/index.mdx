...
1. [Create a Managed Database Instance](/databases/databases-for-postgresql-and-mysql/how-to/create-a-database/) for PostgreSQL and a `lora` table with the following fields: `time` (timestamp type) and `data` (jsonb type).
