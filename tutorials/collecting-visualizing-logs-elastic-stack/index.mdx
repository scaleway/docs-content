...
3. Create another file named `/etc/logstash/conf.d/30-elasticsearch-output.conf`, and copy the following content. Then, save and exit.
    ...
    <Message type="note">
      If you want to add filters that use the Filebeat input, make sure these filters are named between the input and output configuration (between `02` and `30`).
    </Message>
...
Other Beats are available, for example: [Metricbeat](https://www.elastic.co/products/beats/metricbeat) to collect metrics of systems and services, [Packetbeat](https://www.elastic.co/products/beats/packetbeat) to analyze network traffic or [Heartbeat](https://www.elastic.co/products/beats/heartbeat) to monitor the availability of services.
