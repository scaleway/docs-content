...
To start collecting data from the Scaleway Audit Trail, Identity and Access Management (IAM), and Compute APIs using the add-on, you must generate an API key in the Scaleway console and assign the appropriate permissions.
...
Scaleway recommends using Splunk Web for this configuration, but it is also possible to [configure inputs manually via a configuration file](#appendix-manual-add-on-configuration).
...
4. Provide the required information (e.g., polling interval, Scaleway region). Obligatory fields  are marked with an asterisk.
    ...
    - The **Product name** field is optional. If no Product name is given, the input will retrieve data from all the Scaleway products. 
        ...
        On the **Events** tab, select the **Product** field. A drop-down list appears. Use the product name as displayed in the drop-down list. However, make sure that you use lowercase letters only (for example, `instance`) and omit the copyright sign from the name. {/* EDITORIAL COMMENT: Is this correct? */}
...
To perform a simple initial search of data ingested from your Scaleway infrastructure, follow these steps:
