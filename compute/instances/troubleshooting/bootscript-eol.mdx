...
<Message type="important">
  Local volume(s) not supporting UEFI cannot be used to boot any other type of Instance. You will therefore have to retrieve your data and manually migrate it to a Instance supporting UEFI boot.  
  Unfortunately we cannot identify, on the Scaleway side, which customers are using OS images with / without UEFI. This can only be identified by the user themselves.   
...
### Instances using bootscript method and an OS image with UEFI
