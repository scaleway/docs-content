---
title: Domains and DNS FAQ
description: From understanding DNS features to managing domain ownership, explore everything you need to know about domains and DNS zones with Domains and DNS.
tags: dns domain transfer extension
dates:
  validation: 2025-09-23
productIcon: DomainsProductIcon
---

## Overview

### What is the DNS feature?

Domain Name System (DNS) is a domain name management service that allows you to avoid browsing the internet only by IP addresses. It provides human management of addressable resources on the internet. It is possible to create zones and records to manage a subdomain (e.g., `test.mydomain.com`) or specific configurations, such as the MX of your mailbox.

## Specifications

### What are Scaleway's DNS name servers?

Scaleway's DNS name servers are:

- `ns0.dom.scw.cloud`
- `ns1.dom.scw.cloud`

### What are the prerequisites to register a domain name?

To register a domain name, you need:

- A valid email address to make sure your information is updated during the whole period of registration
- A valid debit or credit card

### What are the requirements to use EU, FR, and NL extensions?

To use the `.eu` extension, you must be a citizen of a country in the European Union.

For the `.fr` extension, you must have an address in the European Economic Area and/or Switzerland.

To use the `.nl` extension, you must fill in the **legal form** and **legal form registration number** fields. Additionally, the technical and administrative contacts **must be a natural person**.

### How long is the registration of a domain name valid for?

Your registration is valid for a minimum of 1 year.
You can reserve certain extensions for up to 10 years.

## Pricing and billing

### Why are some domain extensions more expensive than others?

Due to a significant increase in domain pricing across various registrars, Scaleway domains will also experience a price increase.

Find out more about pricing increases in the [Scaleway changelog](/changelog/?product=domains-and-dns).

## Usage and management

### Can I edit my domain name once it has been registered?

It is not possible to change the domain name once the registration has been carried out. However, you can make a new registration with another domain name.

### How long does a domain name transfer take?

A domain name transfer usually takes from 1 to 10 days. However, a domain name transfer can take longer.

### Is domain renewal automatic?

No. You must renew your domain name before the end of its validity period.
If you enable the [auto-renew feature](/domains-and-dns/how-to/configure-autorenew/), your domain name will be automatically renewed.

<Message type="tip">
  You will be notified by email 60 days before your domain name expires. Your domain name will then switch to the `late renew` status.
</Message>

### What are the responsibilities of the domain name holders?

The domain name holders must:

- Comply with the terms and conditions posted by their registrar, including applicable policies from their registrar, the registry, and ICANN
- Review their registrar's current registration agreement, along with any updates
- Assume sole responsibility for the registration and use of their domain name
- Provide accurate information for publication in directories such as `WHOIS` and promptly update this to reflect any changes
- Respond to inquiries from their registrar within fifteen (15) days, and keep their registrar account data and payment information up-to-date

### Which rights are associated with the owner of a domain?

The owner of a domain can update all the technical and administrative parameters referring to their domain name. You must [contact the Support team](https://account.scaleway.com/?service=elementssupport/tickets/create) if you want to modify the contacts you have entered.

### Is it possible to automate the management of DNS service records?

Yes, the DNS service can be automated with the [Domains and DNS API](https://www.scaleway.com/en/developers/api/domains-and-dns/). It is possible to use the API in a CI/CD environment to create test environments, for example.

## Quotas and limitations

### Are there transfer limitations?

All domains registered through [BookMyName](https://www.bookmyname.com/) or [Online](https://account.scaleway.com/?service=dediboxen/login) cannot yet be transferred to Scaleway Domains and DNS.

## Compatibility and integration

### Is the DNS service IPv6 compatible?

Yes, the DNS service is compatible with IPv6.

## Support and troubleshooting

### Why was my domain registration refused?

Your domain registration may be refused if you do not meet the conditions set by the registry.

For instance, if your extension is `.fr`, you must have an address in the European Economic Area and/or Switzerland; otherwise, your domain registration will be declined.

### Why has my current registrar opposed my request for a change of domain name registrar?

The reasons for refusal are generally because the domain name:

- Is locked with the status `client transfer prohibited`: the applicant forgot to remove the padlock from the registrar before requesting the transfer of the domain name
- Was registered for less than 60 days
- Was the subject of a previous transfer for less than 60 days
- Has expired
- Is the subject of a dispute in progress (UDRP procedure, for example)

### What do I do if the domain name I want to register is not available?

If a domain name is unavailable, you can test a light variation of the desired name. You can also add a letter or a number. If you still encounter any issues, you can contact the [Support team](https://account.scaleway.com/?service=elementssupport/tickets).