...
## Context
This tutorial will simulate a project with a production environment running all the time and a development environment that will be switched off on weekends to save costs.
...
Enter the `function` folder and create a `handler.py` file as follows:
  ```py
  import os
...
3. Write a function that will run code you have just written:
    ```json
...
5. Create a cronjob attached to the function to turn your function on every Monday mornings:
    ```json
...
Everything is configured now, deploy your infrastructure using Terraform.
