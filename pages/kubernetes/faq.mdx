...
Renaming a Kubernetes cluster is not a cosmetic change. The cluster name is used in several control-plane components, so when you rename a cluster, it triggers a complete redeployment of the control plane and of the job that injects the managed resources into your cluster.
...
The cluster typically returns to normal operation within 5 to 10 minutes after the rename. Scaleway recommends avoiding cluster renames on production clusters during peak usage periods.