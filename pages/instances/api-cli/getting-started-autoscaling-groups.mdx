...
Autoscaling Groups adjust the number of Instances in a group automatically, based on scaling policies you define. A scaling action (scale-out or scale-in) triggers when a monitored metric, gathered through Cockpit, such as RAM or bandwidth usage, crosses the configured threshold. When a Load Balancer is attached to the group, Scaleway updates it automatically on each scaling action, so that connections stay distributed correctly among the active Instances. You can also configure the group to replace Instances that the Load Balancer marks as unhealthy.
...
<Message type="important">
  The Autoscaling Groups API is in **Private Beta**. [Contact Scaleway Support](https://console.scaleway.com/support) to request Beta access before following this guide.
</Message>
...
- A terminal with [curl](https://curl.se/) installed
- [jq](https://jqlang.github.io/jq/) installed, to parse JSON responses (optional but recommended)

### Create the required resources

Create the following resources in the Scaleway console before you begin.

| Resource | Where to create it | Notes |
|----------|--------------------|-------|
| Load Balancer | Console > Load Balancers > **Create Load Balancer** | Required for auto-healing |
| Backend | Console > Load Balancers > your Load Balancer > **Backends** | Instances created by the group are added to this backend |
| Private Network | Console > Private Networks > **Create Private Network** | Instances created by the group are attached to this network |
...
Get the following resource IDs from the console before continuing.

| Resource | Where to find it |
|----------|-------------------|
| Load Balancer ID | Console > Load Balancers |
| Backend ID | Console > Load Balancers > your Load Balancer > Backends |
| Private Network ID | Console > Private Networks |
