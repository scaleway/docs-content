...
<Message type="tip">
  Replace the IP of the DNS cache server with the one located in the same physical location as your machine for optimal performance. [Check the IPs for each data center](/account/support/scaleway-network-information/#dns-cache-servers-and-ntp-servers)
</Message>