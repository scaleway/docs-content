...
3. Select the upgrade method. Two methods are available:
    ...
    <Message type="note">
      ...
      Keep in mind that reverting the endpoint will not affect the dates stored on the databases. This means that if some entries were added to the upgraded database, they will not be added back to the old version when you change the endpoints.
    </Message>
