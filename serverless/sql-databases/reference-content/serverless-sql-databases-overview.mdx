...
Autoscaling is triggered automatically, based on your database vCPU consumption:
  ...
  - **Scaling up**: after using more than 40% of the currently allocated vCPU for 10 seconds, the vCPU and RAM allocated to your database will increase by 1 vCPU and 4 GB RAM each time, up to a maximum of 15 vCPUs and 60 GB RAM.
...
- SQL request size is limited to 512 KB. A single SQL query sent to Serverless SQL Database cannot exceed 512 KB, but the response to the query is not limited and results tables can exceed 512 KB.
