...
Refer to the [differences between Jobs, Functions and Containers documentation](/serverless-containers/reference-content/difference-jobs-functions-containers/) for more information on the differences between Containers, Functions, and Jobs.
...
### How to add a Load Balancer in front of my Serverless Container?

Load Balancing is integrated directly to Serverless Containers when you invoke them, traffic is handled by our infrastructure without the need to add other products.

Also Load Balancers are compatible only with IPs backends, as Serverless Containers IPs are not predictable it's not possible to integrate directly Containers to Load Balancers.
...
### How to access my Serverless Container with a Private Endpoint?
...
### How to disable public endpoint for Serverless Containers?

Disabling the default public endpoint for Serverless Containers is not yet available. To restrict access to Serverless Containers it's possible to [make the resource private](/serverless-containers/how-to/secure-a-container/#restrict-access-to-your-containers).
...
Health checks will not wake up Serverless Containers if 0 instance are up, to prevent additional costs.
