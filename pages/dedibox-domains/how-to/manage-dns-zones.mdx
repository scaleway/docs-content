---
title: How to manage the DNS zones of a domain
description: This page explains how to manage the DNS zones of a domain in Scaleway Dedibox
...
This document shows you how to manage the DNS zones of your domain.
...
| **MX**      | Allows you set the mail server responsible for a domain (e.g., online.net. IN MX mail.online.net.). The DNS name must be an IPv4, not an IP address or a CNAME. If multiple mail servers are configured, the priority value determines the order in which they are contacted. |
...
<Message type="important">
  - Experts only should modify a DNS zone.
  - Errors in the configuration of a DNS zone may lead to the unavailability of your website or emails.
  - Scaleway does not provide support for web hostings or domain names when the DNS zones have been modified by the user.
</Message>
...
1. From the [Dedibox console](https://console.online.net), click **Domain**. A list of your domains appears.
...
4. Note the following parts of the interface:
    ...
    - The pen icon next to the zone name in the **Active Version** panel lets you manage the zone options. Click the pen to edit the zone name or set it as active.
...
1. From the **Edit DNS zone** tab, make your changes (e.g., adding or deleting an entry) in the current zone shown to you.
...
To facilitate the management of your zones, reserved for experts, Scaleway provides you with the option of sending and downloading your DNS zone in the BIND format. This is the format mostly used for the configuration of a BIND DNS Server. Use BIND to re-import your zones if they were previously in this format, or if you want to work in a format that you are already familiar with.

1. From the **Edit DNS zone** tab, click the arrow in the middle panel next to **Open BIND file options**.
2. Enter the desired content in the text box that appears. Alternatively, if you want to recover your current zone, click the **Download** button.
