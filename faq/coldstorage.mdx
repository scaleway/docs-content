...
It depends on how you use Object Storage. If you consider that a 3-month-old object is rarely used but still has a value, you might want to send it to the Glacier class, for example. Glacier class is ideal for data archiving, and for storing legal documents of long-term backups that do not require immediate access.
...
We recommend that you use Glacier to archive data that you may need only once or twice within a decade. Latencies may vary from 24 to 48 hours to the first byte. The time it takes to restore an object varies depending on the number and size of the files you restore.

For more information, check out [How to restore an object from Glacier class](/storage/object/how-to/restore-an-object-from-glacier/).
