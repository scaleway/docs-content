---
meta:
  title: Bucket policies overview
  description: This page explains what are S3 bucket policies and how they work.
content:
  h1: Bucket policies overview
  paragraph: This page explains what are S3 bucket policies and how they work.
tags: object-storage bucket bucket-policy s3
dates:
  validation: 2023-06-13
  posted: 2021-02-17
categories:
  - storage
  - object-storage
---

## What are bucket policies

<Macro id="storage-bucket-policy" />

The different component strings of a bucket policy allow you to [configure fine-grained permissions](/storage/object/api-cli/combining-iam-and-object-storage/) when combined with [Identity and Access Management (IAM)](/identity-and-access-management/iam/concepts/#iam).

Bucket policies behave like objects: they can be uploaded into buckets. Once you upload, or "put", a bucket policy into a bucket, it takes immediate effect and will from then on define who can access and perform actions on the bucket.

<Message type="important">
  Each bucket can have only one bucket policy.
</Message>

## Bucket policy versions

| Version | ID                | Lifecycle status  |
|:-------:|:-----------------:|:-----------------:|
| V1      | `2012-10-17`      | Deprecated        |
| V2      | `2023-06-07-alpha`      | End of life       |
| V3      | `2023-06-07-alpha`| Current           |

- The `2012-10-17` version is deprecated and its usage is strongly discouraged.

- If you use the `2023-06-07-alpha` version, everything that is not explicitly **allowed** by the bucket policy is **denied**, regardless of what is defined in the IAM policy.

- If you use the `2023-06-07-alpha` version, everything that is not explicitly **denied** by the bucket policy is **allowed**

<Message type="note">
Refer to the [version migration procedure]( /storage/object/api-cli/migrating-bucket-policy-version/) for more information on how to update the version of a bucket policy.
</Message>

## Bucket policies description

Bucket policies use a JSON-based access policy language and are composed of strings, such as: `Version`, `Id`, `Statement`, `Sid`, `Principal`, `NotPrincipal`, `Action`, `NotAction`, `Effect`, `Resource` and `Condition`.

### Version

**Description**
: [IAM](https://en.wikipedia.org/wiki/Identity_management) syntax version

**Required**
: Yes

**Type**
: const

**Value**
: `2023-06-07-alpha`

**Sample**:

```json
{
    "Version": "2023-06-07-alpha",
    "Id": "MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Deny access",
            "Effect":"Deny",
            "Principal": {
                "SCW": "user_id:<USER_ID>"
            },
            "Action": "s3:*",
            "Resource": "<BUCKET_NAME>"
        }
    ]
}
```

### Id

**Description**
: Information about the policy as a whole. The length is limited to 280 characters.

**Required**
: No

**Type**
: string

**Sample**

```json
{
    "Version": "2023-06-07-alpha",
    "Id": "MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Deny Delete actions",
            "Effect":"Deny",
            "Principal":{
                "SCW":"application_id:<APPLICATION_ID>"
            },
            "Action":[
                "s3:DeleteBucket",
                "s3:DeleteObject",
                "s3:DeleteObjects"
            ],
            "Resource":[
                "<BUCKET_NAME>",
                "<BUCKET_NAME>/*"
            ]
        }
    ]
}
```

### Statement

**Description**
: `Statement` defines the rules that should be respected by our policy engine. A policy can have several statements.

**Children**
: `Sid`, `Principal`, `Action`, `Effect` and `Resource`

**Required**
: No

**Type**
: array

**Samples**


```json
{
    "Version": "2023-06-07-alpha",
    "Id":"MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Deny access to all except user",
            "Effect":"Deny",
            "NotPrincipal":{
                "SCW":"user_id:<USER_ID>"
            },
            "Action":"*",
            "Resource":[
                "<BUCKET_NAME>",
                "<BUCKET_NAME>/*"
            ]
        },
        {
            "Sid": "Deny all actions except Get and List",
            "Effect":"Deny",
            "Principal":{
                "SCW":"user_id:<USER_ID>"
            },
            "NotAction":[
                "s3:GetObject",
                "s3:ListBucket"
            ],
            "Resource":"<BUCKET_NAME>/*"
        }
    ]
}
```

### Sid

**Description**
: Provides a way to include information about an individual statement.

**Required**
: No

**Parent**
: Statement

**Type**
: string

**Sample**

```json
{
    "Version": "2023-06-07-alpha",
    "Id":"MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Deny all actions",
            "Effect":"Deny",
            "Principal":{
                "SCW":"user_id:<USER_ID>"
            },
            "Action":"*",
            "Resource":"<BUCKET_NAME>"
        }
    ]
}
```

### Principal

**Description**
: The [user](/identity-and-access-management/iam/concepts#user) or [application](/identity-and-access-management/iam/concepts#application) targeted by the bucket policy.

**Required**
: Yes

**Parent**
: `Statement`

**Supported values**
: : `user_id` / `application_id` / `*`

**Sample**
```json
{
    "Version": "2023-06-07-alpha",
    "Id":"MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Prevent user and application to DELETE",
            "Effect":"Deny",
            "Principal":{
                "SCW":[
                    "user_id:<USER_ID>",
                    "application_id:<APPLICATION_ID>"
                ]
            },
            "Action":[
                "s3:DeleteBucket",
                "s3:DeleteObject",
                "s3:DeleteObjects"
            ],
            "Resource":[
                "<BUCKET_NAME>",
                "<BUCKET_NAME>/*"
            ]
        }
    ]
}
```

### NotPrincipal

**Description**
: Everybody **except** the [user](/identity-and-access-management/iam/concepts#user) or [application](/identity-and-access-management/iam/concepts#application) targeted by the bucket policy.

**Required**
: Yes

**Parent**
: `Statement`

**Supported values**
: `user_id` / `application_id` / `*`

**Sample**

```json
{
    "Version": "2023-06-07-alpha",
    "Id":"MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Deny access to all except user and application",
            "Effect":"Deny",
            "NotPrincipal":{
                "SCW":[
                    "user_id:<USER_ID>",
                    "application_id:<APPLICATION_ID>"    
                ]
            },
            "Action":[
                "s3:*"
            ],
            "Resource":[
                "<BUCKET_NAME>",
                "<BUCKET_NAME>/*"
            ]
        }
    ]
}
```

### Action

**Description**
: Consists of S3 namespace, a colon, and the name of an action. Action names can include wildcards represented by `*`.

**Required**
: Yes (either `Action` or `NotAction`)

**Parent**
: `Statement`

**Sample**

```json
{
    "Version": "2023-06-07-alpha",
    "Id":"MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Deny Delete to user",
            "Effect":"Deny",
            "Principal":{
                "SCW":"user_id:<USER_ID>"
            },
            "Action":[
                "s3:DeleteBucket",
                "s3:DeleteObject",
                "s3:DeleteObjects",
            ],
            "Resource":[
                "<BUCKET_NAME>",
                "<BUCKET_NAME>/*"
            ]
        }
    ]
}
```

#### Supported actions
- `*`
- `s3:*`
- `s3:AbortMultipartUpload`
- `s3:DeleteBucketWebsite`
- `s3:DeleteObject`
- `s3:DeleteObjectTagging`
- `s3:DeleteObjectVersion`
- `s3:DeleteObjectVersionTagging`
- `s3:GetBucketTagging`
- `s3:GetBucketVersioning`
- `s3:GetBucketWebsite`
- `s3:GetLifecycleConfiguration`
- `s3:GetObject`
- `s3:GetObjectTagging`
- `s3:GetObjectVersion`
- `s3:GetObjectVersionTagging`
- `s3:ListBucket`
- `s3:ListBucketMultipartUploads`
- `s3:ListMultipartUploadParts`
- `s3:PutBucketTagging`
- `s3:PutBucketVersioning`
- `s3:PutBucketWebsite`
- `s3:PutLifecycleConfiguration`
- `s3:PutObject`
- `s3:PutObjectTagging`
- `s3:PutObjectVersionTagging`

### NotAction

**Description**
: The `NotAction` element is the opposite `Action`. It corresponds to every action **EXCEPT** the ones listed.

**Required**
: Yes (either `Action` or `NotAction`)

**Parent**
: `Statement`

**Sample**

```json
{
    "Version": "2023-06-07-alpha",
    "Id":"MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Grant List and GET to user and application",
            "Effect":"Deny",
            "Principal":{
                "SCW":[
                    "user_id:<USER_ID>",
                    "application_id:<APPLICATION_ID>"
                ]
            },
            "NotAction":[
                "GetObject",
                "ListBucket"
            ],
            "Resource":[
                "<BUCKET_NAME>",
                "<BUCKET_NAME>/*"
            ]
        }
    ]
}
```

### Effect

**Description**
: Uses **Deny** to indicate that the policy denies access. All actions that are not explicitly denied are allowed.

**Required**
: Yes

**Parent**
: `Statement`

**Sample**
```json
{
  "Version": "2023-06-07-alpha",
  "Id": "MyBucketPolicy",
  "Statement": [
    {
        "Sid": "Deny DELETE to everyone",
        "Effect": "Deny",
        "Principal": "*",
        "Action": "s3:DeleteObject",
        "Resource":[
            "<BUCKET_NAME>",
            "<BUCKET_NAME>/*"
        ]
    }
  ]
}
```

### Resource

**Description**
: Consists in the S3 resource path.

**Required**
: Yes

**Parent**
: `Statement`

**Sample**

* `"<BUCKET_NAME>"` - Grants access to the bucket, but not to the objects inside. If the `s3:ListBucket` action is applied, this resource specification is required.
* `"<BUCKET_NAME>/*"` - Grants access to all objects inside a bucket, but not to the bucket itself. If the `s3:PutObject`, `s3:GetObject` and `s3:DeleteObject` actions are applied, this resource specification is required.
* `"<BUCKET_NAME>/<PREFIX>/*"` - Grants access only to objects with the specified prefix inside a bucket, but not to the bucket itself.

```json
{
    "Version": "2023-06-07-alpha",
    "Id":"MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Grant List and GET to application",
            "Effect":"Deny",
            "Principal":{
                "SCW":"application_id:<APPLICATION_ID>"
            },
            "NotAction":[
                "s3:ListBucket",
                "s3:GetObject"
            ],
            "Resource":[
                "<BUCKET_NAME>",
                "<BUCKET_NAME>/*"
            ]
        }
    ],
    "Statement": [
        {
            "Sid": "Grant Put and Delete to user in a specific folder",
            "Effect":"Deny",
            "Principal":{
                "SCW":[
                    "user_id:<USER_ID>",
                    "application_id:<APPLICATION_ID>"
                ]
            },
            "NotAction":[
                "s3:PutObject",
                "s3:DeleteObject"
            ],
            "Resource":[
                "<BUCKET_NAME>/photos/*"
            ]
        }
    ]
}
```
### Condition

**Description**
: The `Condition` element allows you to specify conditions for when a Policy is in effect.

**Required**
: No

**Parent**
: `Statement`

**Condition Keys**
: `aws:SourceIp`, `aws:Referer`, `aws:CurrentTime`, `aws:EpochTime`

**Examples**

* You can use the `IpAddress` condition to `Allow` actions for specific IP ranges or addresses.

```json
{
    "Version": "2023-06-07-alpha",
    "Id": "MyBucketPolicy",
    "Statement": [
        {
        "Sid": "Grant List and GET from my Instances",
        "Effect": "Deny",
        "Principal": "*",
        "NotAction":[
            "s3:ListBucket", 
            "s3:GetObject"
        ],
        "Resource":[
            "<BUCKET_NAME>",
            "<BUCKET_NAME>/*"
        ],
        "Condition": {
            "IpAddress":{
                "aws:SourceIp": "192.0.2.0/24"
            }
        }
        }
    ]
}
```

* You can allow access only within a set timeframe, by implementing the `DateGreaterThan` and `DateLessThan` conditions.

```json
{
    "Version": "2023-06-07-alpha",
    "Id": "MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Grant List and GET to user for 10 years",
            "Effect": "Deny",
            "Principal": {
                "SCW": "user_id:<USER_ID>"
            },
            "NotAction":[
                "s3:ListBucket",
                "s3:GetObject"
            ],
            "Resource":[
                "<BUCKET_NAME>",
                "<BUCKET_NAME>/photos/*"
            ],
            "Condition":{
                "DateGreaterThan": {
                    "aws:CurrentTime": "2020-01-01T00:00:00Z"
                },
                "DateLessThan": {
                    "aws:CurrentTime": "2030-01-01T00:00:00Z"
                }
            }
        }
    ]
}
```
* You can also allow access according to the `HTTP` referer.

```json
{
    "Version": "2023-06-07-alpha",
    "Id": "MyBucketPolicy",
    "Statement": [
        {
            "Sid": "Allow access to assets from my website",
            "Effect": "Deny",
            "Principal": "*",
            "NotAction": "s3:GetObject",
            "Resource":[
                "<BUCKET_NAME>/assets/*"
            ],
            "Condition": {
                "StringLike": {
                    "aws:Referer": "https://console.scaleway.com"
                }
            }
        }
    ]
}
```
#### Supported conditions
- `IpAddress`
- `NotIpAddress`
- `StringEquals`
- `StringNotEquals`
- `StringEqualsIgnoreCase`
- `StringNotEqualsIgnoreCase`
- `StringLike`
- `StringNotLike`
- `DateGreaterThan`
- `DateGreaterThanEquals`
- `DateLessThan`
- `DateLessThanEquals`


<Navigation title="See Also">
  <PreviousButton to="/storage/object/api-cli/setting-cors-rules/">Setting CORS rules</PreviousButton>
  <NextButton to="/storage/object/api-cli/create-bucket-policy/">Creating and applying bucket policies</NextButton>
</Navigation>
