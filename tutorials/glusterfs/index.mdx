...
GlusterFS is an open-source, scalable network filesystem suited for high data-intensive workloads, such as media streaming, cloud storage, and content delivery networks (CDN). 
...
- Attached [Scaleway Block Storage](/storage/block/how-to/attach-a-volume/) to each GlusterFS Instance
...
1. Install the GlusterFS server on both `gluster01` and `gluster02`:
    ```
    apt install glusterfs-Instance -y
    ```
...
2. Partition the volume and format it with a filesystem (e.g., `ext4`):
     ```
     mkfs.ext4 /dev/sdb
     ```
