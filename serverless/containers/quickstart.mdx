...
<Message type="tip">
  This page explains how to deploy a containers using Scaleway Console but for advanced usage (ex: larger project with dependencies, automation, multiple containers...) [several ways to deploy containers](/serverless/containers/reference-content/deploy-container/) are available.

  For real world examples checkout our [scaleway/serverless-examples repository](https://github.com/scaleway/serverless-examples).
</Message>
...
You can deploy a container either from the [Scaleway Container Registry](/containers/container-registry/quickstart/) or from any other public container registry (DockerHub, AWS Container registries, Gitlab container registry, etc.).
