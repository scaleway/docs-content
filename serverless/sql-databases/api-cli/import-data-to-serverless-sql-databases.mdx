...
- Ensure the database your are importing to is empty, or that you accept to overwrite existing data stored inside your database
...
#### Download your backup on your local machine
1. Open pgAdmin
2. Connect to your existing database server and select it on the left menu (Left-click) to connect to it.
3. Expand the "Databases" submenu, and select the database you want to export (Left-click) to connect to it.
4. In the left menu, right-click on the database you want to export, and select **Backup**.
5. Fill the filename - for instance "my-backup" and keep default format and compression options (ie. Custom Format)
6. Click on **Backup**. The backup creation process will start, and you can follow its progress by Clicking on **View processes**.
7. When the process is finished, you can check your backup is downloaded on your local machine by locating the corresponding file. The default location is `Users/{username}/my-backup` on MacOS.

#### Load your data in your Serverless SQL Database
1. Register your new database in pgAdmin. You can find more information to connect in [How to connect to a Serverless SQL Database](https://www.scaleway.com/en/docs/serverless/sql-databases/how-to/connect-to-a-database/#how-to-set-up-credentials).
2. Select the database server on the left menu (Left-click) to connect to it.
3. Expand the "Databases" submenu, and select your database.
4. In the left menu, right-click on your new database name, and select **Restore**.
5. Fill in the filename of your backup - for instance "my-backup" in this example, and keep default options.
6. Click on the **Data options** tab, and activate the **Do not save - Owner** and **Do not save - Privileges** option.
7. Optional - If your new database is not empty, click on the **Query Options** tab, and activate the **Clean before restore** option. This option will make sure your database is emptied before loading data to avoid potential conflicts with existing data (such as table names or primary key constraints).
8. Click on **Restore**. The restore process will start, and you can follow its progress by Clicking on **View processes**.
9. When the process is finished, you can check your data is now stored in your new database by connecting to it and performing a query.
...
**(Doesn't work currently for Serverless SQL Database, only for Managed Database)**
### Using Airbyte

Requirements: 
- You have access to [Airbyte Cloud](https://cloud.airbyte.com/) or have [Airbyte installed](https://docs.airbyte.com/deploying-airbyte/quickstart) on your machine.
- On Managed Database for PostgreSQL, to avoid any statement timeout errors (eg. `ERROR: canceling statement due to statement timeout`), we recommend you setup the PostgreSQL parameter `statement_timeout` to `1200000` (20 minutes) as recommended by Airbyte for other PostgreSQL-compatible data sources. You can modify this parameter in your [database overview in Scaleway Console](https://console.scaleway.com/rdb/instances), by clicking on **More** and then **Advanced Settings**.


#### Add your existing database as source

1. Click on **Sources**
2. Select **Postgres**
3. Fill in the required information to connect to your existing database:
  - Host, Port, Database Name, Username, Password
  - Security options SSL Modes and SSH Tunnel Method
  - Select **Detect Changes with Xmin System Column** as **Update Method** (other methods are not yet supported)
4. Click on **Set up source**

#### Add your new Serverless SQL Database as destination
...
3. Fill in the required information to connect to your existing database:
  ...
  - Security options SSL Modes and SSH Tunnel Method
  - Select **Detect Changes with Xmin System Column** as **Update Method** (other methods are not yet supported)
4. Click on **Set up destination**
5. Select the streams (tables) you want to load in your new Serverless SQL Database and click on **Next**
6. Change remaining options if required and click on **Finish & Sync**
7. When the process is finished, you can check your data is now stored in your new database by connecting to it and performing a query.
...
Requirements: 
- You have a Fivetran account and can connect to [Fivetran console](https://fivetran.com/dashboard)

#### Connect Fivetran to your Serverless SQL Database
1. From [Fivetran destinations list](https://fivetran.com/dashboard/destinations), click on **Add destination**
2. Enter the destination name (for instance `my-new-database`) and click **Add**
3. Select PostgreSQL and click on **Select**
4. Fill in the required information to connect to your Serverless SQL Database: 
  - Host, Port, User, Password, Database. You can find these information in the [Scaleway Console Overview page](https://console.scaleway.com/serverless-db/databases) for your database. The password is the [IAM Secret Key](https://www.scaleway.com/en/docs/identity-and-access-management/iam/concepts/#api-key) corresponding to the [IAM user](https://www.scaleway.com/en/docs/identity-and-access-management/iam/concepts/#user) or [IAM application](https://www.scaleway.com/en/docs/identity-and-access-management/iam/concepts/#application) you are connecting with. 
  - Select **Connect directly** as **Connection method**
  - Select **Detect Changes via XMIN** as **Update Method** (other methods are not yet supported)
  - If you have setup IP adress access restrictions from your IAM policies, copy the adresses from Fivetran IPs and edit your IAM policies to whitelist them.
5. Click on **Save & Continue**
6. Select the certificate in the list corresponding to `CN=sdb.{region}.scw.cloud`, check its validity and confirm it by clicking on **Confirm**. You can ensure the certificate is fully valid only if all of following checks are true:
...
#### Connect Fivetran to your existing database
1. From [Fivetran connectors list](https://fivetran.com/dashboard/connectors), click on **Add connector**
2. Select your destination (for instance `my-new-database`) and click on **Select**
3. Select PostgreSQL and click on **Set up**
4. Fill in the required information to connect to your existing database: 
  - Schema, Host, Port, User, Password, Database and Connection method.
  - Select **Detect Changes via XMIN** as **Update Method** (other methods are not yet supported)
  - Copy the adresses from Fivetran IPs to whitelist in your firewall if required
5. Click on **Save & Continue**
6. When the database schemas and tables displays, select the one you want to import, then click on **Save & Continue**
7. Select your prefered way of synchronizing future schemas changes and then click on **Save & Continue**.
8. Click on **Start Initial Sync**
9. When the process is finished, you can check your data is now stored in your new database by connecting to it and performing a query.
...
Requirements: 
- You have installed [Meltano](https://docs.meltano.com/getting-started/installation/)

#### Initialize your Meltano project

1. Initialize a Meltano project, for instance named `migrate-postgresql`:
    ```sh
...
2. Go into the newly created directory:
   ...
   ```
3. Add a PostgreSQL-compatible extractor and loader:
   ```sh
...
2. Select the tables you want to load:
   ```sh
...
<Message type="tip">
  Tools like Meltano are more suited to load data only and without keeping constraints on columns types or relations. If you want to load all PostgreSQL objects (such as table constraints, users...), you should use other tools such as [pg_dump/pg_restore](#using-pgdump).
</Message>
...
4. Fill in the required information:
  ...
  - Database Connection Pooling Service: Click on **Create new service**, select **DBCPConnectionPool**, click on **OK** and click on the arrow to the right to access DBCPConnectionPool configuration
5. Click on **Configure** icon for DBCPConnectionPool and fill in the required information:
