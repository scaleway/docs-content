...
Site-to-Site VPN's tunnel is secured using **IPsec** (Internet Protocol Security), a robust suite of protocols that ensures data confidentiality, integrity, and authentication across untrusted networks like the internet. 
...
- **Incoming routes** are the IP prefixes that define ranges of route announcements from the customer gateway to allow. Routes towards these destinations will be propagated, allowing traffic from the Scaleway VPC to be routed via the VPN to your remote infrastructure.
You can allow multiple **outgoing routes** and multiple **incoming routes** per policy.

<Message type="note">
Scaleway Site-to-Site VPN only supports dynamic routing, therefore you cannot set static routes.
</Message>
...
#### Maximum number of learned prefixes

A VPN gateway can learn a maximum of **512 IP prefixes per VPN connection**. This scales with the number of connections terminating on the gateway: a gateway with 2 connections can learn up to 1,024 prefixes, 3 connections up to 1,536, etc.

If a customer gateway announces more prefixes than the limit allows, the [BGP session](/site-to-site-vpn/reference-content/statuses/#border-gateway-protocol-bgp-session-statuses) goes down.
...
- [Created a Scaleway account](/account/setup-lifecycle/create-an-account/) and [added a payment method](/account/setup-lifecycle/create-an-account/#add-your-payment-method)
...
Site-to-Site VPN is available in multiple different Regions and Availability Zones. For the most up-to-date information, check out the [Scaleway console](https://console.scaleway.com/s2s-vpn/fr-par/vpn-gateways) or see [Product availability by region](https://www.scaleway.com/en/product-availability-by-region/).
...
- A maximum of 512 IP prefixes can be learned by a VPN gateway per connection (multiplied by the number of connections).
- IKEv2 SA lifetime is limited to four hours, and ESP SA lifetime is limited to one hour. Automatic key renewal ensures optimal security by mitigating the risk of key compromise, preventing statistical traffic analysis, and ensuring Perfect Forward Secrecy.
...
You are billed for a VPN gateway from the moment you create it, until you delete it. You can [delete a VPN gateway](/site-to-site-vpn/how-to/create-manage-vpn-gateway/) at any time.

Note that:
- The public IPv4 address attached to a VPN gateway incurs a separate charge, with its own hourly rate.
