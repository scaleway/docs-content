...
This means that to resolve the conflict, one or more of the Private Networks involved in the overlap must be [deleted](/vpc/how-to/delete-private-network/), and [recreated](/vpc/how-to/create-private-network/) with a different CIDR block. You can define a custom CIDR block via the **Advanced Settings** when creating a Private Network via the console. Remember that you can use [IPAM](/ipam/) to reserve specific private IP addresses on each Private Network, to use when attaching resources, if needed.
