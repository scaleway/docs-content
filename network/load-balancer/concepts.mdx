...
Tunnel timeout is a configurable value for your Load Balancer's backend. It defines the maximum allowed time (in ms) that a backend server has to process a request.
