...
5. Click **Update volume type** to confirm.
    ...
    <Message type="important">
      When switching from a local volume to a Block volume, and if Autobackup is enabled, be aware that the snapshot type will change. Local volumes utilize logical backups, whereas Block storage employs snapshots. Consequently, the billing for these two types will differ as well.<br />

      Disk size cannot be reduced, so when switching from local to block, the volume size remains the same by default. <br />
