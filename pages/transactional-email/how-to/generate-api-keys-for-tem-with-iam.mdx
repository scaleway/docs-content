...
This page shows you how to generate [API keys](/iam/concepts/glossary/#api-key) with IAM to send [transactional emails](/transactional-email/concepts/#transactional-email) via API and SMTP.
...
1. [Create an IAM application](/iam/principals/create-application/).
2. [Create a policy](/iam/policies-permissions/create-policy/) giving appropriate [permissions](/iam/policies-permissions/permission-sets/) to carry out actions for Transactional Email in the desired Project(s), with the IAM application attached as a principal.
3. [Create an API key for your application](/iam/credentials/create-api-keys/).
