...
8. Configure your Load Balancer's **[backend](/network/load-balancer/concepts/#backends)** by entering a **name**, and the **protocol** and **port** it should use to connect to the backend servers. You should also select a **proxy** protocol option, **health check** method and add the **server IPs** (one or many) of the backend server(s) it should direct traffic to. You can [add additional backends](#how-to-add-backends) after creating your Load Balancer, if required.
...
7. Choose between **using an existing backend** or **creating a new backend**. Your frontend will direct traffic to the backend you choose here.
    ...
    - If creating a new backend, click **Create a new backend**. This will open a new tab in your browser for you to configure your backend. Enter a name, port, protocol, proxy protocol, health check type, server IP(s) and advanced settings as required. See our [dedicated documentation](/network/load-balancer/reference-content/configuring-load-balancer/) for help configuring your backend.
...
5. Enter a name, protocol, port, proxy protocol, health check type,  server IP(s) and advanced settings as required for your new backend. See our [dedicated page](/network/load-balancer/reference-content/configuring-load-balancer/) for help configuring your backend, including details of how to set up SSL bridging, offloading or passthrough.
