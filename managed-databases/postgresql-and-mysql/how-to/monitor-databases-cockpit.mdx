...
- **Node** - Choose a specific node (main, replica, or Read Replica). In an High Availability (HA) cluster, failovers may show the same node as both main and replica. For nodes in an HA cluster, you might see two verions of the same node, the main and the replica. The replica will start appearing as if any failovers happen and will show only metrics for any activity that happened over the failover period.
...
You can also configure [your own managed alerts](/observability/cockpit/how-to/configure-alerts-for-scw-resources/), and configure values for each of the parameters below to define when you want to be alerted:
