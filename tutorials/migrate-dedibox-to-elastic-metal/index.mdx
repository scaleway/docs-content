---
meta:
  title: How to transsfer your data from Dedibox to Elastic Metal
  description: This tutorial provides information how to migrate your existing data from a Dedibox to a Elastic Metal server
content:
  h1: How to transsfer your data from Dedibox to Elastic Metal
  paragraph: This tutorial provides information how to migrate your existing data from a Dedibox to a Elastic Metal server
...
## Data migration from Dedibox to Elastic Metal

This tutorial provides information how to migrate your existing data from a Dedibox to a Elastic Metal server. Its purpose is to guide you in your migration to the resources that best fit your needs for improved stability, performance and reliability. 
...
- You have [created](/compute/elastic-metal/how-to/create-server/) and [installed](/compute/elastic-metal/how-to/install-server/) a Elastic Metal server.
...
In the command above we download Duplicity version `0.8.21`. You may check the [Duplicity](https://duplicity.gitlab.io/duplicity-web/) website for the latest version of the tool.
...
1. To generate the GPG key, launch this command.
    ...
    ```
    Enter and remember a passphrase. You will be asked to define the characteristics of your key. We will go with default settings:
...
4. Edit the file `scw-restore.sh`, and add the following:
  ```sh
  #!/bin/bash
  source .scw-configrc
...
5. Download the data from your bucket to the Elastic Metal server:

  ```
  ./scw-restore.sh /tmp/backup-recovery/
  ```

Once downloaded you can move the data to it's final destination on your new machine. 

<Message type="tip">

You can use duplicity also for regular and incremental backups of your data on the Object Storage platform. Follow our tutorial [How to backup your dedicated server on Object Storage with Duplicity](/tutorials/backup-dedicated-server-s3-duplicity/) for more information.
