...
- Downloaded [a `kubeconfig` file](/kubernetes/how-to/connect-cluster-kubectl/) from the Scaleway console.

## Opening an SSH tunnel to the Kubernetes Kapsule control plane

To connect to the fully isolated Kubernetes Kapsule control plane, you can open an SSH tunnel using the Public Gateway's SSH bastion. This will port-forward the control plane to a local port, to which you can then connect.
...
The downloaded `kubeconfig` file points to the control plane's URL, which is currently unreachable due to its lack of a public IP. However, you can redirect traffic to your local port-forwarded port by editing your `/etc/hosts` file.
