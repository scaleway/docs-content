...
When you activate SSH bastion on your Public Gateway, all [SSH keys held in your Project](https://account.scaleway.com/?service=elementsproject/ssh-keys/) are imported to the SSH bastion. This provides a single point of entry to Instances and other resources behind the bastion, allowing you to connect to them via the Public Gateway, instead of via their public IP addresses.
