...
meta:
  ...
  description: Discover how to setup Ampere-Optimized AI frameworks on Scaleway COP-ARM Instances.
content:
  h1: Setting up Ampere-Optimized AI frameworks on Scaleway COP-ARM Virtual Instances
  paragraph: Discover how to setup Ampere-Optimized AI frameworks on Scaleway COP-ARM Instances.
tags: compute arm ai ampere pytorch onnx tensorflow
categories:
  - instances
dates:
  validation: 2024-02-09
  posted: 2024-02-09
---
...
In this tutorial, we will walk through the process of setting up and using Ampere-Optimized AI fameworks on [Scaleway COP-ARM Instances](https://www.scaleway.com/en/cost-optimized-instances-based-on-arm/).
...
3. Select **ARM64** architecture in the drop-down, then select the COP-ARM offer that best fits your requirements.
    <Message type="tip">
...
5. Adjust the size of your system volume to meet the needs for your models.
6. Leave the IP settings as default, ensuring that a public IPv4 address is assigned to the Instance.
7. Enter a name for your Instance to easily identify it. You can go with the default value if it conveniences you.
8. Verify your SSH key fingerprint is shown. If not, click **Add SSH key** and paste the SSH key you have [pre-generated](/console/project/how-to/create-ssh-key) for secure access to the Instance.
9. Review the cost estimation and, once satisfied, click **Create Instance** to proceed. Allow some time for the Instance to start. Once it is up and running, copy its public IP address.
...
1. Use SSH to access the system by specifying the user as `root` followed by the public IP address of your Instance.
    ...
    ```
2. Execute the following command to update the package lists and install Docker once logged into the Instance.
    ```bash
    apt update && apt install -y docker.io
    ```
    <Message type="tip">
      While the Docker installation completes, visit the [Ampere AI DockerHub](https://hub.docker.com/u/amperecomputingai) and select your desired Ampere-optimized framework, whether it is PyTorch, ONNX Runtime, or TensorFlow.
