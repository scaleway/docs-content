...
**S**tateless **A**uto **A**ddress **C**onfiguration is the simplest way to assign IP addresses in an IPv6 network. Using the [Neighbor Discovery Protocol](/ipv6/concepts/#neighbor-discovery-protocol), devices request the network prefix from the router, and then use this prefix and their own MAC address to create an IPv6 address.

## Stateful DHCPv6

With [stateful DHCPv6](/ipv6/concepts/#stateful-dhcpv6), it is the DHCPv6 server that chooses and assigns IPv6 addresses to clients, unlike with [SLAAC](/ipv6/concepts/#slaac) or [Stateless DHCPv6](/ipv6/concepts/#stateless-dhcpv6)  where the client creates their own IPv6 address. Stateful DHCPv6 is so-called because the DHCPv6 server keeps track of the state of IPv6 addresses on the network.

## Stateless DHCPv6

[Stateless DHCPv6](/ipv6/concepts/#stateless-dhcpv6) involves the initial use of [SLAAC](/ipv6/concepts/#slaac) by a client to create its own IPv6 address, followed by an extra DHCPv6 request to get extra details from the DHCPv6 server, such as DNS server or domain name information. Stateful DHCPv6 is so-called because no server or client is keeping track of the overall state of IPv6 addresses on the network.