...
- For Apache >=2.4.30, use `mod_remoteip`:
    ...
    ```
<Message type="important">Once Proxy Protocol is enabled, Apache will no longer accept direct connections.</Message>
...
### Confiuring Kubernetes Ingress

<Message type="important">
  The legacy `ingress-nginx` project is deprecated and will receive no updates after March 2026. Use the **official NGINX Ingress Controller** (`oci://ghcr.io/nginx/charts/nginx-ingress`). ([Docs](https://docs.nginx.com/nginx-ingress-controller/overview/about/))
</Message>
