...
A design flaw in the VNC service may lead to malicious intends to brute-force into the service. [Read more about the issue in our blog](https://www.scaleway.com/en/blog/m1-how-a-design-flaw-slowed-down-apples-vnc/)

To avoid the the issue, restricting access to the macOS Screen Sharing service to a specific static IP can be done by configuring your network and using the built-in macOS firewall, `pf` (Packet Filter), to control inbound connections.
...
1. Open a **Terminal**.
2. Backup the original `pf.conf` file by copying it to a different location. You might use a command like: `sudo cp /etc/pf.conf /etc/pf.conf.backup`.
3. Edit the `pf.conf` file with a command like `sudo nano /etc/pf.conf`.
4. You'll need to write rules that block all incoming connections on the screen sharing ports except for the desired IP.
    ```
    ...
    ```
    In the example above, `en0` should be replaced with the network interface you're using, and `<static IP>` should be the static IP address you are allowing. Port 5900 is typically used for VNC/screen sharing services.
5. Save and close the file (in `nano`, it's **Control-X** to close, **Y** to confirm changes, and **Enter** to save).
6. Run `sudo pfctl -e` to enable `pf`. You might need to restart your computer for changes to take effect.
7. Try to initiate a Screen Sharing session from the approved IP address and ensure it connects. Also, test from a different IP address to confirm it does not allow the connection.
  <Message type="note">
    ...
    - Remember that this setup can cause issues if the static IP changes for some reason. You can access your m
  </Message>
...
<Message type="important">
  To prevent unauthorized access to your Mac mini's VNC connection, it is recommended to adjust the Mac's firewall settings. Only allow the IP addresses from your own internet connection to connect to the VNC port (port `5900`). For detailed instructions, refer to the official [Apple firewall documentation](https://support.apple.com/en-gb/guide/mac-help/mh34041/14.0/mac/14.0).
</Message>
