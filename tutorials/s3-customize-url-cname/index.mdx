...
1. From the Scaleway console, [create a new bucket](/object/how-to/create-a-bucket/). When entering a name for your bucket, the name must be the same as the domain/subdomain you intend to use with the bucket. For example, to create a bucket accessible at `download.example.com` the bucket name must be `download.example.com`.
2. Set the bucket ACL with [AWS-CLI](/object/api-cli/object-storage-aws-cli/):
    ```
