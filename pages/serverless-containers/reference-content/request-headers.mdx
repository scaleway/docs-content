...
tags: serverless contgainers headers http request-id proxy
dates:
...
| Header | Description |
| :--- | :--- |
| `X-Envoy-External-Address` | The public IP address of the client that initially connected to the Scaleway edge proxy. |
...
| `K-Proxy-Request` | An internal header used by the underlying architecture. |
...
* **Log correlation**: Log the `X-Request-ID` at the start of your container. If a request fails, you can use this ID to search through your [Serverless Containers logs](/serverless-containers/how-to/monitor-container/) to find all entries related to that specific execution.
