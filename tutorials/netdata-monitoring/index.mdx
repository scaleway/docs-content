...
categories:
  - instances
tags: monitoring Netdata discord
dates:
...
- 4 [Instances](/compute/instances/how-to/create-an-instance)
...
3. Run the automatic installer of Netdata:
    ...
    The automatic installer will run some tests to detect the operating system version and install additional dependencies using the `apt` package manager. During the automatic installation, you will be asked to confirm the installation of additional packages using `apt` several times. When prompted, press `Enter` or `Y` to validate.
    ...
    Optionally, Netdata sends by default anonymous [usage statistics](https://github.com/netdata/netdata/blob/master/docs/anonymous-statistics.md) to Google Analytics to identify how the product is used and also for quality assurance statistics.

    It is possible to opt-out from these statistics using an empty file called `.opt-out-from-anonymous-statistics` which is stored in the Netdata configuration directory. It stops the statistics script from running.
...
To facilitate plugin development and to limit the number of threads and processes, Netdata supports plugin orchestrators. Currently, there are orchestrators available for four different programming languages: `C`, `python`, `node.js`, and `bash`.
...
1. Add the following configuration in an Nginx [server block](/tutorials/installation-lemp-ubuntu-bionic/#configuring-a-nginx-server-block):
    ```
