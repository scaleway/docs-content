...
If vCPU and RAM values are too low, it can lead to containers going into error status. It is important to select the correct amount of vCPU and RAM.
The recommended approach is to set high values and then use metrics [(see documentation)](/serverless/containers/how-to/monitor-container/) to monitor
the usage of resources and to adjust the value.
## How to reduce cold-start of Serverless Containers ?

* **Optimise the startup**: Cold-start can be affected by a loading a large number of dependencies and opening lot of resources at startup.
Ensure that your code avoid heavy computations or long-running initialization at statup and optimise the number of loaded libraries.
...
* **Increase resources**: Adding more vCPU and RAM can help to reduce significantly the cold-starts of your container.
