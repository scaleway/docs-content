...
Angular is a development platform that can scale from single-developer projects to enterprise-level applications. The platfrom is built on TypeScript, a superset of JavaScript, and provides features such as:
...
* A suite of developer tools develop, build, test, and update code

<Message type="requirement">

- You have an account and are logged into the [Scaleway console](https://console.scaleway.com)
- You have installed Anglar on your local computer. 
...
1. Run the command below to create a new Angular application. It initalizes a new project and create the files required to build the application.
  ```
  ng new demo-app
  ```
  ...
  Make sure `angular-cli` is installed on your local machine. If you have not installed it yet, install it by running `sudo apt install angular-cli` on Ubuntu/Debian Linux based machines or install it using Homebrew on MacOS: `brew install angular-cli`. 
...
4. Open a web browser and point it to `http://localhost:4200` to see the Angular application: 
  ...
  To stop the developement server press CTRL+C in your terminal. 
...
5. Add a `nginx-configuration.conf` file inside the `demo-app` directory and add the following content to it:
  ...
  The configuation above contains information about expiration headers for images and other content (CSS, HTML, etc.) and switches `gzip` compression on to improve performance of the application.
...
2. Tag the builded Docker image: 
  ```
  ...
  * Make sure to replace `demo-namespace` with the name of your [Container Registry namespace(/compute/container-registry/how-to/create-namespace/).
...
2. Click on the name of the namespace in which you want to deploy the application. The list of your containers displays.
...
3. Click **Deploy a Container**. The container deployment wizard displays. Provide the information to deploy the container:
  - Choose the pushed image in your registries namespace and set the port value to 80. 
  ...
  - You leave the other options on their default values
...
5. Click on the containers' name. The container information page displays. Click the **Container Settings** tab. The container endpoint displays in the container information block. 
