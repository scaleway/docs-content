...
- Deployed a container from [Scaleway Container Registry](/containers/how-to/deploy-container/) or an [external registry](/containers/how-to/deploy-container/)
...
[Queue triggers](/containers/concepts/#queue-trigger) allow you to automatically invoke a container using events stored in a [Scaleway queue](/messaging/concepts/#queues).

Only queues created with the Scaleway Queues product are compatible with Serverless Containers.

The configuration of the queue retention can affect the behavior of the trigger. Refer to the [Considerations to configure event retention for queue trigger inputs](/containers/reference-content/configure-trigger-inputs/) page for more information.
...
[NATS triggers](/containers/concepts/#nats-trigger) allow you to automatically invoke a container using messages sent in a [NATS](/messaging/concepts/#nats) subject.
...
[CRON triggers](/containers/concepts/#cron-trigger) allow you to invoke your containers based on a recurring schedule. The CRON triggers for Serverless Containers use the **UTC** time zone.
...
<Message type="note">
    Refer to the [cron schedules reference](/containers/reference-content/cron-schedules/) for more information.
</Message>
