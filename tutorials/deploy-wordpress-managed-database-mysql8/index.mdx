...
In this tutorial, you will learn how to set up your own [Wordpress](https://wordpress.org) blog on a Scaleway Instance, backed by a Managed Database for MySQL 8. Scaleway Database for MySQL 8 provides up to 48 cores and 256 GB of RAM.
...
- You have a Scaleway Elements Instance running Ubuntu Bionic Beaver (18.04 LTS) or Ubuntu Focal Fossa (20.04 LTS)
...
By default HTTP is a plain text protocol and does not use any encryption. To secure the connection between the client and the server, we are going to use the [TLS protocol](https://en.wikipedia.org/wiki/Transport_Layer_Security) using a certificate issued by the [Let's Encrpt](https://letsencrypt.org/) certificate authority. We are going to use a tool, called `certbot` which manages the configuration of the Apache2 web server, the certificate creation and renewal of it. [Certbot](https://certbot.eff.org/) is developed and maintained by the Electronic Frontier Foundation ([EFF](https://www.eff.org/)].
