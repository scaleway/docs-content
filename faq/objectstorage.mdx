...
We provide an S3-compatible API for programmatic access or usage with any compatible software. Thus, we recommend using dedicated tools like s3cmd to manage large data sets.
...
Yes, this is allowed with tools such as `s3cmd`, but only `Cache-control` or headers prefixed by `x-amz-meta`.
...
Data can be encrypted using third-party solutions, like GPG. It is possible to automatize your backups and encrypt them using a solution like Duplicity or Rclone using the crypt backend.
