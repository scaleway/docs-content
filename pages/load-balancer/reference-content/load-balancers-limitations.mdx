...
\*The buffer size that the Load Balancer allocates per connection to process requests and responses. This limit may be reached, for example, when the Load Balancer is running HTTP [protocol](/load-balancer/concepts/#protocol) for the frontend/backend and the header size exceeds this limit.
