...
2. In the **Endpoint** panel, click **Configure domain**. The following screen displays:
    ...
    <Lightbox src={image2} alt="A screenshot of the Configure Domain screen in the Edge Services part of the Scaleway console. Step one is to set a subdomain, step two is to provide an SSL/TLS certificate" />
...
4. This step depends on whether the domain used in the previous step is managed with [Scaleway Domains and DNS](/domains-and-dns/), or an external domain provider. Choose the appropriate tab below.
    ...
    <Tabs id="edge-cname">
        ...
        <Lightbox src={image3} alt="Verification passed: Your DNS record has been correctly configured" />
...
<Lightbox src={image4} alt="A screenshot of the Scaleway console, showing that an SSL/TLS certificate has been successfully created and has a positive status." />
