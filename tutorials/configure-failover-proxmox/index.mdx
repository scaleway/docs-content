...
<Message type="note">
  ...
  or manually using the [KVM-over-IP device](/bare-metal/hardware/concepts/#kvm-over-ip) of your server.
</Message>
...
7. Configure the network for the container. Set the parameters as follows to ensure your container has network access:
    ...
    - **MAC Address**: Set this value to the [virtual MAC address](/bare-metal/network/reference-content/create-virtual-mac/) associated to the failover IP of the container.
...
8. Configure the network interface. Set the **bridge** to `vmbr0`, the model to `VirtIO (paravirtualized)` and configure the [virtual MAC address](/bare-metal/network/reference-content/create-virtual-mac/) assigned to the failover IP of the virtual machine. The static [network configuration](/bare-metal/elastic-metal/how-to/configure-flexible-ip/) is done in a later step, during the installation of the operating system.  <Lightbox src="scaleway-proxmox_kvm_cpu_05.webp" width="500" alt="" />
