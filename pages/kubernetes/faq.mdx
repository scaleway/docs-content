...
Yes, Kubernetes Secrets are encrypted at rest in etcd using strong encryption. However, only Secrets are encrypted by default — not all data stored in etcd. Encrypting all etcd objects is under consideration for the future, but not currently implemented.
