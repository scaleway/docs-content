...
## How to generate a RSA key pair for Windows Instances

Windows-based Instances require a special type of key known as RSA (Rivest-Shamir-Adleman), a prevalent asymmetric cryptographic algorithm employed for ensuring secure data transmission.
...
2. Run the following command to generate the RSA key pair:
    ```bash
    ssh-keygen -t rsa -b 4096 -C "login" -Z aes256-gcm@openssh.com -f <key_name>
    ```
    Replace `<key_name>` with the desired name for your key pair. For example:
    ```bash
    ssh-keygen -t rsa -b 4096 -C "login" -Z aes256-gcm@openssh.com -f my_windows_key
    ```
