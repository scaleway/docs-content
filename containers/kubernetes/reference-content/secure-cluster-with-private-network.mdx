...
<Message type="tip">
  We recommend [migrating any existing clusters](#how-can-i-migrate-my-existing-clusters-to-private-networks) to a Private Network via the Scaleway console. **Public-only endpoints will become deprecated in October 2023** and will reach end of support in Q1/2024.
</Message>
...
* **Multi-AZ compatibility** allows you to create node pools in serveral Availability Zones  for better resiliance.
* **Lower latency**

You can use the Scaleway console to create new Private Networks and add clusters to existing Private Networks. Each cluster is autoconfigured using a /22 IP subnet, thanks to Scaleway’s IPAM.
...
## Can I have clusters with pools using both controlled and full isolation?
...
You have the option to utilize a Public Gateway when setting up your cluster with [full isolation](#what-is-the-difference-between-controlled-isolation-and-full-isolation). This allows the Kapsule nodes with private IPs to route their outgoing traffic through the Public Gateway. For detailed steps on setting up a Public Gateway, refer to our [Public Gateway documentation](/network/public-gateways/how-to/configure-a-public-gateway/).

Keep in mind that if you remove or detach the Public Gateway from a fully isolated Kapsule pool, it might create a single point of failure in the cluster since the nodes will no longer be able to connect to their control plane.
...
Additionally, [private Load Balancers](/network/load-balancer/concepts/#private-load-balancer) are supported. These Load Balancers have no public IPs in their back- and frontends.
