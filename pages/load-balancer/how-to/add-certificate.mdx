...
- Wildcard domains (e.g., `*.example.com`) are not supported when generating Let's Encrypt certificates directly through the Scaleway Load Balancer. If you need a wildcard certificate, you must generate it externally and import it manually to the Load Balancer.
</Message>
...
Alternatively you can also choose to import a **[self-signed certificate](#how-to-create-a-self-signed-certificate)**, not signed by any publicly-trusted Certificate Authority, but perhaps suitable for development and testing purposes, or internal use.
