...
4. Use `DESCRIBE` to visualize the table structure and to verify everything has been created accordingly:
    ...
    ```
5. Now as we have the table ready, update it by filling in some fictitious employee data into it. When you look at the query, you notice that the value for ID is not in it. This value will be auto-incremented by MySQL when entering the data:
    ```
