...
<Message type="note">
    While the principles discussed are applicable to various technologies and data stores, the examples and demonstrations specifically use MySQL as the database, Redis as the cache, and Entity Framework for .NET as the Object-Relational Mapping (ORM) tool.
</Message>
...
<Message type="requirement">
    ...
    - You have a [MySQL Database Instance](/managed-databases/postgresql-and-mysql/how-to/create-a-database)
    - You have installed the [.NET 7 SDK](https://dotnet.microsoft.com/en-us/download/dotnet/7.0)
</Message>
...
1. Get the public IP and port of your Database Instance. You can locate this information on the **Database Instance Information** page of your instance in the [Scaleway console](https://console.scaleway.com/rdb/instances), under **Public endpoint**.
    <Lightbox src="scaleway-mysql-db-information.png" size="small" alt="Database Instance Information page in the Scaleway Console" />
...
2. Create a new user for your database, granting administrative rights from the **Users page**. Alternatively, use the default user.
    <Lightbox src="scaleway-mysql-user-information.png" size="small" alt="User page in the Scaleway Console" />
...
<Lightbox src="scaleway-redis-db-information.png" size="small" alt="Redis Overview page in the Scaleway Console" />
...
3. During application startup, configure the EasyCaching library to connect to your Redis Instance, and configure it as the second-level cache for Entity Framework. Insert this snippet at the end of `Program.cs`:
    ```csharp
...
Your application is now fully configured to utilize the Scaleway Redis™ instance as a second-level cache for the Scaleway MySQL instance. To test the setup, create a couple of API endpoints.
...
1. Define endpoints to interact with your Product database model for listing, retrieving and adding `ProductModel`s. Add the following snippet at the end of Program.cs:
    ...
    <Message type="note">
        Use `.Cacheable()` on Entity Framework queries to leverage the Redis™ second-level cache:
            ...
            - Any DML operation invalidate the cache on the related rows.
