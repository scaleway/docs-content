...
Prometheus is a flexible monitoring solution that has been in development since 2012. The software stores all its data in a time series database and offers a multi-dimensional data model and a powerful query language to generate reports of the monitored resources.
...
1. Run the following command to create a user for Prometheus and Node Exporter. In this tutorial, we will create it without the login option.
    ...
    To achieve this, we use the parameter `--no-create-home` which skips the creation of a home directory and disables the shell with `--shell /usr/sbin/nologin`.
...
2. Create the folders required to store the binaries of Prometheus and its configuration files.
    ...
    ```
3. Set the ownership of these directories to our `prometheus` user, to make sure that Prometheus can access these folders.
    ```
...
1. Download the latest version of Node Exporter:
    ...
    ```
2. Unpack the downloaded archive. This will create a directory `node_exporter-1.5.0.linux-amd64`, containing the executable, a readme, and a license file:
    ```
...
Prometheus requires some basic configuring before its use. Thus, we need to create a configuration file named `prometheus.yml`

<Message type="note">
  The configuration file of Prometheus is written in [YAML](https://yaml.org/) which strictly forbids the use of tabs. If your file is incorrectly formatted, Prometheus will not start. Be careful when you edit it.
</Message>
...
1. Open the file `prometheus.yml` in a text editor:
    ...
    The `rule_files` block contains information on the location of any rules we want the Prometheus Instance to load.
    ...
    Overwrite the global scrape interval again and set it to 5 seconds. As we are scraping the data from the same Instance Prometheus is running on, we can use `localhost` with the default port of Node Exporter: `9100`.
...
1. Start Prometheus directly from the command line with the following command, which executes the binary file as our `Prometheus` user:
    ...
    ```
2. Open your browser and type `http://IP.OF.YOUR.SERVER:9090` to access the Prometheus interface. If everything is working, we end the task by pressing `CTRL + C` on our keyboard.
    <Message type="note">
...
Prometheus provides a basic web Instance running on `http://your.Instance.ip:9090` that provides access to the data collected by the software.
