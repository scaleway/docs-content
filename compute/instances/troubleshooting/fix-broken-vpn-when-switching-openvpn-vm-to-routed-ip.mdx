...
When switching an OpenVPN Instance to routed IP, the configured VPN will fail to work after the reboot. This is caused by a bug in a script that has been fixed on newer versions of the OpenVPN image.

This can now be fixed by installing a new version of the OpenVPN scripts which has been packaged Scaleway's stable PPA that contained the fixed script.

You can also avoid this situation alltogether by installing the package prior to moving to a routed IP.
