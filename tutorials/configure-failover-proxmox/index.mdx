...
content:
  ...
  paragraph: This page provides information on how to deploy a virtual machine with Proxmox on Scaleway Dedibox.
...
<Message type="note">
  ...
  or manually using the [KVM-over-IP device](/dedicated-servers/concepts/#ipmi) of your server.
</Message>
...
7. Configure the network for the container. Set the parameters as follows to ensure your container has network access:
    ...
    - **MAC Address**: Set this value to the [virtual MAC address](/ip-failover/concepts/#virtual-mac-address) associated to the failover IP of the container.
...
8. Configure the network interface. Set the **bridge** to `vmbr0`, the model to `VirtIO (paravirtualized)`, and configure the [virtual MAC address](/ip-failover/concepts/#virtual-mac-address) assigned to the failover IP of the virtual machine. The static [network configuration](/elastic-metal/how-to/configure-flexible-ip/) is done in a later step, during the installation of the operating system.  <Lightbox src="scaleway-proxmox_kvm_cpu_05.webp" width="500" alt="" />
