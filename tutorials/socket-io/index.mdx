...
tags: Socket.IO socket-io javascript example node-js chat server client node manager
categories:
  - compute
dates:
...
In this tutorial, you will learn how to use Socket.io with Express and create a simple chat server to see the basics of how a client and server work together.
...
3. Create an index.html file in the socket-app folder to display a `Hello World` message in the socket.
...
2. Update the `app.js` file.
  ...
  If we restart the app.js file we will be able to send messages to all other clients when we type anything into our input box and press submit. The div under the hello world message updates.
