...
Tink is an open source and vendor-agnostic toolkit allowing programmers to perform
...
that provides a Tink extension, as it's the case for Scaleway's Key Manager.
It's thus required to install Scaleway's Tink extension in addition to
Scaleway's SDK and Tink itself. Make sure you install the following dependencies:
...
<Message type="note">
In production code you might want to handle errors properly, but
we'll ignore this aspect to focus on concepts, here.
</Message>
...
We'll explain why we need `associatedData` later. For now, let's ignore it.

While this code works as intended, this is not a recommended pattern, and there
are several limitations:
 - It's slow: since the key resides on Scaleway's Key Manager, each encryption
...
with keysets containing only one key in this guide.

Tink can generate keys to be used with the desired algorithm and cipher. Here is
how we generate an AES256-GCM key that will be used as a DEK. We then
store it (encrypted) as a file.
...
needs it to do meaningful work with it, the keyset can be read back.
...
encrypted, but it's authenticated. That is, it must be the same when
...
It's also possible to use a unique DEK for each payload.
...
there exists no "one fits all" approach to make the right key hierarchy. It's
...
It's the context in which you use them that makes them DEKs or KEKs. We usually
assume that keys living in the Key Manager are only used to protect other keys, so they
are usually called "KEKs" with this asusmption in mind. As shown earlier, they can be
used to protect data directly (even if it's not recommended).
</Message>
