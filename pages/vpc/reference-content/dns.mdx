...
The service resolves both regular public records and internal VPC records. The `.internal` at the end of internal addresses is essential in enabling the resolver to distinguish between internal and external/public DNS. For this reason, you should always add `.internal` when addressing resources within your VPC.
...
The DNS service also resolves the following formats, which include the target VPC's name or ID. These formats are required to resolve a resource across a [VPC Peering](/vpc-peering/concepts/) connection:
