...
4. Configure your applications to send metrics and logs to your Cockpit [endpoints](/observability/cockpit/concepts/#endpoints), using your newly-created token for authentication.
    ...
    <Message type="tip">
      If you are using the Scaleway Loki API, you can `POST` through the `Authorization: Bearer <token>` header, or `POST` directly using the following endpoint: `https://api_key:YOUR_COCKPIT_API_KEY@logs.prd.obs.fr-par.scw.cloud/loki/api/v1/push`
    </Message>
    ...
    * Click **New** on the top right corner of your screen and **Import** to import your Grafana dashboard.
