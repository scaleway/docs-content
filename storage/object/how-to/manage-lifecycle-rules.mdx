...
In the Scaleway Console, you can either transfer objects to `GLACIER` class [manually](/storage/object/how-to/change-object-class/) or automatically with lifecycle rules. These rules also allow users to set expiration dates for objects and object versions.
...
6. Filter the objects you want the rule to be applied to by one or both of the following criteria:
    ...
    - **Tags**: comprised of a **key** and a **value** associated with the Key. These tags should be defined prior to the creation of the rule, for either a bucket or an object. You can add one or more tags to your filter. 
...
8. Click **-** or **+** to adjust the timeframe, or type in the exact amount. 
  <Message type="tip">
    ...
    Expiration rules work similarly, objects can be set to be permanently deleted after a given period of time.
