...
[RPNv2](/bare-metal/network/reference-content/configure-rpnv2/) can be configured on any operating system supporting VLANs. The feature is available for selected Dedibox servers.


## Configuring RPNv2 on Debian
...
2. Create another file for the vlan interface (assuming the VLAN ID is `3900`). The file is called `/etc/sysconfig/network-scripts/ifcfg-eth1.3900`:
    ...
    VLAN=YES
    ```
## Configuring RPNv2 on Rocky Linux 9
