...
During the beta phase, if you chose to enable encryption after database creation, you must make an enablement request via a [support ticket](https://console.scaleway.com/support).

Enabling encryption means that your database data will be copied into a new, encrypted block volume. This takes approximately 1h per 100GB of storage. When the volumes are switched, a few seconds of downtime is also expected towards the end of the copying process.
