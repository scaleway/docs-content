...
Serverless Containers support **HTTP/1.1** and **HTTP/2**. **HTTP/1.1** is enabled by default, but some services (e.g., gRPC) only support **http2**.
...
Although such access may be possible for debugging purposes, it will never be performed without the customer’s explicit authorization.
...
Serverless Containers supports Private Network for outgoing traffic (like accessing a database or an instance), but not yet for incoming traffic.

This feature is under development.

### How can I disable the public endpoint for Serverless Containers?

It is not possible to disable the default public endpoint for Serverless Containers. To restrict access to Serverless Containers, you can [make the resource private](/serverless-containers/how-to/secure-a-container/#restrict-access-to-your-containers).
