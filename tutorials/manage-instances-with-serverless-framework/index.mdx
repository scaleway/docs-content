...
For this example we will use the Scaleway SDK and the serverless framework. For more information please refer to the Python SDK Documentation and the [how-to on serverless framework](/compute/functions/api-cli/fun-uploading-with-serverless-framework/)
...
2. Install the Scaleway SDK using `pip`:
    ...
    ```
3. Edit the file `handler.py` in a text editor as follows. Then save the file and exit the editor. 
    ```py
