...
5. Copy the command to connect to a resource, and click **Activate**.
    ...
    You are redirected to your Public Gateway's **Overview** page, where SSH bastion is now activated. All the SSH keys in your [Project credentials](/iam/concepts/glossary/#api-key) at the time of activation are copied to the SSH bastion. The command to use to connect to resources behind the bastion is displayed. See the [SSH bastion documentation](/public-gateways/how-to/use-ssh-bastion/) for further help, including information about restricting connections via the **Allowed IPs** feature.