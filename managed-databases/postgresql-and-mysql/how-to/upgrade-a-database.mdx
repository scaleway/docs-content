...
<Message type="important">
  If you have a Database Instance that uses a local volume, and you want to upgrade to an Instance that uses Block volumes, you must [change the volume type](/managed-databases/postgresql-and-mysql/how-to/change-volume-type) to Block storage first. <br /><br />

  Similarly, if you have a new-generation Database Instance using a [Low Latency Block volume](/managed-databases/postgresql-and-mysql/concepts/#block-storage-low-latency) and want to switch to a first-generation Instance, you must first change the volume type to [Basic Block Storage](/managed-databases/postgresql-and-mysql/concepts/#basic-block-storage).
</Message>
