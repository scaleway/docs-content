---
meta:
  title: How to deploy a self-hosted Penpot Instance using the Docker InstantApp
  description: This page shows you how to deploy a self-hosted Penpot Instance using the Docker InstantApp
content:
  h1: How to ddeploy a self-hosted Penpot Instance using the Docker InstantApp
...
Penpot is the an open-source design and prototyping platform meant for cross-domain teams. It is completely web based and works with open web standards (SVG). Penpot can run in almost any modern browser, no matter of the underlying operating system.

In this tutorial you will learn how to deploy your own Penpot Instance using the Docker [InstantApp](https://www.scaleway.com/en/docs/tutorials/docker-instantapp/).

## Installing Penpot using Docker Compose 

1. In the Scaleway console click **Instances** in the **Compute** section of the navigation bar. Then [deploy](https://console.scaleway.com/instance/servers/create?area=fr-par-1&imageKey=c1b530d8-0ca0-45c4-80db-ba06608287b2&offerName=PLAY2-MICRO) a new Instance using the Docker InstantApp].
...
5. Set the `PENPOT_PUBLIC_URI` to your Penpot domain. Then save the file and exit the editor.
    ...
    ```
6. Update the APT package cache on your system to make sure to have the latest releases available.
    ```
    apt update
    ```
...
9. Create a Nginx configuration file for the reverse proxy:
    ```
    nano /etc/nginx/sites-available/penpot.configuration
    ```
...
1. Start Penpot using `docker-compose`:
    ```
    docker-compose -p penpot -f docker-compose.yaml up -d
    ```

2. Add a Penpot user by running the following command.
    ```
...
4. Enter using the credentials of the previously created user. The Penpot dashboard displays. You can start designing your assets now. 

    <Lightbox src="scaleway-penpot-dashboard.webp" alt="" />


Your Penpot Instance is ready now. For a more advanced configation of additional features, refer to the [official documentation](https://help.penpot.app/technical-guide/configuration/). 