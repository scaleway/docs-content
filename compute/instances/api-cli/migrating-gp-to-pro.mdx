...
This page explains how to migrate from a GP1 Instance with local storage to a PRO2 Instance with [Block Storage](/compute/instances/concepts/#block-volumes) using the [Scaleway APIs](https://developers.scaleway.com/en/) and the [Scaleway CLI](https://github.com/scaleway/scaleway-cli#scaleway-cli-v2). 
...
3. Type the following command to create a PRO2 Instance from the image created by the backup:

  `scw instance server create image=<imageID> zone=fr-par-2 type=PRO2-S`
...
Your PRO2 Instance is created and displays in the [Scaleway console](https://console.scaleway.com/) in the Instances dashboard.
