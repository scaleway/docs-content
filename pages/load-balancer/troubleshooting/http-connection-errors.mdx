...
## My Scaleway Load Balancer is dropping traffic during high load 

You may experience sudden and complete traffic outages on your Scaleway Load Balancer during traffic spikes, even though your backend services remain healthy. This can manifest as brief but full HTTP service disruptions (e.g., 10–15 minute outages), and DNS queries may only return one of your expected Load Balancer IPs during the incident.

### Cause 

The issue stems from hitting connection capacity limits on smaller Load Balancer offers. Note the [connection limits](/load-balancer/concepts/#maximum-connections) for different Load Balancer types. When this limit is reached, the Load Balancer can no longer accept new connections, leading to service degradation or complete unavailability until traffic subsides. 

### Solution 

Upgrade to a larger Load Balancer type to support higher connection loads. You can [resize your Load Balancer](/load-balancer/how-to/resize-lb/) via the Scaleway console at any time.
     
Remember also to [monitor your connection metrics](/load-balancer/how-to/monitor-lb-cockpit/) to anticipate scaling needs. Proactively resizing your Load Balancer prevents capacity-related outages, and ensures consistent availability during traffic peaks. See the [pricing page](https://www.scaleway.com/en/pricing/network/#load-balancer) for full details on different Load Balancer commerical types.
