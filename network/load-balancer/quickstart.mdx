---
layout: docs
title: Quickstart Load Balancer
description: This page shows you how to get started with Scaleway Load Balancer.
excerpt: |
  This page shows you how to get started with Scaleway Load Balancer.
totalTime: PT15M
steps:
  - step: TODO
    type: HowToStep
    url: https://www.scaleway.com/en/docs/network/load-balancer/quickstart
image: /assets/images/docs/scaleway.png
keywords: load balancer cloud computing, scaleway, network, quickstart
dates:
  validation: 2021-05-26
  posted: 2021-05-26
---

[Scaleway Load Balancers](/network/load-balancer/concepts/#load-balancer) are highly available and fully-managed instances which allow you to distribute the workload across your various services. They ensure the scaling of all your applications while securing their continuous availability, even in the event of heavy traffic. They are commonly used to improve the performance and reliability of web sites, applications, databases and other services across multiple servers.

<Message type="requirement">

- You have an account and are logged into the [Scaleway Console](https://console.scaleway.com) 

</Message>

## How to create a Load Balancer


1. Click **Load Balancer** in the **Network** section of the [Scaleway Console](https://console.scaleway.com) side menu. If you have not already created Load Balancer, the product creation page is displayed:
  <Lightbox src="assets/scaleway-lb-creation-page.png" />

2. Click **Create a Load Balancer**. The creation wizard displays.

3.  Complete the following steps in the wizard: 

    - Enter the Load Balancer **name** and **description**. Optionally, you can assign **tags** to help organize your Load Balancers.
    - Choose the **Availability Zone** in which your Load Balancer will be deployed geographically. Currently we provide the following Availability Zones:
      - `PAR1`: Paris 1, France.
      - `PAR2`: Paris 2, France (innovative and sustainable availability zone).
      - `AMS1`: Amsterdam, The Netherlands.
      - `WAW`: Warsaw, Poland.
      <Lightbox src="assets/scaleway-lb-az.png" />
    - Select a Load Balancer **type**.
    - Select an **IP address** that will be assigned to your Load Balancer. If left empty, a new IP address is allocated automatically.
    - Configure your [**Frontend**](/network/load-balancer/concepts/#frontends) and [**Backend**](/network/load-balancer/concepts/#backends).
      - **Frontend** rules include:
        - Frontend rule name
        - Port
      - **Backend** rules include:
        - Backend rule name
        - Protocol: TCP or HTTP
        - Port
        - Proxy
        - Health check type and Health check option.
        - Sticky session with the cookie name associated
        - Server IP

4. Click **Create a Load Balancer**.

## How to add frontends

1. Click **Load Balancer** in the **Network** section of the [Scaleway Console](https://console.scaleway.com) side menu. The Load Balancer dashboard displays. 

2. Click the Load Balancer you want to add frontends to. The Load Balancer information page displays. 

3. Click the **Frontends** tab.

4. Click **Add Frontend**.
  <Lightbox src="assets/scaleway-add-frontends.png" />

5. Enter a **name**, **port** and **protocol** for your new frontend. 

6. Choose between **creating a new backend** or **using an existing backend**.
    - If creating a new backend, enter a name, port, protocol, proxy, health check type, sticky session configuration and server IP
    - If using an existing backend,  click **Use an existing Backend** and select the backend of your choice from the dropdown list.

7. Click **Create Frontend**.

## How to add backends

1. Click **Load Balancer** in the **Network** section of the [Scaleway Console](https://console.scaleway.com) side menu. The Load Balancer dashboard displays. 

2. Click the Load Balancer you want to add backends to. The Load Balancer information page displays. 

3. Click the **Backends** tab.

4. Click **Add Backend**.
  <Lightbox src="assets/scaleway-add-backends.png" />

5. Enter a name, port, protocol, proxy, health check type, sticky session configuration and server IP for your new backend. 

6. Click **Create Backend**

## How to delete a Load Balancer

1. Click **Load Balancer** in the **Network** section of the [Scaleway Console](https://console.scaleway.com) side menu. The Load Balancer dashboard displays. 

2. Click the Load Balancer you want to delete.

3. Scroll down and click **Delete Load Balancer**. 
   <Lightbox src="assets/scaleway-lb-delete.png" />

  <Message type="important">

  This will permanently delete your Load Balancer and all your data will be lost. This action is irreversible.

  </Message>

4. Type **DELETE** in the pop-up box.

5. Tick the checkbox to delete the associated IP address.

6. Click **Delete this Load Balancer**.
<Lightbox src="assets/scaleway-confirm-deletion.png" />
