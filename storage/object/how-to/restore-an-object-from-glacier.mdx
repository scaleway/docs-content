...
5. Click **Restore object from S3 Glacier**.
    <Message type="note">
      Your object remains available in `STANDARD` class for the duration you chose during the restoring process. It will be transferred automatically back to `GLACIER` once the configured period is over.
    </Message>
