...
MLX, an array framework specifically designed for effective and versatile machine learning on Apple silicon, developed by Apple's machine learning research team.

MLX is crafted by machine learning researchers with a focus on catering to their peers, emphasizing a balance between user-friendliness and efficiency in model training and deployment. The framework boasts a deliberately straightforward design, aimed at facilitating seamless extension and enhancement by researchers, fostering the swift exploration of innovative ideas.

In terms of APIs, the Python interface of MLX closely emulates NumPy, incorporating a few distinctive features. Additionally, MLX offers a robust C++ API that harmonizes closely with its Python counterpart.

Taking inspiration from frameworks such as PyTorch, Jax, and ArrayFire, MLX takes a unique path with its unified memory model. Within MLX, arrays reside in shared memory, allowing operations on MLX arrays across diverse supported devices without the need for data copies. This departure marks a notable distinction from other frameworks.

Features of MLX include:

* **Familiar APIs:** MLX provides a Python API closely aligned with NumPy, complemented by a comprehensive C++ API closely mirroring its Python counterpart. Higher-level packages like `mlx.nn` and `mlx.optimizers` closely follwoing PyTorch, simplifying the construction of intricate models.
...
* **Lazy Computation:** Within MLX, computations adopt a lazy approach, materializing arrays only when the need arises.
...
* **Multi-device Support:** MLX operations are versatile, capable of running on supported devices, currently spanning the CPU and GPU.
...
<Message type="requirement">
  ...
  - You have [created an Mac mini](/bare-metal/apple-silicon/how-to/create-mac-mini/)
  - You have native Python >= 3.8 installed on the Mac (preinstalled by default)
</Message> 

<Message type="note">
    MLX is only available on devices running MacOS >= 13.3 It is highly recommended to use MacOS 14 (Sonoma) or later.
</Message>
...
- For a complete documentation about MLX, refer to the [official MLX documenation](https://ml-explore.github.io/mlx/build/html/index.html).
- The [MLX examples repository](https://github.com/ml-explore/mlx-examples) hosts a diverse collection of examples, including:
    ...
    - Large-scale text generation using [LLaMA](https://github.com/ml-explore/mlx-examples/tree/main/llama) and subsequent finetuning with [LoRA](https://github.com/ml-explore/mlx-examples/tree/main/lora).
