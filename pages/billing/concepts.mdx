...
The Billing Alerts functionality within the [Scaleway console](https://console.scaleway.com) allows users to oversee and control their expenses efficiently. This feature allows you to create alerts that notify [users](/iam/concepts/glossary/#user) when predefined budget thresholds are met. Your alerts can be seamlessly delivered through SMS, email, or API webhook channels.
...
Consists of information about the Organization that will be billed. They will appear on the invoice and can be modified on the Scaleway console. Find out more about [how to read an invoice](/billing/faq/#how-do-i-read-my-invoice).
...
<Message type="note">
  You can give users permissions via [policies](/iam/concepts/glossary/#policy), or by adding them to a [group](/iam/concepts/glossary/#group) already attached to a relevant policy.
</Message>

## KYC

**K**now **Y**our **C**ustomer is the process of verifying a client's identity. An ID document such as a passport, driver's license or identity card is required to [complete KYC verification](/account/setup-lifecycle/verify-identity/).
...
A Project is a grouping of Scaleway [resources](/iam/concepts/glossary/#resource). Each Scaleway Organization comes with a default Project, and you can create new Projects if necessary. Projects are cross-regional, meaning resources located in different [regions](/instances/concepts/#region) can be grouped into one single Project. When grouping resources into different Projects, you can use [IAM](/iam/concepts/glossary/#iam) to define custom access rights for each Project.
