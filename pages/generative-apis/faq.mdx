...
### How long does a batch take to be processed (using Batches API)? How can I optimize this time?
We aim to process any batch in at most `24` hours. Past this delay, batch processing will be stopped and any queries remaining to be processed will not be billed.
Batches are processed in their order of creation. You can optimize time before receiving a batch output by splitting one batch into multiple smaller ones.
For example:
- Assuming a batch of `10 000` requests will take `10` hours to be processed.
- Splitting this batch in `10` batches of `1 000` requests each will take the same time, eg. `10` hours to process all batches. However, the first batch output will be provided after `1` hour, the second one after `2` hours, etc.
...
- Using [Batches API](https://console.scaleway.com/generative-api/batches) for non-real time workloads. Requests performed through Batches API do not have rate limit and are billed with a [-50% discount compared to standard model prices](https://www.scaleway.com/en/pricing/model-as-a-service/#generative-apis).
