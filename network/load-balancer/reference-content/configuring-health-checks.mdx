...
Health checks are available in many different types. By default, the heath check type is set to be the same as that of the [backend protocol](/network/load-balancer/reference-content/configuring-backends/#protcol-and-port), that is to say **TLS** or **HTTP**. However, this can be overridden. The following health check types are available:
...
If you select HTTP protocol for your healthcheck, you have the choice to activate or deactivate TLS. 

This setting is inherited from the [backend configuration](/network/load-balancer/reference-content/configuring-backends/#tls-encryption), but can be overriden for health checks if you wish. Activating TLS encrypts connections between the Load Balancer and the backend server(s) during health checks. Note that the backend server should have its own SSL/TLS certificate.
...
This setting is inherited from the [backend configuration](/network/load-balancer/reference-content/configuring-backends/#verify-certificate) and cannot be overriden. If you chose HTTP as the protocol for the Load Balancer's backend and activated TLS, the Verify Certificate setting you chose there is inherited for health checks. If you chose a different protocol for the Load Balancer's backend or didn't activate TLS, the backend server's certificate will **not** be verified during health checks and you do not have the option to activate the setting.
...
The proxy protocol activation setting for health checks is inherited from the [backend configuration](/network/load-balancer/reference-content/configuring-backends/#proxy-protocol) but you can choose to override it.
