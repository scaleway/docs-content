...
<Tabs id="simple_function_call">
    ...
    # Inspect tool calls
    print(response.output[0].content[0].tool_calls)
    ```
    ...
    ```bash
    [ToolCall(
        id='resp-tool-81e63f4f496d429ba9ec6efcff6a86e1',
        type='function',
        function=ToolCallFunction(
            name='get_flight_schedule',
            arguments='{"departure_airport": "CDG", "destination_airport": "LHR", "departure_date": "2024-11-01"}'
        )
    )]
    ```
    </TabsTab>
...
<Tabs id="call_tool">
    ...
    messages = [
        {"role": "user", "content": "What time is the next flight to Paris?"}
    ]
    ...
    if tool_calls:
        ...
        if tool_call.function.name == "get_flight_schedule":
            ...
            # Provide tool output and get final model response
            final_response = client.responses.create(
                model="gpt-4.1",
                input=messages + [
                    response,  # include assistant's tool request
                    {
                        "role": "tool",
                        "tool_call_id": tool_call.id,
                        "name": tool_call.function.name,
