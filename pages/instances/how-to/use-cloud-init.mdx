...
<Tabs>
  <TabsTab label="Console">
    ...
    2. In the list of all Instances, spot the one for which you want to update **user-data**. Click on its **Name** or, at the end of the line, on <Icon name="seeMore" /> then **More info**. The Overview page for that Instance displays.
    3. Click the **Settings** tab.
    4. Scroll down to the **Cloud-init** section.
    5. The <Icon name="toggle" /> icon next to **Use cloud-init** should already be slid to the right, meaning that **user-data** are already set for this Instance.
    6. Edit the contents of the text fields to suit your needs.
    7. Click **Save**.
    8. Start your Instance by clicking **Actions** then **Power on**.
        <Message type="note">
...
- Providing the name of a **meta-data** field as an argument to `scw-metadata` will output only the value of this specific field:
    ...
    my-instance-name
    ```
- You can avoid spending a network roundtrip to the Metadata API by querying previously cached values, if any:
    ```
    $ scw-metadata --cached
    ...
    NAME=my-instance-name
