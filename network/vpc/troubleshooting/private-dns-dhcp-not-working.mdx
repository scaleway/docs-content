...
content:
  ...
  paragraph: This page helps you troubleshoot problems when Ppivate DNS and/or DHCP are not working
...
It does **not** work out of the box for Linxu distributions using Network Manager, such as **RockyLinux**. If your resource is running RockyLinux, or another distribution using Network Manager, you should manually configure the system to use the nameserver `169.254.169.254`. You will then be able to resolve the name of your resources, as well as public domain names.

## DHCP is not working

DHCP should work out of the box for attached resources running any and all Linux distributions. However, when it comes to **ArchLinux**, we are aware of a probblem when it comes to setting a Classless Static Route to our service IP address `169.254.169.254`.
