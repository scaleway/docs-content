...
3. Open a terminal and type the following command to connect to your Instance:
            ...
            ```
4. Open another terminal and type the following command to clone our workshop repository on your local machine:

            ```bash
            git clone git@gitlab.infra.online.net:observability-paas/workshop.git
            ```
5. In the same terminal as step 4, run the following command to send the `workshop` folder into your Instance:
...
6. Add your token's secret key next to `X-Token` in the `metrics` section.
...
8. Add your token's secret key next to `tenant_id` in the `logs` section.
     <Lightbox src="scaleway-logs-section-completed.webp" alt="" />
9. Press `escape`, `:wq` and `Enter` to save and exit the file.
10. Run the following command to launch the Grafana agent:
        ```bash
        ./run.sh
        ```
10. Log in to the Scaleway console and click **Cockpit** in the side menu.
11. Click **Open dashboards** at the top right corner of your screen.
12. Log in to Grafana using your credentials.
13. Click the downward pointing arrow at the top right corner of your screen and select **Import dashboard**.
    <Lightbox src="scaleway-import-dashboard.webp" alt="" />
14. Paste the following dashboard ID `12486`in the **Import via grafana.com** section and click **Load**. Alternatively, download the [dashboard's JSON template](https://grafana.com/grafana/dashboards/12486-node-exporter-full/) and upload it in the **Import dasboard** section.
15. Click **Import**.
16. Configure your dashboard as you please.
17. Select **Scaleway metrics** in the drop-down box and click **Import**.
...
13. Click **Explore** and make sure your logs display as well.
