...
1. Elasticsearch uses a secure HTTPS transaction for its repositories. If not installed yet, install the `apt-transport-https` package use the `apt` package manager:
    ```
...
1. Type the customized URL into your browser.
  ...
  Magento 2.4 sets up multifactor authentication by default. To make your first log in to the admin interface seamless, we will disable MFA via command line. You can enable it on the admin interface at a later time. 

2. Disable MFA with the following command:
  ```
