...
In these dashboards, you can see consumption by IAM principal (user or applications) by filtering through IAM principal label, or opening `Usage details per IAM Principal ID` panel. Note that for low consumption volumes (less than a few million tokens per hour), the exact value displayed on long time ranges (such as a few days), may be inaccurate by several percent. This is a known limitation of Grafana queries using PromQL for discrete values: exact consumption is stored in metrics, but aggregated values in graphs are extrapolated based on instant consumption from several samples. 
To limit this impact you can either:
- ensure you select a narrow timeframe (such as 1 hour) to ensure enough samples exist in this time range to reconstruct an accurate consumption graph
- perform queries using PromQL on the datasource directly
...
- To ensure fixed billing, you can use [Managed Inference](https://www.scaleway.com/en/inference/), which provides the same set of OpenAI-compatible APIs and a wider range of models.
...
Generative APIs targets a 99.9% monthly availability rate, as detailed in [Service Level Agreement for Generative APIs](https://www.scaleway.com/en/generative-apis/sla/).
...
### How long does a batch take to be processed using Batches API, and how do I optimize this time?
...
- Using [Batches API](https://console.scaleway.com/generative-api/batches) for non-real time workloads. Requests performed through Batches API do not have a rate limit and are billed with a [-50% discount compared to standard model prices](https://www.scaleway.com/en/pricing/model-as-a-service/#generative-apis).
