...
<Message type="note">
- If no class is specified via the `--storage-class <class>` option, the object is created as Standard (Multi-AZ) by default.
- [Conditional writes](/object-storage/api-cli/using-conditional-writes/) using the `If-Match` and `If-None-Match` headers are supported.
</Message>
