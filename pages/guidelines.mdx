...
import image2 from './assets/scaleway-console-side-menu.webp'
import image3 from './assets/scaleway-org-dropdown.webp'
import image4 from './assets/scaleway-console-dashboard.webp'
import image5 from './assets/scaleway-instances-creation-page.webp'
import image6 from './assets/scaleway-creation-wizard.webp'
import image8 from './assets/scaleway-screenshot-annotation.webp'


This document shows you how to edit content from the Scaleway documentation website, and sets out the writing guidelines (style guide) for Scaleway Documentation, including grammar, formatting, and standardized terminology. These guidelines should be followed by both internal and external contributors when making additions to the Scaleway Documentation site.

- For details on contributing to the Scaleway documentation as a user, see [documentation guidelines](/guidelines/).
...
Follow this guided tour to discover how to suggest edits to the documentation website.
<GuideFlow src="https://app.guideflow.com/embed/ok8qdq7fqk"/>
...
| Page type | All product documentation pages should conform to one of the [page types listed above](#page-types) | E.g. `how-to`, `api-cli`, `concepts`. |
...
4. Ensure that your file structure is in line with the following image:
    <Lightbox image='scaleway-file-structure.webp' />
5. Click **Launch**
````
</Message>
...
1. In the Scaleway console side menu, go to **Compute** > **CPU & GPU Instances**. The **Instances** dashboard opens.
2. Click the Instance whose flexible IP address you want to detach. The **Overview page** for that Instance opens.
...
Avoid using link text such as `here` or `this page`. Prefer descriptive terms, while keeping link text concise. Internal links are always introduced by `see`.

| ✅ GOOD LINK TEXT                                | ❌ BAD LINK TEXT                                |
| ------------------------------------------ | ----------------------------------------------- |
| For more information, see the [pricing page](https://www.scaleway.com/en/pricing/)  | Find more information [here](https://www.scaleway.com/en/pricing/) |
...
| ------------------------------------------ | ----------------------------------------------- |
|  ````See the [VPC Quickstart](/vpc/quickstart/)````  | ````Refer to the [VPC Quickstart](/vpc/quickstart/)```` <br/> <br/> ````Refer to the [VPC Quickstart](/en/docs/vpc/quickstart/)```` <br/> <br/> ````Refer to the [VPC Quickstart](network/vpc/quickstart)```` |
...
| ------------------------------------------ | ----------------------------------------------- |
|  Anchor is on same page as link: ````Learn more about [InstantApps](#instantapp)````  | ````Learn more about [InstantApps](https://www.scaleway.com/en/docs/instances/concepts/#instantapp/)```` |
| Anchor is on different page : ````Learn more about [InstantApps](/instances/concepts/#instantapp)```` | ````Learn more about [InstantApps](/#instantapp/)````  |
...
See an example of the tabs component in action on the [How to connect to an Instance](/instances/how-to/connect-to-instance/) page.
...
When addressing the user, say "you". When referring to Scaleway, say "Scaleway" (not "we" or "our").

| ✅ DO WRITE                                | ❌ DO NOT WRITE                                 |
| ------------------------------------------ | ----------------------------------------------- |
| You can delete the Instance at any time.   | The user can delete the Instance at any time.   |
| Scaleway offers numerous Instance types.   | We offer numerous Instance types.     |

### Possessive

Do not use possessives with product names, technical terms, or user roles. Express possession by rephrasing with prepositions, using nouns as descriptors, or addressing the reader directly.

| ✅ DO WRITE                                | ❌ DO NOT WRITE                                 |
| ------------------------------------------ | ----------------------------------------------- |
| The Scaleway console                       | Scaleway's console                              |
| The IP address of the instance             | The instance's IP address                      |
| Your credentials                           | The user's credentials                          |

### Capitalization

Always capitalize Scaleway product names (e.g. Instance, Load Balancer, Kubernetes Kapsule, Private Networks).

Do not capitalize names of product features (e.g. security group, snapshot) nor resources (e.g., bucket), **unless** the feature name is a created word or phrase with a meaning specific to Scaleway, (e.g. InstantApp, Easy Deploy).

Titles and headings should be written in sentence case.

### Bold text

Use bold for console UI elements the user must click, select, or find.

Examples: 
✅ Click **Create Instance** to proceed.
✅ The **Instances** dashboard opens.

Do not use bold for emphasis in running prose.
...
- Use numerals for numbers (e.g. "A maximum of 9 backend servers").
...
| ✅ DO WRITE                                | ❌ DO NOT WRITE                                 |
| ------------------------------------------ | ----------------------------------------------- |
| You can create up to 9 snapshots           | You can create up to nine snapshots.               |
...
For dates, use `Month Day, Year` format for optimized readability. Abbreviated format with a 3-letter abbreviation for the month is also accepted when content needs to fit in a tight space (e.g., table).

| ✅ Write         | ❌ Avoid                |
| :--------------- | :--------------------- |
| January 12, 2026 | 12/01/26 or 01/12/2026 |
| Jan 12, 2026     | 12/01/26 or 01/12/2026 |
| January 2026     | 01/2026                |

Avoid ordinals, e.g. `14th January`.

In metadata, always use the ISO format: `YYYY-MM-DD`.

### Times

For times, use the 24-hour format. If it is necessary to use the 12-hour format, add `a.m.` or `p.m.`.
...
All images must be in `webp` format, and stored in the page's `assets` folder. Image names must always start with `scaleway`, and the words should be hyphen-separated.
...
<Lightbox image={image8} alt="A screenshot of a filter toolbar from the Scaleway console, with different dropdowns and aspects of the toolbar annotated with purple circles with white numbers in them" />
...
|-------------------------------|-----------------|
|`<Requirements />`  | ## Before you start <br/><br/> To complete the actions presented below, you must have: |
...
| DO WRITE ✅ | DO NOT WRITE ❌|
|-------------|----------------|
| ## Before you start <br/><br/> To complete the actions presented below, you must have: <br/><br/>- A Scaleway account logged in to the [console](https://console.scaleway.com) <br/>- An [SSH key](/iam/security/create-ssh-key/)<br/>- A valid [API key](/iam/credentials/create-api-keys/) | To complete the actions presented below, you must have: <br/><br/>- You have a Scaleway account<br/>- Have an SSH key<br/> -SSH key |
...
|`- [Owner](/iam/concepts/glossary/#owner) status or [IAM permissions](/iam/concepts/glossary/#permission) allowing you to perform actions in the intended Organization` | - [Owner](/iam/concepts/glossary/#owner) status or [IAM permissions](/iam/concepts/glossary/#permission) allowing you to perform actions in the intended Organization |
|`- An [SSH key](/iam/security/create-ssh-key/)` | - An [SSH key](/iam/security/create-ssh-key/) |
|`- A valid [API key](/iam/credentials/create-api-keys/)` | - A valid [API key](/iam/credentials/create-api-keys/) |
