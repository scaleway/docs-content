...
This quickstart guide is divded into two sections, depending on the protocol you want to use with Messaging and Queuing (SQS/SNS, or NATS).
...
3. Click **Generate credentials**. A pop-up displays.
    <Lightbox src="scaleway-sqssns-credentials.webp" alt="" /> TODO CHECK POPUP
...
5. Copy and save the credentials, before closing the window.
    ...
    <Lightbox src="scaleway-sqs-credentials-panel.webp" alt="" /> TODO CHECK
...
2. Click **SQS**, then **Create queue**. TODO CHECK FLOW.

3. Click the green `+` button to create a new queue. The queue creation wizard displays.
    <Lightbox src="scaleway-create-queue-wizard.webp" alt="" /> TODOC CHECK SCREENSHOT
...
All further actions related to publishing, processing and managing messages, subjects, queues and streams can be done via a supported CLI or SDK. See our documentation on the [SQS/SNS CLI and SDKs](/serverless/messaging/reference-content/snssqs-overview#further-actions) for more information.
...
4. In the **Credentials** panel, click **Generate credentials**. A pop-up displays.
    <Lightbox src="scaleway-nats-credentials.webp" alt="" /> TODO CHECK SCREENSHOT
...
6. Click **Download** and save the file, before closing the window.
    ...
    You are returned to your namespace's overview page. The name of your newly-generated credentials and their access level is displayed in the **Credentials** panem.

    <Lightbox src="scaleway-credentials-list.webp" alt="" /> TODO CHECK SCREENSHOTS
