...
```powershell
winget install -e --id Scaleway.cli
```
...
5. The Scaleway CLI will now check if an SSH key is present in your local machine in the `id_ecdsa.pub`, `id_ed25519.pub`, and `id_rsa.pub` files. 
    ...
    Otherwise, the Scaleway CLI will either confirm that the key is already present in your Scaleway Project, or warn you that no key were found. Refer to the [dedicated documentation](/iam/security/create-ssh-key/) to create an SSH key.
