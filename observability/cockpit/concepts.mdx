...
A Cockpit is an instance of the Observability product that stores metrics, logs, and traces and provides a dedicated dashboarding system on Grafana to visualize them. A Scaleway Project can have only one Cockpit.
...
Data sources are backends that allow you to store and fetch your metrics, logs, and traces. By default data sources are enabled if you are using [Scaleway resources integrated with Cockpit](/observability/cockpit/reference-content/cockpit-limitations/#product-integration-into-cockpit) on your Project.
You can create additional external data sources to store and query metrics, logs, and traces, from your own resources.

## Data types

Data types refer to the categories of data you can collect for monitoring and observability. There are three data types:

- [Metrics](#metrics) used for performance monitoring
- [Logs](#logs) used for event and error monitoring
- [Traces](#traces) used for request-behavior-monitoring

Each data type provides unique insights to optimize app health and user experience
...
You can decide in which region to enable the [alert manager](#alert-manager) and your [managed alerts](#managed-alerts). You can also choose the region in which to create your external [data sources](#data-sources).
