...
**These were a temporary, automatic behaviors which have now been lifted thanks to further integration of Scaleway's [IPAM](/network/vpc/concepts/#ipam) with Public Gateways*. A VPC are no longer auto-created for each Public Gateway, and when you attach a Public Gateway to a Private Network, the Private Network will stay in its original VPC.
