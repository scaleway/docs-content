...
<Message type="requirement">
  ...
  - You have generated an [SSH key](https://www.scaleway.com/en/docs/identity-and-access-management/organizations-and-projects/how-to/create-ssh-key/#how-to-upload-the-public-ssh-key-to-the-scaleway-interface)
</Message>
...
The focus on setting up SNS with maximum automisation, therefore all the infrastructure is handled with Terraform.
...
First, we set up Terraform so we can use it to define our infrastructure.

1. Create a directory called `terraform`.
2. Inside this directory, create a file named `variables.tf`. We need this to define a `public_ssh_key` variable, used to access the Instances we will create later:
    ```
...
2. Create the SSH key that will be used in `sns_tutorial` project to access the instances:
...
Continue to add code to your `main.tf` file as shown below in order to create the SNS topic.
...
2. Create the credentials for SNS:
    ...
    ```
3. Finalise the SNS set up by creating the topic:
    ```
...
2. Create the subscriber Instance itself, with all the tools it will need when it starts up:
  ```
...
3. Create a `terraform_data` resource to wait for the port 8081 to be opened on the subscriber Instance before subscribing to the topic we previously created:
  ```
...
4. Create the SNS topic subscription with the subscriber Instance's endpoint:
  ```
...
5. Create the publisher Instance's IP address:
  ```
...
6. Create the publisher Instance itself, with all the environment variables it will need to publish the notifications to the topic:
  ```
...
7. Check that your `main.tf` file looks like the fully completed example shown on the [GitHub repository](https://github.com/scaleway/serverless-examples/tree/main/mnq/sns-x-instances)



# Run and test your setup

We are now ready to run the command to create all of the infrastructure and resources we have defined in our Terraform files, and carry out the additional steps necessary to test that the publisher is indeed pushing alerts which are being received by the subscriber. The web applications that we defined for each Instance (downloaded from a Docker image) allow us to easily view and check what is going on with each Instance.
1. Run the command `terraform apply`. You are prompted to enter your public SSH key. Execute in your terminal `cat ~/.ssh/`, select with `<tab>` the `*.pub`, paste the result on your local machine to easily access this key, and enter `yes`
...
3. In a browser, go to home page of your subscriber Instance by inserting the IP address copied in the previous step into the following address: ```http://<your_subscriber_ip_address>:8081```.
4. Still in the browser, click `Confirm subscription` to confirm tue subscription to the SNS topic we created. If you have an error because the URL hasn't been received, you can reload the page. It should take less than 10 seconds to appear.
...
7. Go on the home page of your publisher server: ```http://<your_publisher_ip_address>:8081```
<Lightbox src= "publisher-home.webp" alt=""/>
