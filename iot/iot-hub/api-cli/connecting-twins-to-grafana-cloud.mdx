...
Let us consider we want to trace:
- any write on the `state` (even when setting the same value twice)
- changes on the `data` part of the document
- we add a tag to those values in order to recover them easily in grafana.
...
<Navigation title="See Also">
  <PreviousButton to="/iot/iot-hub/api-cli/using-twins/">How to use Cloud Twins</PreviousButton>
