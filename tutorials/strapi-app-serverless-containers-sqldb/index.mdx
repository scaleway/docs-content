...
1. Run the following command to create a [Serverless SQL Database](/sql-databases/reference-content/serverless-sql-databases-overview/) and export its attributes as variables:
...
7. Check that your container runs locally:
    ...
    <Message type="tip">
    The first run of this container will raise a database constraints error such as `error: YupValidationError: This attribute must be unique`, which will not alter any data. This occurs because we use the same [Serverless SQL Database](/sql-databases/) for development and production environments. Hence, initial sample data loading happens twice, once in `NODE_ENV=development` when running `npm run develop` initially and once in `NODE_ENV=production` when running `docker build -t my-strapi-blog .` The second loading triggers errors as data already exists, but will not happen again afterwards for these environments.
    </Message>
...
1. Run the following command to create a [Serverless Containers namespace](/containers/concepts/#namespace) and export its ID as a variable:
    ```bash
...
- **Fine-tune deployment options** such as autoscaling, targeted regions, and more. You can find more information by typing `scw container deploy --help` in your terminal, or by referring to the [dedicated documentation](/containers/how-to/manage-a-container/).
...
- A [Serverless Containers namespace](/containers/concepts/#namespace), that contains a [Serverless Container](/containers/quickstart/), which hosts your Strapi application

- A [Serverless SQL Database](/sql-databases/reference-content/serverless-sql-databases-overview/) which stores posts data
