...
The Handler name is a path to the handler file, suffixed with the function name to use as a handler. In the following example, we use the handler file `handler.rs` inside the `src` folder.
  ...
  ```
1. Make sure to provide the right handler name, namely `src/handler.Handle`
2. You can also specify the file to be executed in your `Cargo.toml`file
  ```toml
    [lib]
    path = "src/handler.rs"
   ```
   in that case you can set the handle as `Handle`
