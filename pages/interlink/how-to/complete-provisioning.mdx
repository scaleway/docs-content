...
import image from './assets/scaleway-hosted-interlink-ordering.webp'
...
2. Click on the InterLink that you want to finish provisioning. The InterLink's **Overview** page displays.
    ...
    <Lightbox image={image2} alt="A screen in the Scaleway console shows an overview of an InterLink's partner, location, region, bandwidth, creation date, pairing key, and IPv4/IPv6 connectivity status." /> TODO UPDATE FROM CONSOLE
...
<Lightbox image={image3} alt="A screen in the Scaleway console shows a listing of InterLinks, with the following information: name, status, partner, location, region and bandwidth." /> TODO UPDATE
