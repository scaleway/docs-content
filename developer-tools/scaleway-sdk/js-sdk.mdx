...
To use the SDK, you need an access key and a secret key for the Scaleway API. Refer to Scaleway's [documentation on API keys](/identity-and-access-management/iam/how-to/create-api-keys/) for instructions on creating and retrieving these keys.
...
2. Scaleway also provides exports of **errors** under the `Errors` namespace, which helps differentiating various error cases, like `Errors.PermissionsDeniedError`.
