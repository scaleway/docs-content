...
10. Create the `carpcontrol.sh` script by clicking on **Diagnostics** > **Edit file** on the first firewall. **Enter** the path `/root/carpcontrol.sh in placeholder.sh` in the form, paste the content and click **Save**.
    ...
    # API Token (Obtain the token at: https://account.scaleway.com/?service=dediboxen/api/access)
    ...
    > `API_TOKEN`: The Scaleway Dedibox API [private token](https://account.scaleway.com/?service=dediboxfr/api/access) > `PF1_IP`: Public IP of the first firewall
    > `PF2_IP`: Public IP of the second firewall
    > `PF1_IPFO`: [failover IP](https://account.scaleway.com/?service=dediboxfr/server/failover) assigned on the first firewall
