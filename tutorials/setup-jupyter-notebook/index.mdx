...
tags: compute gpu jupyter notebook 
excerpt: |
  Jupyter Notebook is a client-server application that allows to edit and run Notebook-documents in a web browser.
totalTime: PT15M
steps:
  - step: Setting up a remote Jupyter Notebook on a GPU instance
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-jupyter-notebook#-Setting-up-a-remote-Jupyter-Notebook-on-a-GPU-instance
  - step: Connecting to the Remote Jupyter Notebook from a Linux / MacOS client
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-jupyter-notebook#-Connecting-to-the-Remote-Jupyter-Notebook-from-a-LinuxMacOS-client
  - step: Connecting to the Remote Jupyter Notebook from a Windows client
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-jupyter-notebook#-Connecting-to-the-Remote-Jupyter-Notebook-from-a-Windows-client
  - step: Setting a Password for Jupyter Notebook
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-jupyter-notebook#-Creating-a-Password-for-Jupyter-Notebook
  - step: Using Jupyter Notebook
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-jupyter-notebook#-Using-Jupyter-Notebook
categories:
  - compute
  - gpu-instances
  - instances
image: /assets/scaleway_jupyter.png
permalink: docs/tutorials/setup-jupyter-notebook
keywords: cloud tutorials, cloud documentations, cloud server, cloud hosting, ubuntu server, debian server, linux server, AwStats, Apache, mail server statistics, ftp
dates:
...
2. [Log yourself](/compute/instances/how-to/connect-to-instance/) into the instance using SSH.
...
2. Open a web browser and go to `http://localhost:7777`. The Jupyter web interface should display:
  ...
  <Lightbox src="scaleway-login_mac.png" alt="" />
...
2. In the menu bar on the left, click **Connection** > **SSH** > **Tunnels**. Then add a new forwarded port:
  ...
  <Lightbox src="scaleway-putty.png" alt="" />
...
4. Keep the PuttY window open in the background and launch a web browser. Go to `http://localhost:7777` to connect to the Jupyter Notebook:
  ...
  <Lightbox src="scaleway-login_win.png" alt="" />
...
<Lightbox src="scaleway-token.png" alt="" />
...
3. Scroll down to the password section of the page and paste the token into the corresponding form. Then enter a safe password and click **Log in and set new password** to take the changes into effect:
  ...
  <Lightbox src="scaleway-setup_password.png" alt="" />
...
3. The Jupyter Editor displays. The first _cell_ is already in place and uses the kernel (Python3) that was choosen when starting the notebook. Write your Python 3 code in the cell. To run the content of a cell click **Play**:
  ...
  <Lightbox src="scaleway-jupyter_editor.png" alt="" />
