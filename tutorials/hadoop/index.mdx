...
1. Visit the [Apache Hadoop Releases page](http://hadoop.apache.org/releases.html) to select the most recent stable release. We will install Hadoop version 3.3.1
...
3. Use `wget` to fetch it:
  ...
  ```
  wget http://mirrors.standaloneinstaller.com/apache/hadoop/common/hadoop-3.3.1/hadoop-3.3.1.tar.gz
  ```
...
4. Copy the link displayed in the browser and use `wget` on our server to download the file.
  ...
  ```
  wget https://dist.apache.org/repos/dist/release/hadoop/common/hadoop-3.3.1/hadoop-3.3.1.tar.gz.mds
  ```

5. Run the following verification:

  ```
  shasum -a 256 hadoop-3.3.1.tar.gz
  ```
  ```
  f837fe260587f71629aad1f4fb6719274e948111dc96ffc5a8e26f27deac5602  hadoop-3.3.1.tar.gz
  ```

6. Compare this value with the SHA-256 value in the .mds file:

  ```
  cat hadoop-3.3.1.tar.gz.mds
  ```
