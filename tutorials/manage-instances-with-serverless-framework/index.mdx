---
meta:
  title: Managing compute Instances with the serverless framework
...
tags: serverless instances python
categories:
  - compute
  - serverless
dates:
...
In this tutorial you will discover some examples of Instances automation using Python and the serverless framework:
...
<Message type="requirement">
  ...
  - You have Python pip installed on your machine

</Message>

## Automatically backup your Instance 

For this example we will use the Scaleway SDK and the serverless framework, for more information please refer to Python SDK Documentation and the [how-to on serverless framework](/compute/functions/api-cli/fun-uploading-with-serverless-framework/)
...
2. Install the Scaleway SDK using `pip`:
    ...
    ```
3. Edit the fily `handler.py` in a text editor. Then save the file and exit the editor. 
    ```py
    ...
    ```
4. Edit your YAML file and add the environmental variable and the backup frequency.
    ```yaml
    functions:
    instancebackup:
    ...
    events:
    - schedule:
        # Specify your frequency using CRON Job(UNIX Format)
        rate: '1 * * * *'
...
For this example we will use the native python library `urllib`, which enables us to write the code down directly into the Scaleway console.
...
2. Edit the file `handler.py` as follows:
    ```py
    import os
