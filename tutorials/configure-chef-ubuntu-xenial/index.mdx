...
content:
  h1: How to use and configure Chef on Ubuntu Xenial
  paragraph: This page explains how to use and configure Chef on Ubuntu Xenial
tags: Instances Chef Ubuntu-Xenial 
steps:
  - step: Core Concepts
    type: HowToStep
...
Chef is a configuration management tool that is written in Ruby and Erlang. It's capable of managing both your on-premise machines and your Scaleway Instances with ease.
...
You can easily manage up to 10000 nodes using Chef. Chef also makes it easy to automate the replication of infrastructure components.
...
- You have a second machine (this could be a local machine or a second Instance) on which to install Chef SDK
...
- **Chef Server:** This central server holds all configuration data that the nodes will use for configuration. We will install this on a Scaleway Instance.
- **Workstation:** This machine holds all the configuration data that can later be pushed to the central Chef server. Several Chef command line utilities are available in this system, which can be used to interact with nodes, update configurations etc. We will use a second Scaleway Instance as our workstation.
...
1. [Connect to your Instance via SSH](/compute/instances/how-to/connect-to-instance/):
  ...
  To install Chef, the hostname of the Instance must be a resolvable fully qualified domain name or IP address. Check that in the following steps:

2. On the Instance where you plan to install Chef, launch:
...
3. Edit the configuration file to add a domain name or IP address where the server can be reached:

  ```
  sudo nano /etc/hosts
  ```

4. Edit the configuration file following the example below:
...
5. Check that the value was set correctly:
  ...
  The command returns the value that you can use to reach your Chef server from anywhere in your infrastructure. This could be your Instance's IP address or fully-qualified domain name.
...
3. Go to the [Chef Infra Server Ubuntu Linux 16.04 Download Page](https://downloads.chef.io/tools/download/infra-server?download=https://packages.chef.io/files/stable/chef-server/14.10.23/ubuntu/16.04/chef-server-core_14.10.23-1_amd64.deb&os=Ubuntu%20Linux%2016.04&version=14.10.23), complete your information in the form and click **Download**.

4. Copy the download URL, and from your terminal launch the installation using `wget` + the URL:

  ```
  wget https://packages.chef.io/files/stable/chefdk/4.13.3/ubuntu/16.04/chefdk_4.13.3-1_amd64.deb?_ga=2.201798231.793288551.1637763130-1348836278.1637763130
  ```
...
6. Call the `reconfigure` command, which configures the components that make up the server to work together in your specific environment:
  ...
  The resulting configurating may take some minutes to complete.
...
The actual infrastructure coordination and configuration does not take place on the Chef server. This work is done on a workstation which then uploads the data to the server to influence the Chef environment. Here, we will use a second Instance.

### Installing the Chef Development Kit (CDK)

The Chef Development Kit, a suite of software designed for Chef workstations.

1. Go to the [CDK Ubuntu Linux 16.04 Download Page](https://downloads.chef.io/tools/download/chefdk?download=https://packages.chef.io/files/stable/chefdk/4.13.3/ubuntu/16.04/chefdk_4.13.3-1_amd64.deb&os=Ubuntu%20Linux%2016.04&version=4.13.3), complete your information in the form and click **Download**.

2. Copy the download URL, and from the terminal of your second Instance, launch the installation using `wget` + the URL:

  ```
  wget https://packages.chef.io/files/stable/chefdk/4.13.3/ubuntu/16.04/chefdk_4.13.3-1_amd64.deb?_ga=2.7729396.793288551.1637763130-1348836278.1637763130
  ```

2. Install the package you downloaded:

  ```
  dpkg -i chefdk_*.deb
  ```

3. Verify that all of the components are available in their expected location through the new chef command:

  ```
  chef verify
  ```

  You will also be prompted to accept the license agreement.
