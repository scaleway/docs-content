...
<Message type="note">
  ...
  It can take anywhere from a few seconds to 24 hours to retrieve the first byte of a average sized file. Your object will remain available in `STANDARD` storage class for the duration you chose during the restore process. It will be transferred automatically back to `GLACIER` once the configured period is over. 
