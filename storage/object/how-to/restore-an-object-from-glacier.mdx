...
If you have numerous files in a bucket that you would like to restore, it may be more convenient to use the command line, for example with the [s3cmd tool](/docs/tutorials/s3cmd/). To restore all files recursively in a bucket, you can invoke this command:

```
s3cmd restore -v --recursive s3://<bucket-name>
```

To count the number of files still in GLACIER (see section on Restore time below, it is not immediate!), you can use:
