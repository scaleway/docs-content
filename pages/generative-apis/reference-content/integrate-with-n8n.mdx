---
title: Integrate Generative APIs with n8n
...
You can add an embedding model to a workflow by adding an **Simple Vector Store** node, and configuring it to use Scaleway Generative APIs. Follow the steps below to see how to create a workflow that takes a chat message as an input, and outputs an embedding of the message (list of vectors).
...
11. Fill in the form, to add where prompted:
    ...
    - The URL `https://api.scaleway.ai/v1` as your base URL
    <Message type="tip">
    If you are using a Project other than your [default Project](TODO), modify the base URL accordingly: `https://api.scaleway.ai/{project_id}/v1`
    </Message>
12. Click **Save**
    The credentials are saved, and an automatic connection test is carried out, displaying the message **Connected tested successfully**. This confirms that n8n can access the `/v1/models` and `/v1/chat/completions` endpoints. 
...
You can test the workflow by opening a chat from the interface, sending it a message, and checking that the output is a list of embeddings. 
