...
---
import UnsupportedRoutingProducts from '@macros/vpc/unsupported-routing-products.mdx'
...
An [owner or manager](/iam/policies-permissions/permission-sets/) of each VPC must independently create a peering connector towards the other, in order for a peering connection to be established. This ensures that peering connections are always mutually consented and prevents unauthorized network access. 
...
You must be the owner of the VPC, or have the correct [permission sets](/iam/policies-permissions/permission-sets/#scoped-by-project) in order to create a peering connector where it is defined as origin. You also need to know the unique VPC ID of the target VPC you want to peer with. Older VPCs must have [activated routing](/vpc/how-to/manage-routing/#how-to-activate-routing) for successful peering.
...
VPC Peering connectors are a regional resource, and are available in multiple regions. For the most up-to-date information, see [Product availability by region](https://www.scaleway.com/en/product-availability-by-region/).
...
- [VPC Peering Troubleshooting](/vpc-peering/troubleshooting/) - Solve any problems you run into with VPC Peering with our troubleshooting guides.
- [VPC Peering FAQ](/vpc-peering/faq/) - Get answers to the most frequently asked questions about VPC Peering.
