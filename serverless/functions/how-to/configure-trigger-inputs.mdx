...
The first thing to calculate is the amount of messages that can be consumed by the function every second, or throughput. This value can be derived from how long it takes for it to complete when inkoked and the max number of instances it may scale up to.
