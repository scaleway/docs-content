...
Regional IP addresses are scoped to a single Scaleway region (such as `FR-PAR`, or `NL-AMS`). If you reserve a public regional IP address for an Instance in `PAR-2`, you can move it to a resource in a different Availability Zone within this Region, such as `PAR-1`. Regional IPs can be used as failover IPs, by switching from a resource in one AZ to a backup resource in a different AZ in case of failure, ensuring high availability for this resource.
...
Zonal IP addresses are scoped to a single Availability Zone. If you reserve a public zonal IP address for an Instance in `PAR-2`, you can move it to a different Instance or VPN gateway in `PAR-2`, but not to a resource in `PAR-1`.