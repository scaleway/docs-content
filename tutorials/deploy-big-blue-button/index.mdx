---
title: Deploy BigBlueButton on your Bare Metal Cloud Server
description: Discover how to deploy the BigBlueButton web conferencing system on your Bare Metal Server
tags: compute webconferencing calls videocalls videoconference bigbluebutton bbb
excerpt: |
  Big
totalTime: PT20M
steps:
  - step: Ordering a Bare Metal Server
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/deploy-big-blue-button/#-Deploying-BigBlueButton-on-a-Scaleway-Bare-Metal-Server)
...
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, ubuntu server, debian server, linux server, IAAS, baremetal, scalability, bare  metal, bigbluebutton, bbb, videoconference, webconference, web conference, conference, self-hosted, sharing
dates:
...
The application can be deployed on [Scaleway Bare Metal Servers](https://www.scaleway.com/en/bare-metal-servers/) and allows you to have access to features such as:
...
This tutorial is composed of several steps. The first section shows you have to deploy BigBlueButton on a Scaleway Bare Metal Server. The second, advanced, section is intended for users who want to take their configurations further.
...
## Deploying BigBlueButton on a Scaleway Bare Metal Server

1. Log yourself into to your [Scaleway console](https://console.scaleway.com). The dashboard displays once logged in.

2. Click **Bare Metal** in the **Compute** section of the side menu:

  <Lightbox src="scaleway-bbb_console_dashboard.png" alt="" />

3. Click **Create a Bare Metal Server** to order a new server:

  <Lightbox src="scaleway-bbb_console_create_server.png" alt="" />

4. Once the server is delivered, click the **Install** button on the Bare Metal start page to install the operating system on the server.
...
The presenter features are available by clicking on the **Presenter Tools** button: <Lightbox src="/assets/images/docs/ensemble/bbb-mod.png" width="45" alt="" />

## Advanced Configuration

In this section, you will learn how to:

- [Configure the administrator account](#-Creating-an-Administrator-Account) to configure advanced settings in the Greenlight web interface.
...
You can also manually install BigBlueButton on a server freshly installed with Ubuntu 16.04 LTS (Xenial Xerus). This can be useful if you installed the server manually using the [KVM over IP device](/compute/bare-metal-cloud/how-to/install-server-from-kvm/).
...
> **Important:** Before installing BigBlueButton, you need to configure a valid hostname for your server. If you have not done so already, you need to purchase a domain name from a [domain name registrar](https://en.wikipedia.org/wiki/Domain_name_registrar) and configure an `A record` pointing to your server's IP address from the registrar's web interface. Refer to the documentation of your domain registrar on information how to do this step. You can buy your domain name at Scaleway Dedibox: [Learn more](https://www.online.net/fr/domain)
