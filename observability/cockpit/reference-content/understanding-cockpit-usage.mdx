...
If you are [using your own Grafana](/tutorials/using-own-grafana/) to visualise metrics, use the following query `sum(rate(observability_cockpit_ingestion_samples_total{is_from_scaleway="false"}[5m])) OR on() vector(0)` to know what your current ingestion rate is.
