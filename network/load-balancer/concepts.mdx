...
TODO: use this info https://wohugb.gitbooks.io/haproxy/content/balance.html

* **Round-robin**  
  In round-robin mode, requests are queued and then each is assigned to a backend server in a defined sequence. The Load Balancer acts like a turnstile, forwarding requests to each backend server in turn as they come through the turnstile. This is the most frequently used balancing method, and the easiest to implement as it is well-suited to infrastructures that have identical backend servers.
* **Least connections**  
...
* **First**  
  In this mode, each request is directed towards the first backend server with available connection slots.
