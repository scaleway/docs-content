...
categories:
  - instances
  - block-storage
tags: block-storage data migration block-storage-low-latency volumes
dates:
...
5. Check that for each volume, the ID displayed in the pop-up matches the one returned in the output in step 3. For example, the `/dev/sda` volume's ID on the console, should match the one returned in the output.
...
4. Unmount the specific partition(s) (/dev/sdb1 in our case) from which you intend to copy data:
        ```
        umount /dev/sdb1
        ```
