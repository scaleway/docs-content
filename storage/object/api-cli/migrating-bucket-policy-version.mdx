...
## Migrating from 2023-04-17 to 2023-06-07-alpha

1. Remove any `Deny` statements from your `2023-04-17` bucket policy, as they do not have any effects.

2. Replace `"Version":"2023-04-17"` by `"Version":"2023-06-07-alpha"`
...
6. Add a new statement to the policy with a `NotPrincipal` element that contains the users and applications targeted in the first statement.

7. Add an `"Action": "s3:*"` element to the second statement. All actions are denied to everyone **EXCEPT** users and applications specified in the `NotPrincipal` element.
...
The example bucket policy below conforms to the `2023-04-17` version. The users and application mentioned in the `Principal` element are allowed to perform the specified actions in the specified buckets.
...
The example policy below has the same effect as the `2023-04-17` version, but conforms to the `2023-06-07-alpha` version:
