...
[SSH keys](/console/my-account/concepts#ssh-key) allow you to securely connect to your Virtual Instances without using a password. An SSH key consists of an [RSA key pair](https://en.wikipedia.org/wiki/RSA_(cryptosystem)), which has two elements:

- An **identification key** (aka private key), which you must keep securely on the computer you want to connect from.
- A **public key** which you must upload to the Scaleway interface. This is then transferred to your Virtual Instance during the boot process.
...
You must upload the content of the public part of the SSH key pair you just generated to the Scaleway interface. This is then transferred to your Virtual Instance during the boot process. You can then connect and authenticate from your local machine (where the private key is) to the remote Virtual Instance (where the public key is).
