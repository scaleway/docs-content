...
This document explains how legacy setups have been automatically migrated with the introduction of [IAM](/identity-and-access-management/iam/).

## Summary

**I**dentity and **A**ccess **M**anagement brings a wealth of new features to your Scaleway account, allowing you to control and manage access in a much more fine-grained way than ever before. **All existing configuation and functionality is automatically migrated into IAM for you**, from the configuration of members and roles, to API keys.
...
| [Members](/console/my-account/concepts/#members)                           | ✅       | ❌        | With IAM, this is replaced by the concept of  **IAM users** of an Organization . Members are automatically migrated to be IAM users of the Organizations where they were members before.                                                                                                                                    |
...
- With IAM, you can invite other Scaleway account holders to be **IAM users** of your Organization
...
In addition, four **groups** have been automatically created in each Organization. Users have been automatically added to the appropriate group, based on the role they had in that Organization pre-IAM. The groups are as follows: 

- **admin** (mapping to the Administrator role)
- **billing** (mapping to the Billing Administrator role)
- **owner**
- **project_owner** (mapping to the Editor role) 

For each group, a corresponding **policy** has been created, where the group is the [principal](/identity-and-access-management/iam/concepts/#principal) of that policy. This policy defines rules that give the exact same rights that the users of the group had prior to the introduction of IAM. In the case of an ex-Billing Administrator, for example, they will now be in the **billing** group, attached to a policy that gives them the neccessary permissions to read and manage an Organization's invoices and billing details, but not to create Projects or resources.
...
With IAM, API keys are longer scoped at Project level. Instead, API keys are associated with a single [IAM user](/identity-and-access-management/iam/concepts/#user) or [IAM application](/identity-and-access-management/iam/concepts/#application), and inherit all their permissions from this user or application. 
...
In order to facilitate the migration of existing Projec-scoped API keys to the new IAM system, [IAM applications](/identity-and-access-management/iam/concepts/#application) have been used.
...
- A corresponding **policy** has been created for each Project/IAM application, where the IAM application is the [principal](/identity-and-access-management/iam/concepts/#principal) of that policy. This policy defines rules that give full read/write permissions to the Project with which the API key was originally associated, thereby ensuring that anyone still using this API key continues to benefit from the same permissions when using it as before the introduction of IAM.
