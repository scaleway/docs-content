...
Each Organization can have up to 100 API keys by default. You can submit a support ticket to request additional quota.

### How many IAM applications can I create?
Each Organization can have up to 100 IAM applications by default. You can submit a support ticket to request additional quota.

### How many IAM groups can I create?
Each Organization can have up to 50 IAM groups by default. You can submit a support ticket to request additional quota.

### How many IAM policies can I create?
Each Organization can have up to 50 IAM policies by default. You can submit a support ticket to request additional quota.

### How many users can my Organization have?
Each Organization can have up to 50 users by default. You can submit a support ticket to request additional quota.
...
You may decide whether to leave these auto-created IAM applications and policies in place, [edit the policies](/iam/policies-permissions/manage-policies/) to change permissions as required, or [generate new API keys for individual users](/iam/credentials/create-api-keys/) to define rights per-user via the creation of [new policies](/iam/policies-permissions/create-policy/) for those users.
...
### How do I change the username of the Owner?
You can use the [Scaleway CLI](/scaleway-cli/quickstart) to change the Owner's username. Run the following command:
```
scw iam user update-username <user-id> username=[new_user_name]
```