...
<Message type="note">
    If you do not want to customize the standard Edge Services endpoint, you do not need CNAME records.
</Message>
...
When you create an Edge Services pipeline to a [backend](/edge-services/concepts/#backend), the backend content is served through the standard Edge Services endpoint, e.g. `pipeline-id.svc.edge.scw.cloud`. If you want to [customize your Edge Services endpoint with your own subdomain](/edge-services/how-to/configure-custom-domain/), you must create a CNAME record to point your subdomain to the Edge Services endpoint. 
  ...
  - If your domain is managed with [Scaleway Domains and DNS](/domains-and-dns/quickstart/), Scaleway will generate the appropriate CNAME record. It will be visible among your Domains and DNS records in the console. Do not modify or delete the CNAME record. If you deactivate Edge Services, Scaleway will automatically delete the CNAME record. 
  - If your domain is managed by an external provider, Scaleway cannot create the appropriate CNAME record. You will be prompted, as part of the process for customizing your Edge Services domain, to create this record yourself with your domain provider.
