...
1. Set the following Environment Vairables at the function level:
...
The following steps are adapted from the [AWS lambda](https://docs.aws.amazon.com/lambda/latest/dg/with-s3-tutorial.html#with-s3-tutorial-create-function-code) tutorial. We recommand using the last version of Node.
...
Once the ImageTransform function is deployed copy its URL (from the Settings Tab in the console or in the Serverless Framework logs) into the BucketScan environment Varible. 
Once everything is well deployed, test the BucketSan functions using its URL
