...
- Created a [functions namespace](/functions/how-to/create-manage-delete-functions-namespace/)
- Created a [function](/functions/how-to/create-a-function/)
...
[Queue triggers](/containers/concepts/#queue-trigger) allow you to automatically invoke a function using events stored in a [queue](/messaging/concepts/#queue).

Only queues created with the Scaleway Queues product are compatible with Serverless Functions.

The configuration of the queue retention may affect the behavior of the trigger. Refer to the [Considerations to configure event retention for queue trigger inputs](/functions/reference-content/configure-trigger-inputs/) page for more information.
...
[NATS triggers](/functions/concepts/#nats-trigger) allow you to automatically invoke a function using messages sent in a [NATS](/messaging/concepts/#nats) subject.
...
[CRON triggers](/functions/concepts/#cron-trigger) allow you to invoke your functions based on a recurring schedule. The CRON triggers for Serverless Functions use the **UTC** time zone.
...
<Message type="note">
    Refer to the [cron schedules reference](/functions/reference-content/cron-schedules/) for more information.
</Message>
