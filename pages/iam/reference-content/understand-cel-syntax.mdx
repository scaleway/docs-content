---
title: Understand CEL syntax
description: The reference for CEL language definition
tags: cel syntax
dates:
  posted: 2026-07-15
...
floating-point numbers. We only support positive, decimal integer literals;
negative integers are produced by the unary negation operator. Note that the
integer 7 as an `int` is a different value than 7 as a `uint`, which would be
written `7u`. Double-precision floating-point is also supported, and the
integer 7 would be written `7.0`, `7e0`, `.700e1`, or any equivalent
representation using a decimal point or exponent.
...
such as expression evaluation, rounding mode, or exception handling. Under the
runtime equality semantics defined later in this document, any two not-a-number
values are treated as unequal, consistent with the IEEE 754 standard (see
[Numbers](#numbers)).
...
representation of the string literal. In addition, the octal escape sequences
are interpreted as octet values rather than as Unicode code points. Both raw
and multiline string literals can be used for byte literals.
...
this data will not map back into CEL as the same value:
...
evaluate to either a value or an error. Here is how different expressions are
evaluated:
...
value or, for dynamically typed values, _a_ CEL type. Where implementations allow
...
In the conditional operator `e ? e1 : e2`, `e1` is the result if `e` evaluates
to `true`, and `e2` is the result if `e` evaluates to `false`. The untaken
branch is presumed to not be executed, though that is an implementation detail.
...
to allow the boolean operators to be mapped to indexed queries, and to align
better with SQL semantics.
...
expression `e1 && e2` can be rewritten as `e1 ? e2 : false`. Similarly,
`e1 || e2` can be rewritten as `e1 ? true : e2`.
...
*   Eliminating all of the above and using only default-cost functions, plus
    aggregate literals, time and space are limited to `O(P * I)`.
    A limiting time example is `size(x) + size(x) + ...`.
    A limiting time and space example is `[x, x, ..., x]`.
Note that custom functions will alter this analysis if they are more expensive
...
variables (similar to type erasure in Java). Thus an implementation can
...
styles are non-overlapping by definition, regardless of their types.
...
expressed within CEL as follows:
...
JSON in a way which most closely matches user expectations. The following
...
Two `list` values are equal if their entries at each ordinal are equal. For
lists `a` and `b` with length `N`, `a == b` is equivalent to:
...
definition of heterogeneous equality. Note that Java and Go proto equality
...
range. For this reason, numeric comparisons across types are supported at
...
All timestamp functions which accept a timezone argument can use any of
...
Note that duration strings should support the following suffixes:
...
The `dyn` type does not exist at runtime, but provides a hint to the
