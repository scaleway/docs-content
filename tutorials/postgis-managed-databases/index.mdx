...
content:
  h1: Using the PostGIS extension on a Managed Database for PostgreSQL
  paragraph: This page shows how to enable and use the PostGIS extension on a Managed Database for PostgreSQL 
...
steps:
  ...
  - step: Downloading and Converting a dataset
    type: HowToStep
...
[PostGIS](https://postgis.net/) is an extension to the PostgreSQL database management system developed by the Canadian company Refractions Research Inc. The extension allows to store and manage Geographic Information Systems (GIS) objects in PostgreSQL. It is available on Scaleway Managed Databases. 
...
2. Create a directory to store the downloaded data and change to it:
  ```
...
5. You will notice several `.dbf`, `.prj`, `.shx` and `.shp` files in the folder. These files together are called a [ShapeFile](https://en.wikipedia.org/wiki/Shapefile), a common digital vector storage format for storing geometric location and associated attribute information for legacy GIS software.
