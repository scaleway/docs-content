...
- [Start a PostgreSQL managed DB Instance](/postgresql-and-mysql/how-to/create-a-database/) and create a `lora` table with the following fields: `time` (timestamp type) and `data` (`jsonb` type)
- Configure a Database Route to push data in there, [see how to](/postgresql-and-mysql/how-to/create-a-database/). The topic filter should be `my-app/things/+/uplink`. Your query should look like `INSERT INTO lora (time, data) VALUES (NOW(), $PAYLOAD)`.
- Start a [new Scaleway Instance](/instances/how-to/create-an-instance/) and setup Grafana, get inspiration from [this tutorial](/tutorials/visualize-timeseries-data-timescaledb-grafana/) (steps 1 to 14, do not enable the TimescaleDB option).
