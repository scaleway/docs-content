...
1. Create a `~/bin` directory, which will be added automatically to your `PATH` on Ubuntu Linux.
    ```
    mkdir ~/bin
    ```
...
Most modules manage a few closely related resources from a single provider and allow you to parameterize the configurations. You can define variables to simplify the management of your infrastructures and to duplicate it in no time.
