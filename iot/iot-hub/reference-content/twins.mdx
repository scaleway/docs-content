...
Create your documents in a way that both REST and MQTT inband API do not write the same. A good example is the `current_state` / `desired_state` pair.
...
To manage concurrency and ensure that each user has a chance to modify the document without losing changes done by another user, the Cloud Twins feature provides locking through version numbers.

- A document is created with version 1.
- For every change made to the document the version is increased in one number.
...
The `_config` document associates configuration values to your documents. Its keys and subkeys enable more twins features.
...
- `_tags`: associates a set of tags (key/value pairs) to the key.
