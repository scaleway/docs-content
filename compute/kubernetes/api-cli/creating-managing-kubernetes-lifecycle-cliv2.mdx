...
A pool is a set of identical Nodes. A pool has a name, a size (its current number of nodes), nodes number limits (min and max), and a Scaleway Instance type, note that DEV1-S, PLAY2-PICO and STARDUST specs (<2GiB RAM) are too small to be used in a pool). Changing these limits increases/decreases the size of a pool. Thus, when autoscaling is enabled, the pool will grow or shrink inside those limits, depending on its load.
