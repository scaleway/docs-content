...
1. To configure at user/local level, open your user SSH configuration file on your local machine with a text-editor such as `nano`: 
    ...
    Paste the following code into the file, then save and exit. Ensure that you make the following replacements:
      - `.priv`: If you attached the Public Gateway to the Private Network via the console, this is the correct value. However, if you used another method such as Terraform, API, or CLI you may need to replace this value - see [above](#how-to-connect-using-the-resources-fully-qualified-domain-name-fqdn).
