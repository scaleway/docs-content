...
excerpt: |
  ...
  In this tutorial, you will learn how to install and configure PrestaShop on Ubuntu Focal Fossa 20.04 on a Scaleway Dedibox dedicated server.
totalTime: PT30M
steps:
  - step: Installing PrestaShop
    type: HowToStep
    url: https://www.scaleway.com/en/docs//tutorials/prestashop-dedibox#Installing-PrestaShop
  - step: Setting up MariaDB
    type: HowToStep
    url: https://www.scaleway.com/en/docs//tutorials/prestashop-dedibox#Setting-up-MariaDB
  - step: Obtaining a Let's Encrypt TLS Certificate
    type: HowToStep
    url: https://www.scaleway.com/en/docs//tutorials/prestashop-dedibox#Obtainig-a-Lets-Encrypt-TLS-certificate
  - step: Configuring PrestaShop
    type: HowToStep
    url: https://www.scaleway.com/en/docs//tutorials/prestashop-dedibox#Configuring-PrestaShop
  - step: Setting up TLS encryption
    type: HowToStep
    url: https://www.scaleway.com//en/docs//tutorials/prestashop-dedibox#Setting-up-TLS-encryption
  - step: Going further
    type: HowToStep
    url: https://www.scaleway.com//en/docs//tutorials/prestashop-dedibox#Going-further
categories:
  - dedicated servers
  - dedibox
image: /assets/scaleway_prestashop.png
tags: dedibox prestashop e-commerce ecommerce webshop onlineshop
permalink: docs//tutorials/prestashop-dedibox
keywords: dedibox tutorials, dedibox documentations, dedicated server, cloud hosting, ubuntu server, e-shop, eshop, web shop, online shop, prestashop, e-commerce, e-retail, shopping, shop, dedibox
dates:
  validation: 2021-09-27
  posted: 2020-08-07
---
...
In this tutorial, you will learn how to install and configure PrestaShop on Ubuntu Focal Fossa 20.04 on a [Scaleway Dedibox](https://www.scaleway.com/en/dedibox/).
...
9. Download the [latest version](http://www.prestashop.com/en/download) of PrestaShop from the official website. At the time of writing this tutorial, it is version 1.7.6.7, and we will use it here. Replace the link with the latest version if required.
...
1. Certbot is available in the _Universe_ repositories of Ubuntu. Make sure they are configured on your system to install the application using the `apt` package manager:
...
4. Certbot will ask you a series of questions, answer them as in the following example:
    ...
    - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    (A)gree/(C)ancel: A <-- Type A to accept the Terms of Service of Let's Encrpyt
    - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
1. Open your web browser and point it to `http://eshop.example.com/install`. The configuration wizard displays. Choose the language you want to use for your e-commerce site and click **Next**:
  ...
  <Lightbox src="scaleway-prestashop_install_1.png" width="900" alt="" />

2. Read the license terms of PrestaShop and tick the box **I agree to the above terms and conditions**, then click **Next**

  <Lightbox src="scaleway-prestashop_install_2.png" width="900" alt="" />

3. The configuration wizard checks the compatibility of your system. If all requirements are fulfilled you see a confirmation message, and you can click **Next** to proceed further with the configuration:

  <Lightbox src="scaleway-prestashop_install_3.png" width="900" alt="" />
...
4. Enter the information about your shop, your account credentials, and choose a secure password for your administrator account. Click **Next** to continue with the installation:
  ...
  <Lightbox src="scaleway-prestashop_install_4.png" width="900" alt="" />

5. Enter the connection details for your database. You have set the credentials for the database user in a previous step. If you want to use PrestaShop with a [Managed Database for MySQL](https://www.scaleway.com/en/database/), you will find the credentials in your Scaleway console. Click **Test your database connection now!** to verify that the connection is successful. Once done, click **Next** to continue.

  <Lightbox src="scaleway-prestashop_install_5.png" width="900" alt="" />
...
6. The setup wizard will create the database structure for PrestaShop. This step may take a moment, and you are being redirected to the next page once this task has completed:
  ...
  <Lightbox src="scaleway-prestashop_install_6.png" width="900" alt="" />

7. The installation is finished. A summary of the installation displays:

  <Lightbox src="scaleway-prestashop_install_7.png" width="900" alt="" />
...
1. Go to `http://eshop.example.com/admin`. The administrator login displays. Connect yourself to the administrator area using the credentials set during the installation:
  ...
  <Lightbox src="scaleway-prestashop_login.png" width="900" alt="" />
...
2. Click **Shop Parameters** > **General** in the menu on the left:
  ...
  <Lightbox src="scaleway-prestashop_config.png" width="200" alt="" />

3. Click the link **Please click here to check if your shop supports HTTPS**. You are being redirected to a secure HTTPS connection.:

  <Lightbox src="scaleway-prestashop_ssl_1.png" width="900" alt="" />
...
4. Enable HTTPS connections by clicking on the corresponding button. By default, PrestaShop uses HTTPS only on the member and checkout section. If you want to use HTTPS on all pages, enable the option by clicking on the corresponding button. Then validate the configuration by clicking on **Save** at the bottom of the page:
  ...
  <Lightbox src="scaleway-prestashop_ssl_2.png" width="900" alt="" />
...
You now have a very basic but working firewall configured on your e-shop server. You can set [additional rules](/tutorials/installation-uncomplicated-firewall/) for more complex setups and configure additional protection of the machine using [fail2ban](/tutorials/protect-server-fail2ban/).
