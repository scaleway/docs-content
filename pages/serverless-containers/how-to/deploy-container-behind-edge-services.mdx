...
<Message type="note">
You cannot currently disable the default endpoint of a **Serverless Container**  and a **Serverless Function** (for example, to keep only a custom domain). The default endpoint remains active. This capability planned for later. **Edge Services** currently relies on this default endpoint to forward requests to the container/function.
</Message>
