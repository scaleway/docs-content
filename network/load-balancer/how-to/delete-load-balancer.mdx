---
title: How to delete a Load Balancer
description: This page explains how to delete a Load Balancer
excerpt: |
  This page explains how to delete a Load Balancer
totalTime: PT15M
steps:
  - step: TODO
    type: HowToStep
    url: https://www.scaleway.com/en/docs/network/how-to/delete-load-balancer/
image: /assets/images/docs/scaleway.png
keywords: load balancer, create, Howto
dates:
  validation: 2021-05-26
  posted: 2021-05-26
---

<Message type="requirement">

- You have an account and are logged into the [Scaleway Console](https://console.scaleway.com) 
- You have [created a Load Balancer](/network/load-balancer/how-to/create-load-balancer)

</Message>

## How to delete a Load Balancer

1. Click **Load Balancer** in the **Network** section of the [Scaleway Console](https://console.scaleway.com) side menu. The Load Balancer dashboard displays. 

2. Click the Load Balancer you want to delete.

3. Scroll down and click **Delete Load Balancer**. 
   <Lightbox src="assets/scaleway-lb-delete.png" />

  <Message type="important">

  This will permanently delete your Load Balancer and all your data will be lost. This action is irreversible.

  </Message>

3. Type **DELETE** in the pop-up box.

4. Tick the checkbox to delete the associated IP address.

5. Click **Delete this Load Balancer**.
<Lightbox src="assets/scaleway-confirm-deletion.png" />

<Navigation title="See Also">
  <PreviousButton to="/network/load-balancer/how-to/use-with-private-network/">How to use a Load Balancer with a Private Network</PreviousButton>
</Navigation>