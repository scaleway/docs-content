...
6. Select a node type. You can keep the same node type, or upgrade to a bigger one.
    <Message type="important">
      If you have a Database Instance that uses a local volume, and you want to upgrade to an Instance that uses Block volumes, you must [change the volume type](/managed-databases/document-database/how-to/change-volume-type) to Block storage first.
    </Message>
