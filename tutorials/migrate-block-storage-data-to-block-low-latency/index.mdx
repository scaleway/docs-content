...
<Message type="important">
  We recommend that you make a [snapshot](/storage/block/how-to/create-a-snapshot/) of your volume before migrating your data. This helps you avoid any potential data loss.
</Message>
...
1. Run the following command to check if you can use the LVM migration method:

    ```
    pvs
    ```

    If an output similar to the following displays, we recommend that you migrate your data using the ## Migrating data with Logical Volume Management (LVM) procedure.
...
- the root system `/` is mounted using `/dev/sda1`, as `/dev/sda` is the System volume. As stated previously in this documentation, data contained in the volume containing the operating system, must not be migrated.
- `/dev/sdc` is mounted as a non-partitioned raw device, since there is no number appended to the device path. For this use case, you must migrate your data using the ## Migrating data without Partition Table or LVM procedure.
- `/dev/sde1` is a partitioned raw device. The naming convention used for it (sde1) is typical for identifying partitions on storage devices. For this use case, one must migrate date their date using the ## Migrating data without LVM but with Partition Table procedure.
