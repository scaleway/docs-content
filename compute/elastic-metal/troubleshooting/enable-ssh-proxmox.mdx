...
Proxmox VE is a complete, open-source server management platform for virtualization. It combines the KVM hypervisor and Linux Containers (LXC), storage and networking functionality - managable trhough a web-based user interface. However SSH connections to the Proxmox VE host are disabled by defalt for security reasons.

You can enable SSH on your machine after the installation of Proxmox from the Scaleway console, if you require SSH access to your Elastic Metal server.
...
1. Login to the Proxmox web interface of your Elastic Metal server.
    <Message type="tip">
