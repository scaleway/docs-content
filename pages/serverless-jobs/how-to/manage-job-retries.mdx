...
- **Retried** — job runs in a `failed` status with a valid exit code
- **Not retried**:
    - job runs in `failed` status **without an exit_code** (null), e.g.,  platform mismatch or some other infrastructure-side error
    — job runs in an `interrupted` status
