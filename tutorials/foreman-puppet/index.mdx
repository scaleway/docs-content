---
title: How to Install and Configure Foreman on Ubuntu Xenial
description: Discover how to install and configure Foreman on Ubuntu Xenial
tags: foreman puppet getting started
excerpt: |
...
totalTime: PT20M
steps:
  - step: Creating your Foreman Server
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-Creating-your-Foreman-Server
  - step: Installing Foreman on Ubuntu Xenial
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-Installing-Foreman-on-Ubuntu-Xenial
  - step: Managing Puppet
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-Managing-Puppet
  - step: The Foreman Web Interface
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-The-Foreman-Web-Interface
  - step: Foreman Dashboard
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-Foreman-Dashboard
  - step: Changing the Admin Password
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-Changing-the-Admin-Password
  - step: Adding NTP Module To Foreman
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-Adding-NTP-Module-To-Foreman
  - step: Overriding Default NTP Pool
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-Overriding-Default-NTP-Pool
  - step: Configuring Foreman Host to Use NTP Module
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-Configuring-Foreman-Host-to-Use-NTP-Module
  - step: Viewing Puppet Configuration
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-Viewing-Puppet-Configuration
  - step: Running the Puppet Agent
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/foreman-puppet/#-Running-the-Puppet-Agent
categories:
  - compute
  - instance-dev
image: /assets/images/docs/scaleway.png
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, ubuntu server, linux server, baremetal, scalability, ARM, physical server, dedicated server, foreman, playbook, hosts, roles, puppet, modules
dates:
...
This tutorial assumes that Foreman is being installed on a fresh instance, which will also act as the Puppet master server. Foreman requires a working Puppet installation to perform the configuration management of hosts.
...
- Port 8140 on the Puppet master must be reachable
...
1. Connect to the instance using SSH:
  ...
  To retrieve the instances IP, list your exiting servers using `scw ps` (Scaleway CLI). For more information on the Scaleway CLI, refer to the tutorial on the [Scaleway Command Line Interface](https://www.scaleway.com/docs/manage-cloud-servers-with-scaleway-cli/).
...
2. Check your instances full hostname to ensure the hostname is set to [FQDN](https://en.wikipedia.org/wiki/Fully_qualified_domain_name)
...
1. Install Puppet 5.x from the Puppetlabs repository as following.
...
2. Add the Foreman repository to the [APT packet manager sources list](https://en.wikipedia.org/wiki/APT_(Debian):
...
During installation, the installation tool will sets up a puppet master on the host, which is fully integrated with Foreman. Run the Puppet agent on the instance to send the first Puppet report to Foreman, This creates the host automatically in Foremans database.
...
<Lightbox src="scaleway-foreman_login.png" alt="" />
...
<Lightbox src="scaleway-dashboard.png" alt="" />
...
Puppet requires accurate time-keeping, therefore the [NTP service](https://en.wikipedia.org/wiki/Network_Time_Protocol) runs on the Foreman host (which acts also as Puppet master).

1. Install NTP and [Stdlib](https://forge.puppet.com/puppetlabs/stdlib) on the Puppet master:
...
2. When connected to the Foreman web UI, click **Configure**> **Puppet**> **Classes**:
   ...
   <Lightbox src="scaleway-configure_classes.png" alt="" />

3. Click the `Import environments from foreman.example.com` button.

  <Lightbox src="scaleway-import.png" alt="" />
...
4. Tick the ntp checkbox next to the item and click **Update** to import the ntp module.
  ...
  <Lightbox src="scaleway-ntp_prod.png" alt="" />
...
<Lightbox src="scaleway-ntp_servers.png" alt="" />
...
2. Click **Edit** on the far right of your Foreman host.
  ...
  <Lightbox src="scaleway-hosts.png" alt="" />
...
3. Click the **Puppet Classes** tab. Within the Available Classes section, click the ntp class, then click the plus sign next to "ntp":
  ...
  <Lightbox src="scaleway-ntp_add.png" alt="" />
...
The ntp class is being moved to the Included Classes section. The next time the Puppet agent checks in, it will have this module applied to it.
  ...
  <Lightbox src="scaleway-ntp_included.png" alt="" />
...
After editing Foreman host's configuration, a redirection to Foreman host's summary page applies.
  ...
  <Lightbox src="scaleway-hosts_dashboard.png" alt="" />
