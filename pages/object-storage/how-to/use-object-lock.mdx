...
2. Run the following command to check if the legal hold was properly applied:
...
**Description**
: Root element of the lock configuration.
...
**Description**
: Enables Object Lock on the bucket.
...
**Description**
: Default retention rule applied to every new object placed in the bucket.
...
**Type**
: String — `GOVERNANCE` or `COMPLIANCE`
...
**Required**
: Use `Days` or `Years`, not both

#### `Years`

**Description**
: Default retention duration in years.
...
**Description**
: Date on which the object retention expires.
...
**Description**
: Enables or disables the legal hold on the object.
...
This example sets up a bucket for strict regulatory retention: all objects are locked in Compliance mode for 7 years and cannot be deleted or overwritten by any user until the period expires.
...
# Set a default 7-year Compliance retention on all new objects
aws s3api put-object-lock-configuration \
  --bucket my-compliance-bucket \
  --object-lock-configuration '{
    "ObjectLockEnabled": "Enabled",
...
Objects uploaded to `my-compliance-bucket` are automatically locked in Compliance mode until 7 years after their upload date. No user can shorten the retention period or delete objects before it expires.
