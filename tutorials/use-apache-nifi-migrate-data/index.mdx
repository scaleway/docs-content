...
tags: objectstorage nifi apache
excerpt: |
  Apache NiFi is application on which we can design data workflows between different storage mediums
totalTime: PT35M
steps:
...
keywords: cloud tutorials, cloud documentations, cloud server, cloud hosting, ubuntu server, debian server, linux server, nifi, apache, s3, terraform
dates:
...
4. Click the small arrow at the right of the line, it will open the configuration window. This will send you to the `NiFi Flow Configuration` window. You'll be able to set your S3 `<ACCESS_KEY>` and S3 `<SECRET_KEY>` by clicking on the dented wheel at the right and entering them in the separate credential fields.
