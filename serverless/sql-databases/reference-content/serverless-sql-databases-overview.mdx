...
## Idle state
After receiving no query for 5 minutes, a Serverless SQL Database will enter into an idle state. It switches from **Running** to **Ready to use**, even if your clients applications are still connected. When switching to **Ready to use**, client connections are maintained, but all temporary data stored at the connection level (i.e. the PostgreSQL session) are deleted (for instance session variables - defined using SET {variable} TO {value}).
...
Autoscaling triggers automatically based on your database vCPU consumption:
  ...
  - **Scaling up**: after using more than 40% the currently allocated vCPU for 10 seconds, the vCPU and RAM allocated to your database will increase by 1 vCPU and 4 GB RAM each time, up to a maximum of 15 vCPUs and 60 GB RAM.

  - **Scaling down**: after using less than 2% of the currently allocated vCPU for 10 seconds, the vCPU and RAM allocated to your database will decrease by 1 vCPU and 4 GB RAM each time, down to a minimum of 1 vCPU and 4 GB RAM while your database is active.
  
<Message type="important">
    These values may change over time to optimize the efficiency of automatic scaling operations.
  </Message>
...
- SQL requests size are limited to 512 KB. A single SQL query sent to Serverless SQL Database cannot exceed 512 kb, but the response to the query is not limited and results tables can exceed 512 kb.
