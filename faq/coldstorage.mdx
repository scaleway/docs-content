...
Standard class is ideal for storing data that is frequently accessed, on-demand content such as streaming or Content Delivery Networks (CDN).
...
<Message type="important">
  ...
  It can take anywhere from a few seconds to 24 hours to retrieve the first byte of an average sized file. From that point, file retrieval is limited to xMo/sec. Unfortunately, we are unable to provide estimates of how long data retrieval will therefore take for each use case.
