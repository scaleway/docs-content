...
Refer to the [dedicated documentation](/containers/reference-content/containers-concurrency/) for more information on container concurrency.
...
Container Registry is the place where your images are stored before being deployed. We recommend using Scaleway Container Registry for optimal integration. See the [migration guide](/containers/api-cli/migrate-external-image-to-scaleway-registry/) for full details.
...
It works similarly to a traditional Linux [cron job](https://en.wikipedia.org/wiki/Cron), using the `* * * * *` format, and uses the **UTC** time zone. Refer to our [cron schedules reference](/containers/reference-content/cron-schedules/) for more information.

## Custom domain

By default, a generated endpoint is assigned to your Serverless resource. Custom domains allows you to use your own domain - see our [custom domain documentation](/containers/how-to/add-a-custom-domain-to-a-container) for full details.
...
Environment variables are key/value pairs injected in your container. They are useful to share information such as configurations with your container. Some names are reserved. [See details about reserved names](/containers/reference-content/containers-limitations/#configuration-restrictions).
...
JWT (JSON Web Token) is an access token you can create from the console or API to enable an application to access your private container. [Find out how to secure a container](/containers/how-to/secure-a-container/#restrict-access-to-your-containers).
...
Serverless Containers offers a built-in logging system based on Cockpit to track the activity of your resources: see [monitoring Serverless Containers](/containers/how-to/monitor-container/).
...
Performance metrics for your Serverless resources are natively available: see [monitoring Serverless Containers](/containers/how-to/monitor-container/).
...
A namespace is a project that allows you to [group your containers](/containers/how-to/create-manage-delete-containers-namespace/).
...
Serverless Jobs are similar to Serverless Containers but are better suited for running longer workloads. See [the comparaison between Serverless products](/containers/reference-content/difference-jobs-functions-containers) for more information.
...
To store data you can use [Scaleway Object Storage](/object/), [Scaleway Managed Databases](/postgresql-and-mysql/), and [Scaleway Serverless Databases](/sql-databases/).
...
* **Error**: something went wrong during the deployment process. [Check our troubleshooting documentation](/containers/troubleshooting/cannot-deploy-image) to solve the issue.
