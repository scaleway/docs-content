...
excerpt: |
  This page provides detailed information about Scaleway IoT Hub Devices.
totalTime: PT5M
...
Devices are any system able to connect to an IoT Hub. They can be a connected device or a piece of software running on a computer, for example. 
...
<Message type="important">
  ...
  We recommend that you to use mutual-TLS connections to exchange messages between the Hub and the Devices.
...
- with the **MQTT username**, matched to a **Device ID** (if the Device is configured to allow insecure connections).
...
<Message type="note">
  ...
  Filters can also be applied to will messages.
