...
Scaleway's Generative APIs - Serverless provides access to pre-configured, serverless endpoints of leading AI models, hosted in European data centers. This allows you to integrate advanced AI capabilities into your applications without managing underlying infrastructure.

### What is Scaleway Generative APIs - Dedicated Deployment?
Scaleway's Generative APIs - Dedicated Deployment is a fully managed service that allows you to deploy, run, and scale AI models in a dedicated environment.
...
### How do I get started with Generative APIs - Serverless? {/*COMMENT: This needs to change depending on whether we'll have 2 quickstarts or 1. */}
...
For details, refer to our [Generative APIs model catalog](/generative-apis/reference-content/model-catalog).
...
There is a free tier available for Generative APIs - Serverless. The free tier allows you to process up to 1,000,000 tokens without incurring any costs. After reaching this limit, you will be charged per million tokens processed. Free tier usage is calculated by adding all input and output tokens consumed by all models used.
...
- To ensure fixed billing, you can use [Generative APIs - Dedicated Deployment](https://www.scaleway.com/en/inference/), which provides the same set of OpenAI-compatible APIs and a wider range of models. {/* COMMENT: Website link should point to Dedicated deployments page. */}
...
No, you cannot increase the maximum number of output tokens beyond the [limits for each model](/generative-apis/reference-content/model-catalog/).
...
By default, you cannot increase the maximum number of concurrent requests beyond the [limits for all models](/organizations-and-projects/additional-content/organization-quotas/#generative-apis). {/* COMMENT: Referenced doc has to be updated with new product name. */}
...
Yes, you need to comply with model licenses when using Generative APIs - Serverless. Applicable licenses are available for [each model in our documentation](/generative-apis/reference-content/model-catalog) and in the console Playground.

### Do model licenses apply when using Dedicated Deployment?
Yes, model licenses need to be complied with when using Generative APIs - Dedicated Deployment. Applicable licenses are available for [each model in our documentation](/generative-apis/reference-content/model-catalog).
...
### Can I use OpenAI libraries and APIs with Scaleway's Generative APIs - Serverless? {/* QUESTION: Is this true for Dedicated too? Should this section be merged with the next one? */}
Yes, Scaleway's Generative APIs - Serverless is designed to be compatible with OpenAI libraries and SDKs, including the OpenAI Python client library and LangChain SDKs. This allows for seamless integration with existing workflows.

### Is Generative APIs - Dedicated Deployment compatible with OpenAI APIs? {/* Dedicated */}
Generative APIs - Dedicated Deployment aims to achieve seamless compatibility with OpenAI APIs. Find detailed information in the [Scaleway Generative APIs - Dedicated deployments as drop-in replacement for the OpenAI APIs](/generative-apis/reference-content/openai-compatibility/) documentation.
