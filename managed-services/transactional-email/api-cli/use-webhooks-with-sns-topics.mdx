...
5. Create a Transactional Email Webhook via the Scaleway API. Replace `<sns_arn_id>` with the ARN identifier you copied earlier.
    ...
    <Message type="tip">
      In this example we use `email_delivered` and `email_dropped` as the event types. To get an extensive list of Transactional Email events,
    </Message>
...
You'll find examples of such calls below:
