...
Routing is activated by default whenever you create a new VPC, and can be activated on pre-existing VPCs by [following these steps](/vpc/how-to/manage-routing/#how-to-activate-routing). To finely control and filter VPC traffic, use [network ACLs](/vpc/reference-content/understanding-nacls/).
...
- A **scope**. This shows whether the route is advertised across the entire VPC, or (in the case of default routes), only certain Private Networks.
...
- You can finely control and filter VPC traffic using [network ACLs](/vpc/reference-content/understanding-nacls/). Alternatively, you can configure NACLs directly on certain resources (e.g., Instances, Elastic Metal servers) using tools such as `iptables` or `nftables`.
...
Scaleway recommends that you build your VPC infrastructure with separation of concerns in mind. Separation of concerns is a fundamental design principle aimed at breaking down large complex systems into smaller, distinct components, each with clear responsibilities and interfaces.
...
You can configure [Network ACLs](/vpc/reference-content/understanding-nacls/) to restrict traffic flow along custom routes. 
...
To exercise more granular control over default route advertisements, Scaleway recommends that you configure [Network ACLs](/vpc/reference-content/understanding-nacls/).
...
Your existing setup may be impacted by the new behavior if you want your custom routes to be scoped only to the next-hop resource's Private Network. In this case, Scaleway recommends that you use [Network ACL rules](/vpc/reference-content/understanding-nacls/) to limit access to the custom route.
