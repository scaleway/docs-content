...
- Installed [Terraform](/terraform/quickstart/) on your local machine
...
- [AWS CLI configured with Scaleway credentials](/object/api-cli/object-storage-aws-cli/#how-to-install-the-aws-cli)
...
5. Before proceeding with the function's logic, improve the Terraform code by adding the following code to your `main.tf` file:
    ...
    ```
    The `null_resource` is used to download and package the correct versions of the libraries that we use with the function. Refer to the [documentation on Python dependencies](/functions/how-to/package-function-dependencies-in-zip/#dependencies) for more information.
