...
You can renew your certificates at any time, when you add a new endpoint, for example.
...
In this page we show you two types of protection modes you can use to verify if the servers connecting to your client are properly encrypted.

We also show you how to run the verification using different languages, such as `psql`, `python`, `go`, and `js`.

## Using verify-ca

Will verify that a server is trustworthy by checking the certificate chain up to the root certificate stored on the client.
...
Will verify that a server is trustworthy by checking the certificate chain up to the root certificate stored on the client and if the hostname specified in the server certificate matches the hostname of the server.
