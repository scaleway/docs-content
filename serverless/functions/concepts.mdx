...
A CRON trigger is a mechanism used to automatically invoke a Serverless Function at a specific time on a recurring schedule. It works similar to a traditional Linux [cron job](https://en.wikipedia.org/wiki/Cron), using the `* * * * *` format.
...
An SQS (Simple Queue Service) trigger is a mechanism that connects a function to an [SQS queue](/serverless/messaging/concepts/#sqs) and invokes the function automatically whenever a message is added to the queue.
