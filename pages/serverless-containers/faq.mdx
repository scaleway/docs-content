...
These services allow you to build highly scalable, event-driven, and pay-as-you-go solutions. Serverless Containers and Functions help you create applications and microservices without worrying about server management, while Serverless Jobs lets you run large-scale, parallel batch-processing tasks efficiently. This leads to faster development cycles, reduced operational overhead, and cost savings.

[Understand the differences between Jobs, Functions, and Containers](/serverless-containers/reference-content/difference-jobs-functions-containers/).
...
Serverless Containers is billed on a pay-as-you-go basis, strictly on resource consumption (Memory and CPU). You only pay for the computing resources you use, with no upfront provisioning costs or charges for idle capacity.

For detailed and up-to-date information on pricing, refer to the [official Scaleway pricing page](https://www.scaleway.com/en/pricing/serverless/#serverless-containers).

* **Memory consumption:** Obtained by multiplying the memory tier chosen by the container run duration.
* **vCPU consumption:** Obtained by multiplying the vCPU tier chosen by the container run duration.
* **Ephemeral storage:** Free of charge. The maximum value of ephemeral storage depends on the memory value.
* **Ingress/Egress:** Free of charge.
...
<Message type="tip">
When deploying a Serverless Container from the Console, a cost estimator is provided directly in the Console.
</Message>
...
<Message type="note">
Your invoice displays your Free Tier usage under the **Serverless Containers Free Tier** section. This section shows the costs saved through the Free Tier, expressed both in euros and in `currency` (an integer value representing the smallest monetary unit, where 1 `currency` = €0.01, i.e., one cent).
</Message>
...
Serverless Containers support **HTTP/1.1** and **HTTP/2**. **HTTP/1.1** is enabled by default, but some services (e.g., gRPC) only support **HTTP/2**.

You can [modify the protocol](/serverless-containers/how-to/manage-a-container/#manage-a-container-deployed-from-the-scaleway-container-registry) used by your container from the **Requests tab** of the **Advanced options**.
...
Yes, Serverless Containers (and also Serverless Functions) are natively integrated. For more information, read [how to set Serverless Containers behind Edge Services](/serverless-containers/how-to/deploy-container-behind-edge-services).
...
No, you cannot use your own TLS certificates. Scaleway uses Let's Encrypt to generate and automatically renew certificates on your [Custom Domains](/serverless-containers/concepts/#custom-domain).

### Can I run any application on Serverless Containers?

Yes. Because Serverless Containers supports any containerized application, you can choose the language, runtime, and framework that best suits your needs. Serverless Containers can host any containerized application that responds to HTTP requests.

### How do I add a Load Balancer in front of my Serverless Container?

Load Balancing is integrated directly to Serverless Containers when you invoke them; our infrastructure handles traffic without the need to add other products.

Also, Load Balancers are compatible only with IP backends. As Serverless Containers IPs are not predictable, it is not possible to integrate Containers directly with Load Balancers.

## Access and security

### Can I connect to my container using SSH?

No, it is not possible to connect to Serverless Containers using SSH. Serverless Containers is a fully managed, stateless compute environment that does not provide direct access to the underlying infrastructure due to several features, such as autoscaling.
...
At the moment, we do not provide encryption at rest on the filesystem of customer containers. However, the underlying volumes comply with our security standards. For more details, refer to our [Security and resilience documentation](https://www.scaleway.com/en/security-and-resilience/).

Within customer containers:

- In sandbox v2, everything under `/tmp` is stored in RAM.
- Everything else is written to disk, but access is restricted to authorized personnel only. Refer to the question *Who are the authorized users?*

### Is there any anti-malware or antivirus system in place for containers to secure the environment?

We do not use any anti-malware or antivirus system to secure customer environments.
Scaleway Container Registry will include vulnerability scanning, [click here to register for the private beta](https://www.scaleway.com/en/betas/#artifact-registry).
...
As a result, application security remains the customer's responsibility.
...
Although such access may be possible for debugging purposes, it is never performed without the customer’s explicit authorization.
...
### How can I access my Serverless Container with a Private Endpoint?

Serverless Containers supports Private Network for outgoing traffic (like accessing a database or an instance), but does not yet support incoming traffic.

This feature is under development.

### How can I disable the public endpoint for Serverless Containers?

It is not possible yet to disable the default public endpoint for Serverless Containers. To restrict access to Serverless Containers, you can [make the resource private](/serverless-containers/how-to/secure-a-container/#restrict-access-to-your-containers).

This feature is under development.

### Is Serverless Containers integrated with Audit Trail?

Yes, Serverless Containers is auditable. For more information, see the documentation about [Supported endpoints in Audit Trail](/audit-trail/reference-content/adt-supported-endpoints/).
...
- With Serverless Containers, you can use images from any Container Registry namespace, as long as they are in the same [Project](/organizations-and-projects/concepts/glossary/#project).
...
Periodic CRON triggers also allow you to keep your containers active during specific time slots to reduce cold start latency, without having to provision a minimum of 1 vCPU at all times.
...
For now, the `HEALTHCHECK` Docker directive has no impact on container readiness. Health checks are planned to be customizable for your applications in the future.

Health checks will not wake up Serverless Containers if no instance is up, to prevent additional costs.
...
- It must not require persistent local storage due to [ephemeral storage](/serverless-containers/concepts/#ephemeral-storage), and must use shared file storage systems like [Object Storage](/object-storage/)
...
Serverless resources are by default [stateless](/serverless-containers/concepts/#stateless) and local storage is ephemeral.
...
* [Scaleway Glacier](/object-storage/): Our dedicated Cold Storage class to secure long-term object storage. Ideal for deep archived data.
...
### Can I allow the IPs of my Serverless Containers?

Due to the nature of serverless computing, Serverless Containers do not have dedicated or predictable IP addresses assigned to them. This means you cannot allow specific IPs for your Serverless Containers.
...
Additionally, Scaleway Serverless Containers supports [Virtual Private Cloud (VPC)](/serverless-containers/how-to/use-private-networks/) and can be attached to a Private Network, which allows you to securely connect your resources in an isolated environment.
...
Insufficient vCPU, RAM, or ephemeral storage can lead to containers entering an error state. Make sure to provision enough resources for your container.
...
A RAM value too low can lead to restarts of Containers due to out of memory errors.

### What are the conditions to use an updated image?

When you push a new image with the same tag (e.g., `:latest`) to the Container Registry, Serverless Containers does **not** automatically use the updated image.

To use the updated image, you must **Redeploy** the container.

Scaling events (scaling up, down, or from zero) do **not** fetch the latest image digest. They use the image digest from the current deployment. Only the **Redeploy** action updates the image digest.

For better traceability and consistency, we recommend using **immutable and versioned tags** (e.g., `:v1.0.0`, `:1.2.3`) instead of mutable tags like `:latest`. This ensures you always know which version of your application is running and makes rollbacks easier.

## Support and troubleshooting

### Why does my gRPC container not respond?

Containers use **HTTP/1.1** by default, yet the gRPC protocol requires **HTTP/2**. You can upgrade the protocol to **HTTP/2** (`h2c`).
...
* **Keep your container warm**: You can use CRON triggers at certain intervals to keep containers warm or set the min-scale parameter to `1` when required.
