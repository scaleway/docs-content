...
There is 2 types of componenents: 
...
🧩 &nbsp;This component use a **native-extended Markdown** style.

Use a triple backquote character ` ``` …` to enclose text

You can precise the language of the code.  
List of languages can be found [here](https://github.com/FormidableLabs/prism-react-renderer/blob/master/src/vendor/prism/includeLangs.js).

**`Component examples`**

```
this is ablock of code
```


Here with use JSX, i.e. ` ```jsx …`
...
Add the metada `no-copy` if you don't need the CopyButton.  
...
🧩 &nbsp;This component use a **Tag** style.
...
- Lists items should be separate with a blank line.  
- Some text must separate definitions, otherwise lists will be sticked.  

Description
: Root level tag for the ObjectLockConfiguration parameters.

Random text for lists separation, it could be a title or inline code…
...
🧩 &nbsp;This component use a **Native Markdown** style.
...
To use code style in the middle of texts lines, enclose text with a single backquote (`) character
...
Inline Console icons are to be used in texts. They are the same as the Scaleway Console icons.
...
🧩 &nbsp;This component use a **Native Markdown** OR a **Tag** style.
