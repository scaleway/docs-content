...
1. Set up your [environment variables](https://github.com/scaleway/scaleway-sdk-go/tree/master/scw#environment-variables), if you have not done so yet. Replace `$SCW_ACCESS_KEY`, `$SCW_SECRET_KEY` and `$SCW_DEFAULT_PROJECT_ID` by their corresponding values.
  ...
  Restic requires permission to perform the `PutObject` action on the bucket. In the next step we will implement a [bucket policy](/storage/object/concepts/#bucket-policy) to grant the permissions. 

2. Create a file named `bucket-policy.json` using a text editor of your choice. In this tutorial we use `nano`.
  ...
  ```json
  {
    ...
    "Id": "MyBucketPolicy",
    "Statement": [{
      "Sid": "access for restic",
      "Effect": "Allow",
      "Principal": {
        "SCW": "project_id:<SCW_PROJECT_ID>"
      },
      "Action": [
        "s3:PutObject",
        "s3:ListBucket",
  ...
  In this example we include other actions beyond `PutObject`. You can configure your policies according to your preferences. To learn more, refer to the [reference documentation](/storage/object/api-cli/bucket-policy/).

  </Message>

3. Run the following command to apply the policy:
...
An output displays informing you of the name and path of your repository.
...
2. Enter the password for the repository when prompted.
  ...
  Restic does not offer a built-in tool for automated backups. Restic recommends using tools like systemd and cron to schedule backup runs. Learn more with the [Restic documentation](https://restic.readthedocs.io/en/latest/040_backup.html#scheduling-backups).
...
3. Run the following command to verify that the backup was successful:
  ...
  ```
  aws s3api list-objects --bucket ahouseisnotahome
  ```
