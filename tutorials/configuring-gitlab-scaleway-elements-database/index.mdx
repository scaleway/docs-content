...
[GitLab](https://about.gitlab.com/) is an open-core Git repository manager providing a wiki, issue tracking and CI/CD pipeline features. Open-core means that the core functionalities of the software are released under an open-source licence with additional optional modules.
...
- You have [enabled the SMTP ports](/console/my-account/how-to/send-emails-from-your-instance) to be able to send emails from your Instance
...
4. Retrieve the Database Instance **Endpoint** from the **Overview** tab, as they this be required for the following steps. The first part, before the colon, is the IP address, and the second part is the port.
...
2. Add the following parameters to the Gitlab configuration file:
    ...
    - `<SCALEWAY_DATABASE_BASE>`: The name of your Scaleway Elements Database. In our example, this is `rdb`
...
7. Once modified save the file and exit the text editor.
...
10. Confirm the database seed by typing `yes` when prompted.
...
3. Log in using the user `root` and the password you defined in step 1.
  ...
  The GitLab Dashboard displays. You can now configure your GitLab installation, add additional users and repositories. All data is stored securely on your Scaleway Elements Database Instance:
