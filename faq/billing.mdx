...
meta:
  title: Billing
  description: Let us address the most common questions you have about plans and pricing.
content:
  h1: Billing
  paragraph: Let us address the most common questions you have about plans and pricing.
image: billing.png
---

## How am I billed for Scaleway Elements services?
...
|             Resource type         |         Billing Starts         |           Billing Stops         |           Comments          |
    ...
    | Bare Metal servers  |         The moment the server is **created**.                | The moment the server is **deleted**. | When a Bare Metal server is powered off, the physical machine remains allocated to the user’s account, meaning the resource continues to be billed. |
    | Apple silicon M1 as-a-Service  |         The moment the Mac mini is **created**.                | The moment the Mac mini is **deleted**. | Due to license constraints, the minimum lease for Apple silicon-as-a-Service is 24 hours. As a result, **a Mac mini M1 can only be deleted after a minimum allocation period of 24 hours**.<br /><br />When a Mac mini M1 is powered off, the physical machine remains allocated to the user’s account, meaning the resource continues to be billed. |
...
<Lightbox src={["assets/Facture1.png","assets/Facture2.png"]} size={"medium"} alt={["Facture1","Facture2"]} />
...
7. The consumption table. This table contains the following information about your resources:
    ...
    - The services and operations consumed in your project (i.e. Instances, Object Storage, Bare Metal servers etc.)
...
Because we are a European company, we are required to charge VAT on all purchases made by European individual customers (B2C).
