...
<Message type="note">
   ...
   * [Dynamic IPs](/compute/instances/concepts/#dynamic-ip) continue to be supported after the migration to routed IPs.
...
### Migrating an existing Instance to routed IP via CLI

To activate the routed IP feature on an existing Instance, you can migrate it.
Any attached IPv4 will be transferred. However, the current IPv6 will be substituted with a new one. Once assigned, this new IPv6 will assigned to the Instance even through stop/start cycles.
...
### Create a new Instance with a routed IP via API
...
### Create a new Instance with one IPv4 only via API
...
### Migrating existing Instances to routed IPs via API
...
### Verifying if an IP is routed or NAT type via API
...
### Verifying routed IPs are enabled for your Instance via API
