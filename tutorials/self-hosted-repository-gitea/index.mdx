...
steps:
  - step: Installing Gitea
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/self-hosted-repository-gitea/#-Installing-Gitea
  - step: Setting up a Nginx SSL/TLS reverse proxy
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/self-hosted-repository-gitean/#-Setting-up-a-Nginx-SSLTLS-reverse-proxy
  - step: Disabling Port 3000 in the Firewall
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/self-hosted-repository-gitea/#-Disabling-Port-3000-in-the-Firewall
categories:
  - compute
  - instances
tags: compute server development gitea git versionning
image: /assets/images/docs/scaleway.png
keywords: cloud tutorials, cloud documentations, cloud server, cloud hosting, ubuntu server, debian server, linux server, IAAS, baremetal, scalability, ARM, physical server, dedicated server, physicalization, server
dates:
...
[Gitea](https://www.gitea.io/) is a self-hosted Git service. It provides lightweight code hosting solution written in Go and published under the MIT license. The tool is a community managed fork of [Gogs](https://gogs.io/).

[Git](https://en.wikipedia.org/wiki/Git) itself is a distributed version-control system for tracking changes in in files.
The tool was originally invented by [Linus Torvalds](https://en.wikipedia.org/wiki/Linus_Torvalds) in 2005 to manage the development of the Linux kernel among other developers contributing to its code-base.
...
5. In a web browser go to `http://<your_instance_ip>:3000/` to access the Gitea application:
  ...
  <Lightbox src="scaleway-gitea_app.png" alt="" />
...
9. Once the form has been submitted, you are logged into your Gitea account:
  ...
  <Lightbox src="scaleway-gitea_dashboard.png" alt="" />

You can now configure your profile and begin to host code on your instance.

## Setting up a Nginx SSL/TLS reverse proxy

For increased security it is possible to recommended to configure NGINX reverse proxy](/tutorials/nginx-reverse-proxy/) with TSL/SSL in front of the Gitea instance, it handles all requests sent from a client to the web interface and adds a layer of increased security and performance.
