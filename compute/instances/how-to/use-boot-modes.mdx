...
## How to change your boot volume

The [boot-on-block](/concepts#boot-on-block) feature allows Instances to boot from attached [Block Storage](/concepts#block-volumes) volumes instead of local volumes. When [creating an Instance](/how-to/create-an-instance), you can select Block Storage as a boot volume. 

Once the Instance is created, you can select a different volume to be boot. 
