...
If you need detailed instructions for installing the add-on in a single-instance deployment, a distributed deployment, or Splunk Cloud, see the [installation walkthrough](#installation-walkthrough-addon) section.
...
#### Installation walkthrough <a name="installation-walkthrough-addon"></a>
...
To manually configure inputs using a configuration file, see [Appendix: Manual configuration](#appendix-manual-add-on-configuration).
...
You can install the Scaleway App via Splunk Web or the command line. It is supported only on search heads instances.
...
If you need detailed instructions for installing the app in a single-instance deployment, a distributed deployment, or Splunk Cloud, see the [installation walkthrough](#installation-walkthrough-app) section at the bottom of this page.
...
#### Installation walkthrough <a name="installation-walkthrough-app"></a>

For detailed guidance on installing a Splunk app in the following deployment scenarios, see [Installing add-ons](https://docs.splunk.com/Documentation/AddOns/released/Overview/Installingadd-ons) in the Splunk Add-ons documentation:
...
The app relies on search macros to simplify the setup of dashboards, alerts, and reports. To learn more about search macros, see [Use search macros in searches](https://help.splunk.com/en/splunk-enterprise/manage-knowledge-objects/knowledge-management-manual/10.4/search-macros/use-search-macros-in-searches).
...
<Tabs>
    ...
    <TabsTab label="CLI">
        ...
        2. Add the following stanza and substitute `&lt;value&gt;` with the correct index.
...
- Since the "Scaleway - Increased number of failed logins" alert relies on logs with `sourcetype=scaleway:audittrail_events:auths`, ensure that the corresponding data input is enabled in the Scaleway Add-on.
...
- Since the report relies on logs with `sourcetype=scaleway:iam:users`, users must ensure that the corresponding data input is enabled in the Scaleway Add-on.
- [Configure search macros](#configure-search-macros).
...
4. Set **Earliest time** and **Latest time** accordingly:
...
4. Set the scheduling as required by your needs.
5. Click **Save**.
...
2. Click on **Configure > All configurations > Assets and identities > Identity lookups**.
3. Click **New > New configuration**.
4. Select the `scaleway_identities` lookup as **Source**.
5. Add a name.
6. Add a category (a short descriptive category for this identity list, for example: 'scaleway_identities').
7. Add a description.
8. Click **Save**.

#### Configure automatic groups lookup
...
- Since the report relies on logs with `sourcetype=scaleway:iam:groups`, ensure that the corresponding data input is enabled in the Scaleway Add-on.
- [Configure search macros](#configure-search-macros).
...
4. Set **Earliest time** and **Latest time** accordingly:
    - If you are receiving the `scaleway:iam:groups` events once per day, please set **Earliest time** to **-24h@h** and **Latest time** to **now**. 
    - If you are receiving the `scaleway:iam:groups` events once per week, please set **Earliest time** to **-7d@d** and **Latest time** to **now**.
5. Click on **Save**.
...
#### Configure automatic applications lookup
...
- Since the report relies on logs with `sourcetype=scaleway:iam:applications`, ensure that the corresponding data input is enabled in the Scaleway Add-on.
- [Configure search macros](#configure-search-macros).
...
- Since the report relies on logs with `sourcetype=scaleway:instance`, ensure that the corresponding data input is enabled in the Scaleway Add-on.
- [Configure search macros](#configure-search-macros).
- The configuration of this mechanism requires [Splunk Enterprise Security](https://www.splunk.com/en_us/products/enterprise-security.html).
...
3. Click **New > New configuration**.
4. Select the **scaleway_assets** lookup as **Source**.
5. Add a name.
...
## Appendix: Manual add-on configuration
