---
title: Understanding resource-level conditions
...
<Message type="note">
  The Scaleway console and API only validate the **syntax** of a condition expression, not whether it is logically possible to satisfy. For example, the following expressions follow the correct syntax but can never evaluate to true, or reference a resource that doesn't exist:
