...
The retry option lets you define how many times a Serverless Job automatically retries on failure. A retry is triggered whenever a job run ends in a `Failed` state, regardless of the exit code returned by the runtime. This includes runs that fail with a `null` exit code, such as those stopped by a platform mismatch or other infrastructure-side error. Retry on `Interrupted` state is not yet supported. Once the job reaches the maximum number of failed retries, it is permanently marked as `failed`.
