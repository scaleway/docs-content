...
3. Configure the locale of the system to `en_US.utf8`:
...
4. Open the file `/etc/environment` in a text editor and add the following lines at the end of the file:
  ...
  CREATE NEW USER ACCOUNT BEFORE STEP 3?
  root@roerpagain:~# sudo adduser mariecurie
  root@roerpagain:~# usermod -aG sudo username
  root@roerpagain:~# localectl set-keymap us && localectl set-locale LANG=en_US.utf8
  root@roerpagain:~# sudo nano /etc/environment
  root@roerpagain:~# reboot
  rowena@ ~ () $ ssh root@151.115.53.65
  Welcome to Ubuntu 20.04.3 LTS (GNU/Linux 5.4.0-84-generic x86_64)
  root@roerpagain:~# whoami
  root
  root@roerpagain:~# su - mariecurie
  mariecurie@roerpagain:~$ whoami
  mariecurie
  mariecurie@roerpagain:~$ 
...
2. In the next step initialize the Frappe framework using the following command. Remember to replace `/home/timmy` with the proper home path of your user:
  ...
  WARNING YOU SHOULD NOT RUN THIS COMMAND AS ROOT
