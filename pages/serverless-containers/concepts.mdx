...
To determine the status of a container, the default health check (referred to as the `liveness_probe` in the API) automatically checks if basic requirements are met, to define the status as `ready`. You can customize the following elements to better fit your use case:
...
- The **timeout** corresponds to the maximum amount of time to wait for a check to complete. If the check does not complete within this time, it is considered failed. Defaults to 1 second.
