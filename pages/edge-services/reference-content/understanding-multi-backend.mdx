...
Now, each pipeline can support **multiple backends**. These can be Load Balancers, Object Storage buckets, or a mixture of both. [Serverless Containers and Serverless Functions](/serverless-containers/how-to/deploy-container-behind-edge-services/) are also available as backend types via the API, CLI, SDK, and Terraform (this feature is not available in the Scaleway console). More backend types will be coming soon. To ensure that traffic reaches the correct background, create **routing rules**. These rules define filters for URL path and/or HTTP method, and route matching requests towards a specified backend:
