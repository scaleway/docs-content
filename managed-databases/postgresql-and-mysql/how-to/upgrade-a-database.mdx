...
meta:
  ...
  description: Guide to upgrading your PostgreSQL or MySQL database on Scaleway.
content:
  h1: How to upgrade a Managed Database for PostgreSQL and MySQL
  paragraph: Guide to upgrading your PostgreSQL or MySQL database on Scaleway.
...
<Message type="important">
  If you have a Database Instance that uses a local volume, and you want to upgrade to a node type that uses Block volumes, you must [change the volume type](/postgresql-and-mysql/how-to/change-volume-type/) to Block Storage first. <br /><br />

  Similarly, if you have a new-generation Database Instance using a [Low Latency Block volume](/postgresql-and-mysql/concepts/#block-storage-low-latency) node type and want to switch to a first-generation node type, you must first change the volume type to [Block Storage Legacy](/postgresql-and-mysql/concepts/#block-storage-legacy).
</Message>
