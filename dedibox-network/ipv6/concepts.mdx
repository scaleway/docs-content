...
**S**tateless **A**uto **A**ddress **C**onfiguration is the simplest way to assign IP addresses in an IPv6 network. Using the Neighbor Discovery Protocol, devices request the network prefix from the router, and then use this prefix and their own MAC address to create an IPv6 address.
...
With stateful DHCPv6, it is the DHCPv6 server who chooses and assigns IPv6 addresses to clients, unlike with SLAAC or Stateless DHCPv6  where the client creates their own IPv6 address. Stateful DHCPv6 is so-called because the DHCPv6 server keeps track of the state of IPv6 addresses on the network.
