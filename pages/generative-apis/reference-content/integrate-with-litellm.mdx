...
- Configure health checks parameters. For instance, when using embeddings models, adding `mode: embedding` under `model_info` in `config.yaml` is required, as [mentioned in LiteLLM Health Checks documentation](https://docs.litellm.ai/docs/proxy/health#embedding-models).
