...
description: This page shows you how to install a Mastodon Community on Ubuntu 20.04 Focal Fossa
excerpt: |
...
brands:
  - elements
tags: compute communication messaging self-hosted social network socialnetwork
image: /assets/scaleway_mastodon.png
permalink: docs/installing-mastodon-community/
keywords: cloud tutorials, cloud documentations, cloud server, cloud hosting, ubuntu server, debian server, linux server, IAAS, baremetal, scalability, ARM, physical server, dedicated server, physicalization, server, communication, messaging, self-hosted, social network, socialnetwork, social-network
dates:
  validation: 2021-05-10
  posted: 2019-03-05
---

[Mastodon](https://www.joinmastodon.org/) is an open-source, self-hosted, social media and social networking service. It allows the hosting of your own instances which may have their own code of conduct, terms of service and moderation policies. There is no central server and Mastodon instances are connected as a federated social network, allowing users from different instances to interact with each other. The platform provides privacy features allowing users to adjust the privacy settings of each of their posts.
...
Mastodon provides the possibility to use [S3 compatible Object Storage](/storage/object/how-to/create-a-bucket/) to store media content uploaded to instances, making it flexible and scalable.
...
4. Mastodon uses the [Yarn](https://yarnpkg.com/en/) packet manager. Install the repository for the required version of Yarn by running the following commands:
...
6. We are going to use `rbenv` to manage Ruby versions. The application must be installed for a single Linux user, therefore, first we must create a user under which Mastodon will run. The `--disabled-login` flag disables direct login to the user account for increased security:

  ```
  adduser --disabled-login mastodon
  ```

7. Then log into the `mastodon` user account and enter into the home directory of the user:
...
8. Setup [rbenv](https://github.com/rbenv/rbenv) and [ruby-build](https://github.com/rbenv/ruby-build):
...
9. Install and enable the [Ruby](https://www.ruby-lang.org/en/) version that is used by Mastodon. This step may last some time:
...
10. Finally install `bundler` by running the following command:
...
2. In the PostgreSQL prompt create the database user for Mastodon and exit from it afterwards:
...
2. Request a certificate by using `certbot` with TLS SNI validation in standalone mode. Replace `example.com` with your domain name:
...
3. Replace `example.com` in the configuration file with your domain or subdomain. Replace it also with your domain in all following occurences.
...
2. Change into the **/home/mastodon/live** directory and run the Mastodon installer:
  ...
  When asked enter the domain name or subdomain of the instance.
  When asked to use Docker choose **No**. Most of the other values are already pre-filled with the correct settings. Edit them if required for your setup.

  To set up Mastodon with Object Storage choose **Amazon S3** as service provider and enter the following information for the account. Valid [API Key](/console/my-project/how-to/generate-api-key/) are required for the configuration with Object Storage.
...
3. Once the application is installed, you will be asked if you want to create an Administrator account for your Mastodon instance. Type `Y` to create the account. Enter the username for the admin user, followed by your email address. A random password will be generated. Take a note of it, as you will require it to connect to your instance.
...
<Lightbox src="scaleway-mastodon_home_new.png" alt="" />
...
<Lightbox src="scaleway-mastodon_feed.png" alt="" />
