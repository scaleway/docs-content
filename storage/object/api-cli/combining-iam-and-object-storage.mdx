...
You can use IAM when you do not need to configure access to specific buckets, and favor simplicity over granularity. IAM is central, and relies on policies to manage a [principal](/identity-and-access-management/iam/concepts/#principal)'s rights in the entire Scaleway ecosystem.
...
Consider an Organization in which a user and an application must perform specific tasks with Scaleway Object Storage in a [project](/identity-and-access-management/organizations-and-projects/concepts/#project) named `S3-project`.
...
2. [Create an IAM policy](/identity-and-access-management/iam/how-to/create-policy/) named `S3-policy` in the [Scaleway console](https://console.scaleway.com/), with the previously created group as a [principal](/identity-and-access-management/iam/concepts/#principal). The members of this group will have the `ObjectStorageFullAccess` right.
