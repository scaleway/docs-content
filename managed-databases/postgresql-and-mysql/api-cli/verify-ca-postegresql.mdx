...
In this page we show you two types protection modes you can use to to verify if the servers connecting to your client are properly encrypted and if the encryption key matches that of your Database Instance.
...
Will verify that a server is trustworthy by checking if the server certificate is valid and if the server hostname is specified in the certificate itself.
