...
All Messaging and Queuing protocols (NATS, SQS and SNS) are now in General Availability, and therefore billable. Billing for Messaging and Queuing is based on:
...
- For SNS: (Number of subscriptions to a topic) * (number of messages) * (message size)
