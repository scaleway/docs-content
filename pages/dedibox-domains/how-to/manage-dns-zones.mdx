...
| **MX**      | Allows you to set the responsible mail server for a domain (online.net. IN mx.online.net.). The DNS name MUST be an IPv4 and can not be an IP address or a CNAME. Priority defines the time in which the servers are contacted if there are more than one. |
