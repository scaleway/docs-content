...
To inject data into Serverless Functions, you can use:
* [Environment variables](/serverless-functions/concepts/#environment-variables) for general purpose data.
* [Secret environment variables](/serverless-functions/concepts/#secrets) for sensitive data.
...
Unauthenticated calls will be rejected, and your function will not receive the incoming request.
...
When a function is set to **Private**, you can use Scaleway IAM policy conditions to define granular access rules using Common Expression Language (CEL): [policy conditions documentation](/iam/policies-permissions/understanding-policy-conditions/).
...
<Message type="note">
  ...
  * **Owner exception**: Users with [Owner status](/iam/concepts/glossary/#owner) possess full administrative rights through global organization policies. Even if your custom policies include IP restrictions, an API key belonging to an Owner can still call the function from any IP address.
</Message>
...
```cel
(cidr("23.45.0.0/16").containsIP(request.ip) || request.user_agent.contains("TrustedCI"))
&& request.time.getDayOfWeek() != 0 && request.time.getDayOfWeek() != 6
```
