...
5. Confirm the move of your worker nodes to routed IPs by typing **MOVE**, then click **Move to routed IPs** to launch the process.
    <Message type="important">
      * This operation is similar to a cluster upgrade. Each node in your cluster will reboot. Ensure having more than one node ready in order to avoid downtime.
...
1. Configure the following variables for your Kapsule cluster:
    ...
    ```
2. Run the following command to launch the migration of existing Kapsule clusters to routed IPs:
    ```
