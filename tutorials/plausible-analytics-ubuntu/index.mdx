...
Plausible Analytics is an open-source web analytics project with the mission to make analytics more privacy-friendly. You can self-host the solution on Scaleway Instances and to reduce the dependency and surveillance of other tools coming from the AdTech world.
...
As Plausible supports only plain HTTP traffic by default, we configure a Nginx reverse proxy to provide HTTPS with a Let's Encrypt certificate to increase security and cache the analytics script for improved performance.
