...
An exit code is an integer returned by a process when it finishes running. A value of `0` indicates success, while any non-zero value indicates an error. For Serverless Jobs, the exit code of a [job run](#job-run) determines its outcome: a non-zero exit code causes the job run to reach the `failed` status, which may trigger a [retry](#retries) if the configured retry count is greater than zero. See [Reason](#reason) (`exited_with_error`) for more information about non-zero exit codes.

Note that the exit code can also be `null` when the job run fails before the container process runs or exits, for example when the reason is `image_not_found` or `technical_error`. In these cases the job run still reaches the `failed` status, but no exit code is returned by the runtime.
...
* **`retrying`**: The job run failed and is being automatically re-executed, based on the configured retry policy. See [Retries](#retries) for more information.

Typical successful flow is: `initialized -> validated -> queued -> running -> succeeded`.

When retries are configured, a failed job run transitions to `retrying` before being re-executed: `initialized -> validated -> queued -> running -> failed -> retrying -> queued -> running -> succeeded`.
