...
## Adding a second Instance to the Private Gateway

1. Click **Instances** in the **Compute** section of the Scaleway console to display the list of your instances. Click **+ Create an Instance**.

2. Enter the details for your new instance and click **Create a new Instance**. The instace is being created. Once this is done, the list of your Instances displays.

3. Click **VPC** in the **Network** section. The list of your Private Networks displays.

4. Click <icon name="more" /> > **More info** to display the details of the Private Network.

5. Click the **Attached Ressources** tab to display a list of resources attached to your Private Network.

7. Select your new Instance in the drop down list and click **Add Instance** to attach it to the Private Network:

  <Lightbox src="scaleway_vpc_add_instance.png" alt="" />

  The new Instance displays in the list of devices connected to the Private Network.

## Configuring port forwarding 

Port forwarding allows to configure a mapping of ports on the IP address of the Public Gateway to IP addresses and ports inside the Private Network. This allows to access services on Virtual Instances in a Private Network without need for a flexible public IP on them. 
...
4. Click the **DHCP** tab. Then click **View DHCP Status** to display the list of devices autoconfigured by the DHCP server: 
  ...
  You notice in the screenshot above, that both entries are **dynamic** types. This means the DHCP server dynamically assigns an IP address to each device. The assigned IP address may change and the port forwarding will stop working. To avoid this, configure a **static** lease. 
...
8. Click **+ Add Static NAT** to conigure a new static NAT rule. In this example, we expose the internal port `22` of the Instance `192.168.0.2` to the external port `2202` of the Public Gateway. Confirm by clicking <Icon name="validate" />:

  <Lightbox src="scaleway_vpc_static_nat.png" />

  <Message type="tip">

  You can select to forward either:
    * TCP traffic
    * UDP traffic
  or both. 
...
You are now logged into the first Instance using the IP address of the Public Gateway: 
