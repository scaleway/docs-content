...
You cannot customize your endpoint with a root domain directly (e.g. `beautiful-domain.com`), only with a subdomain of it.
...
4. In the **Endpoint** panel, click **Configure domain**. The following screen displays:
    ...
    <Lightbox src="scaleway-edge-services-configure-domain.webp" alt="" /> TODO UPDATE

5. Set a subdomain from which your bucket will be accessible. You must already own the root domain. For example,if you own `beautiful-domain.com`, choose any subdomain you like and enter `my-chosen-subdomain.beautiful-domain.com` into the box.

6. The next step depends on the domain provider of the domain you used in the previous step:

    <Tabs id="edge-services-cname">
        <TabsTab label="Scaleway Domains and DNS">
        If your domain (or DNS management) is hosted with [Scaleway's Domains and DNS](/network/domains-and-dns/quickstart/) product, you do **not** need to manually create a [CNAME record](/storage/object/reference-content/cname-record/).

        Scaleway will auto-create the correct CNAME record to point your chosen subdomain to your Edge Services bucket. You will be able to view this CNAME record in the Domains and DNS section of the console. Do not attempt to modify or delete the CNAME record.
        </TabsTab>
        <TabsTab label="External provider">
        If your domain (or DNS management) is hosted with any provider other than [Scaleway's Domains and DNS](/network/domains-and-dns/quickstart/) product, you need to manually create a CNAME record.
...
4. In the **Endpoint** panel, click **Reset**. 
    ...
    A screen displays warning you that this will reset the bucket's domain back to the default Edge Services endpoint. Edge Services will consider your customized subdomain as unknown.

5. Click **Reset domain** to finish.

    <Message type="tip">
    - If you manually created a CNAME record, you should also remember to delete this from your domain provider. If your domain is hosted with Scaleway Domains and DNS however, and we auto-created your CNAME record, we take care of the deletion for you.
    - If you manually created an SSL/TLS certificate for your subdomain, you should also remember to delete it from Secret Manager so that you are no longer billed for it. If you generated a managed Let's Encrypt certificate via Edge Services however, Scaleway takes care of the deletion for you.
    </Message>
