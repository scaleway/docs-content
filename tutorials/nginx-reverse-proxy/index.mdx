...
products:
  - instances
tags: Nginx Reverse-Proxy Ubuntu-Bionic
dates:
...
- **Easy logging and auditing:** Since there is only one single point of access when an Nginx reverse proxy is implemented, logging and auditing become much simpler.
- **Encrypted connection** By encrypting the connection between the client and the Nginx reverse Proxy with TLS, users benefit from an encrypted and secure HTTPS connection, protecting their data.
...
1. Install Certbot on your Instance by using the APT packet manager:
    ...
    ```
2. Certbot provides a plugin designed for the Nginx web server, automating most of the configuration work related to requesting, installing, and managing the TLS certificate:
    ```
    certbot --nginx
    ```
