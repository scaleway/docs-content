...
<Message type="iam">
  **Limitation**
  ...
  Creating a "fully private" cluster (a control plane without a public IP) is currently not possible on Kubernetes Kapsule. The only workaround possible for now is to delete the default ACL rule generated at cluster creation.
</Message>
...
| Isolation   | Controlled isolation<br />(default)                                                                                                                  | Full isolation<br />(optional)                                                                     | 
|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|
| Description | Worker nodes are assigned both private IPs and public IPs.<br />All inbound traffic on the public interface is dropped by default using security groups. | The worker nodes are set without public IPs (100% private network), with the control plane getting a public IP.<br />A [Public Gateway](/public-gateways/how-to/configure-a-public-gateway/) is required. |
| Benefits    | 1. Strong security<br /> 2. Dynamic public IPs to reach out to external providers while avoiding rate limiting                                        | 1. Maximum security <br />2. A stable egress IP for secure connection to external providers         |
| Notice      | Default choice for new clusters. Can be used in combination with pools using full isolation.                                                    | Requires a Public Gateway, which incurs additional costs.                                     |
...
<Message type="tip">
  Refer to the dedicated VPC documentation to learn about [Scaleway DNS for VPC and Private Networks](/vpc/reference-content/dns/).
</Message>
...
Once you create a cluster in Kapsule, all nodes, particularly those with the Private Network feature enabled, are protected by a default security group named `Kapsule default security group`. Any changes made to this security group will apply to all nodes in the cluster, as this security group is shared between your different clusters. You can also specify a `security_group_id` at cluster creation.
...
3. Click the name of the security group that is configured for your Instance, the one named `Kapsule default security group`.
