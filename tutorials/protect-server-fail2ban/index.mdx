...
steps:
  - step: Installing Fail2Ban
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/protect-server-fail2ban/#-Installing-Fail2Ban
  - step: Configuring Fail2Ban
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/protect-server-fail2ban/#-Configuring-Fail2Ban
categories:
  - compute
  - instances
image: /assets/images/docs/scaleway.png
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, ubuntu server, debian server, linux server, IAAS, baremetal, scalability, ARM, physical server, dedicated server, physicalization, server, bruteforce, fail2ban, administration
dates:
...
- You have an Ubuntu Bionic Virtual Instance
...
2. Choose `Internet Site` when asked for the configuration:
  ...
  <Lightbox src="scaleway-postfix-install.png" alt="" />
...
3. Once the installation has completed, open the file `/etc/aliases` and add the following line:
  ...
  To receive notifications by email, it is required that the [email ports are unlocked](https://www.scaleway.com/en/faq/why-can-i-not-send-any-email/).
