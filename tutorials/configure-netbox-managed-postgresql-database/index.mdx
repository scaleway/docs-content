...
content:
  ...
  paragraph: This page shows you how to set up NetBox, a web application designed to help manage and document computer networks, on a Scaleway Instance using the managed PostgreSQL database service.
tags: NetBox dcim ipam PostgreSQL database
categories:
...
3. Install the required packages using the `apt` package manager:
    ...
    ```
4. Download the [latest release](https://github.com/netbox-community/netbox/releases/latest) of NetBox. At the time of writing this tutorial it is version 3.6.3. Unpack it into the `/opt` directory:
    ```
    wget https://github.com/netbox-community/netbox/archive/refs/tags/v3.6.3.tar.gz && tar -xzf v3.6.3.tar.gz -C /opt
    ```
