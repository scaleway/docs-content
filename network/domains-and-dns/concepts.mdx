...
<Concept>
  ...
  Root Servers are a type of [DNS name servers](#dns-name-server) pertaining to [top-level domains](top-level-domain-(tld)). They are the first step in the resolution of any domain name, since they contain information about the authoritative DNS servers for each top-level domain.
</Concept>
