...
1. [Connect to your Serverless SQL Database](/sql-databases/quickstart/#how-to-connect-to-a-database) with a PostgreSQL client (such as `psql`):
    ```bash
...
| pgvector                     | Supports vector similarity search, often used for storing and searching embeddings in machine learning applications. [Query options](https://github.com/pgvector/pgvector?tab=readme-ov-file#query-options) using `SET` command require [to be used in a single transaction](/sql-databases/reference-content/known-differences/#unsupported-configuration-commands) (i.e. between `BEGIN; (...) COMMIT;`)                            |
