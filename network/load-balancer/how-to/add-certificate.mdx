...
Originally, certificates used the **S**ecure **S**ockets **L**ayer (SSL) cryptographic protocol, and are still often referred to simply as SSL certificates. However, it is now standard for such certificates not to use SSL at all, but rather its successor: **T**ransport **L**ayer **S**ecurity (TLS). [Transport Layer Security](https://en.wikipedia.org/wiki/Transport_Layer_Security) is a more up to date security protocol designed to increase privacy and data security for communications over the Internet. 
...
- Encrypted data is sent to the Load Balancer, who can then decrypt it using its certificate.
...
6. Click **Create SSL certificate**.
  ...
  You are returned to the **SSL certificates** tab where your newly-created certificate now appears.
