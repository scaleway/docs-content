...
steps:
  - step: TODO
    type: HowToStep
    url: https://www.scaleway.com/en/docs/compute/functions/how-to/package-your-function-in-zip
categories:
  - compute
---
...
## How to upload your zip file to the Scaleway Console
  ...
  5. On the function creation page, select **Upload a Zip** as **Code entry type**:
    ...
    <Lightbox src="scaleway_upload_zip.png" alt="" />
...
```bash
docker run --rm -v $(pwd):/home/app/function --workdir /home/app/function rg.fr-par.scw.cloud/scwfunctionsruntimes/python-dep3:v1.2.8 pip install -r requirements.txt --target ./package
``` 
...
The example above uses python3 runtime, but you can easily change the docker image from `rg.fr-par.scw.cloud/scwfunctionsruntimes/python-dep3:v1.2.8` to `rg.fr-par.scw.cloud/scwfunctionsruntimes/python-dep2:v1.2.8`
...
Be careful to pack directly the code and its dependencies and not the folder where the project is stored.
