...
content:
  h1: Setting up a Nginx reverse proxy with Object Storage
  paragraph: This page shows you how to configure a Nginx reverse proxy to access Object Storage
...
It is possible to access your bucket via an Instance as a proxy. Transfer of data between your bucket and the compute platform is free of charge, and you can use the bandwidth included with your instance to distribute the files in your bucket. We will use the [Nginx](http://nginx.org/) reverse proxy to provide read-only access to the bucket's contents.
...
2. Make sure the `apt` repositories as well as the software already installed on the Instance is up to date:
  ```
