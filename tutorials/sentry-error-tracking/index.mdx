...
Today's world is running on applications and everyone needs visibility into application health - no matter if you are an engineer, support or product manager. Sentry is an open source solution for monitoring & error tracking in applications. It is written in Python and can provide real-time reporting of errors in applications based on a client/server architecture. You can easily integrate it into your applications via its SDK and let them automatically report the errors via common messaging tools like Slack, IRC and more. This helps reduce the time for development of your applications and increases the user experience.
...
This tutorial details how to setup a [Sentry](https://sentry.io/welcome/) Self-Hosted server on a Scaleway compute Instance to start error tracking in your applications.
...
<Message type="requirement">
  ...
  - You have a [Instance](https://www.scaleway.com/en/virtual-instances/) running on Ubuntu Jammy Jellyfish (22.04 LTS)
</Message>
...
1. Update the system and install the required prerequisites.
    ...
    ```
2.  Sentry runs in a Docker container using `docker-compose`. Install the GPG Key of Docker.
    ```
...
5. Enter the `/opt` directory on your Instance and download the [latest release](https://github.com/getsentry/self-hosted/releases/latest) of Self-Hosted Sentry.
    ...
    ```
    <Message type="note">
      At the time of writing of this tutorial, the latest released version was `23.2.0`.
    </Message>
