...
3. Select **MySQL 8 server** when you are prompted which MySQL product you want to install. Confirm by pressing OK.

4. Verify the configutration file for the `apt` package manager, generated during installation of the package above, by running the command `cat /etc/apt/sources.list.d/mysql.list`. The output should look like the following example:
