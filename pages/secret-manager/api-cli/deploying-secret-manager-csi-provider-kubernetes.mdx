...
The **Scaleway Secret Manager provider for Secrets Store CSI Driver** (also called "the provider" in the remainder of this document) connects these two together: it allows you to get secrets stored in Scaleway Secret Manager and use the Secrets Store CSI Driver interface to mount them directly into your Kubernetes pods as files.

Unlike [External Secrets](https://external-secrets.io), which materializes external secrets as Kubernetes `Secret` objects managed by an operator, the Scaleway Secret Manager provider mounts the secret contents directly into pods through the CSI volume interface, without creating intermediary Kubernetes secrets.

In this tutorial, you will learn how to install this provider on your Kubernetes cluster, define which secrets to mount, and access them from your application pods.
...
- At least one secret already [created in Scaleway Secret Manager](/secret-manager/quickstart/)
...
## Install the Scaleway Secret Manager provider
...
This installs the provider as a `DaemonSet` (so it runs on every node) together with its `ServiceAccount`, `ClusterRole`, and `ClusterRoleBinding`.
...
| `defaultRegion` | The [region](/secret-manager/concepts/#region) of your secrets | none (required) |
| `objects` | Contains the definition of each secret to mount (YAML), either by **Secret ID** or by **Secret Path** (see section [Access secrets](#access-secrets)). | none (required) |
...
- `projectID`: The project ID containing the secret (optional, defaults to `defaultProjectID` or `defaultOrganizationID`)
...
Once the Pod is running, the secrets are mounted as files under the `mountPath`. You can verify that the secrets are accessible inside the Pod:
...
<Message type="note">
Mounted secrets are written to the node's filesystem when the Pod is scheduled. Changes made to the secret in Secret Manager (for example, a new revision) are not reflected automatically in a running Pod; recreate or restart the Pod to pick up the latest revision.
</Message>
...
2. Once these resources have been deleted, uninstall the provider. To uninstall with Helm, execute:
...
Alternatively, if you use a values file, set `debug` to `true`:
