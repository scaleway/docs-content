...
2. Open it in a code editor and paste the code below inside. This statement denies access to everyone **EXCEPT** the specified user.
    ```json
    {
        ...
        "Statement": [
            {
                "Sid": "Prevent user from deleting objects",
                "Effect": "Deny",
                "NotPrincipal": {
                    "SCW": "user_id:<USER_ID>"
                },
    ...
    ```
3. Replace the `<USER_ID>` placeholder with the ID of the user you want to deny actions. You can also deny specific actions to an [application](/storage/object/api-cli/bucket-policy/#principal).
