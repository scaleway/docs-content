...
excerpt: |
  ERPNext is a free and open-source ERP software desigend to be used by manufacteres, distribuors and service companies. It includes a full range of modules including accounting, sales, CRM, purchasing, warehouse management and inventory. Moreover, specialised modules are available tailored to the needs of schools, healthcare, agriculture and non-proift sectors. It uses a MariaDB database to store its dat, is written in Python and based on the Frappé Framework. The application is released under the GNU GPLv3 license and its sourcecode is hosted on GitHub.
totalTime: PT30M
steps:
...
keywords: cloud tutorials, cloud documentations, cloud instance, bare metal, baremetal, baremetal server, erp, odoo, erpnext, open-source
dates:
  validation: 2021-09-14
  posted: 2020-07-28
---
...
1. Executed the following command from the `frappe-bench` directory of your user. Run `bench` with these parameters and replace `timmy` with the name of your user owning the production environment:
