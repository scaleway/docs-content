...
umami is a simple, easy to use, self-hosted web analytics solution. It provides a easier-to-use, open-source, and pprivacy-focused alternative paid solutions such as Google Analytics. 
The tool collects only the metrics relevant to you and displays everything on a single page.

<Message type="requirement">

- You have an account and are logged into the [Scaleway console](https://console.scaleway.com)
- You have a [Container Registry](/compute/container-registry/quickstart/)
- You have a [Managed Database for PostgreSQL](/storage/database/quickstart/)
...
1. Create a [database user](/storage/database/how-to/add-users/) and [grant permissions](/database/how-to/manage-permissions/) to the PostgreSQL database you want to use for umami. 
...
## Preparating the Container Registry
...
1. Select the Serverless Containers namespace you want to use to deploy the application from the **Serverless Containers** section in the Scaleway console. If you do not have a Serverless Containers namespace yet, click **+ Create a namespace** to create a new one.
    <Lightbox src="scalway-serverless-containers-namesp-list.webp" alt="" />
...
2. Enter a name for your container. Optionally you can enter a container description. 

3. Select the resources you want to allocate to the container. The resouces to allocate are depending on the intended load of your application. For the sake of this tutorial choose **128 MB - 70 mCPU**.

4. Configure the minimum and maximum values to autoscale the number of available instances for your container. 
...
7. Set the Secrets. Secrets are an extra-secure type of environment variables which are injected into your container and stored securely:
    ...
    * `DATABASE_URL`: The URL of your PostgreSQL database in the format: `postgres://<db-user>:<db-password>@<postgresql-ip>:<port>/<db-name>?sslmode=require` 
...
2. Copy the URL displayed at **Container Endpoint** and paste in in a web browser. The umami login screen displays.
...
Your Serverless Functions comes with a default entpoint name. You can customize this to a name of your choice using the **Endpoints** feature.

1. Click the **Entpoints** tab. The endpoint information displays. 
...
<Lightbox src="scaleway-serverless-custom-endping.webp" alt="" />