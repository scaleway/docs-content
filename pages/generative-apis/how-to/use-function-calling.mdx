...
Scaleway's Chat Completions API supports function calling as introduced by OpenAI. The Responses API allows not only function calling, but also direct tool-calling by the model, e.g. web and file search. However currently only function calling is supported by Scaleway. [Read more about Chat Completions vs Responses API](/generative-apis/how-to/query-language-models/#chat-completions-api-or-responses-api).

{/* ALTERNATIVE INTRO: This page describes the concept of function calling to allow models to connect to external tools, and explains how to implement it with Scaleway Generative APIs. */}
...
All the [chat models](/generative-apis/reference-content/supported-models) hosted by Scaleway support function calling.
...
See the OpenAPI documentation for a fully worked example on [function calling using the Responses API](https://platform.openai.com/docs/guides/function-calling#function-tool-example). [Find out more about Scaleway's Responses API](/generative-apis/how-to/query-language-models/#chat-completions-api-or-responses-api).
