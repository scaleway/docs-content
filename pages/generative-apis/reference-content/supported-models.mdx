...
\*Some providers recommend to enable reasoning using additional arguments such as `"chat_template_kwargs": {"enable_thinking": true}`. These field are not supported on Generative APIs and we recommand using `reasoning_effort` property which is equivalent.
