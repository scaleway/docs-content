...
title: Run a messaging platform with RocketChat
description: RocketChat is a messaging platform allowing you to run your community with chats, private messaging and file sharing
>>>>>>> 905db15... Update tutorials/run-messaging-platform-with-rocketchat/index.mdx
...
1. Before installing Rocket.Chat, make sure to update the APT package cache and the software already installed on the Instance:
...
5. Update the apt package cache and install MongoDB and Node.js:

  ```
  apt update && apt install build-essential mongodb-org nodejs npm graphicsmagick -y
  ```

6. Install [inherits](https://www.npmjs.com/package/inherits) via npm, the package manager of Node.js, and the node version required by Rocket.Chat:
