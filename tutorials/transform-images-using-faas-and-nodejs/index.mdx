...
description: This page explains how to transform images in an S3 bucket using Serverless Functions and Node JS
tags: serverless functions faas transforming s3 object storage object-storage
categories:
  - serverless
  - functions 
  - storage
...
With this tutorial you learn, how to transform images in a S3 bucket using Serverless Functions and NodeJS:

<Lightbox src="scaleway-faas-image-transform.png" alt="" />

* This project is composed of two functions.
* The first connects to the source object storage bucket and gets all images files from it, then calls the second function to resize each image.
* The second function gets a specific image (whose name is passed through a HTTP-call), resizes it and pushes the resized image to a new bucket.
...
<Message type="requirement">
  ...
  - You have generated your [Scaleway API key's](/docs/console/my-project/how-to/generate-api-key/) (to access your buckets)
...
The following steps are adapted and based on the [AWS lambda](https://docs.aws.amazon.com/lambda/latest/dg/with-s3-tutorial.html#with-s3-tutorial-create-function-code) tutorial.

1. Install `sharp` using fbs `linuxmusl` platform to ensure compatibility with Scaleway’s node runtime:
    ```bash
...
3. Cretate the ImageTransformation function:
    * Connect to Object Storage: 
    ```js
    const s3 = new AWS.S3({
    endpoint: S3_ENDPOINT_URL,
...
Following you find a `Serverless.yml` configuration file to use with the Serverless Framework: 
