...
Another feature of IP mobility is the ability to assign multiple flexible IP addresses to your Instance, allowing you to use both IPv4 and IPv6 protocols.

<Message type="note">
   * Currently, IP mobility is available in the `pl-waw-2`, `fr-par-3` and `nl-ams-2` Availability Zones.
   * [Dynamic IPs](/compute/instances/concepts/#dynamic-ip) continue to be supported after the migration to IP mobility.
   * The field `enable_ipv6` is being deprecated and has no meaning for IP mobility enabled instances.
</Message>
...
First you need to create an IPv4 reservation, then you create a new IP mobility Instance with this IP attached.
...
You can now attach several IPs to one IP mobility Instance and you can mix-up both IPv4 and IPv6 addresses.
