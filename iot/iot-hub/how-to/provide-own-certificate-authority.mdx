...
As a security measure to protect certificates, Scaleway does not store the private keys of custom certificate authorities. Therefore the Hub won't issue certificates for a custom Certificate Authority.
...
3. Prepare the **CA certificate** and a **proof of possession** certificate.  
   ...
   To **generate** a proof of possession, **sign** a certificate having the target **Hub ID** (that looks like `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`) as **Common Name** using the CA certificate.
