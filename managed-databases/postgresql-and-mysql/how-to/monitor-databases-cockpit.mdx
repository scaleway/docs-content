...
- On MySQL, slow queries are not activated by default, but can be activated in the [advanced settings](/postgresql-and-mysql/how-to/configure-advanced-settings/) of the Database Instance. The setting is called `slow_query_log` and the time can be edited with `long_query_time`.
