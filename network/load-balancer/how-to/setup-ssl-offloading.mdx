...
SSL bridging removes SSL-based encryption from HTTPS traffic coming into the Load Balancer (as with SSL offloading), but then initiates a new SSL connection to rencrypt traffic between the Load Balancer and the backend servers.To configure SSL bridging, use TCP protocol on port 443, and add an SSL certificate.

To configure SSL bridging:

1. Create a frontend listening on `port 443` and a backend listening on `port 443` with the `TCP` protocol.

	<Message type="note">

	It is possible to configure other ports, this is simply the traditional configuration.
