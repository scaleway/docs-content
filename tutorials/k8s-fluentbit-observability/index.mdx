...
meta:
  ...
  description: In this tutorial, you will learn to configure Fluent Bit on a Kapsule cluster to forward the container logs and metrics to the Observability Cockpit and visualize them in Grafana.
content:
  h1: Send Kapsule logs and metrics to the Observability Cockpit with Fluent Bit
  paragraph: In this tutorial, you will learn to configure Fluent Bit on a Kapsule cluster to forward the container logs and metrics to the Observability Cockpit and visualize them in Grafana.
...
This process will be done using [Fluent Bit](https://fluentbit.io/), a lightweight logs and metrics processor that acts as a gateway between containers and the Cockpit endpoints, when configured in a Kubernetes cluster.
