...
tags: Instances bare-metal ELK-Stack log Elastic-Stack Elasticsearch Logstash Kibana
steps:
...
10. Start Elasticsearch:
  ...
  To verify if Elasticsearch is running, run a `curl -X GET "localhost:9200"`. The output should be similar to this example:
...
You can now access your Elastic Dashboard at the DNS name of your instance, for example `http://elastic.local`:
