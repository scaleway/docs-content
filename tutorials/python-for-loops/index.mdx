...
In the example below we want to find words with first letter "b" and second letter "i":
...
for word in words:
  if word[0] != "b":
    ...
    if word[1] == "i":
        print("The word", word, "starts with bi-")
  
## Output:
The word bingo starts with bi.
```

### Break and continue exercises

1. Create a loop to check whether the list `[3, 5, 11, 1]` contains at least one number higher than 10, and prints "This list contains a number higher than 10" if it does. Include a break statement. The output should be as follows:

    ```
    This list contains a number higher than 10
    ```

2. Create a loop to check whether the list of lists `[[1, 3, 9], [3, 2], [4, 2, 1, 3]]` contains any lists that a) contain exactly two items, and b) the sum of the items is not greater than 5. Include a continue statement. The output should be as follows:

    ```
    [3, 2] contains two items which have a sum of 5
    ```

## Manipulating variables with for-loops

The example we saw [above](TODO) with the break statement works fine, but if the word bingo **is not** in the list of words, nothing will be printed. This is not very explicit, and might lead us to wonder if the code worked properly or not. 
...
It is **bad practice** to modify a list (or dictionary, or any other sequence) while you are iterating over it. For example, we could try to achieve the above task by removing words in the list when they don't start with **c**, but as words are removed mid-loop, the length of the list changes and this confuses the iterator, which ends up missing out some words as according to their index postion, they have already been checked. The result is therefore inaccurate:

    ```
      
    words = ["deranged", "recondite", "cowardly", "things", 
    "cuddly", "rhyme", "bed", "harm"]
    n = 0

    for word in words:
        print()
        print("iteration", n, "checking word:", word)
        if word[0] != "c":
            print(word, "doesn't start with c")
            words.remove(word)
        else:
            print(word, "starts with c!")
        n+=1

    print("The words that start with c are:", words)
      
    ## Output:
    iteration 0 checking word: deranged
deranged doesn't start with c

iteration 1 checking word: cowardly
cowardly starts with c!

iteration 2 checking word: things
things doesn't start with c

iteration 3 checking word: cuddly
cuddly starts with c!

iteration 4 checking word: rhyme
rhyme doesn't start with c

iteration 5 checking word: harm
harm doesn't start with c
The words that start with c are: ['anger', 'cowardly', 'abject', 'cuddly', 'bed']
    ```
