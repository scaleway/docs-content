...
VPC is currently in public beta, but will shortly be moving into general availability. The arrival of VPC brings changes to the Scaleway Network products portfolio, which are summarized in the table below. Read on to find out more about the detail of these changes.
...
If you're unsure about some of the terminology in this document, refer to the sources below for more explanation beore continuing.
...
**CLI**
- As the CLI does not support multiple versions of an API, new and future versions will drop the VPC v1 API in favour of v2.
...
Security and networking isn't easy. Mistakes can be made and details can be forgotten which can lead to conflicts in your configuration or serious security holes in your infrastructure. 
...
Each Scaleway Project will get a default VPC for each region. From there, it will only take a couple of clicks to create a Private Network with built-in DHCP in a VPC. No network configuration will be required, all the defaults are set up and ready to go. We even take care of making sure that each Private Network gets a large enough subnet that it won't overlap with that of any other Private Networks inside the same VPC.
