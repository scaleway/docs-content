...
## Setting-up a CronJob to update logs

It is recommended to schedule a CronJob to regularly update the AwStats database using newly created log entries, so the stats get updated on a regular basis.
