...
For example to migrate from `node18` to `node20` you need to change the runtime field and redeploy your function. Deployed runtimes are forward compatible but not necessary backward compatible.

Once a function is `deprecated`, `end_of_support` of `end_of_life` you are invited to upgrade your ongoing runtime to take advantage of new features, better performance and security fixes.
