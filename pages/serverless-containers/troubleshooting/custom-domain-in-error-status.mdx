---
title: My custom domain is in error status after adding it to a Serverless Container
description: This page provides solutions to issues encountered while adding a custom domain to Scaleway Serverless Containers.
tags: issues failure error custom-domain container serverless 
dates:
...
Custom domain in `error` status when adding a custom domain to my Serverless Container.

## Causes and solutions
	
When [adding a custom domain to a Serverless Container](/serverless-containers/how-to/add-a-custom-domain-to-a-container/), the following tasks are performed on Scaleway's side:
...
3. If step 2 is OK, we generate a dedicated TLS certificate for that custom domain so it can answer to HTTPS requests.
