...
No special knowledge is required to write bash scripts, as they are plain text files containing the series of commands required to run a specific task. Instead of typing then manually on the shell, you write them in a script and run the script afterward. This can be very useful for system administration tasks as you can automatize tasks. The common file extension for bash scripts is `*.sh`.
