...
4. In the **Cache** panel, use the <Icon name="toggle" /> icon to enable the cache.
    ...
    The **Life time** configuration box displays. This enables you to define, in seconds, how long an object can be stored in the cache before it must be retrieved freshly from the bucket.

    <Message type="tip">
    As an example, a value of 0 means that objects will not be cached, unless they have a separately-defined caching directive.
    </Message>
...
This clears all objects from your cache. Afterwards, fresh copies must be retrieved from the bucket before Edge Services can store them again in the cache. 
...
This allows you to specify the precise objects that you want to clear from the cache. Afterwards, fresh copies must be retrieved from the bucket before Edge Services can store them again in the cache. 
...
5. Enter the path of each object you want to purge. You can purge a maximum of 5 objects at a time.
      ...
      The path for each object should defined from the root of the bucket, starting with a slash. If we imagine a bucket containing one file at the root level called `object1.jpg`, and a subfolder at root level called `videos` containing an item called `my-video.mp4`, we would enter the object paths as follows:
...
5. Click **Disable cache**.
    ...
    Your cache is disabled. Edge Services will now serve content by fetching it from your Object Storage bucket directly.
