...
import image0 from './assets/scaleway-interlink-diag-simple.webp'
import image1 from './assets/scaleway-self-hosted-interlink-ordering.webp'
import image2 from './assets/scaleway-hosted-interlink-ordering.webp'
...
import image7 from './assets/scaleway-interlink-overview.webp'
...
<Lightbox image={image0} alt="A diagram shows how a Scaleway VPC, to the left, can be attached to an InterLink, which then connects to external customer infrastructure." />
...
2. Click **Create InterLink**. The creation wizard displays.
    ...
    <Lightbox image={image3} alt="A screen of the Scaleway console shows the first section of the InterLink order form. The different sections are explained in the steps below." />
...
- **Self-hosted InterLinks**: Your InterLink is now fully created. However, you must complete InterLink setup on the customer side by configuring your network device. For full details, see [How to complete self-hosted InterLink setup](/interlink/how-to/configure-interlink/#how-to-set-up-your-network-device-self-hosted-only)
    <Lightbox image={image1} alt="A four step flow chart shows the following steps: 1. User orders InterLink via Scaleway, 2. User requests InterLink from partner, 3. Partner accepts InterLink request, 4. Partner provisions InterLink." /> 
...
<Lightbox image={image6} alt="A five step flow chart shows the following steps: 1. Create InterLink, 2. Attach VPC, 3. Attach routing policy, 4. Set up network device (self-hosted InterLinks only) 5. Activate route propagation. After these steps are complete, traffic can flow across the InterLink" />
...
2. Click on the InterLink that you want to configure. The InterLink's **Overview** page displays.
    ...
    <Lightbox image={image7} alt="A screen in the Scaleway console shows an overview of a self hosted InterLink's location, region, bandwidth, creation date, ASN, and VLAN ID. Underneath are the 'Attached VPC', 'Route propagation' and 'Tags' panels" />
