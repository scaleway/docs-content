...
- Wildcard domains (e.g., `*.example.com`) are not supported when generating Let's Encrypt certificates directly through the Scaleway Load Balancer. If you need a wildcard certificate, you must generate it externally import it manually to the Load Balancer.
</Message>
