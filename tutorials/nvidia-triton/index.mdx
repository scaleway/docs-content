...
categories:
  - compute
tags: compute gpu nvidia triton storage
dates:
...
<Message type="requirement">
  ...
  - You have [created GPU Instance](/compute/instances/how-to/create-an-instance/) that runs Ubuntu GPU 12
  - You have [activated your observalibility Cockpit](/observability/cockpit/how-to/activate-cockpit/)
  - You have sudo privileges or access to the root user
  - You have basic understanding of machine learning models and Docker.
...
6. Upload the example model folder to your bucket in Scaleway Object Storage. You can use the [Scaleway Object Storage API](/docs/storage/object/api-cli/using-api-call-list/), any S3 compatible tool, or web interface to upload the model folder.
    <Message type="tip">
      You can use the `s3cmd` [command-line tool](/docs/tutorials/s3cmd/) or any other S3 compatible tool to upload your data.
    </Message>
...
1. Log into your GPU Instance using SSH.
    ...
    ```
3. Edit the Triton server configuration file, typically named `config.pbtxt`. This file should be located in the Triton Inference Server installation directory.
4. Configure the model repository path by adding or modifying the `model-repository` parameter:
   ```bash
...
1. Pull the Triton Inference Server container image:
   ...
   ```
   <Message type="tip">
     In case Docker is not preinstalled on your GPU Instance, follow the [Docker installation guide](/tutorials/install-docker-ubuntu-jammy-jellyfish/) before pulling the image.
   </Message>
...
Triton Inference Server provides metrics that can be exported for monitoring. Let's set up Prometheus and Scaleway Cockpit to visualize these metrics.
