...
- [Owner](/iam/concepts/glossary/#owner) status or [IAM permissions](/iam/concepts/glossary/#permission) allowing you to perform actions in the intended Project
...
Edge Services forwards incoming requests to your Serverless Container and Serverless Functions through its default (auto-generated) endpoint, not through a custom domain. It cannot reach a private or IAM-secured container, so the backend container must be public.
</Message>
...
- A valid [Scaleway API key](/iam/credentials/create-api-keys/) with appropriate permissions
...
<Message type="note">
You cannot disable the default endpoint of a Serverless Container or Serverless Function, for example to keep only a custom domain. This capability is planned for a future release. The default endpoint stays active, and Edge Services relies on it to forward requests to the container or function.
</Message>
