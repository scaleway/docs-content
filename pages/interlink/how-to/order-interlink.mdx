...
6. Attach up to two [routing policies](/interlink/concepts/#dedicated-physical-connection). InterLink needs a policy for each IP traffic type it should route: all routes are blocked except those whitelisted in the policy(ies). Alternatively, skip this step and attach routing policies later.
