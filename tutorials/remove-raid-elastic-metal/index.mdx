...
Each Elastic Metal server is using a RAID1 configuration after installation from the Scaleway console. If you want to change the RAID configuration of the server, you can modify the RAID array using [rescue mode](/compute/elastic-metal/how-to/use-rescue-mode/).
...
2. Login to the server using the `root` account:
...
5. Remove the mdadm devices using the `mdadm --remove` command: 
    ...
    This is normal behavior. Ignore it, and go ahead with the newt step.
...
6. Verify the RAID configuration has been removed using the `lsblk` and `cat /proc/mdstat` commands: 
