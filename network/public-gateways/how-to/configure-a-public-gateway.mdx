...
This page shows you how to attach a [Public Gateway](/network/public-gateways/concepts/#public-gateway) to a Private Network, how to configure DHCP and NAT and how to enable SMTP.
...
6. Click **Attach Private Network** to finish. You are taken back to the Private Networks tab, where the network you attached now appears, along with the services configured and the IP address of the Public Gateway.
...
4. DHCP is configured per Private Network. Select a Private Network from the drop-down menu to review its configuration.
    - Click **View DHCP Status** to view the current MAC/IP associations (aka DHCP leases).
    - Click <Icon name="toggle" /> to **Configure DHCP** and click <Icon name="edit" /> **Edit** to modify the network's DHCP configuration.
