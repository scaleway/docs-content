...
Entra can sometimes complain that some path is not supported by Scaleway's SCIM implementation. For example Entra might show errors like the following:
...
This type of errors indicate that Entra is trying to reconcile data that Scaleway does not understand. To fix this, follow the next steps:
1. Click **Attribute mapping** under the **Manage** menu.
2. Click **Provision Microsoft Entra ID Users**.
     This section controls how Entra ID attributes map to Scaleway SCIM fields.
3. Find the element EntraID complained about. In the previous example, find the line for which the `Microsoft Entra ID Attribute` column reads `displayName`, then click **Delete**.
