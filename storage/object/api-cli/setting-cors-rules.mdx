...
excerpt: |
  The CORS (cross-origin resource sharing) standard describes new HTTP headers which provide browsers a way to request remote URLs only when they have permission. Although some validation and authorization can be performed by the server, it is generally the browser's responsibility to support these headers and honor the restrictions they impose.
totalTime: PT10M
steps:
  - step: Configure CORS on an Object Storage Bucket
    type: HowToStep
    url: https://www.scaleway.com/en/docs/object-storage-cors/#-Setting-CORS-on-an-Object-Storage-Bucket
  - step: Getting the CORS Configuation of a Bucket
    type: HowToStep
    url: https://www.scaleway.com/en/docs/object-storage-cors/#-Getting-the-CORS-Configuration-of-a-Bucket
  - step: Verifying the CORS Configuration of a Bucket
    type: HowToStep
    url: https://www.scaleway.com/en/docs/object-storage-cors/#-Verifying-the-CORS-Configuration-of-a-Bucket
  - step: Deleting the CORS configuration of a Bucket
    type: HowToStep
    url: https://www.scaleway.com/en/docs/object-storage-cors/#-Deleting-the-CORS-Configuration-of-a-Bucket
categories: 
  - storage
brands: 
  - elements
...
permalink: docs/object-storage-cors/
keywords: cloud tutorials, cloud documentations, cloud server, cloud hosting, ubuntu server, debian server, linux server, IAAS, baremetal, scalability, ARM, physical server, dedicated server, physicalization, server, minio, migration, object storage, s3
dates:
...
An example of a cross-origin request: The frontend JavaScript code for a web application served from _http://webapplication.com_ uses `XMLHttpRequest` to make a request for _http://customerapi.io/data.json_. Another example might be JavaScript that calls files in an Object Storage bucket, like web fonts, downloads etc. It is possible to configure CORS for each bucket with [`aws-cli`](/storage/object/api-cli/object-storage-aws-cli.mdx).
