...
description: This page shows you how to use placement groups using the Scaleway Elements API
steps:
  - step: placement groups Principles
    type: HowToStep
    url: https://www.scaleway.com/en/docs/using-placement-groups-from-the-api/#-Placement-Groups-Principles
  - step: placement groups in Practice
    type: HowToStep
    url: https://www.scaleway.com/en/docs/using-placement-groups-from-the-api/#-Placement-Groups-in-Practice
  - step: Creating a placement group
    type: HowToStep
    url: https://www.scaleway.com/en/docs/using-placement-groups-from-the-api/#-Creating-a-Placement-Group
  - step: Adding Multiple Existing Instances to a placement group
    type: HowToStep
    url: https://www.scaleway.com/en/docs/using-placement-groups-from-the-api/#-Adding-Multiple-Existing-instances-to-a-Placement-Group
  - step: Adding a Single Instance to a placement group
    type: HowToStep
    url: https://www.scaleway.com/en/docs/using-placement-groups-from-the-api/#-Adding-a-Single-Instance-to-a-Placement-Group
  - step: Removing an Instance From a placement group
    type: HowToStep
    url: https://www.scaleway.com/en/docs/using-placement-groups-from-the-api/#-Removing-an-Instance-From-a-Placement-Group
categories: 
  - compute
tags: compute placement-group server
...
Placement groups have two operating modes. The first one is called `max_availability`. It ensures that all the Virtual Instances that belong to the same group will not run on the same underlying hardware. The second one is called `low_latency` and does the exact opposite: it brings Virtual Instances closer together to achieve higher network throughput.
...
For more information and to see the full capabilities of placement groups, refer to the [full api documentation](https://developers.scaleway.com/en/products/instance/api/#placement-groups-d8f653).
...
- Instances must be VM only (Bare Metal servers are not supported yet)
