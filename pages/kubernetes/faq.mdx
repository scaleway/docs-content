...
Yes, Kubernetes Secrets are encrypted at rest in etcd using strong encryption. As of now, only Secrets are encrypted by default. A: Yes, Kubernetes Secrets are encrypted at rest in etcd using strong encryption. As of now, only Secrets are encrypted by default. While there are no current plans to encrypt all objects in etcd, this may be considered in the future based on security requirements and operational best practices.
