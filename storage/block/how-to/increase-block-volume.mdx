...
excerpt: |
  This page explains how to increase your Block Storage volume
totalTime: PT15M
steps:
  - step: TODO
    type: HowToStep
...
keywords: Block Storage, increase, Howto
dates:
...
## How to increast the volume size from the Scaleway console
...
## How to increase the parition size of the volume

<Message type="tip">

It is recommeneded to make a backup of your data using the [Snapshot](https://www.scaleway.com/en/docs/compute/instances/how-to/create-a-snapshot/) feature, before increasing the partition size of your volume to avoid any data loss. 
...
2. Use `lsblk` to identifiy your Block Storage volume by its mount point. In the following example the volume is mounted at `/mnt/block-volume`: 
...
3. [Unmount](https://www.scaleway.com/en/docs/storage/block/how-to/unmount-a-volume/) the volum:

  ```
  umount /mnt/block-volume
  ```

4. Check that the volume is not mounted any longer by running the `lsblk` command. 
...
6. Run `e2fsck` on the Block Storage volume to check that everything went well. Replace `/dev/sdX` with the name of your block volume in the following command:
  ...
  An output like the following example displays. The status of your filesystem shall be `clean`
