...
Scaleway is also responsible for the operational upkeep of the operating system and system applications on the nodes. This includes essential components like CoreDNS, Kubeproxy, Container Networking Interface (CNI), and Container Storage Interface (CSI), which are vital for the optimal functioning of the Kubernetes cluster and its associated resources. To ensure your nodes are always using the latest version of the underlying OS image it is recommended to limit their lifespan to a period of 30 days and to replace them regularly. 
...
Furthermore, it is expected that you use the appropriate interfaces for managing resources, such as `kubectl` and the Scaleway Kubernetes console. However, please note that if you make any modification to the pre-installed components, such as Kubelet, at the node or system level as mentioned earlier, the responsibility for managing those components will be transferred to you.
