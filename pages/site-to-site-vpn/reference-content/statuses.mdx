...
A Site-to-Site VPN connection also always has a **status**, separate to that of the VPN gateway which can be retrieved via the API using the **Get a connection** call. The connnection status is based on the [tunnel status](), [BGP session status](), and attachment of a routing policy.
