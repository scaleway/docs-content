...
<Message type="requirement">
  ...
  - You are part of the VPC Regional Private Networks Private Beta
...
2. Email the Project ID to vramiro@scaleway.com. You can find the Project ID in your Scaleway console at the URL `https://console.scaleway.com/<project-name>/settings`.

3. Create a Private Network for the Paris region via the API:
...
4. Create resources of your choice (e.g. Instances, Elastic Metal servers, Public Gateways etc) either via the console or the API. Make sure you save the resource ID after creation. For example, [create an Instance](/compute/instances/how-to/create-an-instance/) and note the Instance ID (`server-id` in the API).

5. Attach the resource to the Regional Private Network you created, via the API. For an Instance, use the following API call:
...
## Migrating zonal Private Networks to VPC regional Private Networks

To migrate all the Private Networks in an Organization, use the following API call:

    ```
    curl --request GET \
      --url https://agw.stg.fr-par-2.internal.scaleway.com/vpc/v2/regions/fr-srr/private-networks/migrate-zonal \
