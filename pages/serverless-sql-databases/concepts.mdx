...
Credentials are the login and password (also called "username" and "password" in PostgreSQL vocabulary) used by a user, or an application to connect to a database. With Scaleway Serverless SQL Databases, logins are [IAM principal IDs](/iam/concepts/glossary/#principal) and passwords are [IAM secret keys](/iam/concepts/glossary/#api-key).
