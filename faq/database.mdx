...
Currently, you can not connect PAR2 or PAR3 Instances to a Database Instance located in the **Paris** region using Private Network.
...
However, you can use the Database virtual IP (VIP) in a DHCP-controlled environnement by [reserving the IP](/network/vpc/how-to/configure-a-public-gateway/#how-to-review-and-configure-dhcp) in the DHCP server configuration to be sure it is not assigned, then configure it on the Private Networks endpoint.
...
## What should I use as virtual IP? How to calculate CIDR netmask?
