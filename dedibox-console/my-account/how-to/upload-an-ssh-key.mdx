---
title: How to upload and use an SSH key
description: This page explains how to upload and use an SSH key.
excerpt: |
  This page explains how to upload and use an SSH key
steps:
  - step: TODO
    type: HowToStep
    url: https://www.scaleway.com/en/docs/dedibox-console/my-account/how-to/upload-an-ssh-key/
image: /assets/images/docs/scaleway.png
keywords: Dedibox, console, SSH, ssh key, key, Howto
dates:
  validation: 2021-07-13
  posted: 2020-03-12
---

SSH keys are an alternative way to authenticate identity on computer systems, and are more secure than passwords. Each SSH key consists of a private and a public part and can be [generated](https://www.scaleway.com/en/docs/configure-new-ssh-key/) on your computer. The public part can be uploaded to the Scaleway Dedibox console and transferred to your dedicated server during installation for passwordless authentication purposes.

<Message type="requirement">

  - You have an account and are logged into the [Dedibox Console](https://console.online.net) 

</Message>

1. Click your identifier in the top right corner of the console to open the pop-up menu. Then click **SSH keys** in the menu.

    <Lightbox src="scaleway-dedibox-console-ssh-key-menu.png" width="300" alt="" />

    A page listing all your existing SSH keys displays (if you don't have any SSH keys, the listing will be empty).

2. Click **Add SSH key** to display the SSH key form.

3. Enter a description of your choice (e.g. your computer's name), then enter the public key in the form.

4. Click **Add SSH key** to upload the public key to your Scaleway Dedibox account.

You can now select your key during the installation of your Dedibox and use it to authenticate on your server.

<Message type="note">

If you upload another key or change it, you need to replace it manually on your Dedibox. Keys are only transferred from the console to your server during installation.

</Message>

<Navigation title="See Also">
  <PreviousButton to="/dedibox-console/my-account/how-to/update-email/">How to your account email</PreviousButton>
  <NextButton to="/dedibox-console/my-account/how-to/outsource-dedibox/">How to outsource a Dedibox</NextButton>
</Navigation>