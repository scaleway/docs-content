...
IP addresses from within a given subnet (that is to say, allocated from a certain CIDR block which is used for a certain Private Network) display with a slash at the end. In the case of an IPv4 adddress this could look like `172.16.16.2/22`. This format encapsulates information both about the IP address itself, and which subnet it is from. If you conigure an Instance manually with this address, it sets both the correct IP and a route to the correct subnet.
</Concept>
