...
1. From the Scaleway console, [create a new bucket](/storage/object/how-to/create-a-bucket/). When entering a name for your bucket, the name must be the same as the domain/subdomain intended to be used with it. For example, to create a bucket accessible at `download.example.com` the bucket name must be `download.example.com`.
