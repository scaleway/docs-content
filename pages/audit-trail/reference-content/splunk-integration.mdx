...
4. Provide the required information (e.g., polling interval, Scaleway region). Obligatory fields are marked with an asterisk.
    ...
    - The **Product name** field is optional. If no product name is specified, the input will retrieve data from all the Scaleway products. 
        ...
        To retrieve a product name, go to the [Scaleway console](https://account.scaleway.com/), select **Monitoring > Audit Trail** in the side menu, and click the **Product** field on the **Events** tab. Use the product name as displayed in the dropdown. However, make sure that you use lowercase letters only (for example, `instance`) and omit the copyright sign from the name.
