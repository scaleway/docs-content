...
To connect with IAM, create one or more [API keys](https://www.scaleway.com/en/docs/iam/how-to/create-api-keys/), each associated with a [policy](https://www.scaleway.com/en/docs/iam/how-to/create-policy/) granting at least the following permission sets: `KafkaConsumerAccess`, `KafkaManagerAccess`, and `KafkaProducerAccess`
The following **Go**, **Python**, and **Java** code snippets demonstrate how to connect to the cluster, using sample consumer and producer.

1. Connect with Go or Python
...
2. Connect with Java
  ...
  Donwload Kafka and export it in the PATH
  ```
  ...
  For each consumer and producer client, create the `config.properties` file
  ```java
  ...
  Run topics, consumer or producer commands
  ```bash
  ...
  All these code snippets can be found on the console, on the kafka cluster dashboard, **Connectivity section** by clicking **Connect**
