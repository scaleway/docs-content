...
The version upgrade is only available for:
  ...
  - Database Instances using Block volumes. If your Database Instance uses a local volume, you must first [change the volume type](/postgresql-and-mysql/how-to/change-volume-type/) to a Block volume and try again.
</Message>
...
- A [PostgreSQL Database Instance](/postgresql-and-mysql/how-to/create-a-database/) running on an outdated engine version
...
3. Select the upgrade method. Two methods are available:
    ...
    <Message type="note">
      Follow the [migrating endpoints via the CLI](/postgresql-and-mysql/api-cli/migrating-endpoints/) procedure to quickly migrate your endpoints. You can use this procedure to revert the migration anytime. <br /><br />
