...
To activate JSON mode, provide the `response_format` parameter in with `{"type": "json_object"}`. It is possible to define a JSON schema using the `schema` property of `response_format`, ensuring the generated output meets your requirements.
