...
Writing bash scripts does not require any special skills. You just write down the commands you want the computer to follow in a text file, instead of typing them one by one. This is handy for automating tasks, like managing files or folders. Bash scripts usually end with `*.sh`.
