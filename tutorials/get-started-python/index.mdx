...
5. Enter the following into the Python shell. The values of `cats` and `dogs` should both remain at 3 from our previous input.
    ...
    Our `elif` condition (else if) told Python to check for the possibility that the number of cats and dogs were equal (represented with `==`) and added an ouput for this possibility. The printed statement is correct!
    ...
    <Message type="tip">
      Note the difference between using a single `=` to assign a value to a variable (`cats = 2`) versus using a double `==` as the **equals to** operator, which tests whether two values/variables have the same value  (`if cats == dogs`). Don't confuse the two!
    </Message>
...
**Print statements**: "printing" in Python just means to display an output on the screen. The print statement syntax is `print("hello world!")`.
