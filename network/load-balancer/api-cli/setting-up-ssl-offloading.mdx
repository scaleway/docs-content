...
6. The certificate details are presented in the form of a `json` list. Copy the value of the first line starting with `id` and set it as a variable:
	...
	```
7. Create a [new frontend](https://developers.scaleway.com/en/products/lb/api/#post-8499de) by specifying the **IDs of the Load Balancer**, **an existing backend** and the **certificate**. Specify also the `inbound_port` (Port `443` for the default `HTTPS` port), on which the frontend will listen for incoming connections.
	```
