---
meta:
  title: Cockpit
  description: Discover Scaleway's Cockpit
content:
  h1: Cockpit
hero: assets/scaleway-cockpit.webp
category: observability
---

## What is Scaleway's Cockpit?

Scaleway's Cockpit is a monitoring and alerting solution designed for multi-source observability. Cockpit allows you to collect, store, and analyze metrics and logs from your infrastructure and applications, giving you insights into your system's performance and behavior.

## How does Cockpit integrate with other Scaleway products?

Cockpit integrates seamlessly with Scaleway’s ecosystem. It provides pre-built dashboards and alerts to monitor other Scaleway products' performance.

## How am I billed for using Cockpit with my Scaleway data?

There are three pricing plans you can choose from when using Scaleway's monitoring Cockpit:

| Plan type | What does the plan include?                                                                                           | Cost           |
|-----------|-----------------------------------------------------------------------------------------------------------------------|----------------|
| Free      | Free storage for metrics, logs, and traces, for a period of **3 days for metrics** and **1 day for logs and traces**. | Free           |
| Premium   | **31 days of storage for metrics**, and **7 days for logs and traces**.                                               | €29 per month  |
| Expert    | **365 days of storage for metrics**, and **31 days for logs and traces**.                                             | €129 per month |

## How am I billed for using Cockpit with external data?

Cockpit’s pricing is based on two dimensions: the volume of external metrics, logs, and traces ingested, and the duration of storage.

- External metrics are billed €0,15 per million of samples ingested, per month.
- External logs are billed €0,35 per GB ingested, per month.
- External traces are billed €0,35 per GB ingested, per month

You will not be charged for querying external data.

## How do I send my own data to my Cockpit?

You can send your data using our [dedicated documentation](/observability/cockpit/how-to/send-metrics-logs-to-cockpit/).
If you have not set up any agent, check out our [documentation on how to configure a Grafana agent](/observability/cockpit/api-cli/configuring-grafana-agent/).

## Which Scaleway products are integrated into Cockpit?

The following products are integrated into Scaleway's Cockpit:

 - IoT Hub
 - Cockpit
 - Load Balancers
 - Private Networks
 - Public Gateways
 - Database RDB MySQL
 - Redis
 - Database RDB PostgreSQL
 - Messaging and Queuing NATS
 - Messaging and Queuing SQS
 - Messaging and Queuing SNS
 - Instances
 - Object Storage
 - Serverless Containers
 - Serverless Functions
 - Secret Manager
 - Kubernetes Kapsule
 - Kubernetes Kosmos

Refer to our [documentation on understanding Cockpit usage and pricing](/observability/cockpit/reference-content/understanding-cockpit-usage/) for more information on how to reduce extra costs.
Read our [Cockpit capabilities and limits documentation](/observability/cockpit/reference-content/cockpit-limitations/#product-integration-into-cockpit) to find out more about product integration.

<Message type="important">
 Sending metrics and logs for Scaleway resources or personal data using an external path is a billable feature. In addition, any data that you push yourself is billed, even if you send data from Scaleway products. Refer to the [product pricing](https://www.scaleway.com/en/pricing/?tags=available,managedservices-observability-cockpit) for more information.
</Message>