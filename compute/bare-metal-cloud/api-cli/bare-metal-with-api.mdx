---
title: Deploying a Bare Metal server with one API call
description: This page shows you how to deploy a Bare Metal server running on your favourite Operating System with one API command 
...
2. Retrieve a list of all Operating systems available in the desired Availability Zone. Currently all Bare Metal servers are located in the AZ `fr-par-2`: 
...
3. Create the Bare Metal server from the API using cURL and the `POST` command on the endpoint `/baremetal/v1/zones/fr-par-2/servers`: 
...
4. The server is being delivered to your account and is automatically being installed on the Operating System choosen. You can retieve the status of the installation using the following API call: 
    ...
    Your new Bare Metal server will be installed and ready to be used in usually less than **15 minutes**, all API features are listed in the [API documentation](https://developers.scaleway.com/en/products/baremetal/api/).

<<<<<<< HEAD
For more information about Bare Metal servers, refer to our [dedicated product documentation](/compute/bare-metal-cloud) and the [FAQ](https://www.scaleway.com/en/faq/bare-metal-servers/).
=======
