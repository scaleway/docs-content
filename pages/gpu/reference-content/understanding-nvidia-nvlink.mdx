...
| Feature           | NVLink 4.0 (H100-SXM)     | NVLink 5.0 (B300-SXM)     | PCIe 5.0                           |
|-------------------|---------------------------|---------------------------|------------------------------------|
| **Use case**      | High-performance computing, deep learning | High-performance computing, deep learning | General-purpose computing, graphics |
| **Bandwidth**     | Up to 900 GB/s (aggregate, multi-GPU) | Up to 1800 GB/s (aggregate, multi-GPU) | 128 GB/s (x16 bidirectional)     |
| **Latency**       | Lower than PCIe (sub-microsecond) | Lower than PCIe (sub-microsecond) | Higher compared to NVLink        |
| **Communication** | Direct GPU-to-GPU           | Direct GPU-to-GPU           | Through CPU or PCIe switch         |
| **Memory sharing** | Unified memory space across GPUs | Unified memory space across GPUs | Requires CPU intervention (higher overhead) |
| **Scalability**   | Multi-GPU direct connection via NVSwitch | Multi-GPU direct connection via NVSwitch | Limited by PCIe lanes             |
| **Efficiency**    | Optimized for GPU workloads | Optimized for GPU workloads | More general-purpose               |
