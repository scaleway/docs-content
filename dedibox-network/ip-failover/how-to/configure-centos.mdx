...
You will find below, information on how to configure a [failover IP](/dedibox-network/ip-failover/concepts/#failover-ips) on Dedibox servers running on CentOS.
...
2. From the Netowrk Manager interface select **Edit a connection** and select the interface you want to add an alias to. Then Click **Edit...**.

3. Browse through the **Add ...** section to add an additional IP address. 

4. Enter the details of your failover IP and save the settings to generate the configration file. 

5. Check the configuration file, to verify that the failover IP address has been added: 
