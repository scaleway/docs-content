...
3. Complete the following steps in the wizard: 
    ...
    - Add **Volumes**. This is an optional step. Volumes are storage spaces used by your Instances. You can leave the default settings of a maximum local storage, or choose how much [local](/compute/instances/concepts#local-volumes) and/or [block](/compute/instances/concepts#block-volumes) storage you want. Your **system volume** is the volume on which your Instance will boot. The system volume can be either a local or a block volume. 
    
    <Message type="important">

    Please note that:  
      - a volume with an OS image should be at least 10GB (125GB for a GPU OS).
      - if several Block Storage volumes are connected to your Instance, the first volume will contain the OS and is required to boot the Instance. You may change your boot volume later in Advanced Settings.
