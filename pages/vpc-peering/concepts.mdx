...
When two VPCs are peered, DNS resolution can extend across the peering connection. To resolve a resource located in a peered VPC, you must include the target VPC name or ID in the DNS query, since Private Network names are only guaranteed to be unique within a single VPC.
