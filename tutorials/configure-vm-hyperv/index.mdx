...
A Windows distribution can be installed on a Scaleway Dedibox server in two ways: through the [Dedibox console](https://account.scaleway.com/?service=dediboxen/login) or through [KVM over IP](<https://en.wikipedia.org/wiki/KVM_switch#KVM_over_IP_(IPKVM)>).
...
10. Assign a virtual MAC address to the failover IP from the [Dedibox console](https://account.scaleway.com/?service=dedibox) to make sure that the virtual machine is able to communicate on the internet. Refer to the [dedicated documentation](/dedibox-ip-failover/how-to/create-virtual-mac/) for more information about how to create a virtual MAC address. Click **Settings** in the VM section of the menu.
