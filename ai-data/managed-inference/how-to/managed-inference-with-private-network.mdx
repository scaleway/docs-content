...
3. Transfer the TLS certificate to the Instance using the `scp` (secure copy) command to securely transfer the certificate from your local machine to the Scaleway Instance.
   - Example command:
     ...
     ```
   - Replace placeholders in the command aboive as follows:
...
5. Open a text editor, e.g., `nano`, and create a new script:
     ```bash
     nano inference.py
     ```
6. Paste the sample Python code for the script into your text editor:
    <Message type="tip">
    This script takes an example of a conversational task with a request sent to an [LLM through the Chat Completions API](https://www.scaleway.com/en/docs/ai-data/managed-inference/reference-content/openai-compatibility/#chat-completions-api).
    </Message>
    ```py
    import requests
...
6. Save and Exit the Editor. Save the file with `CONTROL+O`, then exit `nano` with `CONTROL+X`.
7. Make the Script executable using the following command:
     ```bash
...
5. Click **Detach Private Network** to confirm removal of the private endpoint for your deployment.
    ...
    <Message type="important">
      Managed Inference deployments must have at least one endpoint, either public or private. Consequently, users can't detach their private endpoint without having a public one.
    </Message>