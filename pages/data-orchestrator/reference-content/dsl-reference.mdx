...
| Property&nbsp;name | Type | Required | Description|
|:--|:---|:---|:---|
| `if` | string | no | A [runtime expression](https://github.com/open-workflow-specification/specification/blob/main/dsl.md#runtime-expressions), if any, used to determine whether or not the task should be run.<br />The task is considered skipped if not run, and the *raw* task input becomes the task's output. The expression is evaluated against the *raw* task input before any other expression of the task. |
...
The [Scaleway Serverless Function Call](#serverless-function-call-exclusive-to-data-orchestrator) allow you to call default endpoint of yours Serverless Functions, as a http call.

Instead of using an HTTP endpoint (with potential authentication), `call-Serverless-Function` simply uses the function's identifier (`id`) and region (`region`) to invoke it, and handles authentication for you.

#### Properties

| Property&nbsp;name | Type | Required | Description                                    |
|:-----|:----|:--------|:-----------------------------------------------|
| `id` | string | yes | The id of the scaleway serverless function.    |
| `region` | string | yes | The region of the scaleway serverless function |
| `method` | string | yes | The HTTP request method. |
| `headers` | map | no | A name/value mapping of the HTTP headers to use, if any. |
| `body` | any | no | The HTTP request body, if any. |
| `query` | map[string, any] | no | A name/value mapping of the query parameters to use, if any. |
...
Instead of using an HTTP endpoint (with potential authentication), `call-Serverless-container` simply uses the container's identifier (`containerdId`) or the container's name (`name`) + namespace's identifier (`namespaceName` or `namespaceId`) and region (`region`) to invoke it, and handles authentication for you.

#### Properties

| Property&nbsp;name        | Type | Required | Description                                                                                                                                                                                                                                                                                                                                                          |
|:------------|:----|:--------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `name`          | string |   no   | The name of the Serverless Container.<br />*Required if `containedId` not set.*                                                                                                                                                                                                                                                                                        |
| `namespaceId`   | string |   no   | The ID of the Serverless Container namespace.<br />*If `containerId` not set: required if `namespaceName` is not set.*                                                                                                                                                                                                                                                 |
| `namespaceName` | string |   no   | The name of the Serverless Container namespace.<br />*If `containerId` not set: required if `namespaceId` is not set.*                                                                                                                                                                                                                                                 |
| `containerId`   | string |   no   | The ID of the existing Serverless Container.<br />*Required if `name` not set.*                                                                                                                                                                                                                                                                                        |
