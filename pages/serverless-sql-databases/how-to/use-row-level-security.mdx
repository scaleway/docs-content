...
This requires setting up different [IAM permissions sets](/iam/policies-permissions/permission-sets/) for each role (**ServerlessSQLDatabaseFullAccess** or **ServerlessSQLDatabaseReadWrite** for one role, and **ServerlessSQLDatabaseDataReadWrite** for the other).
...
1. Create a new [IAM application](/iam/principals/create-application/).

2. Create a new [IAM policy](/iam/policies-permissions/create-policy/), and add the **ServerlessSQLDatabaseDataReadWrite** permission set to the application you just created.
...
3. Create an [API Key](/iam/credentials/create-api-keys/) for this application, and connect to your Serverless SQL Database with this application.
    ```bash
