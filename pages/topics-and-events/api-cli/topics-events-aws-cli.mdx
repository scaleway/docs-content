...
2. Get the function endpoint from the [Scaleway console](https://account.scaleway.com/?service=elementsfunctions) under "Functions" -> "[YOUR-FUNCTION-NAMESPACE]" -> "[YOUR-FUNCTION-NAME]" -> "Function Settings" tab -> "Function Endpoint"
...
6. Use the following command to delete the Scaleway queue (if you had a Scaleway Queues target):
    ...
    <Message type="tip">
    For **lambda**, delete the function (if necessary), using the [Scaleway console](https://account.scaleway.com/?service=elementsfunctions)
    </Message>
