...
Credentials give services and platforms access to your Messaging namespace, enabling them to connect to the host system. Credentials are [protocol](#messaging-protocol)-specific: for [SQS/SNS](#sqssns) credentials different levels of permissions can be defined, whereas NATS credentials give full access to the messaging namespace. Refer to our [Messaging Reference Content](/compute/messaging/reference-content/) for more information.
...
Scaleway's Messaging product is a [message broker](#message-broker) tool that allows you to transfer messages between different microservices and platforms. This enables them to "talk" to each other effectively even if they are not otherwise compatible. Messaging enables and simplifies microservices application development and allows you to build highly scalable, reliable, distributed applications. 
