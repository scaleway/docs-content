...
A users primary responsibility is to deploy and manage the applications or services that are planned to run on the Kubernetes cluster.

Users are accountable for deploying any resources to the cluster and managing their configurations.

To manage resources effectively, it is essential to use appropriate interfaces like `kubectl` and the Scaleway Kubernetes console. Note, however, that the responsibility for managing pre-installed components such as Kubelet at the node or system level shifts to the user when they modify them.

For the security and stability of the user's cluster, Scaleway strongly advises applying patches and security updates regularly. Minor version updates can be directly applied to each cluster through the Scaleway console or CLI to ensure proper functionality.

To keep the cluster's nodes up to date, it is recommended to set a lifespan limit of 30 days for each node and replace them regularly with the latest version of the underlying OS image. This practice ensures that the cluster operates with the most recent and secure infrastructure.
