...
JSON mode allows you to guide the language model in outputtiong well-structured JSON data that matches a provided schema.
To activate JSON mode, provide the `response_format` parameter with `{"type": "json_object"}`. It is possible to define a JSON schema using the `schema` property of `response_format`, ensuring the generated output meets your requirements.
JSON mode is useful for applications like chatbots or APIs, where a consistent, machine-readable format is essential for easy processing.
...
Structured outputs enable you to format the model's responses to suit specific use cases. To activate structured outputs, provide the `response_format` parameter with `{"type": "json_schema"} `.
...
By specifying the expected response format through the API, you can make the model consistently deliver the output your system requirements.
