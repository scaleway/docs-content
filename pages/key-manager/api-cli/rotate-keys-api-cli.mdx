...
- For keys with the `external` origin (BYOK), rotation is performed by importing new key material with the `ImportKeyMaterial` endpoint, instead of using `RotateKey`. Refer to the [Using Bring Your Own Key with the Scaleway API and CLI](/key-manager/api-cli/byok-api-cli/) documentation for more information.
