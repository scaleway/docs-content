...
1. (Optional) Install `nodejs`, `gunpg2`, `ruby` and `curl`. When using Rails, is is required to install Node.js. The asset pipeline compiler of Rails requires a Javascript runtime. The version of `Node.js` that you install does not matter. If you do not use Rails, skip this and proceed to the next step.
    ```
...
<Message type="note">
  Routes are matched in the exact order that they are defined in your code, this means that the second route in the example below will never get matched because we have the exact same pattern further up in the code.
</Message>
