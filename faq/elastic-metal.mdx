...
The Windows Server licenses exist in 2 versions (Standard and Datacenter) with different prices. Depending on the version you choose, you have certain features (roles) available as well as limitations in the Standard version. You can find all the limitations on the Microsoft website.

## What is the price of Windows Server licenses?

Windows Server 2019 or 2022 licenses are priced on a monthly basis (no prorating, calendar month) and defined according to 3 criteria: the number of physical CPUs, the number of cores per CPU and the OS version: Standard or Datacenter. You pay your license month by month, with your Elastic Metal whether it is billed per hour or per month.

## Can I transfer the monthly license to another server ?

The proposed licenses are only valid on the Elastic Metal servers that we propose and it will not be possible to recover it for an external use.  
Each installation of Windows imply the invoicing of a new Windows license (except in case of reinstallation of Windows on an Elastic Metal with a Windows installation provided by our installation system).
