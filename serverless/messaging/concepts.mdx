...
Standard queues represent the default queue type, and offer an at-least-once message delivery system. Unlike [FIFO queues](#fifo-queue), Standard queues provide only best-effort attempts to deliver messages in order and prevent the same message being received more than once.
