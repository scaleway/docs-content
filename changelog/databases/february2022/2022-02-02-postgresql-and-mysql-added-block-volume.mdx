---
title: Database with Block Storage is now in general availability
status: added
author:
    fullname: 'Join the #database channel on Slack.'
    url: 'https://slack.scaleway.com/'
date: 2022-02-02
category: databases
product: postgresql-and-mysql
---

With this feature, you can:

* [Change storage and node size](/postgresql-and-mysql/how-to/create-a-database/) independently to optimize your costs and adapt to your application's traffic.
* Scale up your storage volume from 5GB to 10TB with no downtime.
* Make Snapshots of your data in seconds for any size of the volume and recover a previous state of your data in case of accidental data alteration.

