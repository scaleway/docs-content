...
Scaleway [Smart Labeling](ADD_LINK) is a platform that allows you to annotate images or videos, either by yourself or via a team of annotators. Smart Labeling currently uses Intel's CVAT as its interface: [open-source Computer Vision Annotation Tool](https://github.com/openvinotoolkit/cvat). Smart Labeling facilitates the use of CVAT in a cloud-native manner, with labeling software running on-demand, annotations safely persisting in an automatically backed-up database, and datasets stored in cost-efficient S3 buckets.
