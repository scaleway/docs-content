...
<Message type="important">
  - You can only upgrade to a Database Instance with the same storage type. For example, if you have a Database Instance with local storage, and want to upgrade to one that uses Block storage, you must first [change the volume type](/managed-databases/postgresql-and-mysql/how-to/change-volume-type).
</Message>
