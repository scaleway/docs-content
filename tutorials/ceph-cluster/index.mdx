...
4. Type the following command to add the user to the sudoers configuration so it has sudo rights:
...
5. Type the following command to install an NTP client on your Ceph nodes:
  ...
  ```
    ...
    <Message type="note">
     ...
     The installation of an NTP client allows you to avoid synchronization issues caused by time-drift.
...
7. Repeat all the previous steps 3 times to setup each of your 3 nodes.
...
1. Type the following command to install `aws-cli` and `awscli-plugin` on your machine using `pip`, the Python packet manager:
