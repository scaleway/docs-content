...
| HTTP/2 | <Icon name="validate"/>                 | <Icon name="validate"/>                    | <Icon name="validate"/>                | <Icon name="validate"/>                   |
| HTTP/3 | <Icon name="validate"/>                 | ❌                    | ❌                | ❌                   |

In brief:

- Scaleway Load Balancers support HTTP/2 connections to frontends, and HTTP/2 connections from backends to backend servers.
...
When [TLS is activated](TODO) on your backend, HTTP/1 and HTTP/2 support are automatically enabled as default for the backend itself, and all attached frontends. No additional configuration is required.
[TODO: check: does the user have to have added a certificate as well for the frontend to support HTTP2 when attached to a TLS backend?]

## Configuring your Load Balancer for HTTP/3

HTTP/3 is only supported for frontends that:
- Are attached to an HTTP backend (TODO: with TLS activated?)
- Have an SSL/TLS certificate. 

HTTP/3 is not supported on backends in any case.

To create a frontend that allows HTTP/3 connections:

1. From the [Load Balancer page](https://console.scaleway.com/load-balancer/) of the Scaleway console, click the Load Balancer you want to activate HTTP/3 on.

2. Click the **Frontends** tab.

3. Click **Add Frontends**.

4. In the **SSL certificates** box, select an SSL certificate to add to your frontend. See our dedicated documentation on [generating an SSL/TLS certificate](TODO).

5. Under **Configure backend**, ensure that you choose a backend using HTTP protocol (TODO and TLS?)

6. Under **Advanced Settings**, select **Enable HTTP/3**.

7. Click **Create Frontend**.

TODO: can a user also edit an existing frontend to enable HTTP/3 ?
