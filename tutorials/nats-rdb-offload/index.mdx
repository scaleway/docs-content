...
<Lightbox src="scaleway-nats-offload-archi.png" alt="" />
...
To properly follow this tutorial, we recommend you [create a dedicated Project](/identity-and-access-management/organizations-and-projects/how-to/create-a-project/) in your Scaleway account.
You can name it "RDB and NATS Tutorial". It will hold all your Scaleway resources created for this tutorial, so they will be easier to find.
...
- Generated NATS credentials (and saved them under your project directory)
- Created a Managed Dabatase (/managed-databases/postgresql-and-mysql/quickstart/) with mySQL


## Create a NATS stream

Refer to the [following documentation](/serverless/messaging/api-cli/nats-cli/) to create your NATS stream.
We recommend keeping stream specifications as simple as possible. 
There is no need for replication or specific parameters for this tutorial.

When your NATS server is up and running, check you did not forgot anything for the Scaleway [context](/serverless/messaging/api-cli/nats-cli/#define-a-context) by running the following code in a new window of your terminal:
...
## Connect to your Scalesway Managed Database
...
7. Follow the instructions in the terminal to validate your password
    You should have access to the mysql environment 
...
2. Paste the following Python code into the file:
   ...
   This is normal as both applications are communicating through the same NATS server.
...
```
Messaging and Queuing NATS is a Serverless product and is only charged for when you are moving messages through the broker. This eliminates the need to worry about unexpected billing when you are not using it.
