...
2. Create a folder named **BucketScan** in the **FunctionsTutorial** folder and access it.
3. run the following command to create the **ImageTransform** function:
...
6. Press the `ESC` key, type `:wq` and press `Enter` to save the file.
7. Run the following command in the same terminal to download the required dependencies and packages:
