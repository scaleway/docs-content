...
content:
  ...
  paragraph: This page shows you how to setup Gitlab with a managed PostgreSQL database
...
- You have an Instance or an Elastic Metal server running Ubuntu Bionic Beaver (18.04)
...
5. Set your domain name in the `EXTERNAL_URL` variable, then install GitLab using `apt`:
  ...
  GitLab will automatically request a [Let's Encrypt](/tutorials/configure-nginx-lets-encrypt/) certifcate for your domain name. Make sure you have pointed the `A-Record` of your domain to your Instance's IP address to obtain a valid certificate.
...
3. Enter the username (`gitlab`) and a secure password. In the **Permissions** tab, give `ALL` permissions to the user on the `rdb` database.
