---
title: How to change the account language
description: Switch languages in the Scaleway account interface.
tags: account language change
dates:
  validation: 2025-09-03
  posted: 2021-06-22
---
import Requirements from '@macros/iam/requirements.mdx'


You can choose which language you would like to view the [Scaleway console](/account/concepts/#console) in. This language will also be used for any marketing and product communications Scaleway sends you.

<Requirements />

- A [Scaleway account](https://account.scaleway.com/?service=elements)

1. Log into the [Scaleway console](https://account.scaleway.com/?service=elements).
2. Access your [Profile Settings](https://account.scaleway.com/?service=elementssettings/account/profile).
3. Click the **Edit profile** button, in the **User information** section. A pop-up displays.
4. Select your preferred language from the drop-down list, then click **Confirm changes**.

Your console will now be displayed in your chosen language, which will also be used for all marketing and product communications you receive. You can change your preferred language at any time by repeating this procedure.