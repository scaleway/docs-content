---
meta:
  title: Setting up a KVM Hypervisor on Bare Metal servers
  description: This page shows you how to setup a virtual machine with KVM on a Bare Metal
content:
  h1: Setting up a KVM Hypervisor on Bare Metal servers
  paragraph: This page shows you how to setup a virtual machine with KVM on a Bare Metal
steps:
  - step: Installing the KVM Hypervisor and a First Guest
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/install-kvm-bmaas/#-Installing-the-KVM-Hypervisor-and-a-First-Guest
  - step: Forwarding Ports to Guests with IPtables
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/install-kvm-bmaas/#-Forwarding-Ports-to-Guests-with-IPtables
categories:
  - compute
  - bare-metal
tags: console security KVM-Hypervisor servers Bare-Metal First-Guest 
...
- You have created a [Bare Metal server](/compute/bare-metal-cloud/how-to/create-server/) running Ubuntu Bionic Beaver (18.04)
...
9. Start the virtual machine again, after a few seconds the Ubuntu login appears:
  ...
  The VM has configured its network automatically with DHCP and is able to communicate with the outside world by passing all traffic over the virtual bridge.
  However it is not possible to receive external connections with on the virtual machine until now.
...
3. Retrieve the MAC address of the virtual machine in preparation of assigning an IP statically, by running `virsh dumpxml VM_NAME`, i.e.:
...
5. Add this line line after the <range ...> section. Make sure to edit the MAC-address and the name to the values of your virtual machine:
...
8. Configure a port forwarding that forwards the port `5678` from the host machine to port `22` on the guest machine, to be able to SSH into it:
