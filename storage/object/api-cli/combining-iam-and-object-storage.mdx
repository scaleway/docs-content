...
Scaleway Object Storage allows you to manage access to your resources with two features, [Identity and Access Management (IAM)](/iam/concepts/#iam) and [Bucket policies](/object/api-cli/bucket-policy/).
...
In order to perform actions on a bucket, a [principal](/object/concepts/#principal) (user or application) must be granted access via an IAM policy, and optionally via a bucket policy.
...
4. [Create and push a bucket policy](/object/api-cli/create-bucket-policy/) in each bucket in which you want fine-grained access.
...
- The user must be able to read ([GET](/object/api-cli/object-operations/#getobject)) and write ([PUT](/object/api-cli/object-operations/#putobject)) objects in a bucket A.

- The application must be able to read ([GET](/object/api-cli/object-operations/#getobject)) objects in bucket A, and to write ([PUT](/object/api-cli/object-operations/#putobject)) and [DELETE](/object/api-cli/object-operations/#deleteobjects) objects in a bucket B.
...
3. [Create a bucket policy](/object/api-cli/create-bucket-policy/) to manage access to bucket A.
    ```json
    {
    "Version": "2023-04-17",
        "Id": "bucket-A-policy",
    ...
    ```
4. [Create a bucket policy](/object/api-cli/create-bucket-policy/) to manage access to bucket B.
    ```json
    {
    "Version": "2023-04-17",
        "Id": "bucket-B-policy",
...
5. [Push the policies](/object/api-cli/create-bucket-policy/) to the corresponding buckets.
...
<Message type="note">
Refer to the [Bucket policies overview](/object/api-cli/bucket-policy/#bucket-policies-description) for more information on the different elements of a bucket policy.
</Message>
