...
In a serverless architecture, a container is not running constantly, but is rather activated only when there is an event. A trigger is a mechanism that connects the container to an event source and enables the container to run automatically in response to specific events.

The trigger can be an HTTP request, a message from a queue or stream, a CRON schedule etc. It is configured during the container deployment, and it determines how and when the container will run.
