...
GitLab serves as a web-based manager for Git repositories, offering features like wikis and issue tracking. It provides a myriad of functionalities, including management of git repositories, code reviews, issue tracking, and wikis, among others.

Scaleway offers a GitLab image which is based on Ubuntu 22.04 LTS (Jammy Jellyfish) and incorporates GitLab 16.3.3. By default, this image supports [routed IPs](/compute/instances/api-cli/using-routed-ips/).
...
5. Select the **GitLab** image in the **InstantApps** tab.
    <Lightbox src="scaleway-instantapp-gitlab.webp" alt="" />
...
2. You find your GitLab password in the `MOTD` when you login to your Instance.
    ...
    ################################################
    ```
3. Open the GitLab web interface in a web browser by copy/pasting the URL shown in the MOTD, or the Instances IP address in your browsers navigation bar. GitLab's login screen displays.
4. Login using the user `root` and the password shown in the MOTD of your Instance.
...
<Message type="note">
  In order to prevent spam, our infrastructure does not allow access to remote mail ports by default. However, since GitLab requires the ability to send emails, yoh have to enable access to these ports. Your account needs to undergo a validation proces before you can enable the SMTP ports. [Learn more about identiy verification](/console/my-account/how-to/verify-identity).
</Message>
