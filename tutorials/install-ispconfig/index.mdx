...
<Message type="requirement">
  ...
  - You have [created your SSH key](/identity-and-access-management/organizations-and-projects/how-to/create-ssh-key/)
  - You have [created an Instance](/compute/instances/how-to/create-an-instance/) that runs Ubuntu (for performance reasons, it is recommended to use an Instance with at least 4 GB of RAM)
...
1. Install the [PureFTPd](https://www.pureftpd.org/project/pure-ftpd) FTP-server and quotas by running the following command.
    ...
    ```
    Once installed, open the file `/etc/default/pure-ftpd-common` in your favorite text editor and make enable `VIRTUALCHROOT` by setting the value to _true_:
    ```
    [...]
    # VIRTUALCHROOT:
