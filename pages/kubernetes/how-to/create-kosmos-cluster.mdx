...
2. Specify the following for each pool:
    * For **Scaleway** pools:
      ...
      * Configure **pool options** or retain default settings. Configuration options include:
        ...
        * **Scheduling & Placement** tab: Configure [startup taints and/or taints](#how-to-configure-startup-taints-or-taints), if required. You can also link the pool to a [placement group](/instances/concepts/#placement-groups).
      * Configure an optional [cloud-init](#how-to-configure-a-cloud-init-script) script to customize your nodes. 
...
The [startup taints](/kubernetes/concepts/#startup-taints) and [taints](/kubernetes/concepts/#taints) you configure for your Scaleway pool are automatically applied to any new node created within the pool.

1. On the page of the pool that you are configuring, click the **Scheduling & Placement** tab.
...
[Cloud-init](/kubernetes/concepts/#cloud-init) enables you to set custom configuration that will be applied at boot time to the Instances acting as the nodes in your Scaleway pool. 
...
By attaching node labels to a Scaleway pool, you can:
