...
The `SetClusterType` API call allows you to edit your cluster's control plane from shared to dedicated offers and vice versa. Keep in mind, however, that certain conversions are not possible.
For example, it is not possible to move workloads from Kapsule to Kosmos, nor from Kosmos to Kapsule. You will need to create a new cluster. Refer to the [API documentation](https://www.scaleway.com/en/developers/api/kubernetes/#path-clusters-change-the-cluster-type) for more information.
...
`kapsule-agent` is a software developed internally at Scaleway. It is used to assist in managing node lifecycles. Its primary function is to set up the network during the initial boot, and toggle the SSH server on or off based on a k8s label of each node.

### Is etcd encryption enabled in our Kubernetes clusters?

Yes, Kubernetes Secrets are encrypted at rest in etcd using strong encryption. However, only Secrets are encrypted by default — not all data stored in etcd. Encrypting all etcd objects is under consideration for the future, but is not currently implemented.

### How can I have data persistence on my cluster?

Kapsule is a managed Kubernetes engine. By definition, the nodes of your clusters can be deleted, replaced, or restarted if the applications running on them require it or if a node suddenly stops responding. It means that Kubernetes clusters' nodes are to be considered stateless.
...
### What are the impacts of renaming a Kubernetes cluster?

Renaming a Kubernetes cluster is not a cosmetic change. The cluster name is used in several control-plane components, including the autoscaler configuration. When you rename a cluster, it triggers a complete redeployment of the control plane and of the job that injects the managed resources into your cluster.

This operation can cause temporary disruption to your workloads, including:

- Pods stuck in `ContainerCreating` state
- Internal DNS resolution errors
- ConfigMap mounting failures
- API server availability issues during the redeployment
- Error messages such as `the object has been modified; please apply your changes to the latest version and try again`

The cluster typically returns to normal operation within 1 to 2 hours after the rename. Scaleway recommends avoiding cluster renames on production clusters during peak usage periods.