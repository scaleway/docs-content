...
3. Access the `/etc/frr/daemons` file, and set the `bgpd=no` parameter to `bgpd=yes`.
...
4. Create the `/etc/frr/frr.conf` file, and add the following content to it:
    ...
    !
    router bgp 65000
      bgp router-id 169.254.0.7/31 # your IP in the BGP session - see connection overview in the Scaleway console
      neighbor 169.254.0.6/31 remote-as 12876 # the other IP in the BGP session
      neighbor 169.254.0.6/31 description TransitProvider
      !
...
1. Log in to the customer gateway using SSH if you are not already. Make sure to replace the placeholder with the appropriate value:
