...
Note that in this example, the first line where the free tier applies will not display in your current Scaleway bills by model, but will instead be listed under `Offer deducted - Generative
APIs Free Tier` and located under your `default` project (regardless of the exact project you performed queries configuring URL such as `api.scaleway.ai/{project_id}/v1/chat/completions`).
