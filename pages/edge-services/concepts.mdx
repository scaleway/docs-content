...
The endpoint can be customized with a user-defined subdomain, allowing you to replace the standardized endpoint with the subdomain of a domain you already own, e.g. `http://my-own-domain.com`. Using customized endpoints requires an associated [certificate](#certificate), and [CNAME record](#cname-record).
...
You can create an Edge Services pipeline for each of your Object Storage buckets or Load Balancers. Note that caching, WAF, and other optional parts of the pipeline can be enabled and disabled at will. The customization of the endpoint is also optional.
