...
There are two IAM permission sets specific to Generative APIs - Dedicated Deployment: `InferenceFullAccess` (allowing access to create, read, update, and delete a deployment) and `InferenceReadOnly` (allowing read-only access). Alternatively, wide-scoped permission sets such as `AllProductsFullAccess` will also allow access. {/* QUESTION: Verify if the name of the permission set is correct, especially InferenceFullAccess. */}
