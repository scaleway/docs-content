...
<Message type="important">
  The security measures listed on this page, except enforcing MFA, apply only to [IAM members](/iam/concepts/glossary/#member).
</Message>
...
If [Multifactor Authentication (MFA) is enabled](/account/login-credentials/use-2fa/) for a member you can disable it anytime. Disabling MFA is useful if the member needs to reset MFA.
...
5. Click **Disable MFA**. A pop-up appears.
    <Message type="important">
      Keep in mind that disabling MFA means a member will no longer be required to sign in with MFA. If [MFA is enforced](/iam/security/enforce-mfa/) at the Organization level, the member will have a grace period allowing them to enable it again.
    </Message>
...
<Message type="tip">
  Refer to the [How to enforce MFA](/iam/security/enforce-mfa/) documentation page for more information.
</Message>
...
<Message type="important">
  Locked Members cannot connect to the Organization until they are [manually unlocked](/iam/principals/manage-members/#how-to-unlock-a-member). Upon regaining access, the grace period resets, giving them another chance to meet security requirements.
</Message>
