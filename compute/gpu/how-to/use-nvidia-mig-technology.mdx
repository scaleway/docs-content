...
<Message type="important">
  ...
  * To avoid confusion, we will use the term GPU Instance in this document to designate the Scaleway GPU Instance. And we use *"MIG partition"* in the context of the MIG feature.
</Message>
...
With MIG, you are able to see and schedule jobs on virtual MIG partitions as if they were physical GPUs. MIG works with Linux operating systems and supports containers using Docker Engine, with support for Kubernetes and virtual machines using hypervisors such as Red Hat Virtualization and VMware vSphere.
...
* **Complete isolation:** Each MIG partition is isolated from the others, ensuring that workloads running in one Instance do not interfere with or impact the performance of workloads in other Instances. With MIG, each MIG partition's processors have separate and isolated paths through the entire memory system - the on-chip crossbar ports, L2 cache banks, memory controllers, and DRAM address busses are all assigned uniquely to an individual Instance.
...
1. Log in to your GPU Instance as root user using SSH.
2. Check the status of the MIG mode of your Instance running nvidia-smi. It shows that MIG mode is disabled:
    ...
    +-------------------------------+----------------------+----------------------+
    ```
2. Enable MIG mode on the GPU by running the following command:
    ```s
    root@my-h100-instance:~# sudo nvidia-smi -i 0 -mig 1
    Enabled MIG Mode for GPU 00000000:01:00.0
    All done.
    ```
3. Verify that MIG mode is enabled on the GPU by running the following command:
    ```s
...
These profiles determine the available sizes and functionalities of the MIG partitions that users can generate.
Additionally, the driver supplies details regarding placements, which specify the specific types and quantities of instances that can be established. 
...
1. Run the command `nvidia-smi mig -lgip` to retrieve a list of the available MIG profiles for the Instance.
    ```s
...
1. Run the following command to divide the H100 GPU Instance into four slices (MIG partitions):
    ...
    * `-cgi 9,19,19,19`: This flag is used to specify the MIG partition configuration. The numbers following the flag represent the MIG partitions for each of the four MIG device slices. In this case, there are four slices with configurations 9, 19, 19, and 19 compute instances each. These numbers correspond to the profile IDs retrieved previously. Note that you can use either one of:
    ...
    * `-C`: This flag automatically creates the corresponding compute instances for the MIG partitions.
    ...
    <Message type="note">
      ...
      You can utilize the [NVIDIA MIG Partition Editor](https://github.com/nvidia/mig-parted) (`mig-parted`) tool for automated assistance in this process.
...
2. Verify the MIG configuration of the GPU by running the following command:
    ```s
...
4. Run nvidia-smi on three 1g.10gb MIG partitons to display their characeristics:
    * MIG 1g.10gb 1
    ```s
...
1. If you intend to remove all MIG partitions and their corresponding compute instances, run the following command:
    ```s
    ...
    ```
2. Verify that the MIG partitions have successfully been removed by running the `nvidia-smi` command:
    ```s
    +-----------------------------------------------------------------------------+
    | MIG devices:                                                                |
    +------------------+----------------------+-----------+-----------------------+
