...
2. Run the following command to check where the plugin was installed.
  ...
  Required-by:
  ```
3. Create the `~/.aws/config` by running the following command:
  ```
  aws configure
  ```
4. Open the `~/.aws/config` file with a text editor. 

5. Edit the file according to example below. Make sure you add a `[plugins]` section including `endpoint = awscli_plugin_endpoint` and `cli_legacy_plugin_path = path_to_plugin` to the file. Replace `path_to_plugin` with the path you copied on step 2.
