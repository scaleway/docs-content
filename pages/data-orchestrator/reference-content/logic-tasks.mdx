...
<Tabs>
  ...
  <TabsTab label="Try-catch">
    A **Try-catch** allows you to handle pre-identified errors (e.g., authentication, authorization, timeout, etc.). Failed tasks can even be retried before proceeding with a fallback task. This is a basic example of a Serverless Job that is tried up to 4 times. Then if it still fails, a fallback task is executed.
      ```yaml
      document:
        dsl: 1.0.3
        namespace: <namespace_name>
