...
<Message type="requirement">
  ...
  - You have [created an SNS/SQS Messaging and Queuing namespace](/compute/messaging/how-to/create-namespace/)
  - You have [generated credentials](/compute/messaging/how-to/create-credentials/) for your SNS/SQS Messaging and Queuing namespace
...
<Message type="note">
  ...
  The `Endpoint` for Scaleway Messaging and Queuing is `https://sqs-sns.mnq.fr-par.scw.cloud`. The values for the access and secret keys should be the credentials you [generated](/compute/messaging/how-to/create-credentials/) for your SNS/SQS namespace.

</Message>

Once connected to the SNS service, you can use any of the SDK's available functions. Be aware though that some functions are not [supported by Scaleway Messaging and Queuing](/compute/messaging/reference-content/sns-support), so make sure to check the link for more details on these. See the [official SDK documentation](https://pkg.go.dev/github.com/aws/aws-sdk-go/service/sns) for more information on getting started with the SDK, or keep reading for some code examples.
...
<Message type="note">
  The `endpoint_url` for Scaleway Messaging and Queuing is `https://sqs-sns.mnq.fr-par.scw.cloud`. The values for the access and secret keys should be the credentials you [generated](/compute/messaging/how-to/create-credentials/) for your SNS/SQS namespace.
</Message>

Once connected to the SNS service, you can use any of the SDK's available functions. However, some functions are not [supported by Scaleway Messaging and Queuing](/compute/messaging/reference-content/sns-support/), so do check the link to make sure. See the [official SDK documentation](https://boto3.amazonaws.com/v1/documentation/api/latest/index.html) for more information, or keep reading for some code examples.
...
<Message type="note">
  The `endpoint_url` for Scaleway Messaging and Queuing is `https://sqs-sns.mnq.fr-par.scw.cloud`. For the access and secret key values, use the credentials you [generated](/compute/messaging/how-to/create-credentials/) for your SNS/SQS namespace.
</Message>

Once connected to the SNS service, you can use any of the SDK's available functions. However, some functions are not [supported by Scaleway Messaging and Queuing](/compute/messaging/reference-content/sns-support), so do check the link to make sure. See the [official SDK documentation](https://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/SQS.html) for more information, or keep reading for some code examples.
