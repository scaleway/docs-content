...
5. Set the DNS resolver by adding the following lines to the file. Then save it and exit the text editor:
    ...
    ```
    <Message type="tip">
      Replace the IP of the DNS cache server with the one located in the same physical location as your machine for optimal performances (https://www.scaleway.com/en/docs/console/account/reference-content/scaleway-network-information/#dns-cache-servers)
    </Message>
