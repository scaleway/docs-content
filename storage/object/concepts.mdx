...
excerpt: |
  This page explains all the concepts related to Object Storage that you might encounter while using Scaleway products and ressources.
totalTime: PT15M
steps:
  - step: TODO
    type: HowToStep
...
permalink: docs/storage/object-storage/concepts
...
A resource-based policy option. It allows users to grant access to buckets to other Scaleway projects and organizations. By default, all S3 resources in a project are private and can be accessed only by users of the project. To grant access to outside users, [a policy file can be added to a bucket via an API call or the AWS-CLI](/storage/object/api-cli/bucket-policy/).
...
- `Standard`: The Standard class for any upload; is suitable for on-demand content like streaming or CDN.
