...
=======
>>>>>>> a2efe62b (fix(mnq): modify faq for GA (#2231))
...
The console now points to the new and improved version of Messaging and Queuing, `v1beta1`. Any SQS or SNS resources created with the API or the console prior to the date of SQS and NATS going into General Availability, exist within the old `v1alpha1` version of Messaging and Queuing.

You can still access these resources via the [v1alpha1 API](https://www.scaleway.com/en/developers/api/messaging-and-queuing/). Be aware however that 30 days after the date of SQS and NATS going into General Availability, your old resources will be deleted.
...
You should stop using v1alpha1, and switch to v1beta1. The old API (v1alpha1) will shortly be retired, and all resources still existing within it will be deleted.

You must migrate by recreating your SQS and/or SNS resources on the new v1beta1 API (or via the console). NATS resources will be automatically migrated.
