...
Docker is a platform as a service (PaaS) tool that uses OS-level virtualization to deliver applications in packages called containers. Each container is isolated from the others and each of them bundles their own software, libraries, and configuration files. Unlike virtual machines, containers share the services of a single operating system kernel. This reduces unnecessary overhead and makes them lightweight and portable. Docker containers can run on any computer running macOS, Windows, or Linux, either on-premises or in a public cloud environment, such as Scaleway Elements.

All [Scaleway GPU Instances](https://www.scaleway.com/en/gpu-instances/) come with prebuilt AI Docker images which can be launched as soon as you connect to your Instance. Each image provides a different AI environment, based on Anaconda or Pipenv, depending on the GPU OS image. When you launch one of these images, you are in your chosen environment within seconds with all your favourite python packages already installed. Using Docker for your AI projects in this way allows you to ensure that your working environments are both **isolated** and **portable**, since they are in containers which can be easily transferred between machines.
...
3. Use the following command to pull/run the Docker container. Remember to replace `[OPTIONS]` with any relevant options parameters, and `<IMAGE_NAME:TAG>` with the image name and tag of your chosen image, as shown in the reference content linked above:
  ...
  If you created your GPU Instance with the Ubuntu Focal GPU OS11 Operating System image, refer to the [pipenv documentation](/compute/gpu/how-to/use-pipenv/) for how to use the pipenv virtual environment inside the container.
