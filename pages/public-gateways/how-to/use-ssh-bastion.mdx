...
5. Copy the command to connect to a resource, and click **Save SSH bastion settings**.
    ...
    You are redirected to your Public Gateway's **Overview** page, where SSH bastion is now activated. All the SSH keys in your [Project](/organizations-and-projects/concepts/glossary/#project) at the time of activation are copied to the SSH bastion.
...
1. Ensure you have [activated SSH bastion](#how-to-activate-ssh-bastion).
...
If you add new SSH keys to your [Project](/organizations-and-projects/concepts/glossary/#project) after activating SSH bastion, you will need to perform a reimport to update the bastion with the new keys.
...
1. To configure at user/local level, open your user SSH configuration file on your local machine with a text-editor such as `nano`: 
    ...
    Paste the following code into the file, then save and exit. Ensure that you make the following replacements:
      - `.<private-network-name>`: The name of your Private Network
...
## How to monitor SSH bastion

Scaleway Cockpit allows you to monitor the logs of SSH Bastion. Refer to the [dedicated documentation](/public-gateways/how-to/monitor-public-gateways/) for more information on how to monitor your Public Gateways.
