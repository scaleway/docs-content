...
2. Add the **Server IP addresses** of the servers the Load Balancer will forward requests/connections to. These will be your backend servers. 
        ...
        <Message type="note">
        If you want your Load Balancer to communicate with its backend servers over an attached [Private Network](/network/vpc/concepts/#private-networks), ensure that you enter the servers' private IP addresses on that network. This is necessary in the case of a [private Load Balancer](/network/load-balancer/concepts/#private-load-balancer), but may also be desired for public Load Balancers attached to Private Networks where backend servers are on the same Private Network.
        </Message>
