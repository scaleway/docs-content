...
* Connect servers which do not have a RPN interface to the RPN
...
2. Click **Configure** next to the RPN VPN service you want to configure. The RPN VPN information displays.
    ...
    If you do not have any RPN VPN service yet, you can click **Order RPN VPN** directly on the page to order it.
...
2. Edit the client configuration file `/etc/openvpn/client.conf` in a text editor. For example `nano`: 
...
3. To start automatically the VPN during the boot process of the server, edit the file `/etc/default/openvpn` and insert the following line:

    ```
    AUTOSTART="client"
    ```

    This line allows to start automatically [OpenVPN](/tutorials/install-openvpn/) with the configuration file located at /etc/openvpn/client.conf

Once the connection to the RPN VPN server has been established, you may check if you can see a new interface, using your RPN VPN IP:
