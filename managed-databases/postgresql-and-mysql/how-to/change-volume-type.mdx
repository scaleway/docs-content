...
5. Click **Update volume type** to confirm.
    ...
    <Message type="important">
      - When switching from a local volume to a Block volume, and if [Autobackup](/managed-databases/postgresql-and-mysql/how-to/enable-autobackup) is enabled, be aware that the snapshot type will change. Local volumes utilize logical backups, whereas Block storage employs snapshots. Consequently, the billing for these two types will differ as well.
