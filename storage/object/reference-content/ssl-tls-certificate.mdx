...
For Scaleway Edge Services, you must import the certificate, certificate chain and private key in `.PEM` format**. The file should look like this:
...
- **Primary SSL certificate**: The certificate issued by the CA for your domain name. **Issued for**: Your name and public key. **Issued by**: CA's name and signature.
- **Intermediate certificate**: The intermediate certificate chaining your primary certificate to the root certificate. **Issued for**: CA's name and public key. **Issued by**: The Root CA's name and signature.
- **Root certificate**: The root certificate by the CA, for the trusted CA itself. **Issued for**: The Root CA's name and public key. **Issued by**: Root CA's signature and public key (self signed).
