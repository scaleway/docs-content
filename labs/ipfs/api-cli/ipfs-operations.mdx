---
meta:
  title: Understanding IPFS Pinning
  description: This page shows IPFS Pinning API.
content:
  h1: Understanding IPFS Storage
  paragraph: This page shows IPFS Storage API.
tags: ipfs pinning storage ipfs-pinning ipfs-storage volume labs cid
dates:
  validation: 2023-05-25
  posted: 2023-05-25
categories:
  - labs
  - storage
  - ipfs-storage
  - pinning
  - ipfs-pinnings
---

You can leverage our IPFS pinning service to store permanently your data on the public IPFS network. You can do it via a Content-ID (CID) or a web URL. You can [create](/storage/ipfs/api-cli/volumes-operations) and manage your volume via the [Scaleway Command Line Interface](https://github.com/scaleway/scaleway-cli).

## Scaleway Command Line Interface overview

The [Scaleway Command  Line Interface (CLI)](https://github.com/scaleway/scaleway-cli) allows you to create and interact with your pins. You will be able to create, add/delete content, and manage volumes.

<Macro id="iam-requirements" />

<Message type="requirement">
    - You have an account and are logged into the [Scaleway Console](https://console.scaleway.com/)
    - You have generated your [API key](/identity-and-access-management/iam/how-to/create-api-keys/)
    - You have downloaded and installed the [Scaleway CLI v2](https://github.com/scaleway/scaleway-cli#scaleway-cli-v2)
</Message>

## Pin a content by its CID

This operation allows you to add content by its CID in a volume.

<Message type="note">
  It suppose that the content pointed by the CID must already be present on the public IPFS network.
</Message>

**Post request**
```sh no-copy
POST /ipfs/v1alpha1/regions/fr-par/pins/create-by-cid HTTP/1.1
Host: agw.fr-par-2.internal.scaleway.com
User-Agent: scaleway-sdk-go/v1.0.0-beta.7+dev (go1.20.3; linux; amd64) scaleway-cli/2.14.0-1
Content-Length: 169
Content-Type: application/json
X-Auth-Token: 6a55c9c4-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```

**CLI command**
```
scw ipfs ipfs add-cid volume-id=11111111-1111-1111-1111-111111111111 cid=Qmdi7ERksspfxWXfU8ATRUt7iCjZJbEbDrUoMDtjnbdTwo name=MyNewPinByCID
```

**Response**
```
PinID           22222222-2222-2222-2222-222222222222
Status          queued
CreatedAt       now
Cid.Cid         Qmdi7ERksspfxWXfU8ATRUt7iCjZJbEbDrUoMDtjnbdTwo
Cid.Name        MyNewPinByCID
Cid.Meta.AppID  00000000-0000-0000-0000-000000000000
```

## Pin a content by its URL

This operation allows you to add content by URL into your volume. The request failed if the content is not reachable.

**Post request**
```sh no-copy
POST /ipfs/v1alpha1/regions/fr-par/pins/create-by-url/ HTTP/1.1
Host: agw.fr-par-2.internal.scaleway.com
User-Agent: scaleway-sdk-go/v1.0.0-beta.7+dev (go1.20.3; linux; amd64) scaleway-cli/2.14.0-1
Content-Length: 121
Content-Type: application/json
X-Auth-Token: 6a55c9c4-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```

**CLI command**
```
scw ipfs ipfs add-url volume-id=11111111-1111-1111-1111-111111111111 url=https://labs.scaleway.com/images/Logo-labs-horizontal-white.svg name=MyNewPinByCID
```

**Response**
```
PinID           22222222-2222-2222-2222-222222222222
Status          queued
CreatedAt       now
Cid.Cid         -
Cid.Name        MyNewPinCID
Cid.Meta.AppID  00000000-0000-0000-0000-000000000000
```

<Message type="note">
  If the response is ingest you can retrieve the CID or from your URL.
</Message>

## Get your pins

This operation allows you to get information about a pin ID. You can retrieve the CID associated with the pin ID or discover the CID behind your URL.

**Sample request**
```sh no-copy
GET /ipfs/v1alpha1/regions/fr-par/pins/22222222-2222-2222-2222-222222222222?volume_id=11111111-1111-1111-1111-111111111111 HTTP/1.1
Host: agw.fr-par-2.internal.scaleway.com
User-Agent: scaleway-sdk-go/v1.0.0-beta.7+dev (go1.20.3; linux; amd64) scaleway-cli/2.14.0-1
X-Auth-Token: 6a55c9c4-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```

**CLI command**
```
scw ipfs ipfs get-pin-id pin-id=22222222-2222-2222-2222-222222222222 volume-id=11111111-1111-1111-1111-111111111111
```

**Response**
```
Status          pinned
CreatedAt       5 minutes ago
Cid.Cid         QmaGNR5PedL8kiTUMtsPeFFodL9HnYBMuUqAEQrv6CQH4F
Cid.Name        value:"MyNewPinByCID"
Cid.Meta.AppID  00000000-0000-0000-0000-000000000000
```

<Message type="note">
  If the response is ingested you will retrieve the CID of your content pointed by the URL.
</Message>

<Message type="important">
  If the status is failed, you can retrieve information about the CID or the URL and retry.
</Message>

## Remove a pin

This operation allows you to remove a pin within a volume from its pin ID. The content will be removed if you were the last owner of that associated content.

<Message type="note">
  If the operation is successful no output will be returned.
</Message>

**Delete request**
```sh no-copy
DELETE /ipfs/v1alpha1/regions/fr-par/pins/22222222-2222-2222-2222-222222222222?volume_id=11111111-1111-1111-1111-111111111111 HTTP/1.1
Host: agw.fr-par-2.internal.scaleway.com
User-Agent: scaleway-sdk-go/v1.0.0-beta.7+dev (go1.20.3; linux; amd64) scaleway-cli/2.14.0-1
X-Auth-Token: 6a55c9c4-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```

**CLI command**
```
scw ipfs ipfs rm-pin-id pin-id=22222222-2222-2222-2222-222222222222 volume-id=11111111-1111-1111-1111-111111111111
```

## Listing your pins

This operation allows you to list all your pins from one of your volume.

**Get request**
```sh no-copy
GET /ipfs/v1alpha1/regions/fr-par/pins?order_by=created_at_asc&page=1&status=unknown_status&volume_id=11111111-1111-1111-1111-111111111111 HTTP/1.1
Host: agw.fr-par-2.internal.scaleway.com
User-Agent: scaleway-sdk-go/v1.0.0-beta.7+dev (go1.20.4; linux; amd64) scaleway-cli/2.0.0+dev
X-Auth-Token: 6a55c9c4-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```

**CLI command**
```
scw ipfs ipfs list-pins volume-id=11111111-1111-1111-1111-111111111111
```

**Response**
```
{"total_count":0,"pins":[]}
```