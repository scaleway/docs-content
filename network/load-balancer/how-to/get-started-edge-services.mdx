...
The origin host must be associated with the origin Load Balancer, and only one host may be set per pipeline. If your Load Balancer is associated with multiple hosts, you can create a separate Edge Services pipeline for each. Each host will therefore get its own Edge Services endpoint and cache.
...
The endpoint from which a given Edge Services pipeline can be accessed, e.g. `https://pipeline-name.svc.edge.scw.cloud`. When a client requests content from the Edge Services endpoint, it is served by Edge Services and its cache, rather than from the Load Balancer and its backend servers directly. Edge Services automatically manages redirection from HTTP to HTTPS
...
4. Configure the [origin](#origin) for this pipeline:
        ...
        <Message type="tip">
        For Kubernetes Load Balancers, see our [dedicated documentation](TODO) for help selecting the correct frontend and port.
        </Message>
