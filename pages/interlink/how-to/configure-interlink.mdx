...
3. In the **Attached VPC** section, click **Attach VPC**. A pop-up displays:
    ...
    <Lightbox src="scaleway-interlink-attach-vpc.webp" alt="TODO MODIFY A screen in the Scaleway console invites the user to attach a VPC to complete the connection from their external infrastructure, across the InterLink to their Scaleway resources. A dropdown menu allows a VPC to be selected. A banner reminds users to check any attached route filter lists for compatibility with the selected VPC." />
...
3. In the **Route propagation** section, click **Attach routing policy**. A pop-up displays.
    ...
    <Lightbox src="scaleway-attach-allowed-routes.webp" alt="TODO MODIFY A pop-up in the Scaleway console prompts a user to select a routing policy from a drown down list." />
