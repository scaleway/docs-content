...
If you notice any errors when running a SMART diagnosis on your disk, open a [support ticket](/account/support/open-a-support-ticket/) and ask for the disk to be replaced, indicating the serial number with the result of the `smartctl` command:
...
<Message type="important">
  ...
  If your server appears on the list and its storage status is indeed `FAILED`, open a [support ticket](/account/support/open-a-support-ticket/).
</Message>
...
<Message type="note">
If you encounter **Health status: Failed** or **Failing Now**, the disk is considered out of order. Make sure that you have backups, then open a [support ticket](/account/support/open-a-support-ticket/) and ask for the disk to be replaced, indicating the serial number with the result of the `smartctl` command.
</Message>
