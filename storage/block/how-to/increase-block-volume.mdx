...
<Message type="tip">
  It is recommeneded to make a backup of your data using the [snapshot](/compute/instances/how-to/create-a-snapshot/) feature, before increasing the partition size of your volume. This helps to avoid any data loss. 
</Message>
...
2. Use the `lsblk` command to identify your Block Storage volume's mount point. In the following example the volume is mounted at `/mnt/block-volume`: 
    ...
    ```
3. [Unmount](/storage/block/how-to/unmount-a-volume/) the volume:
    ```
...
7. Run `e2fsck` on the Block Storage volume to check that everything went well. Replace `/dev/sdX` with the name of your block volume in the following command:
    ...
    ```
8. [Remount](/storage/block/how-to/mount-and-use-volume/) the volume to access the files stored on it: 
    ```
