...
content:
  h1: Provisioning a hosted InterLink
  paragraph: Get help with understanding all the different stages of ordering and provisioning a hosted Scaleway InterLink, from placing the initial order, to dealing with error messages, and deleting the resource if no longer needed.
...
<Lightbox image={image2} alt="A four step flow chart shows the following steps: 1. User creates InterLink via Scaleway, 2. User requests InterLink from partner, 3. Partner accepts InterLink request, 4. Partner provisions InterLink." />
