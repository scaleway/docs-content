...
### How to upgrade runtime version?

Serverless Functions runtimes are updated by Scaleway, if you need a specific version we recommend to use Serverless Containers instead.

Serverless Containers and Serverless Functions are very similar, check our page [how to migrate from Functions to Containers](/serverless-functions/how-to/migrate-functions-to-containers/).
...
### How to access my Serverless Function with a Private Endpoint?

Serverless Functions supports Private Network for outgoing traffic (like accessing a database or an instance) but not yet for incoming traffic.

This feature is under development.

### How to disable public endpoint for Serverless Functions?

Disabling the default public endpoint for Serverless Functions is not yet available. To restrict access to Serverless Containers it's possible to [make the resource private](/serverless-functions/how-to/secure-a-function/#restrict-access-to-your-functions).

This feature is under development.

### How to add a Load Balancer in front of my Serverless Function?

Load Balancing is integrated directly to Serverless Functions when you invoke them, traffic is handled by our infrastructure without the need to add other products.

Also Load Balancers are compatible only with IPs backends, as Serverless Functions IPs are not predictable it's not possible to integrate directly Functions to Load Balancers.
