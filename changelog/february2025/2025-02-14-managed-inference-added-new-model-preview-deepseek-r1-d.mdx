---
title: New model preview Deepseek R1 Distilled Llama 70B
status: added
author:
    fullname: ''
    url: 'https://slack.scaleway.com'
date: 2025-02-14
category: ai-data
product: managed-inference
---

[Deepseek R1 Distilled Llama 70B](https://www.scaleway.com/en/docs/managed-inference/reference-content/deepseek-r1-distill-llama-70b/) is now available on Managed Inference.

Deepseek R1 Distilled Llama improves Llama model performance on reasoning use cases like mathematics or code.

