...
excerpt: |
  Hadoop is an open-source framework. It is provided by Apache to process and analyze very huge volume of data. It is written in Java and currently used by Google, Facebook, LinkedIn, Yahoo, Twitter etc. Similar to data residing in a local file system of personal computer system, in Hadoop, data resides in a distributed file system which is called as a Hadoop Distributed File system.
totalTime: PT30M
steps:
  - step: Installing the Default JRE/JDK
    type: HowToStep
    url: https://www.scaleway.com/en/docs/how-to-install-hadoop-standalone-on-ubuntu/#-Installing-the-Default-JREJDK
  - step: Installing Hadoop Standalone Mode
    type: HowToStep
    url: https://www.scaleway.com/en/docs/how-to-install-hadoop-standalone-on-ubuntu/#-Installing-Hadoop-in-Standalone-Mode
  - step: Configring Hadoop's Java Home
    type: HowToStep
    url: https://www.scaleway.com/en/docs/how-to-install-hadoop-standalone-on-ubuntu/#-Configuring-Hadoops-Java-Home
  - step: Running Hadoop
    type: HowToStep
    url: https://www.scaleway.com/en/docs/how-to-install-hadoop-standalone-on-ubuntu/#-Running-Hadoop
  - step: MapReduce Example
    type: HowToStep
    url: https://www.scaleway.com/en/docs/how-to-install-hadoop-standalone-on-ubuntu/#-MapReduce-Example
categories:
  - compute
brands:
  - elements
permalink: /tutorials/hadoop
image: /assets/images/docs/scaleway.png
keywords: cloud tutorials, cloud documentations, cloud instance, scaleway cli, command line interface, cloud hosting, ubuntu server, debian server, linux server, physical server, dedicated server, hadoop, data, volume, big data, java, apache
dates:
...
Similar to data residing in a local file system of a personal computer system, in Hadoop, data resides in a distributed file system which is called as a Hadoop Distributed File system. Hadoop is suitable for:

- **Big Data Analysis:** Big Data tends to be distributed and unstructured in nature. Hadoop processes the logic (not the actual data) that flows to the computing nodes, less network bandwidth is consumed. This concept is called as data locality concept which helps increase the efficiency of Hadoop-based applications.
...
1. Visit the [Apache Hadoop Releases page](http://hadoop.apache.org/releases.html) to select the most recent stable release. We will install Hadoop version 3.1.1

2. Copy the link to the release binary.

  <Lightbox src="scaleway-apacheDL.png" alt="" />

3. Use `wget` to fetch it:

  ```
  wget http://mirrors.standaloneinstaller.com/apache/hadoop/common/hadoop-3.1.1/hadoop-3.1.1.tar.gz
  ```
  ...
  <Lightbox src="scaleway-checksumDL.png" alt="" />

4. Copy the link displayed in the browser and use `wget` on our server to download the file.

  ```
  wget https://dist.apache.org/repos/dist/release/hadoop/common/hadoop-3.1.1/hadoop-3.1.1.tar.gz.mds
  ```

5. Run the following verification:

  ```
  shasum -a 256 hadoop-3.1.1.tar.gz
  ```
  ```
  f837fe260587f71629aad1f4fb6719274e948111dc96ffc5a8e26f27deac5602  hadoop-3.1.1.tar.gz
  ```

6. Compare this value with the SHA-256 value in the .mds file:

  ```
  cat hadoop-3.1.1.tar.gz.mds
  ```
