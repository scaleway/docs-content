...
In cases where your clusters are large and containing numerous resources such as nodes, pods, and custom resources (CRDs), or when experiencing a large number of simultaneous requests to the apiserver from controllers like argocd and velero,  it is possible for the CPU and memory usage to spike. Consequently, the performance of the apiserver may noticeably decline, leading to sluggishness or unresponsiveness. You might encounter errors like `"EOF"` when using `kubectl`.
