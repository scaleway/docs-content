...
With stateful DHCPv6, the DHCPv6 server itself chooses and assigns IPv6 addresses to clients, unlike with SLAAC or Stateless DHCPv6  where the client creates their own IPv6 address. Stateful DHCPv6 is so-called because the DHCPv6 server keeps track of the state of IPv6 addresses on the network.
