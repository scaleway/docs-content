...
tags: routed ip cloud scaleway-api migration
dates:
...
Another feature of directly routed public IPs is the ability to assign multiple flexible IP addresses to your Instance, allowing you to use both IPv4 and IPv6 protocols.

<Message type="note">
   * Currently, the routing of public IPs is available in the `pl-waw-2`, `fr-par-3` and `nl-ams-2` Availability Zones.
   * [Dynamic IPs](/compute/instances/concepts/#dynamic-ip) continue to be supported after the migration to routed public IPs.
   * The field `enable_ipv6` is being deprecated and has no meaning for routed public IP enabled Instances.
</Message>

## How to enable public IP routing?

<Message type="note">
  Currently, the direct routing of public IPs is disabled by default. If you're interested in exploring its functionality, you need to explicitly enable it.
</Message>

In this section, we will guide you through the process of creating a new Instance with a directly routed public IP. We will also provide instructions on migrating your existing legacy Instances.
...
<Message type="important">
    Keep in mind that once the migration to directly routed IPs is complete, there is **no way to revert it**.
</Message>

## Creating a routed public IP

You can request directly routed public IP addresses using the Scaleway API. Available IP types are IPv4 and IPv6.
...
## Create a new Instance with a directly routed public IP
...
<Tabs id="create-instance">
    <TabsTab label="Request">
        ...
        ```
        <Message type="tip">
            In order to create an Instance with a routed public IP, you have to add `"routed_ip_enabled": true` to your payload.
        </Message>
    </TabsTab>
    <TabsTab label="Response">
        ```json
...
First you need to create an IPv4 reservation, then you create a new Instance with this public IP attached.
...
<Tabs id="create-instancev4">
    <TabsTab label="Request">
        ...
        ```
        <Message type="tip">
            In order to create an Instance with a directly routed public IP, you have to add `"routed_ip_enabled": true` to your payload.
        </Message>
    </TabsTab>
    <TabsTab label="Response">
        ```json
...
You can use a specific server action to migrate an existing (legacy network) Instance to directly routed IPs.
...
<Tabs id="verify-ip">
    ...
    <TabsTab label="Response - Routed public IP">
        Compared to the previous response, an Instance with directly routed public IP would have a different `type`:
        ```json
        {
            "ip": {
                "address": "51.158.38.9",
...
## Verifying directly routed IPs are enabled for your Instance

You can verify if your Instance is enabled for directly routed IPs through the `/servers` endpoint:
...
<Tabs id="create">
    ...
    <TabsTab label="Response - Routed public IP">
        Compared to the previous response, an Instance with directly routed public IPs would have the `routed_ip_enabled` flag set to `true`:
        ```json
        {
            "server": {
                "allowed_actions": [
