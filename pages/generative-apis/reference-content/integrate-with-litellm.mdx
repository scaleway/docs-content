...
- Configure the health check parameters. For instance, when using embedding models, you must add `mode: embedding` under `model_info` in `config.yaml`, as [mentioned in the LiteLLM Health Checks documentation](https://docs.litellm.ai/docs/proxy/health#embedding-models).
