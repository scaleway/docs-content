...
The Kubernetes version of the existing nodes in your multi-cloud pool cannot be upgraded. Once a node is provisioned, it will stay in the Kubernetes version it started with. To upgrade, you will have to detach a node, and add a new one:
...
2. Add a new node to your pool. It will start up with the new version of Kubernetes.
3. Using kubectl, drain one of the old nodes in your pool.
4. Detach the drained node. Keep in mind you will need to deprovision or recycle the node in your cloud provider.
