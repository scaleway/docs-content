...
Secret protection is a feature that allows users with the `SecretManagerFullAccess`, `SecretManagerSecretWrite`, and `AllProductsFullAccess` [permission sets](/identity-and-access-management/iam/reference-content/permission-sets/), to enable it on your secrets so that they cannot be deleted even though they can be read and modified. 
This ensures that your critical information is protected against accidental deletion while maintaining secret management flexibility.
