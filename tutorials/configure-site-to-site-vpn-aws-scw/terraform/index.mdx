...
This tutorial explains how to establish a secure, high-availability Site-to-Site VPN connection between a **Scaleway VPC** (e.g., for a Kapsule Kubernetes cluster) and an **AWS VPC** (e.g., for an RDS database) using Terraform.

The connection uses the **IKEv2** protocol with a Pre-Shared Key (PSK) and **BGP** (Border Gateway Protocol) for dynamic routing.
...
<Step title="Scaleway Network Initialization">
...
<Step title="AWS Network & Customer Gateway">
...
<Step title="Configuring the VPN Tunnel (IPSec & BGP)">
...
<Step title="Deploy Initial Infrastructure (Phase 1)">
In this phase, you execute the first `apply` to create the networks and gateways. This generates the necessary IPs and the Pre-Shared Key (PSK) on Scaleway, which are needed for the final connection.

1. Run the **first** `terraform apply`.
We target the infrastructure components first to ensure the Scaleway Gateway generates the PSK before we attempt to fully configure the AWS Connection.
```bash
terraform apply -target=module.scaleway_infra -target=aws_vpc.main -target=aws_customer_gateway.scw_side

```

2. Note the **Public IP** of the Scaleway Gateway output by Terraform, if you need to verify it against the AWS Customer Gateway.

</Step>

<Step title="Retrieve PSK and Deploy Tunnel (Phase 2)">
...
3. Run the **second** `terraform apply`.
