...
- [Created a Cockpit token](/cockpit/how-to/create-token/) with the **Rules** > **Metrics** and  **Rules** > **Logs** permissions and saved it in a secure place {/*EDITORIAL COMMENT: Are there any other permissions that should be enabled?*/}
- [Retrieved your Metrics and Logs data source URLs](/cockpit/concepts/#data-sources) from the [Scaleway console](https://console.scaleway.com/cockpit/dataSource)
...
{/*EDITORIAL COMMENT: Shall we mention the requirement to install Prometheus and Promtail and configure them so that they scrape metrics/logs from the target Sxcaleway resource and push them toward Mimir/Loki? */}
...
- Rules endpoint: `/prometheus/api/v1/rules`
...
Make sure that you create your token in the same region as the data source you want to use it for, and that it has the **Rules** > **Metrics** and **Rules** > **Logs** permissions.
...
For more information, see the following Grafana documentation:
...
Retrieve rules from a specific namespace:
...
Delete a specific Mimir rule group in a namespace:
...
Delete all Mimir rule groups in a namespace (including the namespace itself):
...
Delete a specific Loki rule group in a namespace:
...
Delete all Loki rule groups in a namespace (including the namespace itself):
...
{/*EDITORIAL COMMENT: Need a paragraph about how to incorporate this into their DevOps workflow. */}