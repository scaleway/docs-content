...
3. Create the `~/.aws/config` by running the following command:
  ...
  aws configure
  ```
4. Open `~/.aws/config` in a text editor and edit it as follows:
  ```
  [plugins]
...
5. Generate a credentials file using the command:
  ```
  aws configure
  ```
6. Open the `~/.aws/credentials` file and configure your [API key](/console/my-project/how-to/generate-api-key/) as follows:
  ```
  [default]
