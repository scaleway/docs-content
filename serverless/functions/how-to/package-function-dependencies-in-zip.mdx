...
- A [functions namespace](/functions/how-to/create-manage-delete-functions-namespace/)
- installed [jq](https://jqlang.github.io/jq/download/)

## How to package a function into zip file

There are [different methods to deploy functions](/functions/reference-content/deploy-function/) and some of them require the function to be packaged into a `zip` file.
...
<Tabs id="tab1">
  <TabsTab label="Scaleway Console">
    ...
    1. Package your dependencies on your local computer, as explained in the [Configure your package](/functions/how-to/package-function-dependencies-in-zip/#how-to-package-a-function-into-zip-file) section.
    ...
    <Message type="note">
    Refer to our [dedicated documentation](/functions/how-to/create-a-function/) for more information on how to create Serverless Functions
    </Message>
...
<Tabs id="tab1">
  <TabsTab label="Python">
    ...
    3. Provide a custom handler name. Here, the handler is `handlers/hello.say_hello`.
        <Message type="note">
          By default, the handler path is `handler.handle` (def handle in handler.py). Refer to the [Functions handlers reference](/functions/reference-content/functions-handlers/) for more information.
        </Message>
  </TabsTab>
  <TabsTab label="Node">
...
Like that, you can create 3 functions with the same zip file simply by changing the [handler](/functions/concepts/#handler) parameter to match the handler you want:
- `myFuncA.handlerA`
- `myFuncA.handlerB`
- `myFuncB.handlerC`

You can also create a single Serverless Function with several handlers in the same zip file, and change the [handler](/functions/concepts/#handler) parameter according to your needs.
