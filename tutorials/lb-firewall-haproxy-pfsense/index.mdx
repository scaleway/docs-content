...
excerpt: |
  pfSense is an open-source firewall and router distribution, based on FreeBSD and released under the Apache license. The configuration and management of the software can be done through a web-based interfaces, requiring no knowledge of the underlying FreeBSD system. The source code of the project is available on GitHub.
totalTime: PT30M
steps:
  - step: Core Concepts
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/lb-firewall-haproxy-pfsense/#-Installing-pfSense
  - step: Configuring Synchronisation
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/lb-firewall-haproxy-pfsense/#-Configuring-Synchronisation
  - step: Block Storage Performances
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/lb-firewall-haproxy-pfsense/#-Configuring-Failover-IPs
  - step: Setting up HAProxy / Acme
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/lb-firewall-haproxy-pfsense/#-Setting-up-HAProxy--Acme
...
categories:
  ...
  - dedicated-servers
  - dedibox
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, ubuntu server, debian server, linux server, baremetal, scalability, ARM, physical server, dedicated server, physicalization, pfsense, ha, high-availability, haproxy
dates:
...
<Message type="requirement">
  ...
  - You have [created a Bare Metal server](/compute/bare-metal-cloud/how-to/create-server) or have a [Scaleway Dedibox](https://www.scaleway.com/en/dedibox/)
...
The installation of pfSense is done using the [KVM-over-IP device](/compute/bare-metal-cloud/how-to/install-server-from-kvm/) of the Bare Metal Cloud Server or the Scaleway Dedibox.

</Message>

1. Connect to the [KVM-over-IP device](/compute/bare-metal-cloud/how-to/install-server-from-kvm/) of your Bare Metal Cloud Server or Scaleway Dedibox.
...
6. Choose the Keymap for your pfSense installation. By default, the `US` keymap is preselected. Select the desired keymap from the list and confirm by pressing Enter:
  ...
  <Lightbox src="scaleway-pfsense_install_03.png" alt="" />
...
7. Choose the disk to install pfSense on. If your server has more than one disk, choose `ada0` as installation destination. Press Enter to confirm.
  ...
  <Lightbox src="scaleway-pfsense_install_04.png" alt="" />
...
8. Select the partition scheme for the volume. The default option is `MBR`. You can keep this value and confirm by pressing Enter:
  ...
  <Lightbox src="scaleway-pfsense_install_05.png" alt="" />
...
9. The setup wizard starts copying the data from the virtual CD ROM drive to the servers hard disk. This may take a while depending on the connection speeds:
  ...
  <Lightbox src="scaleway-pfsense_install_06.png" alt="" />
...
10. The automatic installation finishes. Choose **Yes** and confirm by pressing on Enter to open a shell in the newly installed system to run a manual configuration:
  ...
  <Lightbox src="scaleway-pfsense_install_07.png" alt="" />
...
12. In the configuration file, scroll down to the network configuration section:
  ...
  <Lightbox src="scaleway-pfsense_install_08.png" alt="" />
...
<Lightbox src="scaleway-pfsense_welcome.png" alt="" />
...
1. Open a web browser and point it to `https://<server_public_ip>`. The pfSense Web Interface displays. Log in using the user **admin** and the password **pfsense**:
  ...
  <Lightbox src="scaleway-pfsense_setup_01.png" alt="" />
...
2. Once logged in, the pfSense setup wizard displays:
  ...
  <Lightbox src="scaleway-pfsense_setup_02.png" alt="" />
...
4. Enter general information about the pfSense server. These include the **Hostname** and the **Domain** of the server. Leave **Primary DNS Server** and **Secondary DNS Server** empty and the option **Override DNS** checked. Click **Next** to continue:
  ...
  <Lightbox src="scaleway-pfsense_setup_04.png" alt="" />
...
3. Go to **System** > **Advanced** and change the **TCP port** in the **WebConfigurator** settings to `31337` to match the previously configured rules.
  ...
  <Lightbox src="scaleway-pfsense_configure_tcp_port.png" alt="" />
...
1. From the pfSense web interface, click **System** > **High Avail. Sync**.
