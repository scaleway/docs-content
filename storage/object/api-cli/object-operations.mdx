...
<Message type="note">
  ...
  To define the [storage class](/storage/object/concepts/#storage-class) of the object directly upon creation, use the `--storage-class <class>` option with `awscli` or add the `x-amz-storage-class <class>` header if you're using the S3 API directly. You can specify one of the following classes: `STANDARD`, `ONEZONE_IA`, `GLACIER`. Example: `x-amz-storage-class: ONEZONE_IA`. 
