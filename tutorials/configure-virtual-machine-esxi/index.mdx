...
There are two ways to install EXSi on a Dedibox server: through the [Dedibox console](https://account.scaleway.com/?service=dediboxen/login) or through [KVM over IP](https://en.wikipedia.org/wiki/KVM_switch#KVM_over_IP_(IPKVM)).
