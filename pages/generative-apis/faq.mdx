...
Caching is performed automatically based on heuristics to optimize both throughput and availability.

For *Serverless* models, cache is persisted automatically and evicted based on request frequency. For workloads with recurring prefixes (e.g., conversational or agentic use cases), cache hit ratio is targeted to be between 50% and 90%, though not guaranteed. Cached input tokens are billed at a discount price and consumption can be monitored in [Scaleway Cockpit](/cockpit/).

For *Dedicated Deployments*, input cache is fully dedicated to your usage and already included in the prices. Available cache size is proportional to the vRAM available on the GPU Instance type selected.
