...
- A connection string provides the necessary information and parameters to establish a connection between an [IAM user](/iam/concepts/glossary/#user) or [application](/iam/concepts/glossary/#application), and the database. The string is written as follows:
    ```bash
...
- Connection parameters provide the necessary information and parameters to connect an [IAM user](/iam/concepts/glossary/#user) or [application](/iam/concepts/glossary/#application) to a database. The parameters are expressed in the `KEY="value"` format, as follows:
...
2. Select the [principal](/iam/concepts/glossary/#principal) to connect to the database, who can be either:
    - Yourself, as an [IAM user](/iam/concepts/glossary/#user), or
    - An [IAM application](/iam/concepts/glossary/#application) you [previously created](/iam/principals/create-application/) in the IAM section of the console.
    <Message type="note">
        Refer to the [IAM documentation](/iam/concepts/overview/) for more information on identities and access rights.
    </Message>
3. Set up the **connection string** or **connection parameters**.
    <Tabs>
        <TabsTab label="With an existing API key">
        <br/>
        If you already have an [API secret key](/iam/concepts/glossary/#api-key), replace the `[INSERT_SECRET_KEY_HERE]` placeholder with your **secret key** as shown below.
        - Connection string:
            ```
        ...
        <br/>
        If you do not have an [API key](/iam/concepts/glossary/#api-key), click **Generate new secret key**.
