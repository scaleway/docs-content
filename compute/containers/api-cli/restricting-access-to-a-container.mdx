...
This page shows you how to restrict access to your container by making it private. This prevents unauthorized launching of the container. Contrary to private functions, private containers need to be configured to prevent unauthenticated requests to access the container.
...
Privacy can be ensured via **J**SON **W**eb **T**okens. A JWT Token can be retrieved ffrom the API-endpoint `GET /issue-jwt`. Depending on the parameters, a JWT token can be valid for either a function, a container, or a namespace:
