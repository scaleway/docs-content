...
totalTime: PT10M
steps:
  - step: Creating a Kubernetes Cluster using kubeadm on Scaleway Elements
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/cloud-controller-manager/#-Creating-a-Kubernetes-Cluster-using-kubeadm-on-Scaleway-Elements
categories:
  - compute
  - kapsule
tags: compute mediaserver media video sharing conference call
image: /assets/images/docs/scaleway.png
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, ubuntu server, debian server, linux server, IAAS, baremetal, scalability, ARM, physical server, dedicated server, physicalization, server, kubernetes, kubeadm, ubuntu, cloud-controller-manager
dates:
...
A [Cloud Controller Manager](https://kubernetes.io/docs/tasks/administer-cluster/running-cloud-controller/) is a daemon that embeds cloud-specific control loops. It can be used to deploy resources in the Scaleway Elements ecosystem.
...
The goal of this step is to create a `Kubernetes` cluster using `kubeadm` on Scaleway Elements [Virtual Instances](https://www.scaleway.com/en/virtual-instances/).
...
2. Deploy a `LoadBalancer` service and verify a public ip is assigned to this service. The service will automatically create a [managed Load Balancer](https://www.scaleway.com/en/load-balancer/) on the Scaleway Elements platform.
...
You have sucessfully deployed a cluster with `kubeadm` and the `scaleway-cloud-controlle-manager`. For more information about the Scaleway Cloud Controller Manager, follow the project on [GitHub](https://github.com/scaleway/scaleway-cloud-controller-manager).
