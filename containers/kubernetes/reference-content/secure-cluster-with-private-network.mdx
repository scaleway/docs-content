...
<Message type="important">
  Be aware that any existing Kapsule cluster **will be destroyed** and recreated by adding the `private_network_id` attribute over an existing terraform definition.
