...
Health checks are available in many different types. By default, the health check type is set to be the same as that of the [backend protocol](/network/load-balancer/reference-content/configuring-backends/#protocol-and-port), that is to say **TCP** or **HTTP**. However, this can be overridden. The following health check types are available:
