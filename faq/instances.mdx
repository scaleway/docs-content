...
Both are being updated automatically when the Instance moves to another hypervisor (and thus its IPv6, and the private IPv4, changes). You can either use these hostnames directly or configure a [CNAME record](/network/domains-and-dns/concepts//#resource-records) linking to it. 
