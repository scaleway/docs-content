...
## How to check server with no RAID controller
...
2. Run the following command from the root account (or precede it with `sudo`):
    ...
    ```
    <Message type="note">
      The identifier `sdx` refers to your hard disk, replacing x with a, b or c etc. depending on the disk you want to test.
    </Message>
