...
- `PreferNoSchedule` tries to avoid scheduling them but will fall back if no other node fits
- `NoExecute` both evicts any already‑running non‑tolerating pods and prevents new ones from being scheduled
