...
| :--------------------- | :------------------------------------------------------------------------------------------------ | :---------------------------------------------------------------------------------------------------------------------------------- | --- |
| `$SCW_ACCESS_KEY`      | The [access key](/iam/credentials/create-api-keys/) of a token                                         | `$SCALEWAY_ACCESS_KEY` (used by Terraform/OpenTofu)                                                                                 |     |
| `$SCW_SECRET_KEY`      | The [secret key](/iam/credentials/create-api-keys/) of a token                                         | `$SCW_TOKEN` (used by the CLI), `$SCALEWAY_TOKEN` (used by Terraform/OpenTofu), `$SCALEWAY_ACCESS_KEY` (used by Terraform/OpenTofu) |     |
