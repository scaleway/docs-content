...
4. Click **+ Attach a Private Network**. The Private Network configuration wizard displays.
  ...
  * Enter the static IP for the Database Instance. The static IP can be any valid IP from a private address space, as specified in [RFC 1918](https://datatracker.ietf.org/doc/html/rfc1918).
    ...
    <Message type="tip">
      You can use the database virtual IP (VIP) in a DHCP-controlled environment by [reserving the IP](/network/public-gateways/ic-gateways/how-to/configure-a-public-gateway/#how-to-review-and-configure-dhcp) in the DHCP server configuration to be sure it is not assigned to any other Instance, then configure it on the Private Networks endpoint.
    </Message>
