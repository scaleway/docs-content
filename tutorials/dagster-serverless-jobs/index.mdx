...
In this tutorial, we will create a [Serverless Job](/serverless/jobs/quickstart) using Dagster to scrape the [Hacker News webstite](https://news.ycombinator.com/) to retrieve the top articles' title, author, and URL.
...
<Message type="requirement">
  ...
  - You have installed [Docker](https://docs.docker.com/engine/install/) on your local machine to build and push the image
...
1. Run the following command to install Dagster:
    ```
...
The Serverless Database ensures the communication between the different Dagster enements, especially to notify the UI that the scheduler has completed the execution of a job. It also stores the different events occurring during the execution.
...
- The source code is available in the [dagster-scaleway](https://github.com/cyclimse/dagster-scaleway) repository.
