...
domain_example.com_ is the `domain` and has only one owner who can manage it.

By default, the domain only has a root zone and no subdomains.

The `zone` for _example.com_ allows you to configure records such as:
...
But multiple zones (i.e. subdomains) allow:
