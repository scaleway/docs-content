...
When [TLS is activated](/network/load-balancer/reference-content/configuring-load-balancer/#configuring-a-backend) on your HTTP backend, HTTP/1 and HTTP/2 support are **automatically enabled by default**...

- ... on the backend, if the backend server is configured to support HTTP/2.
- ... on all attached frontends with an [SSL/TLS certificate](/network/load-balancer/how-to/add-certificate/).

## Configuring your Load Balancer for HTTP/3

HTTP/3 is supported on for frontends meeting the following conditions:
