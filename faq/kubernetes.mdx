...
`kapsule-agent` is a software developed internally at Scaleway and is used to assist in managing the lifecycle of nodes. Its primary function is to set up the network during the initial boot, and toggle the SSH server on or off based on a k8s label of each node.
