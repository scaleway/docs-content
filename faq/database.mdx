...
[Private Networks](/network/vpc/quickstart/) allows you to configure your own Layer-2 network with IPv4 adresses. Thanks to this feature, you enhance the security of your systems architecture by isolating it from the Internet. 
...
This unleashes the DB performances by reducing the latency between your nodes and increases the security of your databases, as Instances in your Private Network can directly to your Database Instance, without passing over the public Internet.

You can create a new Database Instance on a Private Network or connect an existing Database Instance by adding a new Private Networks Endpoint on it. 

## How many Private Networks can I attach? 

During the public beta, you can connect one Private Networks on each Database Instance.
...
## How is the Database private failover IP is managed?

The virtual IPv4 used to expose the Database Instance service is automatically routed to the new promoted node and announced within 10s to the neighbor Compute Instances.

## Can I remove the LoadBalancer Endpoint from my Database Instance? 
You can not remove the LoadBalancer Endpoint from your Database Instance, but you can block all access by emptying the allowed IPs list from the console or via the API.

## Can I limit the IPs allowed to reach my Database Instance on a Private Network?

It is not possible to limit access to a Database Instance on a Private Network. We recommend to create a dedicated Private Network for your Database Instances and add only the relevant middleware application Instances to this Private Network.

## Can I add an IPv6 to my Database Instance?

IPv6 is not supported with Private Networks on Database Instances.

## Where is Private Network available for Database Instances?

Private Network is available for Database Instances in PAR1, AMS1 and WAW1.

You can not connect PAR2 or PAR3 compute instances to a Database Instance located in PARIS region using Private Network.

| RDB Region         	| Private Network Region                                                                               	|
|--------------------	|------------------------------------------------------------------------------------------------------	|
| Amsterdam (nl-ams) 	| nl-ams-1 <Icon name="validate" />                                                                    	|
| Paris (fr-par)     	| fr-par-1 <Icon name="validate" />   fr-par-2 <Icon name="cancel" />  fr-par-3 <Icon name="cancel" /> 	|
| Warsaw (pl-waw)    	| pl-waw-1 <Icon name="validate" />                                                                    	|

## Can I assign a Database VIP with DHCP?

No, you need to provide a static IPv4 with CIDR notation on the same subnet as the one configured on your Private Network.
However, you can use the Database VIP in a DHCP-controlled environnement by [reserving the IP](/network/vpc/how-to/configure-a-public-gateway/#how-to-review-and-configure-dhcp) in the DHCP server configuration to be sure it is not assigned, then configure it on the Private Networks endpoint.
...
## What should i use as VIP? How to calculate CIDR netmask?
