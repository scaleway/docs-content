...
2. 
  1. For Debian & Ubuntu, update the `apt` package manager repositories, then upgrade the software already installed on the machine to the latest version available in the repositories: 
    ```
    apt update && apt upgrade -y
    ```
  2. For Arch Linux, update the `pacman` package manager repositories.
    ```
    pacman -Syu
    ```
3.
  1. For Debian & Ubuntu, install DHCPCD using the `apt` package manager:
    ```
    apt install dhcpcd5
    ```
3.
  2. For Arch Linux, install DHCPCD using the Arch Linux package manager:
    ```
    pacman -S dhcpcd
    ```
