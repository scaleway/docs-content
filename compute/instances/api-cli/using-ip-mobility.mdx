...
It is important to note that even if you do not assign a public IP to your Instance, you can still use the private IP to connect with other Instances.
In addition, the public network is provided by a network component that manages a carrier-grade NAT (Network Address Translation), establishing a one-to-one mapping between the public IP and the private IP assigned to the Instance during provisioning.

With IP Mobility, private IPs are no longer in use. Instead, traffic is routed directly to the public IP of the Instance. The main advantage of this approach is that your underlying IP remains unchanged when you stop and start the Instance or when it gets provisioned on another hypervisor. However, it also means that it will not have network access if you do not attach any public IP to your Instance.
Another feature of IP Mobility is the ability to assign multiple public IP addresses to your Instance, allowing you to choose between IPv4 and IPv6 protocols.
