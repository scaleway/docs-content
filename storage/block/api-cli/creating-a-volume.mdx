---
meta:
  title: Creating a Block Storage volume via the API
  description: This page explains how to creat a Block Storage volume using the API.
content:
  h1: Creating a Block Storage volume via the API
  paragraph: This page explains how to creat a Block Storage volume using the API.
steps:
  - step: TODO
    type: HowToStep
    url: https://www.scaleway.com/en/docs/storage/block/api-cli/creating-a-volume
dates:
  validation: 2021-06-02
  posted: 2020-01-10
---

<Message type="requirement">

  - You have a [Scaleway account](/console/my-account/how-to/create-an-account/)   
  - You have [configured your API keys](/console/my-project/how-to/generate-api-key/)

</Message>

1. Create a block volume from the API by launching the following command:

  ```
  $ curl -q \
    -H "X-Auth-Token: $SECRET_KEY" \
    -H 'Content-Type: application/json' \
    -X POST \
    -d '{"name": "block-volume-101", "organization": "'$SCW_ORGANIZATION'", "size": 10000000000, "volume_type": "b_ssd"}' \
    https://api.scaleway.com/instance/v1/zones/fr-par-1/volumes
  ```

2. To retrieve the current status of the block volume, query the volume endpoint.

  ```
  $ curl -q \
    -H "X-Auth-Token: $SECRET_KEY" \
    -H 'Content-Type: application/json'  \
    https://api.scaleway.com/instance/v1/zones/fr-par-1/volumes/b3a42fb1-e85c-46e9-b0a6-9adb62278295
  {
    "volume": {
      "size": 10000000000,
      "state": "available",
      "name": "block-volume101",
      "modification_date": "2019-09-03T09:57:53.742130+00:00",
  ...
      "id": "b3a42fb1-e85c-46e9-b0a6-9adb62278295",
      "volume_type": "b_ssd",
      "server": null
    }
  }
  ```


<Navigation title="See Also">
  <NextButton to="/storage/block/api-cli/attaching-a-volume">Attaching a volume</NextButton>
</Navigation>
