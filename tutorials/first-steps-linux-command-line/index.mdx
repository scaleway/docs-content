...
You may be used to using a **G**raphical **U**set **I**nterface (**GUI**) for your machine, such as Windows or MacOS. These GUIs make it easy to do everything visually, with clicks of your mouse to open, and close programs and complete thousands of different tasks.
...
To find out where you are (i.e. what your working directory is), use the `pwd` command. It stands for **p**rint **w**orking **d**directory. Type it into your terminal and hit enter:
...
You can move between directories with the `cd` command. It stands for **c**hange **d**directory.
...
You can make a new directory with the `mkdir` command. It stands for **m**ache **dir**ectory.
...
3. Log in as the new user with the `su` (**s**witch **u**set) command:
