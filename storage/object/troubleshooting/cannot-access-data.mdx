...
- Make sure there is no [bucket policy](/object/concepts/#bucket-policy) that prevents you from deleting the bucket. If there is one, make sure that it [explicitly allows you](/object/api-cli/bucket-policy/) to perform the desired action.

- Make sure that [Object lock](/object/api-cli/object-lock/) has not been enabled on the bucket you try to access.
