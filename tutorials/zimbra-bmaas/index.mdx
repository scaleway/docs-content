...
excerpt: |
  The Zimbra Open Source Collaboration Suite is designed to provide an end-to-end mail solution that is scalable and highly reliable. It consits of both, a client and server components. The web client provides a full-featured collaboration suite that supports email and group calendars. 
  Messaging architecture is built with open technology and well-known standards. It provides POP3 and IMAP access and relies on open-source applications such as Postfix, ClamAV and SpamAssassin.
totalTime: PT30M
steps:
...
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, ubuntu server, scalability, physical server, dedicated server, server, network, messaging, mail, zimbra, email, bare metal cloud server, bare metal, baremetal
dates:
