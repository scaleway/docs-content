...
A principal is an entity that can be attached to a policy. Currently, you can attach either a [user](/iam/principals/users-groups-and-applications/#users), a [group](/iam/principals/users-groups-and-applications/#groups), or an [application](/iam/principals/users-groups-and-applications/#applications) to a policy. You can also select **AllUsers** or **AllApps** to attach the policy to every user or every application in your Organization at once. Only one principal can be attached to each policy.
...
A rule is the part of a policy that defines the permissions of the policy's [principal](/iam/concepts/glossary/#principal), and the [scope](/iam/concepts/glossary/#scope) of these permissions. It consists of a **scope** (at Project level or Organization level) and one or more **permission sets** (eg. "list all Instances").
...
<Message type="note">
  You can find a detailed list of all permission sets available at Scaleway in the permission sets [reference page](/iam/policies-permissions/permission-sets/).
</Message>
...
A condition is an additional layer of restrictions for your rule. You can allow access to specific user agents or IP addresses, and allow actions to be performed only at certain dates and times. Conditions are defined through [CEL](/iam/concepts/glossary/#common-expression-language-cel) expressions. In general, a condition expression consists of one or more statements that are joined by logical operators (`&&`, `||`, or `!`).

Conditions can be set up and configured in the Scaleway console.

Refer to the [Understanding policy conditions](/iam/policies-permissions/understanding-policy-conditions/) documentation page to learn how they are set up and how you can define them.
