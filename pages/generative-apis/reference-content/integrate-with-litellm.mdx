...
Note that for `/v1/embeddings` endpoint, parameter `encoding_format` must also be added and set to `float`. 
...
- Configure the health check parameters. 
  - For instance, when using embedding models, you must add `mode: embedding` under `model_info` in `config.yaml`, as [mentioned in the LiteLLM Health Checks documentation](https://docs.litellm.ai/docs/proxy/health#embedding-models).
  - As of version `1.81.12`, LiteLLM does not yet support performing healthchecks on `/v1/audio/transcriptions` endpoint: LiteLLM performs healthchecks using `/v1/chat/completions` query format and thus fails.
