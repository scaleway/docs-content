...
Adding a cron schedule to a specific task allows you to run it automatically at fixed times and dates. Cron jobs are specified using a unix-cron format. You can run a task as often as every minute, up to once a year.

## Cron schedule format

Cron jobs are explressed using the `* * * * *` format, which corresponds to the fields below.
...
<Message type="note">
  - The month and the day of the wee can be both exressed using numerical or alphabetical Values
...
| `15,45 12-17 * * WED`      | Runs at minute 15 and 45 from 12:15 to 17:45 every Wednesday                                          |
