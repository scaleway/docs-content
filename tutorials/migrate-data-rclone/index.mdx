...
The `sync` command from rclone should be prefered to `copy`, because it deletes files from destination.
...
Then sync buckets your other Block Storage proider -> SCW and SCW -> SCW:

```bash
# sync from ovh to scaleway (nl-ams)
rclone --config rclone.conf sync ovh-gra://my-bucket/my-key-prefix scw-nl-ams://my-bucket/my-key-prefix
