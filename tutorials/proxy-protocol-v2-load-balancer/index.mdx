...
tags: compute
excerpt: |
  The Scaleway managed Load Balancer supports the PROXY Protocol v2.
  It allows an application, like a web server like Apache or Nginx, to retrieve client information of a user passing via a load balanced infrastructure.
totalTime: PT15M
steps:
  - step: Enabling PROXY Protocol from the Management Console
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/proxy-protocol-v2-load-balancer/#-Enabling-PROXY-Protocol-from-the-Management-Console
  - step: Configuring Proxy Protocol in Nginx Web Server
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/proxy-protocol-v2-load-balancer/#-Configuring-Proxy-Protocol-in-Nginx-Web-Server
  - step: Configuring Proxy Protocol in Apache Web Server
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/proxy-protocol-v2-load-balancer/#-Configuring-Proxy-Protocol-in-Apache-Web-Server
categories:
  - network
  - load-balancer
...
The [PROXY Protocol](https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt) allows an application, like a web server like Apache or Nginx, to retrieve client information of a user passing via a load balanced infrastructure.
...
When using a [Load Balancer](/network/load-balancer/concepts) in front of your web servers, the load balancers IP address is passed by default to your web servers. This result is visible in the access log files of the web server:
...
1. When creating a new load balancer or backend rule, enable the **Proxy** option in the Backend settings:
  ...
  <Lightbox src="scaleway-lb_proxy.png" alt="" />

2. To enable the PROXY protocol for a load balancer that already exists, enter the **Load Balancer** section of the [management console](https://console.scaleway.com) and click the **Backends** tab.

3. Click the Backend rule to modify:

  <Lightbox src="scaleway-lb_backend_list.png" alt="" />

4. Enable the **Proxy** option for the rule before saving the configuration:

  <Lightbox src="scaleway-lb_edit_backend.png" alt="" />
