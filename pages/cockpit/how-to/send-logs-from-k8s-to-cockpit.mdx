...
description: Learn how to send the logs of your pods to your Cockpit using Scaleway's comprehensive guide. This tutorial covers sending Kubernetes pod logs to Scaleway Cockpit for centralized monitoring and analysis using Grafana, ensuring efficient monitoring and log analysis in your infrastructure.
...
Note that the example Helm chart can be configured to send both logs and metrics to your Cockpit. In this tutorial, we are interested in the logs part only. For information about setting up metrics collection, see our documentation about [sending metrics from your Kubernetes cluster to Cockpit](/cockpit/how-to/send-metrics-from-k8s-to-cockpit).
...
clusterMetrics:
  ...
  # Already included in Scaleway Datasource
  controlPlane:
    enabled: false
...
podLogsViaLoki:
  ...
  staticLabels:
    source: "podLogs"
  # Add a indexed label containing the podname from structured metadata for ease of filtering in dashboards
  extraLogProcessingStages: |-
    stage.labels {
      values = {
        pod = "pod",
  ...
  alloy:
    alloy:
      logging:
        level: info
        format: logfmt
      # Easier debug, but consumes more ressources
      liveDebugging:
        enabled: false
...
<Message type="tip">
 ...
 In the example configuration, we only keep pods with the annotation `cockpit/logs=true`.
...
To learn how to set up log collection using the Scaleway CLI, see [Kubernetes Monitoring with Helm (Manual Installation)](https://github.com/scaleway/k8s-monitoring/tree/main/helm-only).
