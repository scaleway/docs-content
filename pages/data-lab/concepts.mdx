---
title: Clusters for Apache Spark™ - Concepts
description: Learn the fundamental concepts of Scaleway Clusters for Apache Spark™.
tags: data lab clusters for apache spark notebook jupyter processing
dates:
...
An Apache Spark™ cluster is an orchestrated set of machines over which distributed/Big Data calculus is processed. In the case of Scaleway Clusters for Apache Spark™, the Apache Spark™ cluster is a Kubernetes cluster with Apache Spark™ installed in each Pod. For more details, check out the [Apache Spark™ documentation](https://spark.apache.org/documentation.html).

## Clusters for Apache Spark™

Clusters for Apache Spark™ (formerly known as Data Lab for Apache Spark™) allows you to create an Apache Spark™ cluster distributed across multiple worker nodes to accelerate the processing of large datasets, saving time and enabling faster access to actionable insights. 

Your cluster (formerly called a Data Lab) acts as a project setup that comes with an optional notebook for data processing, analysis, and exploration. The cluster is available through a VPC. It includes the required infrastructure and tools to allow data engineers and data scientists to leverage the power of the cluster for their pipeline or use it to explore data, create models, and gain insights.

## Endpoint

A point of connection to the cluster. The endpoint is associated with an IPv4 address and a port and contains information on whether the endpoint is read-write.
- Notebook endpoint: when the option is chosen, an endpoint is provided to connect the notebook to the Spark™ cluster
- Private endpoint: the endpoint needed to connect the Spark™ cluster to other resources within the same Private Network
...
A fixture is a set of pre-defined data used for testing purposes. In the context of Clusters for Apache Spark™, fixtures are essential for ensuring that your code is properly tested and validated before being deployed to production.
