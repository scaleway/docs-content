...
[Private Networks](/network/vpc/quickstart/) allow you to configure your Layer-2 network with IPv4 addresses on Managed Database for Redis™ Instances.
...
In a cluster topology, you must [provide a private IPv4 address](/faq/databases-for-postgresql-and-mysql/#what-should-i-use-as-virtual-ip-how-do-i-calculate-the-cidr-netmask) to each node. For instance, if you have a three nodes cluster, you must declare 3 IPs on the same L3 network. The 3 IPs must be able to connect to each other.
