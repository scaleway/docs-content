...
```
<Message type="important">
 - If you have not configured your environment in your local machine, make sure you replace `\<project_id\>` with your [Project ID](/console/project/concepts/#project), `$REGION` with `fr-par` and `$TOKEN` with your secret key. If you do not replace the Project ID, the API displays a "permission denied" error message.
