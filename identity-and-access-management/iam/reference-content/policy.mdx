...
Policies are statements that define the permissions of users, groups and applications in Scaleway Organizations.

They are composed of:

- a [principal](#principal) - a user, group or application
- one or more IAM rules - consisting of [permission sets](#permission-sets) and a [scope](#scope)
...
A rule is the part of a policy that defines the permissions of the policy’s [principal](#principal), and the [scope](#scope) of these permissions. It consists of a **scope** (at Project level or Organization level) and one or more **permission sets** (eg. "list all Instances"). A policy can have one or many rules. 
...
If we attribute the rule above to a principal (user, group or application), we create a policy:
...
The policy states that the application "production-c" can create, list, delete and manage Instances and Databases in Project A, but for Object Storage it can only list and read the resources.

If necessary, we could add more rules in the policy, giving "production-c"s additional permissions in other Projects, or at Organization level.

<Navigation title="See Also">
  <PreviousButton to="/identity-and-access-management/iam/reference-content/user-groups-and-applications/">IAM users, groups and applications</PreviousButton>
