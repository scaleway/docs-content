...
5. On debian based distrib, hold apt packages to avoid unattended upgrades:

    ```bash
    echo "scaleway-cni-plugins hold" | sudo dpkg --set-selections
    echo "scaleway-containerd hold" | sudo dpkg --set-selections
    echo "scaleway-kubectl hold" | sudo dpkg --set-selections
    echo "scaleway-kubelet hold" | sudo dpkg --set-selections
    echo "scaleway-runc hold" | sudo dpkg --set-selections
    ```
...
2. On debian based distrib, unhold apt packages before upgrade:

    ```bash
    echo "scaleway-cni-plugins install" | sudo dpkg --set-selections
    echo "scaleway-containerd install" | sudo dpkg --set-selections
    echo "scaleway-kubectl install" | sudo dpkg --set-selections
    echo "scaleway-kubelet install" | sudo dpkg --set-selections
    echo "scaleway-runc install" | sudo dpkg --set-selections
    ```

3. Run the installer program as you would do for a fresh node install, with the additional option `-self-update`. If the option is not available, redownload the program from S3 bucket.
4. Now the node will register itself with the Apiserver. Once it is ready, you will see the same node with two kubelet versions. The older node should end up `NotReady` after 5m, you can safely delete it with `kubectl`.
5. On debian based distrib, hold apt packages to avoid unattended upgrades:
    ...
    echo "scaleway-runc hold" | sudo dpkg --set-selections
    ```
6. Detach the older node in Scaleway API.
