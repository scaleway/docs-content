...
- **Load Balancing:** A Nginx reverse proxy can perform load balancing which helps distribute client requests evenly across backend servers. It also improves redundancy so if one server goes down, the reverse proxy will simply reroute requests to a different server according to the routing policy.
...
=======
1. Open a terminal and type the following command to update the APT package cache and install the Nginx web server via the package manager:
>>>>>>> fix(how to): fixed typos in vpc troubleshooting
...
=======
2. Type the following command to disable the default virtual host that is pre-configured when Nginx is installed via Ubuntu's package manager apt:
>>>>>>> fix(how to): fixed typos in vpc troubleshooting
...
4. Paste the following Nginx configuration in the text editor: 
  ...
  ```
The proxy server redirects all incoming connections on port 80 to the Webfsd server, listening on port 8000. Edit the port value depending on the applications specific port.
