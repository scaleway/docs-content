...
- **Necessity of `scaleway-ecosystem` and `cloud-init`:** These packages are not strictly required for routed IPs. However, if they are installed, it is recommended to update them to the latest version.
...
* For **Ubuntu and Debian** Instances:
    ```bash
    dpkg -l | grep scaleway-ecosystem || echo "scaleway-ecosystem is not installed"
    apt upgrade scaleway-ecosystem cloud-init -y
    ```

* For **CentOS and Fedora** Instances:
    ```bash
    rpm -qa | grep scaleway-ecosystem || echo "scaleway-ecosystem is not installed"
