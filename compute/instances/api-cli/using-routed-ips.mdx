...
<Message type="important">
  * Legacy instance types (STARDUST and VC-XX) are not compatible with the routed IP feature.
  * Using static network configurations during migration poses risks of losing internet connectivity (using DHCP is recommended).
  * Updating the `scw` and `cloud-init` packages is crucial before launching the migration.
</Message>
...
To activate the routed IP feature on an existing Instance, you can migrate it with the command shown below.
...
### Migrating existing Instances to routed IPs via the API
