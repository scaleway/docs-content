...
- [Installed and configured the AWS-CLI](/object/api-cli/object-storage-aws-cli/) to use with Scaleway Object Storage
- [Created an Object Storage bucket](/object/how-to/create-a-bucket/)
...
1. Run the command below to create a backup. Include the path to your repository (in this example, the path is the endpoint of your bucket), and the path of the directory you want to back up (`~/<my-directory>` in the example).
    ...
    <Message type="tip">
      By default, the backups you create are stored using `STANDARD` [storage class](/object/concepts/#storage-class). If you want to use a different storage class, you can add the `-o s3.storage-class=<STORAGE_CLASS>` option to your command, as shown below:
      ```
