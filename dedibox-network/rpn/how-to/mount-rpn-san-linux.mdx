---
title: How to mount a RPN-SAN volume on Linux
description: This page explains how to mount a RPN-SAN volume on Linux
dates:
...
3. Add the following configuration lines to the configuration: 
    ```
...
3. Type `dmesg` to see the kernel logs. You should see some lines like in the following example
    ...
    You can now use your SAN disk as any other disk.

## Important recommendations for use

As seen in the exaple abouve, the disk is available as `/dev/sdd.` It is not guaranteed that at the next reboot the same drive letter will be used, especially if you add other targets, or if your hardware configuration changes. It is therefore recommended not to use the names `/dev/sdX` in your configuration files.

### How to address devices when using LVM
LVM automatically uses UUIDs to recognize the partitions it uses. No problem in this case.

### How to address devices when using direct disk formatting

<Message typem="note">

This method is not recommended, because it does not allow you to evolve afterwards, by aggregating several RPN-SAN storage spaces.

</Message>

If you format directly the device or a partition of it, use the link created by the system in `/dev/disk/by-uuid/` or the fstab notation `UUID=XXX-XXX...` to address it.

### How to address devices direct use of the device

If your application does not support (unlike LVM) the auto-detection of the partitions associated with it via the UUID, it is recommended to use the link in `/dev/disk/by-id/`.

This is the available path that will be the most stable over time.