...
This page shows you how to send a simple transactional email in `JSON` format to multiple recipients using the `additional_headers` parameter using the Scaleway Transactional Email API.
...
5. Copy the following template. Make sure that you replace the placeholder information with your own.
        ...
        cat > mail.json <<EOF
        {
            ...
            "attachments": [
                {
                    "name": "file.html", # Replace with the name of the file your want to send
                    "type": "text/html; charset=\"utf8\"", # Replace with the attachment type you want to send
                    "content": "" # Replace with the content you want to send
                }
            ],
            "additional_headers": [
                {
...
7. Run the following command to check that your email has been sent. Make sure that you replace `email-id` with the ID of the email you retrieved in the output of the previous step.

        ```
        curl --request GET \
        --url https://api.scaleway.com/transactional-email/v1alpha1/regions/fr-par/emails/email-id \
