...
In this tutorial, we will build a simple architecture to store and automatically convert images to PDF. The focus is on transferring large messages using a messaging service, specifically the NATS server from Scaleway. Our setup involves using the Object Storage service for image storage and the Serverless Function service for conversion.
...
1. A producer implemented as a shell script responsible for uploading an image to the Object Storage bucket and sending a message to the NATS server.
2. A consumer implemented as a serverless Python function designed to receive the message, parse it, extract the image URL, download it from the bucket, convert it to a PDF, and then re-upload the resulting PDF.
...
- A NATS server responsible for receiving and dispatching messages sent by our producer.
- A triggering mechanism in NATS that forwards the message to our consumer.

## Prepare the infrastructure

1. Create a `main.tf` file and paste the following code to import the necessary providers.
    ```terraform
    terraform {
      required_providers {
        scaleway = {
...
5. Add these resources to create a NATS account and your NATS credential file:
    ```terraform
...
6. You can run `terraform init` and `terraform apply` to create the resources in your Scaleway account. 
...
To upload the file, we will use the AWS CLI configured with the Scaleway endpoint and credentials, because the Scaleway Object storage is fully compliant with S3.
...
4. Next create the code of your function. In the file `large_messages.py`, import the libraries that we need and create a empty handler function.
    ```python
    import os
    import boto3
