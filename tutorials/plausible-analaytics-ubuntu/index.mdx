...
## Configuring a Nginx reverse proxy for Plausible

As Plausible supports by default plain HTTP traffic by only, we configure a Nginx reverse proxy to provide HTTPS with a Let's Encrypt certificate to increase security.
...
6. Run `certbot` to obtain a TLS certificate for your domain name and let the application configure Ngninx automatically. 
  ...
  ```
  <Message type="tip">
    For more details on using Certbot, refer to our [dedicated documenation](/tutorials/nginx-reverse-proxy/#adding-tls-to-your-nginx-reverse-proxy-using-let's-encrypt).
  </Message>
