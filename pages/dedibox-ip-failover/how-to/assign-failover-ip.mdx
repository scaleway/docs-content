...
Your failover IP can be routed at any time to your servers in just a few minutes by using the [Dedibox console](https://console.online.net) or through the [Dedibox API](https://account.scaleway.com/?service=dediboxen/api/). By switching your failover IP addresses from one server to another, the hosted services remain available without reconfiguring the IP address.
