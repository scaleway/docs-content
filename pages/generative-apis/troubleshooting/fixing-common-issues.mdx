...
- Use [Managed Inference](/managed-inference/), where these quotas do not apply (your throughput will be limited only by the amount of Inference Deployment you provision).
...
- Use [Batches API](https://console.scaleway.com/generative-api/batches) for non-real time workloads. Requests performed through Batches API do not have a rate limit and are billed with a [-50% discount compared to standard model prices](https://www.scaleway.com/en/pricing/model-as-a-service/#generative-apis).
- Use [Managed Inference](/managed-inference/), where concurrent request limits do not apply. Note that exceeding the number of concurrent requests your Inference deployment can handle may impact performance metrics.
...
- Use [Managed Inference](/managed-inference/), where no capacity is dedicated to your usage only.
