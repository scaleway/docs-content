...
Setting the preferred Project does not automatically give the API key bearer permissions for Object Storage in this Project. Ensure that the user/application is either the Owner of the Organization, or has a [policy](/iam/concepts/glossary/#policy) giving them appropriate permissions for Object Storage in this Project. Note that the application of Object Storage permissions can take up to 5 minutes after creating a new rule or policy.
</Message>
...
To create an API key via the Scaleway console, follow the steps detailed in [how to create an API key](/iam/credentials/create-api-keys/).
...
<Message type="note">
    While creating an API key, you have the option to skip the step of selecting a preferred Project. Note that if you select this option, the Project that you are currently navigating in the console (i.e. the one that is selected in your [Project dashboard](/organizations-and-projects/concepts/glossary/#project-dashboard)) will be automatically selected as the preferred Project for Object Storage. This can still be edited later.
</Message>

It is also essential to make sure the API key [has permissions to carry out Object Storage actions within this Project](/iam/policies-permissions/permission-sets/). Ensure that a suitable policy is attached to the IAM user/application associated with this API key.
...
- [Owner](/iam/concepts/glossary/#owner) status or [IAM permissions](/iam/concepts/glossary/#permission) allowing you to create an IAM application.

1. [Create a new IAM application](/iam/principals/create-application/) called `my-test-app` without attaching any policies.
    <Lightbox image={image3} alt="" />

2. [Create an API key](/iam/credentials/create-api-keys/) for the `my-test-app` application. Select the preferred Project for the API key as `default`.
...
5. [Create a policy](/iam/policies-permissions/create-policy/) with the name `object-storage-full-access` and the IAM application you created in step 1 as the principal. When adding rules, create a rule giving `Access to resources` > `default` Project, and the `Storage` permission set `ObjectStorageFullAccess`.
...
1. [Create a new Project](/organizations-and-projects/project/create-a-project/) in the Organization, with the name `my-second-project`.
    <Message type="note">
    You must be Owner of the Organization, or have the `ProjectManager`, `IAMManager` or `OrganizationManager` [permissions](/iam/policies-permissions/permission-sets/) to create a Project.
    </Message>
