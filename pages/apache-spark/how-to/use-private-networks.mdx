...
10. Set the following parameters (remember to remove the hashtags to uncomment the parameters):
...
13. Access [http://localhost:8888](http://localhost:8888), then enter the token generated while executing the `jupyter lab` command.
    ...
    <Message type="tip">
    To find the token, look for this line in the terminal: `http://127.0.0.1:8888/lab?token=<TOKEN_VALUE>`
    </Message>

You now have access to your Apache Spark™ cluster via a Private Network, using a JupyterLab notebook deployed on an Instance.
