...
excerpt: |
  ERPNext is a free and open-source ERP software designed to be used by manufacturers, distributors and service companies. It includes a full range of modules including accounting, sales, CRM, purchasing, warehouse management and inventory. Moreover, specialized modules are available tailored to the needs of schools, healthcare, agriculture and non-profit sectors. It uses a MariaDB database to store its dat, is written in Python and based on the Frappé Framework. The application is released under the GNU GPLv3 license and its source code is hosted on GitHub.
totalTime: PT30M
steps:
