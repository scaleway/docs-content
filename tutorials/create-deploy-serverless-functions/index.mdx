...
meta:
  ...
  description: This pages shows you how to create and deploy functions to transform images in an S3 bucket using Serverless Functions and Triggers
content:
  h1: Transforming images in an S3 bucket using Serverless Functions and Triggers - Deployment
  paragraph: This pages shows you how to create and deploy functions to transform images in an S3 bucket using Serverless Functions and Triggers
categories:
  - serverless
...
6. Store the token securely, as you will not be able to access them later.
...
7. Enter the following **environment variables**:
    ...
    - `DESTINATION_BUCKET`: the name of your destination bucket (e.g.: `dest-images-{YourName}`).
9. Click **Create function**. Deploying a function takes a few minutes to complete.

### Ading an SQS trigger to the ImageTransform function
