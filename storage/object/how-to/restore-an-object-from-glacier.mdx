...
<Message type="note">
  The `s3cmd restore` command does not support permanent restores, and only restores objects for one day by default. Use the `--restore-days=NUM` parameter to specify the desired number of days.
</Message>
