...
```python
import cirq
from cirq_scaleway import ScalewaySampler
...
# Example: 'EMU-QSIM-H100' targets Qsim running on an NVIDIA H100
PLATFORM_NAME = "EMU-QSIM-H100"

# 3. Initialize the Sampler
sampler = ScalewaySampler(
    project_id=PROJECT_ID,
    secret_key=SECRET_KEY,
    platform=PLATFORM_NAME
)

# 4. Create a Circuit (e.g., Bell State)
...
# The circuit is sent to Scaleway, executed by Qsim on GPU, and results returned.
...
print(f"Submitting to Qsim backend: {backend.name}")
