...
[Understand the differences between Jobs, Functions, and Containers](/serverless-functions/reference-content/difference-jobs-functions-containers/).
...
Serverless Functions offers several runtimes and versions. In some cases, more control is required or specific dependencies needs some runtime tuning. For these cases, read [How to migrate from Functions to Containers](/serverless-functions/how-to/migrate-functions-to-containers/).
...
### How do I upgrade the runtime version?

Serverless Functions runtimes are updated by Scaleway. If you need a specific version, we recommend using Serverless Containers instead.

Serverless Containers and Serverless Functions are very similar. [Learn how to migrate from Functions to Containers](/serverless-functions/how-to/migrate-functions-to-containers/).
...
* **Resource provision:** in order to mitigate cold starts, users can choose to keep some function instances warm (by setting the minimum scale value). We then charge for the provisioned resources similarly to the Resource consumption component.
...
For detailed and up-to-date information on pricing, refer to the [official Scaleway pricing page](https://www.scaleway.com/en/pricing/serverless/#serverless-functions).

#### Examples

<Message type="note">
This calculation is provided for illustrative purposes only. For detailed and up-to-date information on pricing, refer to the [official Scaleway pricing page](https://www.scaleway.com/en/pricing/serverless/#serverless-functions).
</Message>

<Message type="tip">
When deploying a Serverless Functions from the Console, a cost estimator is provided directly in the Console.
</Message>
...
* **Resources consumption**
  * *Service usage duration:* 30 000 000 requests x 0.15 s = 4 500 000 seconds used
  * *Memory conversion:* 128 MB = 0.125 GB
  * *Resources consumed:* 4 500 000 s x 0.125 GB = 562 500 GB-s
  * *Free tier:* 400 000 GB-s
  * *Resourced billed:* 562 500 - 400 000 = 162 500 GB-s
  * *Cost:* 162 500 x €0.0000170 = **€2.76**
...
**Total monthly cost: €7.11**

##### Example 2: With resources provisioning

| Criteria                     	| Value        	|
|-------------------------------|--------------	|
| Number of requests           	| 60 000 000 	|
...
* **Resources consumption**
  * *Service usage duration:* 60 000 000 requests x 0.1 s = 6 000 000 seconds used
  * *Memory conversion:* 128 MB = 0.125 GB
  * *Resources consumed:* 6 000 000 s x 0.125 GB = 750 000 GB-s
  * *Free tier:* 400 000 GB-s
  * *Resourced billed:* 750 000 - 400 000 = 350 000 GB-s
  * *Cost:* 350 000 x €0.0000170 = **€5.95**
...
* **Provisioned functions consumption:**
  ...
  * *Cost:* 324 000 x €0.0000050 = **€1.62**

* **Requests:**
  * *Free tier:* 1 000 000 requests
  * *Billed requests:* 60 000 000 - 1 000 000 = 59 000 000 requests
  * *Cost:* 59 000 000 x €0.00000015 = **€8.85**

**Total monthly cost: €16.42**
...
<Message type="note">
Your invoice displays your Free Tier usage under the **Serverless Function Free Tier** section. This section shows the costs saved through the Free Tier, expressed both in euros and in `currency` (an integer value representing the smallest monetary unit, where 1 `currency` = €0.01, i.e., one cent).
</Message>
...
Integration is straightforward. Serverless Functions and Containers can be triggered by events from [Queues](/queues/concepts/#queue) and [Topics and Events](/topics-and-events/concepts/#topics-and-events), and can easily communicate with services like [Managed Databases](/managed-databases-for-postgresql-and-mysql/) or [Serverless databases](/serverless-sql-databases/). [Serverless Jobs](/serverless-jobs/) can pull data from [Object Storage](/object-storage/), or output processed results into a database. With managed connectors, APIs, and built-in integrations, linking to the broader Scaleway ecosystem is seamless.

### Can I use Serverless Functions with Edge Services?

Yes, Serverless Functions (and also Serverless Containers) are natively integrated. For more information, read [how to set Serverless Containers behind Edge Services](/serverless-containers/how-to/deploy-container-behind-edge-services).
...
### How do I access my Serverless Function with a Private Endpoint?

Serverless Functions supports Private Networks for outgoing traffic (like accessing a database or an instance), but not yet for incoming traffic.

This feature is under development.

### How can I disable the public endpoint for Serverless Functions?

Disabling the default public endpoint for Serverless Functions is not yet available. To restrict access to Serverless Containers, you can [make the resource private](/serverless-functions/how-to/secure-a-function/#restrict-access-to-your-functions).

This feature is under development.

### How do I add a Load Balancer in front of my Serverless Function?

Load Balancing is integrated directly to Serverless Functions when you invoke them; our infrastructure handles traffic without the need to add other products.

Also, Load Balancers are compatible only with IP backends. As Serverless Functions IPs are not predictable, it is not possible to integrate Functions directly with Load Balancers.
...
### Can I allow the IPs of my Serverless Functions?

Due to the nature of serverless computing, Serverless Functions do not have dedicated or predictable IP addresses assigned to them. This means you cannot allow specific IPs for your Serverless Functions.
...
Additionally, Scaleway Serverless Functions supports [Virtual Private Cloud (VPC)](/serverless-functions/how-to/use-private-networks/) and can be attached to a Private Network, which allows you to securely connect your resources in an isolated environment.
...
At the moment, we do not provide encryption at rest on the filesystem of customer functions. However, the underlying volumes comply with our security standards. For more details, refer to our [Security and resilience documentation](https://www.scaleway.com/en/security-and-resilience/).
...
### Is there any anti-malware or antivirus system in place for functions to secure the environment?
...
### Is Serverless Functions integrated with Audit Trail?

Yes, Serverless Functions is auditable. For more information, see the documentation about [Supported endpoints in Audit Trail](/audit-trail/reference-content/adt-supported-endpoints/#serverless-functions).
