---
meta:
  title: Deploy Strapi headless CMS using Serverless Containers and Serverless SQL Databases
...
content:
  h1: Deploy Strapi headless CMS using Serverless Containers and Serverless SQL Databases
...
## Deploy Strapi headless CMS using Scaleway CLI
...
3. Start Strapi in development mode:
    ...
    Your browser should open automatically at the address `http://localhost:1337` to create a local administrator account.
    Define your credentials and create the account.

4. Congrats ! You can now access Strapi Administration Panel at `http://localhost:1337/admin`. Once connected with your local administrator account, you can browse and edit content.
...
4. Add the following code to it:
    ...
    Docker will now ignore node dependencies and other unecesseray files during build process.
...
6. Run the following command to create random secrets required by Strapi and export its attributes as variables:
    ```bash
...
7. Check that your container runs locally:
    ...
    <Message type="tip">
    The first run of this container will raise database constraints error such as `error: YupValidationError: This attribute must be unique` which will not alter any data. This is because for simplicity in this tutorial, we use the same **Serverless SQL Database** for development and production environments. Hence initial sample data loading happens twice, once in `NODE_ENV=development` environment when running `npm run develop` initially and once in `NODE_ENV=production` environment when running `docker build -t my-strapi-blog .` The second loading triggers errors as data already exists, but will not happen again afterwards for these environments.
    </Message>
    

8. Access `http://localhost:1337/admin` in a browser. Strapi Administration Panel displays.
...
4. (Optional) You can check that Strapi APIs are working with the following command or going to `http://localhost:1337/api/articles` in your browser:
    ```bash
    curl -X GET http://localhost:1337/api/articles | jq
    ```
...
To secure your deployment, we will now add a dedicated [IAM application](/identity-and-access-management/iam/concepts/#application), give it the minimum required permissions, and provide its credentials to your the Strapi container.
...
8. Redeploy your Serverless Container:
    ```bash
    scw container container update $CONTAINER_ID environment-variables.DATABASE_HOST=$DATABASE_HOST environment-variables.DATABASE_PORT=$DATABASE_PORT environment-variables.DATABASE_NAME=$DATABASE_NAME environment-variables.DATABASE_USERNAME=$SCW_APPLICATION_ID secret-environment-variables.0.key='DATABASE_PASSWORD' secret-environment-variables.0.value=$SCW_APPLICATION_SECRET secret-environment-variables.1.key='APP_KEYS' secret-environment-variables.1.value=$APP_KEYS secret-environment-variables.2.key='API_TOKEN_SALT' secret-environment-variables.2.value=$API_TOKEN_SALT secret-environment-variables.3.key='ADMIN_JWT_SECRET' secret-environment-variables.3.value=$ADMIN_JWT_SECRET secret-environment-variables.4.key='TRANSFER_TOKEN_SALT' secret-environment-variables.4.value=$TRANSFER_TOKEN_SALT secret-environment-variables.5.key='JWT_SECRET' secret-environment-variables.5.value=$JWT_SECRET redeploy=true
    ```
...
2. Run the command below to make sure the environment variables are properly set:
    ...
    This command displays your access_key and secret_key in the two last lines of the ouptut. The `ORIGIN` column should display `env (SCW_ACCESS_KEY)` and `env (SCW_SECRET_KEY)`, and not `default profile`.
...
1. Create a Strapi blog template
    ...
    ```
    This blog templates stores the database only locally yet.
...
5. Add the following code to it:
    ...
    # Before starting the server, creates an admin user or updates its credential if a user already exists with this email
...
2. Add the `REGISTRY_ENDPOINT`, `DEFAULT_PROJECT_ID`, `ADMIN_EMAIL` and `ADMIN_PASSWORD` environment variables to Terraform:
    ...
    ```
    You should edit `ADMIN_EMAIL` and `ADMIN_PASSWORD` values with your own email and password. Optionally, you can also edit `ADMIN_FIRSTNAME` and `ADMIN_LASTNAME` values to change default admin firstname and lastname.
    <Message type="note">
        ...
        If the admin password or email do not meet requirements, the container will no start.
    </Message>
...
6. Copy the container URL from the output and paste it into your browser. The main Strapi page displays. Click on "Open the administration" or add `/admin` to your browser URL to access Strapi Administration Panel.

    <Message type="tip">
    The first deployment can take a few minutes. You can check the deployment status with the following command:
    ```sh
    scw container container list name=tutorial-strapi-blog-tf
    ```
    When the status appears as `ready`, you can access the Strapi Administration Panel via your browser.
    </Message>

7. (Optional) You can check that Strapi APIs are working with the following command or going to `https://{container_url}:1337/api/articles` in your browser:
    ```bash
    curl -X GET https://{container_url}:1337/api/articles | jq
    ```
...
8. Once you are done, run the following command to stop all your resources:
    ```hcl
    terraform destroy
    ```
...
- Create a secondary production environment by duplicating your built container, building it in `NODE_ENV=production` environment, running `npm run start`, and plugging it onto another **Serverless SQL Database**. This will let you for instance edit content-types which is not possible in production.
