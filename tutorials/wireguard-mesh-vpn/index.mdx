...
In this second part we will spin up a private mesh network to create a secure private connection between your different servers and Virtual Instances. A mesh network allows your devices to communicate between each other using a virtual Private Network without the need for a central server:
...
- `192.168.1.1` is a randomly chosen private IP address for the VPN interface of the virtual Virtual Instances located in `PAR-1`.
- `192.168.1.2` is a randomly chosen private IP address for the VPN interface of the virtual Virtual Instances located in `AMS-1`.
...
Create the file `/etc/wireguard/wg0.conf` with the following content:
  ...
  ```
  [Interface]
  PrivateKey = PAR-1 Virtual Instances private key
  Address = 192.168.1.1
  ListenPort = 52345

  [Peer]
  PublicKey = AMS-1 Virtual Instances strings
...
Create the file `/etc/wireguard/wg0.conf` with the following content:
  ...
  ```
  [Interface]
  PrivateKey = AMS-1 Virtual Instances private key
  Address = 192.168.1.2
  ListenPort = 52345
