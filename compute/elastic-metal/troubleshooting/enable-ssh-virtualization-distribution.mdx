...
Elastic Metal servers support the installation of virtualization solutions Proxmox and ESXi. These solutions can be managed from either a web interface or a management tool and therefore the direct SSH access to machines running one of these solutions is disabled by default. However, you can enable SSH for avanced management tasks if required.
