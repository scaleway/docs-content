...
excerpt: |
  In this tutorial you will learn how to deploy a Software as-a-Service (SaaS) application, based on a SaaS application developed using Django Template. 
totalTime: PT15M
steps:
  - step: Setting up Scaleway Elements Object Storage for your static files
    type: HowToStep
    url: https://www.scaleway.com/en/docs/deploying-saas-with-elements/#-Setting-up-Scaleway-Elements-Object-Storage-for-your-static-files
  - step: Setting up Scaleway Database
    type: HowToStep
    url: https://www.scaleway.com/en/docs/deploying-saas-with-elements/#-Setting-up-Scaleway-Database
  - step: Setting up Container - block-storage credentials 
    type: HowToStep
    url: https://www.scaleway.com/en/docs/deploying-saas-with-elements/#-Setting-up-Container-- block-storage-credentials 
  - step: Building and pushing your SaaS Docker image
    type: HowToStep
    url: https://www.scaleway.com/en/docs/deploying-saas-with-elements/#-Building-and-pushing-your-SaaS-Docker-image
  - step: Setting up environment variables
    type: HowToStep
    url: https://www.scaleway.com/en/docs/deploying-saas-with-elements/#-Setting-up-environment-variables
  - step: Deploying your application
    type: HowToStep
    url: https://www.scaleway.com/en/docs/deploying-saas-with-elements/#-Deploying-your-application
categories: 
  - compute
  - container-- block-storage
image: /assets/images/docs/scaleway.png
permalink: docs/deploying-saas-with-elements/
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, server, object storage, kubernetes, kubernetes kapsule, database, kubernetes secret, docker, saas, software as-a-Service, container - block-storage
dates:
...
In this tutorial you will learn how to deploy a Software as-a-Service (SaaS) application, based on a SaaS application developed using Django Template. You will use the following Scaleway products:
...
* [**Container - block-storage**](https://www.scaleway.com/en/container-- block-storage/), to store your Docker images
* [**Kubernetes Kapsule**](https://www.scaleway.com/en/kubernetes-kapsule/), for your production environment

Along the way, you'll learn how to store environment variables with Kubernetes secrets and use ImagePullSecrets to pull your private images. We also give you some tips for going further with your deployed SaaS application, including how to expose it with a Scaleway Load Balancer.

	
<Message type="requirement">

  - You have already created a SaaS application using [Django Template](https://docs.djangoproject.com/en/3.1/topics/templates/) (Note: you may still follow this tutorial if you have used another technology for your SaaS application, but you will need to adapt the Django settings used throughout this tutorial)
  - You have created an account and are logged into [console.scaleway.com](https://console.scaleway.com)
  ...
  - You have [created a Scaleway Elements Managed Database](/storage/database/how-to/create-a-database/)
  - You have [created a private Container - block-storage namespace](/compute/container-- block-storage/how-to/create-namespace)
...
1. Take a look at your Django application's `settings.py` file. Natively, Django doesn't manage the S3 protocol for storing static files, and it will provide you with a basic configuration at the end of this file:
...
2. Append the following code to your `settings.py` file. It will add new variables to set up the use of Object Storage as the static files' storage engine. It is recommended to keep the previous settings by default for development purposes.
...
## Setting up Container - block-storage credentials 

As noted in the requirements for this tutorial, you should already have [created a private Scaleway Container - block-storage namespace](/compute/container-- block-storage/how-to/create-namespace/). For this tutorial, we use a namespace called `my_- block-storage_namespace`.

1. Log into your Container - block-storage with docker. You can do this in your terminal by using the following command, as you already set up your `SECRET_ACCESS_KEY` environment variable in an earlier step. Just make sure to use your correct username, and namespace region (in this case `nl-ams`):

  ```
  echo SECRET_ACCESS_KEY | docker login rg.nl-ams.scw.cloud -u <my_user_name> --password-stdin
  ```

2. Enable Kubernetes to pull from your private - block-storage namespace by using the `ImagePullSecret` configuration in your manifest. Type the following into your terminal command line, being sure to replace the information in the angled brackets for your own case as necessary:

  ```
  kubectl create secret docker-- block-storage my-private-- block-storage-access \
...
<Message type="note">
  ...
  You can see that this secret is not of type `generic` with `from-literal` tags, but of type `docker-- block-storage`, with docker-specific tags. To learn more about secrets and their differences, you can go to the [official Kubernetes documentation page](https://kubernetes.io/docs/concepts/configuration/secret/).
...
3. Tag your image, with the following command:
  ...
  ```
  docker tag my_web_app:latest rg.nl-ams.scw.cloud/my_- block-storage_namespace/my_web_app:latest
  ```

4. Push the tagged image to your private Container - block-storage namespace:

  ```
  docker push rg.nl-ams.scw.cloud/my_- block-storage_namespace/my_web_app:latest
  ```
...
1. Type the following into your terminal, being sure to replace the information in angled brackets with the correct values for your application:
  ...
  spec:
      ...
      template:
          ...
          spec:
              containers:
              - name: my_web_app
                  image: rg.nl-ams.scw.cloud/my_- block-storage_namespace/my_web_app:v1
                  ports:
                  - containerPort: 8000
                  envFrom:
                      - secretRef:
              ...
              imagePullSecrets:
              - name: my-private-- block-storage-access
  ```

  In this file, we can see that:

    - We link to our docker image in our private Container - block-storage namespace (`image: rg.nl-ams.scw.cloud/my_- block-storage_namespace/my_web_app:v1`) by using the credentials set as secret (`imagePullSecrets`)
