...
[Scaleway Object Storage](/object/concepts/#object-storage) is a highly resilient and versatile service that guarantees the reliability and accessibility of your data, while being fully [Amazon S3-compatible](/object/concepts/#amazon-s3) and user-friendly.
...
<Message type="tip">
  [Prefixes](/object/concepts/#prefix) and the `MaxKeys` parameter allow you to target smaller object batches.
</Message>
...
The physical distance to the hardware hosting your Object Storage can also have an impact on performance, especially on latency. Make sure to benchmark the different [regions](/object/concepts/#region-and-availability-zone) where Object Storage is available to compare latency on your mission-critical operations.
...
<Message type="tip">
  Consider using multipart uploads for objects larger than 50 MB. Refer to [our documentation on managing multipart uploads](/object/api-cli/multipart-uploads/) for more information.
</Message>
...
- you can [automate the removal of incomplete multipart uploads](/object/api-cli/multipart-uploads/#aborting-a-multipart-upload) to clear the parts that are no longer needed if an upload fails.
...
Try benchmarking several clients to upload objects, such as [MinIO CLI](/object/api-cli/installing-minio-client/), [awscli](/object/api-cli/object-storage-aws-cli/), [Rclone](/object/api-cli/installing-rclone/), or [s5cmd](https://github.com/peak/s5cmd).

Leverage the different [storage classes](/object/concepts/#storage-class) offered by Scaleway, and benchmark them according to your needs to find the right configuration for your deployment.
...
[Enable Scaleway Edge Services](/object/how-to/get-started-edge-services/) to achieve better performance on `GET` 
