...
4. Complete the `scrape_hn.py` file by adding the function code [as shown in the repository](https://github.com/scaleway/serverless-examples/blob/main/mnq/serverless-scraping/scraper/handlers/scrape_hn.py). The main code blocks are briefly explained as follows:
    ...
    ```
As explained in the [Scaleway Functions documentation](/functions/how-to/package-function-dependencies-in-zip/#how-to-configure-your-package), dependencies need to be installed in a `package` folder, and uploaded as an archive. This is done via Terraform, but if you decide to deploy using another system, you can create the archive as follows:
    ```bash
    cd scraper
...
- **Scaleway Queues resources**: [console documentation](/messaging/quickstart/) | [API documentation](https://www.scaleway.com/en/developers/api/messaging-and-queuing/sqs-api/)
- **Managed Database for PostgreSQL**: [console documentation](/postgresql-and-mysql/quickstart/) | [API documentation](https://www.scaleway.com/en/developers/api/managed-database-postgre-mysql/)
- **Serverless Functions**: [console documentation](/functions/quickstart/) | [API documentation](https://www.scaleway.com/en/developers/api/serverless-functions/)
  - **Serverless Functions triggers**: [console documentation](/functions/how-to/add-trigger-to-a-function/) | [API documentation](https://www.scaleway.com/en/developers/api/serverless-functions/#path-triggers-list-all-triggers)
</Message>
...
To make sure the data is correctly stored in the database, you can [connect to it directly](/postgresql-and-mysql/how-to/connect-database-instance/) via a CLI tool such as `psql`.
...
Here are some possible extensions to this basic example:
 ...
 - Use a [Serverless Container](/containers/quickstart/) instead of the consumer function, and use a command line tool such as `htmldoc` or `pandoc` to convert the scraped articles to PDF and upload the result to a [Scaleway Object Storage bucket](/object/quickstart/).
 - Replace the Managed Database for PostgreSQL with a [Scaleway Serverless Database](/sql-databases/quickstart/), so that all the infrastructure lives in the serverless ecosystem! *Note that at the moment there is no Terraform support for Serverless Database, hence the choice here to use Managed Database for PostgreSQL*.