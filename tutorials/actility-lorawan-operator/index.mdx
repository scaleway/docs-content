...
1. [Create a Database Instance](/postgresql-and-mysql/how-to/create-a-database/) for PostgreSQL, add a `lora` table with fields such as `time` (timestamp type) and `data` (jsonb type).
