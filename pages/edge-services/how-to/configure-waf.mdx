...
description: Learn how to configure a Web Application Firewall (WAF) for Edge Services. Protect your Load Balancer backend from threats and malicious requests, and fine tune your settings to pick the right paranoia level and exclusions for your use case.
dates:
...
- An [Edge Services pipeline](/edge-services/how-to/create-pipeline/) for a Load Balancer backend
...
2. In the **Web Application Firewall (WAF)** panel, click **Enable WAF**. A configuration pop-up displays.
...
4. Select a WAF **mode**. Requests judged to be malicious can either be **blocked** and prevented from passing to the backend, or **logged** but allowed to pass.
...
3. Make edits to the filters as required. You cannot add more than one filter of each type (maximum of one path regex and one HTTP method filter per exclusion).
...
2. In the WAF panel, click **Edit exclusions** to enter Edit mode.

3. Click the <Icon name="delete" /> icon next to the exclusion you want to delete. 
...
2. In the WAF panel, click **Disable WAF**. A confirmation pop-up displays.
