...
- Generated your [API key](/iam/credentials/create-api-keys/)
- Installed `curl` on your local computer
...
3. Replace the following information in the command above:
    * `$SECRET_KEY`: Your Scaleway API [secret key](/iam/concepts/glossary/#api-key)
...
1. Copy and paste the following script into a text file, and save it as `db_mig_script.sh`.
    ```bash
        ...
        # Check if instance_id exists and is reachable with current cli configuration
        check_instance () {
            if [ "$1" = "" ]; then
        ...
        old_major_version=$(instance_major_version "$old_instance")
        new_major_version=$(instance_major_version "$new_instance")
