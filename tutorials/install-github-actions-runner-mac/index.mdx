...
1. Go to [Github](https://github.com/), navigate to the main page of your repository and click **Settings**.
2. In the left sidebar, click **Actions**, then click **Runners**.
3. Click **New self-hosted runner**. For the image, select **macOS** image and for the architecture, select **ARM64**.
...
5. [Connect](/bare-metal/apple-silicon/how-to/connect-to-mac-mini-m1/) to your Mac mini M1 using VNC. The runner service needs a graphical session to be started, hence using SSH won't work.
6. Open a terminal and execute the commands that you noted in step 4. Don't launch the runner yet.
...
Once you have installed the runner, you will need to configure it to use the dependencies that you need for your workflows.

1. Install the dependencies that you need for your workflows. For example, if you need to use [Homebrew](https://brew.sh/index_fr), a popular package manager for macOS, you can install it with the following command:
    ```sh
    /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
    ```
    Unlike in online runners where the dependencies have to be installed every that a workflow is run, with self-hosted runners you can prepare all the dependencies that you need.
