---
title: How to connect to the default Websocket network
description: This page explains how to connect to the default Websocket network
excerpt: |
  This page explains how to connect to the default Websocket network.
totalTime: PT15M
steps:
  - step: TODO
    type: HowToStep
    url: https://www.scaleway.com/en/docs/iot/iot-hub/how-to/connect-default-websocket-network/
image: /assets/images/docs/scaleway.png
keywords: Bare Metal, create, Howto
dates:
  validation: 2021-06-02
  posted: 2019-09-01
---

A default Websocket network is automatically created with each Hub. Similarly to the default network, it cannot be deleted.

<Message type="requirement">

- You have an account and are logged into the [Scaleway Console](https://console.scaleway.com)
- You have [created an IoT Hub](/iot/iot-hub/how-to/create-hub)

</Message>


1. [Add a Hub device](/iot/iot-hub/how-to/add-device) if you haven't already done so.  
To keep this tutorial simple, choose to `Allow Insercure` connections. Insecure connections allow connections to the hub without encryption layer.

2. Configure the physical device (or client) to connect through the network, using the following parameters:

    - **Host**: The network endpoint.
    - **Port**: Use `80` for plaintext or `443` for TLS.
    - **MQTT Username**: The *Device ID* from the device you created earlier.
    - If your client asks for a password, just pass in an empty string.
    
    <Message type="important">

    Mutual TLS authentication is not yet available on this network, which means that devices that connect over this network must `Allow Insecure` connections.

    </Message>

3. Connect and exchange messages.

<Navigation title="See Also">
  <PreviousButton to="/iot/iot-hub/how-to/connect-default-mqtt-network/">How to connect to the default MQTT network</PreviousButton>
  <NextButton to="/iot/iot-hub/how-to/setup-use-rest-network/">How to set up and use a REST network</NextButton>
</Navigation>