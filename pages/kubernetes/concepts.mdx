...
cloud-init is an industry-standard tool used to apply custom, predefined configurations to an Instance upon startup. In a Kubernetes context, you can define initial setup tasks (such as creating users or upgrading system packages) that automatically execute when an Instance is first provisioned as a worker node. On Scaleway Instances, cloud-init retrieves this custom configuration directly from user-data. See [How to use cloud-init with Scaleway Instances](/instances/how-to/use-cloud-init/) for more informatio about cloud-init and Scaleway Instances.
