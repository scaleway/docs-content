...
Run the following command to update the apt package manager, upgrade the software installed already installed on the server and to download and install Duplicity and the other required software:
...
As everything is installed and ready, we can now configure and script our interactions between our server and the cloud.
...
2. Add the following lines to `.scw-configrc`. Make usre you replace the necessary values with the details of your Scaleway API key, Object Storage bucket and GPG key. You also need to enter a path to the desired backup folder:
