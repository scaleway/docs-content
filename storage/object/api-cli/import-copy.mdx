...
tags: object storage command bucket s3 copy replicate import
dates:
...
- Recurrently copy new and updated files from the source directory to the target one
...
<Message type="note">
  When applied to a versioned bucket, the sync command copies only the most recent version of the objects.
</Message>
...
To import copied objects to a bucket in the same [Project](/console/project/quickstart/), use the `aws s3 sync` command.

<Message type="important">
  Objects in the bucket must have the `Standard` or `One Zone - IA` class. If an object has the `Glacier` class, it must be restored beforehand. Refer to [this documentation page](/storage/object/how-to/edit-storage-class) to change the storage class of an object.
</Message>
...
Replace `<SOURCE_BUCKET>` with the name of the bucket where the copy is currently stored, and `<TARGET_BUCKET>` with the name of the bucket the copy will be stored in.

## Importing copied objects from a different project

To import a copy of objects in a bucket to another bucket in a different [Project](/console/project/quickstart/), you need to implement a [bucket policy](/storage/object/api-cli/bucket-policy).
