...
While there are many popular configuration management systems available for Linux systems, such as [Chef](/tutorials/configure-chef-ubuntu-xenial/), Ansible is the simplest to get started with.

Ansible works by configuring client machines from a computer that has the Ansible components installed and configured. It communicates over standard SSH channels to retrieve information from remote machines. This means that any computer you can administer through SSH can also be administered through Ansible.
...
1. Connect to your server using SSH:
    ...
    If you do not know your server IP, you can list your existing servers using `scw ps` (Scaleway CLI). For more information on the Scaleway CLI, refer to the tutorial on the [Scaleway Command Line Interface](https://github.com/scaleway/scaleway-cli/#installation/).

    The server IP can also be retrieved from the Scaleway console. Once logged in, check the **IP Addresses** in the **Servers** tab of the left menu.
...
## Installing Ansible from the PPA repository
...
1. Ansible tracks all the servers through an [inventory file](https://docs.ansible.com/ansible/latest/user_guide/intro_inventory.html). We need to set up this file first before we can communicate with our other computers.
...
The `all` means all hosts listed in the hosts file. However, it is also possible to specify:

- A group: `ansible -m ping servers`
- An individual host: `ansible -m ping host1`
- Multiple hosts by separating them with colons: `ansible -m ping host1:host2`

For more information on Ansible commands or playbooks, refer to the official [Ansible documentation](https://docs.ansible.com/ansible/2.4/ansible-playbook.html).
