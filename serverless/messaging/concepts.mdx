...
A subscription is a connection between a client or endpoint, and a topic. By creating a subscription, the subscribed endpoint receives messages and notifications published to the topic. You can [create subscriptions](/messaging/how-to/create-manage-subscriptions/) for HTTP/S endpoints, Scaleway queues, and Scaleway Serverless [Functions](/functions/quickstart/) and [Containers](/containers/quickstart/).
...
| Serverless Functions and Containers | A Scaleway Serverless [Function](/functions/quickstart/) or [Container](/containers/quickstart/) <br />   | - It must have a [public](/functions/concepts/#privacy-policy) privacy policy  <br /> - It must be in a [namespace](/functions/concepts/#namespace) from the same [Project](/organizations-and-projects/concepts/#project) and [region](#region) as the topic |
