...
Cockpit allows you to [push your own metrics](https://www.scaleway.com/docs/observability/cockpit/how-to/send-metrics-logs-to-cockpit/) for various resources. The pricing of our service is based on the number of samples that are ingested per month.
...
If you are [using your own Grafana](https://www.scaleway.com/docs/tutorials/using-own-grafana/) to visualise metrics, use the following query `sum(rate(observability_cockpit_ingestion_samples_total{is_from_scaleway="false"}[5m])) OR on() vector(0)` to know what your current ingestion rate is.
