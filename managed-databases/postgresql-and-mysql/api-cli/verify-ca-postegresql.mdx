...
Will verify that a server is trustworthy by checking if the server certificate is valid and if the hostname specified in the certificate itself, matches the hostname of the server.
