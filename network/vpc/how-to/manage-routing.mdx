...
3. Click the **Routing** tab.
    ...
    - Attach a Public Gateway to a Private Network and set it to advertize a default route. This generates a **default route to the internet**.
    ...
    As all routes are currently managed by Scaleway, you do not need to take any action with your route table. If and when you delete Private Networks from the VPC, or delete a Public Gateway advertizing the default route, these routes will be automatically deleted from your route table.
...
- Managed Databases are not currently compatible with routing. The VPC cannot automatically route between Managed Databases on different Private Networks, or (for example) between a Managed Databases on one Private Network and an Instance on a different Private Network.
