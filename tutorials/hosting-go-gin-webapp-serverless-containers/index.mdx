...
1. Create a `Dockerfile` at the root of your project:
    ...
    <Message type="note">
    **Why multi-stage builds?** Multi-stage builds create smaller, more secure images by separating the build environment from the runtime image. The final image only contains your compiled binary and templates, no Go compiler, build tools, or unnecessary dependencies. Using `scratch` (an empty base image) resulting in faster deployments (lower cold starts), lower storage costs, and a reduced attack surface.
    </Message>
