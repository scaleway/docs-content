...
Node labels are key‑value pairs attached to a node. By referencing those labels in a Pod spec you can:

- **Require** the Pod to run only on nodes that match the label(s), or  
- **Prefer** the Pod to run on nodes that match the label(s).

To express those constraints, you specify one or both of the following node selection mechanisms for a Pod:
...
These constraints are evaluated against every node in a node pool during scheduling. 
...
A startup taint is a temporary taint that is applied while a node is initializing. The taint remains in place until a controller or you remove it. For example, Cilium adds a default startup taint, and when Cilium completes initialization, Cilium's controller automatically removes that taint, allowing workloads to be scheduled.
