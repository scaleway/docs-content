...
The service resolves both regular public records and internal VPC records. The `.internal` at the end of internal addresses is important in enabling the resolver to distinguish between internal and external/public DNS. For this reason, you should always add `.internal` when addressing resources within your VPC.
...
```
DNS cannot resolve or forward queries between different VPCs. It can only resolve hostnames for resources within the same VPC, though they may be on different Private Networks within that VPC. See [DNS resolution across VPC Peering connections](#dns-resolution-across-vpc-peering-connections) for the formats used to resolve resources in a peered VPC.
...
- Instances: `resource_uuid` corresponds to the [Private NIC](https://www.scaleway.com/en/developers/api/instances/#path-private-nics-list-all-private-nics) ID
...
- Public Gateways: `resource_uuid` corresponds to the [GatewayNetwork](https://www.scaleway.com/en/developers/api/public-gateways/#path-gateway-networks-list-public-gateway-connections-to-private-networks) ID.
</Message>

## DNS resolution across VPC Peering connections

To resolve a resource located in a different, peered VPC, you must identify the target VPC in the DNS query, using either its name or its ID.

Specifying the target VPC is mandatory for cross-VPC resolution. Private Network names are only guaranteed to be unique within a single VPC, so disambiguation is required to avoid conflicts between Private Networks of the same name in different, peered VPCs.

When [VPC Peering](/vpc-peering/concepts/) is used to connect VPCs, the DNS service also resolves the following formats, which include the target VPC's name or ID:

```
<resource_name>.<private-network_name>.<vpc_uuid>.internal
<resource_name>.<private-network_uuid>.<vpc_uuid>.internal
<resource_uuid>.<private-network_uuid>.<vpc_uuid>.internal
<resource_uuid>.<private-network_name>.<vpc_uuid>.internal
<resource_name>.<private-network_name>.<vpc_name>.internal
<resource_name>.<private-network_uuid>.<vpc_name>.internal
<resource_uuid>.<private-network_uuid>.<vpc_name>.internal
<resource_uuid>.<private-network_name>.<vpc_name>.internal
```

<Message type="tip">
Cross-VPC DNS resolution follows the same transitivity rules as VPC Peering routing: it works across a direct peering connection, and through an intermediary VPC that has transitive peering enabled, but not through an intermediary VPC where this setting is not enabled. See [DNS resolution](/vpc-peering/concepts/#dns-resolution) for more information.
</Message>
