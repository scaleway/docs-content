...
description: This page explains to setup a mailserver on Ubuntu Bionic Beaver.
excerpt: |
...
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, ubuntu server, debian server, linux server, baremetal, scalability, ARM, physical server, dedicated server, cloud-init, fedora server, centos server
categories:
  - compute
  - instances
dates:
...
Before you continue with this tutorial, some pre-work is required to make sure your mail server will be working.
...
2. Before starting the installation of the mail server, make sure that there is no other mail software already installed:
    ...
    If you receive a message `Failed to stop sendmail.service: Unit sendmail.service not loaded.` you can ignore it. It will only tell you that sendmail has not been installed, so it can't be removed.

    </Message>

## Installing PostfixAdmin

All mailboxes will belong to virtual users, to manage them we need one system user which will be the owner of all mailboxes and will be used by all virtual users to access email on the server. The home directory of the user will be `/var/mail/vmail` and all mailboxes will be stored in that directory:
...
1. Set a root password for MariaDB:
    ...
    The setup tool will ask you a few questions:
...
9. Enter the email address of the admin, and answer the questions of the CLI.
    ...
    Certbot will automatically request the certificate and rewrite the Nginx configuration file. The file `/etc/nginx/sites-enabled/mail.example.com.conf` should look as the following example:
...
9. Once the MySQL configuration files are created, update the configuration of Postfix:
    ...
    Local delivery will be handled by Dovecot’s local delivery agent. It will take mail from a MTA (Postfix) and deliver it to a local user’s mailbox.
    ```code
    sudo postconf -e "virtual_transport = lmtp:unix:private/dovecot-lmtp"
    ```

10. Configure the TLS parameters by using the Let’s encrypt SSL certificate:
...
23. Edit the file `/etc/dovecot/dovecot-dict-sql.conf.ext` and add the following content. Make sure your MySQL credentials are correct:
...
To protect yourself from spam and to to increase the trust level of your mails, we use Rspamd and create DKIM and DMARC DNS records.

### Installing Redis

Redis is used by Rspamd as as a storage and caching system. It can easily be installed with apt:
...
7. Add an proxy in the Nginx configuration file (for example: `/etc/nginx/sites-enabled/mail.example.com.conf`):
...
2. Open the file `/etc/dovecot/conf.d/20-lmtp.conf` and edit is as following:
...
3. Open the file `/etc/dovecot/conf.d/20-imap.conf` and edit is as following:
...
8. Create a script, named `/var/vmail/mail/sieve/global/spam-global.sieve`, that will be triggered each time when you manually move an email into the spam folder:
    ```code
...
DomainKeys Identified Mail ([DKIM](https://en.wikipedia.org/wiki/DomainKeys_Identified_Mail)) is an email authentication method designed to detect email spoofing. It allows the to verify the origin of an email by affixing a digital signature to it. Verification of the signature is carried out using the signer's public key published in the DNS. It can be used to detect fraudulent mails.
...
To read your mails comfortable and directly within your web browser, we will install a Roundcube web mail interface.
...
2. Login to your MariaDB server and create a MySQL database for Roundcube:
    ```code
    mysql -u root -p
    ```
