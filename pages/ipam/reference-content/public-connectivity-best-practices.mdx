...
You can also add a *W**eb **A**application **F**irewall to your Load Balancer via our [Edge Services](/edge-services/reference-content/understanding-waf/) product. This adds an extra level of protection to your Load Balancer, by filtering requests and blocking those judged to be malicious.
...
- Installing a third-party **W**eb **A**application **F**irewall that can filter out malicious traffic such as requests containing attack patterns, or requests from blacklisted IPs.
