...
dates:
  validation: 2022-09-21
  posted: 2022-09-24
---

You can use the Scaleway CLI to create, export and import snapshots of your Instances. A snapshot is a QCOW2 image of your Instances volumes and stored in a Scaleway Object Storage bucket. It can be imported into any Availability Zone, as long as it is in the same region as the bucket in which it hosted.
...
The following arguments and flags are avialble to customize your command:
...
To create a snapshot in in a specific zone (e.g. `fr-par-1`) from the given volume ID run the following command. Remember to replace `11111111-1111-1111-1111-111111111111` with the actual ID of your volume. 
```
$ scw instance snapshot create zone=fr-par-1 volume-id=11111111-1111-1111-1111-111111111111
```

## Creating a named snapshot i the default zone from a given volume ID 
...
When importing existing QCOW / QCOW2 files from Object Storage as Instance snapshot, make sure that the file has the suffix `.qcow` or `.qcow2`. 
