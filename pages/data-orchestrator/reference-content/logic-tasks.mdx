...
<Tabs>
  ...
  <TabsTab label="Fork">
    **Fork properties**
    ...
    |   `compete`   |      boolean       |    no    | - If `false` (default value), the task returns an array that includes the outputs from each branch, preserving the order in which the branches are declared. <br/> - If `true`, only the jobs following the first competing job to succeed will be running afterwards. Other branches will be dropped. |
  ...
  <TabsTab label="Switch">
    ...
    **Switch case properties**
    ...
    Define a switch case with a condition for matching and an associated action to execute upon match.
