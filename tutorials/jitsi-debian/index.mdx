...
6. Run the following script to generate a [Let's Encrypt](https://letsencrypt.org) SSL certificate for your instance:
  ...
  <Message type="tip">
    ...
    In case the generation of the Let's Encrypt certificate fails, run the following command: `dpkg-reconfigure jitsi-meet-web-config` to reconfigure the package `jitsi-meet-web-config` before relaunching the certificate request.
