...
### Can I transcribe audio in streaming?
Stream are only supported for audio output, but not input yet.
As a workaround, you can send only small chunks of audio of a few seconds and activate output streaming with:
```bash
curl https://api.scaleway.ai/v1/audio/transcriptions \
  ...
  -F stream=true
```
Audio streaming will start as soon as the first chunk of 30 seconds is processed, hence after only a few seconds. This can close to realtime enough for many user / audio agent interactions.
...
Since billing is done per second of audio input for models such as `whisper-large-v3`, this method does not add additional costs besides the duration of each audio files in seconds.
