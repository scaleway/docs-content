...
1. Pull the container image from the registry using `docker pull` or `kubectl pull`.
...
Tools like Docker buildx support multi-platform builds, allowing you to build images for multiple architectures simultaneously. Ensure that your Dockerfile specifies the target architecture appropriately.

Example Dockerfile snippet for ARM:
```
...
Kubernetes can pull the appropriate image based on the architecture of the node where it's scheduled to run.

Example manifest list usage:
```
...
- Ensure that all dependencies and libraries used in your application are compatible with the ARM architecture. Tools like `file` command can help identify the architecture of binary files.
...
For more information, refer to our documentation [How to deploy both x86 and ARM images in Kubernetes](/containers/kubernetes/how-to/deploy-x86-arm-images/).