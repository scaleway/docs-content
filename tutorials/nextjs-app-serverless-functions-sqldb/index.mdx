...
Your application cannot store any data persistently yet. We will now add serverless storage in the next section.
...
```
Note that this repository differs slightly in configuration, as it will dynamically render pages and not pre-render them in the Next.js app build phase.
</Message>
...
However, your application would then connect with your [user credentials](/iam/concepts/glossary/#user), which is not a recommended security practice.
To secure your deployment, we will now add a dedicated [IAM application](/iam/concepts/glossary/#application), give it the minimum required permissions, and provide its credentials to your application.

## Secure and redeploy your application

1. Run the following command to create an [IAM application](/iam/concepts/glossary/#application) and export it as a variable:
    ```bash
...
2. Create an [IAM policy](/iam/concepts/glossary/#policy) giving your application rights to access the database:
    ```bash
    ...
    ```
    The permission `ServerlessSQLDatabaseFullAccess` lets your application read and write data or create new databases. You can restrict this later to fit your database use cases, using `ServerlessSQLDatabaseRead` or `ServerlessSQLDatabaseReadWrite` permission sets.

3. Create an [IAM API Key](/iam/concepts/glossary/#api-key) and export its secret key as an environment variable:
    ```bash
...
2. Run the command below to make sure the environment variables are properly set:
    ...
    This command displays your access_key and secret_key in the last two lines of the output. The `ORIGIN` column should display `env (SCW_ACCESS_KEY)` and `env (SCW_SECRET_KEY)`, and not `default profile`.
...
1. Build your application container with:
    ...
    ```
    <Message type="tip">
      An error message `Error: connect ECONNREFUSED` might display when building. This is linked to Next.js pre-rendering stage without any database to connect to yet but will raise any issue at runtime because all pages will be dynamically rendered.
    </Message>
...
- An [IAM policy](/iam/concepts/glossary/#policy) which grants your application the right permissions

- An [IAM application](/iam/concepts/glossary/#application), used as a principal for your IAM policy

- An [IAM API key](/iam/concepts/glossary/#api-key) used as credentials to authenticate your application to the database
...
If you encounter any issues, first check that you meet all the requirements.
...
- You have **Docker Engine** installed. Running the `docker -v` command in a terminal should display your currently installed Docker version:
    ...
    ```
- You have the right [IAM permissions](/iam/policies-permissions/policy/), specifically **ContainersRegistryFullAccess**, **ContainersFullAccess** and **ServerlessSQLDatabaseFullAccess**.