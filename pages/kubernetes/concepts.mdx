...
Auto-upgrade allows you to schedule a maintenance window for your cluster to be upgraded to a more recent version of Kubernetes. The upgrade process will occur within 2 hours from the time defined by you.

## Cloud-init

Cloud-init is an industry-standard tool used to apply custom, predefined configurations to an instance upon startup. In a Kubernetes context, you can define initial setup tasks (such as creating users, upgrading system packages, configuring trusted CA certificates, etc.) that automatically execute when a node is first provisioned. On Scaleway Kapsule pools, cloud-init retrieves this custom configuration directly from the user-data of the pool. 

## Cluster

A cluster is a set of machines, called nodes, running containerized applications managed by [Kubernetes](#kubernetes). A cluster has several worker nodes and at least one control plane. See the [control plane](#control-plane) concept for more information regarding the cluster's limitations. Scaleway allows you to create 2 configurations of clusters: [Kapsule](#kubernetes-kapsule), for clusters comprising Scaleway Instances, and [Kosmos](#kubernetes-kosmos), for multi-cloud clusters. Clusters can be tailored to availability requirements depending on the cluster availability types: zonal (single-zone or multi-AZ) and regional.
...
A **dedicated Control Plane** provides an isolated control plane exclusively for your cluster.
All control plane components run on infrastructure dedicated to your Organization, offering enhanced performance and isolation. This ensures no sharing of control plane infrastructure with other customers, resulting in predictable performance without potential noise from neighboring clusters. It is designed for critical production workloads requiring maximum reliability and is better suited to regulated industries with strict isolation requirements. This option has a higher cost, due to the dedicated infrastructure and resources allocated to your Organization.
...
A **mutualized Control Plane** is a Kubernetes control plane shared across multiple clusters. In this configuration, the core control components, such as the API server, scheduler, controller manager, and etcd, are hosted on a shared infrastructure managed by Scaleway.
This approach is cost-effective as infrastructure resources are shared between multiple customers. Scaleway handles full operational management, including updates, patches, and monitoring. It is suitable for development, testing, staging environments, or production workloads with standard availability requirements. Despite the shared infrastructure, logical isolation between clusters ensures security and privacy within the multi-tenant architecture.
