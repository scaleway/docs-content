...
categories: 
  - compute
tags: compute Terraform Scaleway-infrastructures cloud Bare-Metal Instance HashiCorp-Terraform
...
6. If the output obtained is the same as the one above, it is now time to apply the new configuration and to create the Instance using Terraform by running `terraform apply`. Confirm the execution of the plan by typing `yes` when prompted: 
...
Terraform allows us to add additional resources to infrastructures and is capable of managing both [Instances](https://www.scaleway.com/en/virtual-instances/) and [Bare Metal servers](https://www.scaleway.com/en/bare-metal-servers/). Let us add a Bare Metal Server to our Terraform project using the [Bare Metal module](https://www.terraform.io/docs/providers/scaleway/r/baremetal_server.html) of the Scaleway provider.
