...
When defining custom user-data on a node pool:

- Users are responsible for troubleshooting issues caused by cloud-init scripts (user-data named "cloud-init" may break nodes)
...
- When defining custom pool user-data, users are responsible for troubleshooting issues caused by cloud-init scripts (user-data named "cloud-init" may override standard configurations and break nodes)
