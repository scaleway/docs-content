...
The different elements of a bucket policy allow you to [configure fine-grained permissions](/object-storage/api-cli/combining-iam-and-object-storage/) when combined with [Identity and Access Management (IAM)](/iam/concepts/glossary/#iam), as shown in the table below.
...
- With the `2023-04-17` version, only actions explicitly **allowed** by the bucket policy are permitted, if the principal is also **allowed** by an [IAM policy](/iam/policies-permissions/create-policy/). **Deny** statements are therefore useless.
...
**Description**
: Defines the targets of the bucket policy, which can be a [user](/iam/concepts/glossary/#user) or an [application](/iam/concepts/glossary/#application). You must use `user_id` and/or `application_id`, or `*` to grant access to "everyone".
