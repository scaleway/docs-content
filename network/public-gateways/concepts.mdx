...
tags: network availability-zone dhcp dns flexible-ip nat private-ip ssh-bastion egress 
categories:
  - network
---
...
An Availability Zone refers to the geographical location in which your [Public Gateway](/network/public-gateways/concepts/#public-gateway) is created. To connect resources to Public Gateways, they must be within the same Availability Zone. Find below the list of Availability Zones by region:
...
The **D**omain **N**ame **S**ystem (DNS) is a naming system for devices connected to the Internet or Private Networks. Most prominently, DNS servers translate text-based domain names (eg www.scaleway.com) to numerical [IP addresses](#ip-address) eg (51.158.66.220). A Public Gateway acts as a local DNS server for the devices within a Private Network, resolving the devices' IP addresses into their hostname.
