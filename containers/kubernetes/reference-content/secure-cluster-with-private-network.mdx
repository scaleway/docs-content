...
Scaleway Kubernetes Kapsule provides a managed environment to create, configure and run a cluster of pre-configured machines for containerized applications. This allows you to create Kubernetes clusters without the complexity of managing the infrastructure.

All new Kubernetes clusters are deployed with a Scaleway [regional Private Network](/network/vpc/concepts/#regional-private-networks). **We recommend migrating existing clusters to a regional Private Network as well, using the Scaleway console.**
...
## Can I have pools with nodes in different AZ?

Yes. It is possible to create nodes in different AZ

Ci dessous un script terraform qui provisionne 2 pools de noeuds
...
Option 2: cluster existant # Grace au field "private_network_id", attacher un PN dans son cluster
...
Ceci va initier le migration du reseau du cluster vers un réseau prive, au meme titre que ce que la console peut faire une migration en 2 clics:

2. Puis via CLI ou TF, ajouter des pools de noeuds dans de nouvelles zones...
CLI

scw k8s pool create cluster-id=XX node-type=YY zone=ZZ name=foo size=1 autohealing=true
TF
