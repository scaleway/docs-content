...
**Condition keys**
: `aws:SourceIp`, `aws:SourceVpc`, `aws:Referer`, `aws:CurrentTime`, `aws:EpochTime`, `aws:SecureTransport`, `s3:prefix`
...
* You can restrict a statement to the requests coming from a given VPC with the `aws:SourceVpc` condition key. The condition matches the requests that reach the bucket over the VPC endpoint, which requires [Object Storage private access](/vpc/how-to/enable-object-storage-private-access/) to be enabled on the VPC:
...
```json
{
  ...
  "Statement": [
    {
      "Sid": "Allow access from my VPC only",
      "Effect": "Allow",
      "Principal": "*",
      "Action": ["s3:ListBucket", "s3:GetObject"],
      "Resource": ["<BUCKET_NAME>", "<BUCKET_NAME>/*"],
