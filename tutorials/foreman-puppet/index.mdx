...
This tutorial assumes that Foreman is being installed on a fresh instance, which will also act as the Puppet primary server. Foreman requires a working Puppet installation to perform the configuration management of hosts.
...
2. Check your Instances full hostname to ensure the hostname is set to a valid [FQDN](https://en.wikipedia.org/wiki/Fully_qualified_domain_name).
...
During installation, the installation tool will sets up a puppet primary on the host, which is fully integrated with Foreman. Run the Puppet agent on the instance to send the first Puppet report to Foreman, This creates the host automatically in Foremans database.
