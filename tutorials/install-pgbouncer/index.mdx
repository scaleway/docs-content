...
## PgBouncer Overview
PgBouncer is a connection pooler for PostgreSQL. It sits between the application and the PostgreSQL server. PgBouncer opens multiple connections to the database and serves it to the application. This way, the application reduces connection opening costs and gives a performance boost.

There are three types of pooling modes:
- Session: The server connection will be released back to the pool after client disconnects. (Default pooling method.)
...
In this tutorial, you can choose the pooling mode that best suits for your workload.

<Message type="requirements">

    - You have an account and are logged into the [Scaleway Console](https://console.scaleway.com)
    - You have [configured your SSH Key](/console/my-project/how-to/create-ssh-key/)
    - You have an Instance running on Ubuntu or Debian
...
The **PostgreSQL Global Development Group (PGDG)** provides a apt repository. After importing the repository, you can install the PgBouncer package.
...
1. Set up the PostgreSQL server details in `/etc/pgbouncer/pgbouncer.ini`:
   ...
   * = host=POSTGRESQL_IP port=POSTGRESQL_PORT
   ```
    ...
    You may want to update `listen_addr` to listen to TCP connections on all addresses. You can use '`*`' for it. (You cam also set a list of IP addresses):

    </Message>

2. Create the `/etc/pgbouncer/userlist.txt` file that contains the user allowed to login from PgBouncer:
    ```"
    "USERNAME" ""
    ```
...
3. Add the IP address of the PgBouncer server to the PostgreSQL `pg_hba.conf` file:
   ...
   ```
    ...
    You should reload or restart PostgreSQL for apply the pg_hba.conf changes.

    </Message>

4. Reload the `pgbouncer.service` for apply the configurations:
   ```
   systemctl reload pgbouncer.service
   ```

5. Now, you can connect the PostgreSQL through PgBouncer (PgBouncer default port: 6432):
    ```
...
1. To manage PgBouncer, you need to connect a special database called `pgbouncer` with `psql`. For that, you need to install a `postgresql-client-VERSION` package. The latest version **PostgreSQL 14** currently.  
   ```
   apt install postgresql-client-14
   ```

2. You need to configure an admin user to connecting to `pgbouncer` database.
   ```
   admin_users pgbouncer
   ```

3. After configuring the PgBouncer admin user, you need a reload the `pgbouncer.service` to apply the changes.
   ```
   systemctl reload pgbouncer.service
   ```

4. Now you can connect to `pgbouncer` database with `psql`:
   ```
   psql -h 127.0.0.1 -U pgbouncer -d pgbouncer -p 6432
   ```

## Useful admin console commands
Here a few commands that can be used after connecting to the `pgbouncer` database. It is helpful when used for troubleshooting:
...
## Making an online restart

- You can do an online restart without terminating the connections. PgBouncer launches a new process and loads open sockets from running PgBouncer. After that, old process is stopped and the new process resumes. This way, connections will not be interrupted. This is very useful when upgrading PgBouncer.
   ```
...
5. Reload the `pgbouncer.service` for apply the configurations:
   ```
   systemctl reload pgbouncer.service
   ```
