...
Compared to Generative APIs, no usage quota is applied to the number of tokens per second generated, since the output is limited by the GPU Instance size and number of your Managed Inference Deployment.

### How can I monitor performance?
Managed Inference metrics and logs are available in [Scaleway Cockpit](https://account.scaleway.com/?service=elementscockpit/overview). You can follow your deployment metrics in real-time, such as tokens throughput, requests latency, GPU power usage, and GPU VRAM usage.
