...
During the installation or reinstallation of a Windows Server operating system, an administrator password is configured by you (Administrator account).

In case you have lost this password, you can reset it via rescue mode.
...
6. Edit the `Administrator` account by running the following command:
    ...
    Select: [q] >
    ```
7. Type `1` and press enter to clear the password of the user:
    ```
    Select: [q] > 1
    Password cleared!
    ```
8. Type `q` and press enter to quit editiong the user. A summary of the files edited displays:
    ```
    ...
    ```
9. Type `y` and press enter to validate to write the files.
    ```
...
1. [Activate remote access](https://www.scaleway.com/en/docs/bare-metal/elastic-metal/how-to/activate-remote-access/) on your server to be able to connect to it using the KVM over IP device.
2. Open the remote console to visualize the screen output of your machine.
3. Open the start menu and click on the the `Administrator` user name to open the user settings.
