...
Focalboard is an open-source, multilingual, self-hosted project management tool developed by the team around Mattermost. 
It helps your organization maintain a single source of truth so teams stay aligned to complete tasks, reach milestones, and achieve their goals. 
Focalboard provides a full suite of features such as Kanban, table, gallery, and calendar view. Filters help you to stay focussed on the highest priority items. 
The application is a privacy-caring alternative to Trello, Notion, and Asana and can be installed easily on Scaleway Instances.
...
In the following steps you make sure that the software installed on your Instance is up-to-date, you install the required software packages and download and unpack the Focalboard application.
...
3. Install the required software packages using the `apt` package manager.
    ...
    ```
4. Download and unpack Focalboard. In this tutorial, we use v7.8.2 of Focalboard, but you can find the **lates**t release in the project's [GitHub repository](https://github.com/mattermost/focalboard/releases).
    ```
...
We use Nginx aas a proxy in front of the application to prevent it from direct access and by providing an additional layer of security through TLS encryption.
...
Focalboard uses a MariaDB database to store it's data. Initialize the database engine, create a table and user for Focalboard.
...
We configure a service that will will keep the Focalboard server running across reboots.
...
To avoid the Focalboard application being directly reachable via plain HTTP on Port 8000, we configure [a firewall](/tutorials/installation-uncomplicated-firewall/) to block all incoming ports, except HTTP, HTTPS, and SSH.
