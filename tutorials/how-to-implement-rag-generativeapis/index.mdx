...
Create a .env file and add the following variables. These will store your API keys, database connection details, and other configuration values.
   ...
   SCW_REGION=fr-par
   SCW_BUCKET_ENDPOINT="https://{{SCW_BUCKET_NAME}}.s3.{{SCW_REGION}}.scw.cloud" # S3 endpoint, e.g., https://name.s3.fr-par.scw.cloud

   # Scaleway Generative APIs endpoint
   SCW_GENERATIVE_APIs_ENDPOINT="https://api.scaleway.ai/v1"
   ```

## Setting Up Managed Database
...
In the context of using Scaleway’s Generative APIs and the `sentence-t5-xxl` model of this example, TikToken tokenization is not necessary because the model you are using (sentence-transformers) works with raw text and handles its own tokenization internally.
...
By setting `tiktoken_enabled=False`, you ensure that raw text is sent to Scaleway's Generative APIs endpoint, which is what the sentence-transformers model expects to process. This guarantees that the embedding generation process works smoothly with Scaleway's infrastructure.
