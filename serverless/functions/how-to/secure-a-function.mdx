...
content:
  ...
  paragraph: This page explains how to  how to secure a Serverless Function
...
## Use Secrets to store access keys and sensible information

Instead of using environment variables (which are stored in clear) for your functions, use Secrets. These are information usable through environment variables but are stored encrypted. 
...
6. Click **Deploy a Function** to submit your Secrets and redeploy your function. 

### Configure Secrets using the Serverless framework

Add `secret` to your functions description (more information in the [plugin documentation](https://github.com/scaleway/serverless-scaleway-functions#security-and-secret-management))
We recommend to use them with global environment variables or a `.env` file stored independently (and kept secret).
...
Unauthenticated calls will be rejected, and your function will not be triggered. This feature is handy if an event triggers your function (CRON, and soon SQS or NATS trigger) or if you put them behind an API gateway or a proxy server (see examples in serverless-examples).
...
6. If required create an access token for your function:
    ...
    * Enter a description and set an optional expiration date. To have a permanent valid token tick **No expiration date**. Then click **Generate token**.
...
6. Enter a description and set an optional expiration date. To have a permanent valid token tick **No expiration date**. Then click **Generate token**.
    The token is displayed. Copy and store the token in a secure environment.
    <Message type="important">
        The token key will be shown only once. Copy and safely store it before leaving this page.
    </Message>
...
You can generate access credentials to inject in other applications (containers, functions ...) directly from terraform using the [function_token resource](https://registry.terraform.io/providers/scaleway/scaleway/latest/docs/resources/function_token)

## Set up alerts in Observability Cockpit (upcoming feature)

Using Scaleway [Observability Cockpit](https://www.scaleway.com/en/docs/observability/cockpit/quickstart/), a managed Grafana solution to which all your functions are connected, you can:
