...
- **Backend servers health check**: Shows, for each backend server identified by its IP address and the name of the frontend it is attached to, its current status (`up` (i.e. healthy), or `down` (unhealthy))s.
