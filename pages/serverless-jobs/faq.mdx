...
<Message type="info">
This calculation is provided for illustrative purposes only. For detailed and up-to-date information on pricing, refer to the [official Scaleway pricing page](https://www.scaleway.com/en/pricing/serverless/#serverless-jobs).
</Message>
...
<Message type="note">
Your invoice displays your Free Tier usage under the **Serverless Jobs - CPU Pack** section. This section shows the costs saved through the Free Tier, expressed both in euros and in `currency` (an integer value representing the smallest monetary unit, where 1 `currency` = €0.01, i.e., one cent).
</Message>
...
To inject specific commands into a job run, you can add a [startup command and arguments](/serverless-jobs/concepts/#startup-commands-and-arguments) to a job.
...
Serverless resources are [stateless](/serverless-jobs/concepts/#stateless) by default, local storage is ephemeral.
