...
Adding a cron schedule to a specific task allows you to run it automatically at fixed times and dates. CronJobs are specified using a unix-cron format. You can run a task as often as every minute, or as infrequently as once a year.

## Cron schedule format

CronJobs are expressed using the `* * * * *` format, which corresponds to the fields below.
