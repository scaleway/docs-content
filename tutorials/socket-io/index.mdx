...
meta:
  ...
  description: This tutorial shows how to install and how to use Socket.io 
content:
...
1. Create a folder and change directory. In this tutorial we call the folder `myapp`.
...
1. Create a new file in the socket-app folder (`myapp`). 

  ```
  nano app.js
  ```

2. Paste the following content in the file, save and exit.
...
3. Create an index.html file in the socket-app folder (`myapp`) to display a `Hello World` message in the socket.
...
5. Open a browser and enter `http://<IP_address>:4200`. It should display the following:
...
You must set up your server locally for the following steps to work. 
...
2. Update the `app.js`
  ...
  You can see we are listening on messages. After receiving data we inform all other clients, including the socket that sent the message, the data.
