...
- **Toggle activated**: Additional settings to limit backend load are activated and appear for you to configure. These additional settings are **Max simultaneous** and (if you previously [activated sticky sessions](#sticky-sessions)) **Queue timeout**.
  ...
  - **Max simultaneous**: Defines the maximum number of simultaneous requests (for HTTP) or simultaneous connections (for TCP) to any single backend server. A value of 5 means that each backend server will have a limit of 5 connections (even if, for example, there are only three servers in the backend). This setting is particularly relevant when using the [First available](#balancing-method) balancing method. 
      ...
      - Pass the request/connection to a different backend server that still has slots available, unless no backend server has available slots in which case the Load Balancer indicates to the client that the request cannot be handled (e.g. `503 service unavailable`), or
