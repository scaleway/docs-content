...
6. Enter the following information in the pop-up form:
    ...
    - **IP control action**: Choose the action to take if the IP(s) specified match.
...
- An ACL which gives access to traffic based on the filter condition `allow if`, will therefore reject all traffic that doesn't match that filter condition.
