...
Scaleway Kubernetes team allows the usage of every new Kubernetes version as soon as the version is officially released and certified by the [Cloud Native Computing Foundation](https://www.cncf.io/), which usually makes the versions available in Kubernetes Kapsule and Kubernetes Kosmos within a few days after the release.

Each minor Kubernetes version is supported by Scaleway Kubernetes products for at least twelve month before being deprecated.

## Version management recommandation
It is recommended that every Kubernetes cluster is maintained up-to-date with the latest minor version of Kubernetes available and the latest patch.

### Cluster auto-upgrade
The auto-upgrade functionality provided by Kubernetes Kapsule & Kubernetes Kosmos on the control plane and the managed nodes versions of Kubernetes can be activated using the API, CLI, or the Scaleway Console. For this functionality to operate correctly, the `kube-system` namespace of a cluster must not be altered or have any user-induced behaviors that could interfere with the auto-upgrade functionality.

## Support
Before opening a support ticket related to a cluster behavior or a Kubernetes component, we highly recommend that the cluster is upgraded to the latest version of Kubernetes. If the upgrade to a new minor version is not possible, the latest patch of the version running on the cluster must be applied.
...
* **Deprecation:** Version is removed from cluster creation option. Customers are contacted via a support ticket that they are using a deprecated version of Kubernetes and are asked to update their clusters.
...
## Version information
For a complete changelog update, please check the official [Kubernetes changelog](https://kubernetes.io/releases/).

The following documentation will only list the main feature changes for each version and provide a link to the version release announcement on Kubernetes official blog.
