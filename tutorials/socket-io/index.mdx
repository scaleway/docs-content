...
2. Paste the following code in the file, save and exit.
  ...
  `App.get` routes HTTP requests to the specified path with a specific call-back function. The `res` object represents the HTTP response that an Express app sends when it gets a request. The `sendFile` sends the file to the specified path. The `server.listen` opens a port and listesn for requests coming in.
