...
3. Retrieve the URL of the metrics data source you want to add from the [Data sources tab](https://console.scaleway.com/cockpit/dataSource) of the Scaleway console. Make sure that the region in the URL is the one where your data source is located. Make sure that you also add `/prometheus` at the end of the URL. Your metrics' URL should look like the following:
    ```bash
...
8. From the **Advanced settings** section > **Performance**, select **Prometheus** in the **Prometheus type** drop-down.
9. Select the latest version in the **Prometheus version** drop-down.
...
1. From the Grafana homepage, click **Add your first data source**.
2. Select the Prometheus data source in the list that displays.
3. Retrieve the URL of the internal metrics data source you want to add from the [Data sources tab](https://console.scaleway.com/cockpit/dataSource) of the Scaleway console. Make sure that the region in the URL is the one where your data source is located. Make sure that you also add `/prometheus` at the end of the URL. Your metrics' URL should look like the following:
    ```bash
    https://xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx.metrics.cockpit.fr-par.scw.cloud/prometheus
    ```
4. Paste your metrics' URL in the URL field of the **Connection** section:
...
3. In the **Alertmanager** section > **Implementation** field, select **Mimir**.
3. Copy and paste the following URL in the URL field of the **HTTP** section:
    ```bash
    https://alertmanager.cockpit.fr-par.scw.cloud
    ```
4. From the **Custom HTTP headers**, click **+ Add header**.
5. Enter `X-Token` in the **Header** field.
6. Enter your token's secret key in the **Value** field.
7. Click **Save & test** to confirm.

## Creating your data source for your external traces

1. From the Grafana homepage, click **Add your first data source**.
2. Select the Tempo data source in the list that displays.
3. Retrieve the URL of the external traces data source you want to add from the [Data sources tab](https://console.scaleway.com/cockpit/dataSource) of the Scaleway console.
