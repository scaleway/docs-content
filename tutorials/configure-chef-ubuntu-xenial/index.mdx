...
1. Connect to your remote machine via SSH. Follow the instructions depending on your remote machine type: [Instance](/compute/instances/how-to/connect-to-instance/), [Bare Metal](/compute/bare-metal-cloud/how-to/connect-to-server/) or [Dedibox](/dedibox/dedicated-servers/how-to/connect-to-server/).
...
3. From your local machine's browser, go to the [Chef Infra Server Download Page](https://downloads.chef.io/tools/infra-server), and click **Download** next to the version of Ubuntu you are running.
...
6. From the terminal of your remote machine, launch the installation using `wget` + the link you copied. Make sure to use your own link and not the one shown below as an example:

  ```
  wget https://packages.chef.io/files/stable/chef-server/14.10.23/ubuntu/20.04/chef-server-core_14.10.23-1_amd64.deb?_ga=2.xxxxxxxx.xxxxxx.xxxxxxxxx-xxxxxxxx.xxxxxx
  ```
...
8. Call the `reconfigure` command, which configures the components that make up the server to work together in your specific environment:
  ...
  The configuration then runs. This may take some minutes to complete.
...
1. Go to the [CDK Ubuntu Linux 16.04 Download Page](https://downloads.chef.io/tools/chefdk), and click **Download** next to the version of Ubuntu you are running.

2. Complete your information in the form that displays and click **Download**.

3. Right-click **Download ChefDK directly** and copy the link. 

4. On the machine you want to install ChefDK, launch the installation using `wget` + the link you copied. Make sure to use your own link and not the one shown below as an example:

  ```
  wget https://packages.chef.io/files/stable/chefdk/4.13.3/ubuntu/20.04/chefdk_4.13.3-1_amd64.deb?_ga=2.xxxxxxxx.xxxxxxxxxxx.xxxxxxxxxxx-xxxxxxxxxxx.xxxxxxxxxxx
  ```
...
7. You are prompted to accept the license agreement. Enter `yes` to agree.
...
By default, you cannot connect to a Scaleway Instance or Bare Metal server with a password. For more information, refer to the dedicated [Ubuntu Forum](https://askubuntu.com/questions/101670/how-can-i-allow-ssh-password-authentication-from-only-certain-ip-addresses)
