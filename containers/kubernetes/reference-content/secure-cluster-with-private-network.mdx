...
All new Kubernetes clusters are deployed with a Scaleway [Private Network](/network/vpc/concepts/#private-networks). **We recommend migrating existing clusters to a Private Network as well, using the Scaleway console before Q1/2024 as public endpoints will be deprecated.**

Currently, by default, worker nodes are still shipped with public IP addresses.
...
Yes, you can use a Public Gateway to exit outgoing traffic from your Kapsule nodes having private IPs.
...
|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|----------------------------------------------|
| Description | Worker nodes are assigned both private IPs and public IPs. All inbound traffic on public interface is dropped by default using Security Groups. | Worker nodes are set without public IPs (100% private network). A Public Gateway is required. | Clusters without a Private Network attached. |
...
- **End of support** for legacy public clusters. All clusters will be required to migrate to controlled isolation (Private Networks) by Q1/2024
