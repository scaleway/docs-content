...
The legacy public network is provided through a network component managing a carrier-grade NAT with NAT IP addresses.
When an Instance is started, it is assigned a NAT IP address that is associated with the underlying node hosting the Instance.
