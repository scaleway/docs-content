...
description: This page shows how to run a NodeJS + Express Server on Serverless Container 
categories:
  - serverless
  - containers
tags: serverless containers nodejs express server
dates:
...
- You have NodeJS, Npm installed on your computer
...
We first need to create a simple Web Server with Node JS and the Express.js framework.
...
2. Install the `express` library using the following command: 
...
3. Create a `server.js` file with the following code, it creates a simple Web Server using Express.js:
...
1. Create a Dockerfile using the following command: 
    ...
    ```
    <Message type="tip">
    A Dockerfile is a document that contains all the commands a user could call on the command line to create the image.
    </Message>
...
2. Open the Dockerfile in a text editor and specify the following actions:
    ...
    * Copy your code inside the Docker image:
    ...
    The final Docker File should look like the follwing example:
...
2. [Push](https://www.scaleway.com/en/docs/compute/container-registry/how-to/push-images/) the created image into the [container registry](https://www.scaleway.com/en/docs/compute/container-registry/quickstart/) linked to your Containers namespace
    ...
    The registry name start with `funcscw` + the namespace name.
...
You have build, pushed and deployed your first webapplication using NodeJS and Express Server on Scaleway Serverless Containers. For more information about the Containers product, refer to our [dedicated product documentation](https://www.scaleway.com/en/docs/compute/containers/quickstart/).
