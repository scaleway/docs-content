...
When provisioned with a [minimum scale](#min-scale) of `0`, Serverless Containers scale down to zero active instances as long as they are not triggered. While idling, they do not consume any resources, which allows you to reduce the cost of your infrastructure.
...
- The `args` instruction defines the arguments passed to the `command` instruction. Arguments can reference environment variables, as shown in the example below.
...
The endpoint is the URL generated to access your Serverless Container. A container can have a public endpoint and a private endpoint if it is attached to a [Private Network](/serverless-containers/concepts/#private-networks).

### Public endpoint

An endpoint is the URL generated to access your resource. It can be customized with [custom domains](#custom-domain). **[TBC]** If private endpoint is enabled, the public endpoint can be disabled to isolate the container from the public internet.

### Private endpoint

A Serverless Container attached to a Private Network can have a private endpoint to securely communicate with other attached resources.

### Private endpoint certificate

The private endpoint certificate is a TLS certificate generated for the private endpoint.
...
Scaleway Serverless Containers automatically inject several HTTP headers into every incoming request. These headers provide metadata about the request, the client, and the routing path through the Scaleway infrastructure. For more information, see the [header reference documentation](/serverless-containers/reference-content/request-headers/).

## Health check

Serverless Containers supports two types of health checks:
...
Both probe types share the same configuration options:
- **Probe type**: defines the type of check to perform.
    - The **TCP** probe checks whether the TCP connection of your Serverless Container is open on the specified port.
    - The **HTTP** probe lets you define a custom path to your Serverless Container. Use it to check that all requirements are met, such as the connection to the database.
- **Failure threshold**: the maximum number of checks that can fail before declaring the Serverless Container in error. Defaults to 30 for liveness probe.
- **Interval**: the duration in seconds between every check. Defaults to 10 seconds for liveness probe.
- **Timeout** the maximum amount of time to wait for a check to complete. If the check does not complete within this time, it is considered failed. Defaults to 1 second for liveness probe.
- **Path** (HTTP probe type only): the endpoint to call your Serverless Container (example: `/health`).

**Additional information for the startup probe**

Your application must listen on all network interfaces (`0.0.0.0`) for the startup probe to succeed. For example, if your application listens on port `8080`, it should bind to `0.0.0.0:8080` rather than `127.0.0.1:8080`.

**How many times can a health check fail before the container is considered in error?**

The maximum time before a container enters the `error` state depends on the configured **interval** and **failure threshold**:

For example, with the following configuration, the container enters the `error` state after **300 seconds (5 minutes)** of consecutive failed health checks:
...
See FAQ: [How do Serverless Containers health checks work?](/serverless-containers/faq/#how-do-serverless-containers-health-checks-work)
...
Serverless.com (Serverless Framework) is a tool that allows you to deploy serverless applications without having to manage Serverless Containers API calls. Write and deploy a YAML configuration file, everything else is handled automatically, even the image building.
...
For Serverless Containers, a trigger is a setting that can start events that automatically interact with your service.
...
Refer to the [dedicated documentation](/serverless-containers/how-to/add-trigger-to-a-container/#cron-triggers) for more information on how to add triggers to a Serverless Container.
...
Refer to the [dedicated documentation](/serverless-containers/how-to/add-trigger-to-a-container/#queue-triggers) for more information on how to add triggers to a Serverless Container.

### NATS trigger

A NATS trigger is a mechanism that connects a container to a [NATS](/nats/concepts/#nats-messaging-system) subject and invokes the container automatically whenever a message is published to the subject.
...
Refer to the [dedicated documentation](/serverless-containers/how-to/add-trigger-to-a-container/#nats-triggers) for more information on how to add triggers to a Serverless Container.
...
Serverless Containers API versions available are `v1beta1` and `v1`. Refer to the [v1 migration guide](/serverless-containers/reference-content/v1-migration-guide).
