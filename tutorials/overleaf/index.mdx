...
Overleaf is a collaborative writing and publishing system that utilizes LaTeX, a powerful typesetting software, allowing individuals to create structured, professional documents online.
It is particularly favored by academics and professionals for producing scientific documents, research papers, theses, and presentations due to its precise formatting and styling capabilities, and it supports real-time collaboration between multiple authors.
...
<Message type="requirement">
  ...
  - You have an Instance running on the Docker Image or has Docker installed
...
<Message type="tip">
  Overleaf requires Docker. Make sure to run this tutorial on an Instance running the Docker InstantApp image or make sure [Docker is installed](/tutorials/install-docker-ubuntu-jammy-jellyfish/) on the Instance.
</Message>
...
7. Open the file `./config/overleaf.rc` in a text editor and edit the line `NGINX_HTTP_PORT=80`. Set the value to `NGINX_HTTP_PORT=8877`, then save the file and exit the editor.

## Configuring a systemd script

If the previous command runs overleaf correctly, it is time to create a systemd daemon service.

1. Create and open the file `/etc/systemd/system/overleaf.service` in a text editor of your choice and past the following content into it:
    ```
    [Unit]
...
4. Add the following line which will stop Nginx, renew the certificate and restart the web server daily at 4:30 in the morning:
    ...
    ```
5.  Edit the file `/etc/nginx/sites-enabled/default` so that it looks like the following example, to configure Nginx in a way, that it will serve our Overleaf Community Edition site as default site:
    ```
    server {
        listen       80;
        listen       [::]:80;

        server_name  my.Overleaf Community Edition.tld;
    ...
    server {
      ...
      ssl_certificate /etc/letsencrypt/live/my.Overleaf Community Edition.tld/fullchain.pem;
      ssl_certificate_key /etc/letsencrypt/live/my.Overleaf Community Edition.tld/privkey.pem;
