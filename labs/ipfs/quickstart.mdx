...
- Data is **shared**: public IPFS nodes can fetch and host your pinned content. Your data could be hosted **anywhere** from America, Asia and everything in between. Scaleway is not able to delete it once the content is delivered to external peers.
...
Now that we have a volume, we can pin content from [CIDs](/en/docs/labs/ipfs/concepts/#content-identifier-(cid)) and URLs to it. Let's see how it works with a CID.

<Message type="note">
  The content pointed by the CID must already be present on the public IPFS network.
</Message>
