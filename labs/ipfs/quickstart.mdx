...
- Data is **shared**: public IPFS nodes can fetch and host your pinned content. Your data could be hosted **anywhere** from America, Asia and everything in between. Scaleway is not enabled to delete it once the content is delivered to external peers.
- Data is **not encrypted**: we **do not apply any encryption** algorithm over your data.
</Message>
...
Now that we have a volume, we can pin content from CIDs (content identifier) and URLs to it. Let's see how it works with a CID.

<Message type="note">
  It supposes that the content pointed by the CID must already be present on the public IPFS network.
</Message>
