...
{/*<Message type="note">
  Vous pouvez télécharger le modèle de responsabilité partagée relatif aux Instances en français [ici](/pdf/???.pdf/). Veuillez noter que, pour les valeurs contractuelles, la version anglaise fait foi.
</Message>*/}
...
- Specific provisions for HDS (Health Data Hosting)
...
Scaleway manages the Kubernetes control plane (either Kapsule or Kosmos), which consists of various components responsible for managing and maintaining the cluster and its state, and scheduling applications. This includes components such as the control plane itself: *etcd*, *API server*, *scheduler*, etc.

Scaleway takes care of Kubernetes system applications such as CoreDNS, kube-proxy, Container Networking Interface (CNI), and Container Storage Interface (CSI), which are required for the normal run of the Kubernetes cluster and its associated resources.
...
- Publishing documented SLAs for managed control planes
...
- Kubernetes resource configuration (Deployments, Services, ConfigMaps, Secrets, CRDs)
- RBAC (Role-Based Access Control) and user access permissions
...
- Configuring firewall rules (ACLs) and security groups
...
- Configuring these add-ons according to his specific needs when applicable
...
<Message type="note">
  The **auto-upgrade** feature applies patch updates only within the same minor version. It covers both the control plane and managed nodes. Ensure the `kube-system` namespace remains unmodified to avoid upgrade failures.
</Message>
...
- **Deprecation**: Announced in the [Changelog](https://changelog.scaleway.com)
...
## Specific provisions for HDS (Health Data Hosting) with Kapsule Dedicated Offer
...
Users are responsible for backing up their data with appropriate storage solutions like HDS block storage. Scaleway must backup Kapsule control-plane configuration and remain able to restore cluster state after cluster malfunction if any.

### Data Residency

So that Scaleway can guarantee that data processing remains within authorized data centers in Paris users must:

- Create Kapsule clusters only in Paris authorized region (fr-par)
- Create associated storage and network resources in Paris region
- **Not route cluster traffic outside of fr-par region**
...
- Enforcing HTTPS for ingress and egress cluster traffic, i.e. accordingly configure load balancers and gateways
- Handling certificates generation and renewal for transit
...
- Providing compliant Storage APIs to store at rest Health data with Kapsule CSI
...
- Configuring and maintaining monitoring & tracing solutions for his workload, eventually using Scaleway Cockpit solution
- Activating and configuring Audit Trail solution 

Scaleway is responsible for:

- Providing metrics & logging interfaces to monitor control-plane components (including Audit trail capabilities)
...
#### Step 2: Migrate container images to your new container registry
...
#### Step 4: Configure kubectl on your new Kubernetes cluster

#### Step 5: Adapt Kubernetes manifests and configurations
...
#### Step 6: Migrate persistent Data and storage
...
2. Restore data to newly created cluster.
...
#### Step 7: Deploy applications to your new cluster

#### Step 8: Update networking and DNS configurations

#### Step 9: Test and validate deployments

Conduct functional, performance, and end-to-end testing to verify the applications work as expected in the new environment.

<Message type="info">
  This documentation should be read with Scaleway's HDS agreement and applicable certifications. If you have any questions, contact HDS support for clarification.
</Message>