...
The user can now access the Dedibox console through their unified Scaleway account.

1. Log in as the user (or ask them to log in) using their Scaleway credentials at [account.scaleway.com](https://account.scaleway.com/login?service=dedibox).
2. If prompted, select "Dedibox" as the service to access.

If permissions are correctly configured, the Dedibox resources will be visible in the console.

<Message type="note">
For users who linked their accounts before 28 October 2025, they can still access the Dedibox console directly at [https://console.online.net/](https://console.online.net/). All other users must use the unified login process.
</Message>

<Message type="tip">
  For further information about Scaleway IAM, refer to the dedicated [IAM documentation](/iam/concepts/quickstart/).
</Message>
