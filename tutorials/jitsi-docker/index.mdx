...
4. Create a `.env` file by copying and adjusting `env.example`:
  ...
  cp env.example .env
  ```
4. Generate and configure strong passwords in the security section options of .env file by running the following bash script
  ```
  ./gen-passwords.sh
  ```
5. Create the required config directories for Jitsi Meet: 
  ```
  mkdir -p ~/.jitsi-meet-cfg/{web,transcripts,prosody/config,prosody/prosody-plugins-custom,jicofo,jvb,jigasi,jibri}
  ```
6. Run `docker compose up -d`, open a web browser and access the Jitsi Meet UI at `https://<YOUR_INSTANCE_IP>:8443`
  <Message type="note">
