...
1. The installation is done using the `apt` package manager of Ubuntu. Update the `apt` packet cache prior to installation, to make sure to install the latest available version in the Ubuntu repository:
