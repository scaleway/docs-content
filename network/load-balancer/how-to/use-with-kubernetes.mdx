...
By default, when you create a Load Balancer for your cluster using a yaml manifest such as the one shown above, it will be assigned a public IP address at random. When you delete the Load Balancer, the IP address will also be deleted and cannot be retrieved to transfer to another Load Balancer service in your cluster.
...
The full list of annotations is available [here](https://github.com/scaleway/scaleway-cloud-controller-manager/blob/master/docs/loadbalancer-annotations.md) as part of the Scaleway Cloud Controller Manager documentation.
