...
**Edge Services** forwards incoming requests to your **Serverless Container** and **Serverless Functions** via its default (auto-generated) endpoint, not via a custom domain. The container used as an Edge Services backend must be public. **Edge Services** cannot reach a private or IAM-secured container.
</Message>
...
<Message type="note">
You cannot currently disable the default endpoint of a **Serverless Container**  and a **Serverless Function** (for example, to keep only a custom domain). The default endpoint remains active. This capability is planned for later. **Edge Services** currently relies on this default endpoint to forward requests to the container/function.
</Message>
