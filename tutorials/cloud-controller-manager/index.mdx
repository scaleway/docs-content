...
For the purpose on this example, you will need to create three Ubuntu Bionic Instances:
...
4. Execute the `kubeadm join` command on each of the worker nodes to join the cluster:
...
1. Create a `k8s-scaleway-secret.yml` containing the following information:
...
2. Deploy a `LoadBalancer` service and make sure a public IP is assigned to this service. The service will automatically create a [managed Load Balancer](https://www.scaleway.com/en/load-balancer/) on the Scaleway Elements platform.
