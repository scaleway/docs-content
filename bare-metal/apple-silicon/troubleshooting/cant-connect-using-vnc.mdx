...
- If rebooting fails or if the server was forced to shut down, wait 24 hours for automatic restart. See [I have shut down my Mac mini](/bare-metal/apple-silicon/how-to/shutdown-mac-mini/).
...
Ensure your remote connection client is compatible with macOS. Incompatible remote desktop (VNC) clients can cause failed connection attempts. For recommended VNC clients, see [How to access remote desktop](#).
...
For further assistance, visit our [Slack Community](https://slack.scaleway.com/) in the #apple-silicon channel.

For persistent issues, consult our [fail2ban tutorial](#) and ensure proper security configurations are in place.