---
meta:
  title: Replacing a disk in a software RAID
  description: Replacing a disk in a software RAID
content:
  h1: Replacing a disk in a software RAID
  paragraph: Replacing a disk in a software RAID
...
2. Log in to the server using the rescue account:
    ...
    The rescue credentials are available from your servers status page in the Scaleway console.

    </Message>

2. Run to following command to make sure all disk caches are written to the disk:
...
4. Visualize the existing `mdadm` RAID devices by running the following command:
    ...
    The faulty device is market with `(F)`.
