...
This article describes basic steps to set up a deployment-ready VPS Instance using Capistrano with RVM, Puma as the application server, and Nginx as the reverse proxy. Let's assume we already have a working rails application, hosted on a remote git repository.
...
- An [Instance](/compute/instances/how-to/create-an-instance) or [Elastic Metal server](/bare-metal/elastic-metal/quickstart/#how-to-create-an-elastic-metal-server) running on Ubuntu Focal Fossa (20.04 LTS)
...
Then it will route this traffic to one or several application servers.

The second step is the application server.
...
In this tutorial, we only installed basic dependencies, but many other services could be installed, such as [elasticsearch](https://github.com/elastic/elasticsearch-rails), [redis](https://redis.io/), and more.
