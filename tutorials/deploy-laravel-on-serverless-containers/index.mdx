...
Laravel applications make use of [queues](https://laravel.com/docs/10.x/queues) to process long running jobs in the background. As this feature of the Laraval framework is nearly always used, we will configure it and leverage the [Messaging & Queuing](https://www.scaleway.com/en/messaging-and-queuing/) product from Scaleway. The [Scaleway Developer documentation](https://www.scaleway.com/en/docs/serverless/messaging/) provides clear information on how this managed service works and can be configured.
...
3. Create webserver configuration files. Nginx will be used to serve the static assets and to forward the requests to the php-fpm pool for processing. In this example, we create the following configuration files `stubs/nginx/http.d/default.conf` and `stubs/nginx/nginx.conf`.
...
1. [Create a  ScalewayContainer Registry namespace](/containers/container-registry/how-to/create-namespace/) in the `PAR` region. Set the visibility to `Private` to avoid having your container retrieved without proper authentication and authorization.
...
The Scaleway documentation website provides a Quickstart on how to [create and manage a Serverless Container Namespace](https://www.scaleway.com/en/docs/serverless/containers/quickstart/).
...
2. Deploy the application. Click **+ Deploy a Container** once the namespace is created, and follow the instructions of the creation wizard. Select the registry namespace and the previously uploaded Docker image and configure the listening port (Nginx webserver is listening on port 80). For the CPU and memory, define at least 560mVPCU and 256MB respectively. To reduce the limitations due to [cold start](/serverless/containers/concepts/#cold-start), we will run at least 1 instance.
...
By default, some metrics will be available in the Scaleway console. However, to better understand how your application behaves under load, you can enable [Scaleway Cockpit](https://www.scaleway.com/en/cockpit/).
