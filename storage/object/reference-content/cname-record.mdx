...
<Message type="note">
CNAME records cannot be created at root level, e.g. to point `example.com` towards `otherdomain.com`. This is why you cannot set a root domain to point towards Edge Services. You must always use a subdomain, e.g. `blog.example.com`.
</Message>
...
However, if you choose to [customize your Edge Services endpoint with your own subdomain](/storage/object/how-to/get-started-edge-services/#how-to-configure-a-custom-domain), a CNAME record is required to point your subdomain to the Edge Services endpoint for your bucket. 

- If your domain (or DNS management) is hosted with [Scaleway's Domains and DNS](/network/domains-and-dns/quickstart/) product, you do **not** need to manually create a [CNAME record](/storage/object/reference-content/cname-record/). When you customize your Edge Services endpoint and specify your Scaleway-hosted subdomain, Scaleway will auto-create the correct CNAME record to point the subdomain to your Edge Services bucket. You will be able to view this CNAME record in the Domains and DNS section of the console. Do not attempt to modify or delete the CNAME record.

- If your domain is hosted with a provider external to Scaleway, you **do** need to create a CNAME record. You will be prompted to do so as part of the Edge Services domain configuration process. See the next section for more information.

     <Lightbox src="scaleway-edge-services-configure-domain.webp" alt="" /> TODO CHANGE
...
| Subdomain error - root domain not allowed | Make sure you are using a subdomain (e.g. `blog.example.com`) and not a root domain (e.g. `example.com`). It is not possible to point a root domain towards Edge Services, as CNAME records cannot be created at root level.   |