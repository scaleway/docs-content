...
3. Click the **Edge Services** tab.
    ...
    <Lightbox src="scaleway-edge-services-os-splash.webp" alt="A screenshot of the Scaleway console shows the Edge Services tab for the selected bucket: it is in an empty state, with a button inviting you to enable Edge Services." />
...
5. Click **Create pipeline** to confirm.
    ...
    Your bucket's [visibility](/object/concepts/#visibility) can be set to **private**, but any objects within it that you want to expose via Edge Services must be set to [**public** visibility](/object/how-to/manage-object-visibility/). However, in the case that you are using Edge Services with bucket website, objects can remain private.
