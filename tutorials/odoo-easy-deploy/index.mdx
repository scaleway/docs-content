...
But how can you initiate your Odoo journey? Here's where we step in. This guide will walk you through the process of setting up Odoo on a [Scaleway Kubernetes Kapsule](/containers/kubernetes/quickstart/) utilizing the [Easy-Deploy](/containers/kubernetes/how-to/enable-easy-deploy/) feature, easily accessible right from within the Scaleway console. In minimal time, you'll have Odoo up and operational, primed to advance your business endeavors. Just adhere to the steps outlined in the sidebar on the right, and let's commence the journey!
<Message type="tip">
