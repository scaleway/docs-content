...
<Message type="requirement">
  ...
  - You have a [Instance running on Ubuntu](/compute/instances/how-to/create-an-instance/).
</Message>
...
2. Update the apt packet cache and upgrade the software already installed on the Instance:
    ...
    ```
3. Strapi requires Node.js (version 12.x and latest) in order to run, as well as npm (version 6.x and latest). Use the following command to add a maintained and up to date repository for Debian-based Node.js:
    ```
...
5. Check that Node.js and npm are installed in minimum versions 12.x and 6.x respectively:
    ...
    An output displays confirming the versions you have installed for each.
6. Strapi uses the [yarn package manager](https://yarnpkg.com/). Run the following commands to install the repository for the required version of yarn:
    ```
    curl -sS https://dl.yarnpkg.com/debian/pubkey.gpg | apt-key add -
    echo "deb https://dl.yarnpkg.com/debian/ stable main" | tee /etc/apt/sources.list.d/yarn.list
    apt update && apt install yarn
    ```
7. Run the following command to make sure yarn is installed:
    ```
    yarn -v
    ```

    An output displays confirming the installed version.
...
1. Navigate to your home directory and create a new folder for your Strapi project:
    ...
    ```
2. Access your directory and install Strapi in it. Then, create a new project called `strapi-project` and a directory called `backend` in which we will store Strapi's blog template:
    ```
    cd strapi-projects
    yarn create strapi-app backend my-strapi-project --quickstart --template @strapi/template-blog@1.0.0 blog
    ...
    <Message type="note">
      ...
      - The `-- template` flag allows you to steup a blog that contains pre-built collection-types and single-types.
    </Message>

    You should see Strapi install and run (this may take a few minutes). You can find useful information at the end of this output that you may want to read and take note of, particularly about the different commands like `yarn develop` and `yarn start` that you can use to run your Strapi app in different modes. The output should end with a message like the following:
    ...
    We are now ready to start configuring our Strapi app.
...
In this step, we will use the Strapi dashboard to configure Strapi for a very simple blog use case. First we need to create a SuperAdmin user, then set up a database to hold our blog posts, then write a "Hello World" blog post and see how to access it. Make sure you are in the `backend` directory.

1. In your terminal, run `yarn build` to build your Strapi application, then run `yarn start`.
2. Open a browser and go to `http://<Instance IP Address>:1337/admin`. Make sure you replace `<Instance IP Address>` with your Instance's Public IP.

    The following output displays:
    <Lightbox src="scaleway-create-admin.webp" alt="" />

3. Complete the form to create an admin user, then click **Let's Start**.

    The following screen displays:

    <Lightbox src="scaleway-strapi-dashboard.webp" alt="" />

    Next, we will essentially define a new database to hold our blog content.
4. Click **Create Your First Content Type**:

    The following screen displays:

    <Lightbox src="scaleway-create-collection-type.webp" alt="" />
5. Enter `Blog` as the display name (UID will auto-complete), and click `Continue`:
