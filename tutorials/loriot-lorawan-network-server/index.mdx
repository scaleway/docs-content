...
- [Start a PostgreSQL managed DB instance](/databases/databases-for-postgresql-and-mysql/how-to/create-a-database/) and create a `lora` table with the following fields: `time` (timestamp type) and `data` (jsonb type)
- Configure a Database Route to push data in there, [see how to](/databases/databases-for-postgresql-and-mysql/how-to/create-a-database/). The topic filter should be `my-app/things/+/uplink`. Your query should look like `INSERT INTO lora (time, data) VALUES (NOW(), $PAYLOAD)`.
