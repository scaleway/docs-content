---
meta:
  title: How to add a custom domain to a function
  description: This page explains how to add a custom domain to a function
content:
  h1: How to add a custom domain to a function
  paragraph: This page explains how to add a custom domain to a function
tags: functions custom-domain
dates:
  validation: 2023-01-02
  posted: 2022-02-21
categories:
  - serverless
---

Each function comes with an automatically generated domain on which it is available. However it is possible to assign your own domain name to make it easier to use your functions.

<Macro id="iam-requirements" />

<Message type="requirement">
  - You have an account and are logged into the [Scaleway console](https://console.scaleway.com)
  - You have a running [Serverless Function](/serverless/functions/how-to/create-a-function/)
  - You have a domain name
  - If you are using **Cloudflare DNS**, please see the note below regarding proxying
</Message>

1. Add a CNAME record pointing to your function's default URL from the [domains dashboard](/network/domains-and-dns/how-to/manage-dns-records/).
2. Click the **Endpoints** tab on your Function's Overview page.
    <Lightbox src="scaleway-functions-domain.webp" alt="" />
3. Click **+ Add endpoint**.
4. Enter your custom domain URL and validate the form.
5. Connect to your function using your custom domain once you have checked the CNAME and TLS fields.

<Message type="note">
  Be careful, for now **Custom Domains** do not support websockets. If you need to use websockets you have to configure them with the original function URL.
</Message>

## Cloudflare DNS

If you are using Cloudflare DNS to configure your custom domain, you need to set the `CNAME` record's proxy status to **DNS Only** while adding it to your function. We perform a number of validation checks on our side when configuring your domain, and if the `CNAME` record is proxied during these checks, they will fail.

Once you have configured your domain and your function is responding correctly, you can set the `CNAME` record back to `Proxied` if you wish.

The [Cloudflare documentation on Proxy Status](https://developers.cloudflare.com/dns/manage-dns-records/reference/proxied-dns-records/) describes how to change the Proxy Status of a `CNAME` record via the Cloudflare web UI.

<Navigation title="See Also">
  <PreviousButton to="/serverless/functions/how-to/manage-a-function/">How to manage a function</PreviousButton>
  <NextButton to="/serverless/functions/how-to/connect-function-postgresql-database/">How to connect a function to a PostgreSQL database</NextButton>
</Navigation>
