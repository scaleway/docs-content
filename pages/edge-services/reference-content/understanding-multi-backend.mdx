...
Each pipeline can now support multiple backends — Load Balancers, Object Storage buckets, or a combination of the two. [Serverless Containers and Serverless Functions](/serverless-containers/how-to/deploy-container-behind-edge-services/) are also supported as backend types, but only via the API, CLI, SDK, and Terraform (not yet available in the Scaleway console). Additional backend types are planned for future releases.

To make sure traffic reaches the right backend, you can define routing rules. Each rule specifies filters based on URL path and/or HTTP method, and routes any matching requests to the backend you specify:
