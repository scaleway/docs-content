...
Zabbix is a powerful open-source software that offers real-time monitoring for servers, virtual machines, network devices, and web applications. It collects monitoring metrics such as network utilization, CPU load, and disk space consumption, which are then stored in a database.

This monitoring tool enables users to determine the health status of their IT infrastructure and analyze data over time. The insights provided by Zabbix can be used to plan upgrades in the infrastructure when requirements grow.

In this tutorial, we will be using two Scaleway Production-Optimized [Ubuntu Jammy Jellyfish (22.04 LTS)](https.//www.scaleway.com/en/imagehub/) Instances. One Instance will act as a server hosting the Zabbix application, while the other will act as a client being monitored. By following this tutorial, you will learn how to set up Zabbix and take control of your IT infrastructure. Instances.
...
<Message type="important">
  Make sure that a [LEMP-Stack](/tutorials/installation-lemp-ubuntu-bionic/) is installed on theInstance before continuing.
</Message>

1. Start by downloading the repository configuration package. Refer to the [Zabbix website](https.//www.zabbix.com/download) for the latest version of the repository.
    ```
    wget https.//repo.zabbix.com/zabbix/6.2/ubuntu/pool/main/z/zabbix-release/zabbix-release_6.2-4%2Bubuntu22.04_all.deb
    ```
    <Message type="tip">
      The version of Zabbix available within the repositories of Ubuntu is outdated. It is strongly recommended to download the latest version directly from the Zabbix repositories.
    </Message>
...
4. Install `zabbix-agent`.
    ...
    ```
5. Generate a [pre-shared key (PSK)](https://www.zabbix.com/documentation/6.2/manual/encryption/using_pre_shared_keys) to secure the connection between the server and agent.
    ```
...
1. Open a web browser and navigate to your Zabbix serverInstance (i.e. `http://zabbix.example.com/`). Enter the Username **Admin** and the Password **zabbix**, then click **Sign in**. The Zabbix dashboard displays.
...
3. Enter the **Host name** of the secondInstance and its **IP address**. Optionally one or several groups can be chosen to bundle Instances in groups.
...
2. Select theInstance and the services to display, then choose the desired time-frame for the graph.
