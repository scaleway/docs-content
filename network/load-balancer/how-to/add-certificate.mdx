...
6. Combine the certificate and private key into a `.pem` file, which is the format required in the Scaleway console when adding the certificate to the Load Balancer: 
    ```
    cat csr.txt private_key.txt > cert.pem
    ```
