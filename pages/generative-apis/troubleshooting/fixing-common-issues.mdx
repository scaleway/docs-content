...
- Disable structured output in your query: ensure `response_format` is set to `{ "type": "text"}`. 
  - Explanation: when `response_format` is set to `{ "type": "json_schema", "json_schema": {...} }}`, model output will be forced to comply with the valid JSON Schema grammar for each newly generated token. This means a valid JSON structure representing tool calls cannot be generated.
