...
If the HTTP-01 challenge fails and the custom domain is in `error` status, see [Troubleshooting](/serverless-functions/troubleshooting/custom-domain-in-error-status/).
