...
import scalewayInterlinkOverviewRequesting from './assets/scaleway-interlink-overview-requesting.webp'
...
2. Click on the InterLink that you want to finish provisioning. The InterLink's **Overview** page displays.
    ...
    <Lightbox image={scalewayInterlinkOverviewRequesting} alt="A screen in the Scaleway console show's an overview of an InterLink's partner, location, region, bandwidth, creation date, pairing key, and IPv4/IPv6 connectivity status." /> 
    TODO UPDATE CONSOLE
...
<Lightbox image={scalewayInterlinkListing} alt="A screen in the Scaleway console shows a listing of InterLinks, with the following information: name, status, partner, location, region and bandwidth." /> 
TODO UPDATE CONSOLE
