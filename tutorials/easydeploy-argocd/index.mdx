...
meta:
  ...
  description: Learn how to deploy Argo CD on Scaleway Kubernetes clusters using the Easy Deploy feature. This comprehensive guide provides step-by-step instructions for deploying Argo CD, which pulls application definitions from a Git repository to maintain the desired state across your Kubernetes clusters, ensuring optimal continuous delivery and GitOps practices.


content:
  h1: Deploy Argo CD on Scaleway Kubernetes clusters using Easy Deploy
  paragraph: Learn how to deploy Argo CD on Scaleway Kubernetes clusters using the Easy Deploy feature. This comprehensive guide provides step-by-step instructions for deploying Argo CD, which pulls application definitions from a Git repository to maintain the desired state across your Kubernetes clusters, ensuring optimal continuous delivery and GitOps practices.
...
Therefore making it ideal for managing complex Kubernetes environments with precision and reliability.
...
6. Select the **Argo CD** application.
    <Message type="tip">
        If you cannot find Argo CD on the first page, use the search bar or navigate through the library using the arrow buttons.
    </Message>
7. Optionally, customize the default configuration for Argo CD using [Helm Charts](/tutorials/kubernetes-package-management-helm/). If you do not require any customized configuration you can skip this step.
8. Enter a name and a Kubernetes namespace for your application. If no name is entered, Argo CD will be installed in the default namespace of the cluster.
