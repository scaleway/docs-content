---
title: How to deploy a Serverless Container behind an Edge Service
description: Deploy a Serverless Container behind a Scaleway Edge Service pipeline using the CLI or Terraform. Configure HTTPS termination and a public endpoint.
...
This guide walks you through deploying a Serverless Container behind a Scaleway Edge Service pipeline. Create a namespace and deploy a container. Then configure an Edge Service pipeline with stages for backend routing, TLS termination, and DNS to expose a public endpoint.

<Message type="note">
This same procedure applies to **Serverless Functions**. The Edge Service backend stage supports both backends. Replace the container namespace and container resources with the equivalent Function namespace and function resources, and use the corresponding function ID in the backend stage configuration.
</Message>
...
After creating the container, note the `id` (container ID) from the output. You need it to attach the container to an Edge Service.
...
The stages must be linked in a chain. Update the TLS stage to link to the backend stage:
