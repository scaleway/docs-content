...
Scaleway VPC provides managed DNS resolution for resources attached to a Private Network, letting you address resources by hostname rather than IP address. [Read more about VPC DNS](/vpc/reference-content/dns/).

When two VPCs are peered, DNS resolution can extend across the peering connection. To resolve a resource located in a peered VPC, you must include the target VPC's name or ID in the DNS query, since Private Network names are only guaranteed to be unique within a single VPC.

Cross-VPC DNS resolution follows the same rules as [transitive peering](#transitive-peering): it works across a direct peering connection, and through an intermediary VPC where transitive peering is enabled, but not through an intermediary VPC where it is not.
