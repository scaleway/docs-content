...
excerpt: |
  This page shows you how to configure a Cloudera CDH cluster using two or more machines running on Ubuntu Bionic
totalTime: PT15M
steps:
...
8. Comment-out the line `bind-address` by putting a `#` in front of it to enable remote connections to the MariaDB server:
  ...
  Once uncommented the database server will be reachable from any machine connected to the Internet. It is recommended to limit the access to the IPs of the cluster by setting up a [firewall](/tutorials/uncomplicated-firewall) on the machine.
