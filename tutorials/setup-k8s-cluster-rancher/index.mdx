...
tags: kubernetes rancher k8s cluster
excerpt: |
  Rancher is an open-source container management platform, providing an graphical interface making container management easier. The GUI of Rancher makes it easy to manage secrets, handle roles and permissions. It allows to scale nodes and pods, set up load balancers without the requirement of a command line tool or the editing of hard to read YAML files.
totalTime: PT15M
steps:
  - step: Spinning up the required Instances
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-k8s-cluster-rancher#-Spinning-up-the-required-Instances
  - step: Installing Rancher
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-k8s-cluster-rancher#-Installing-Rancher
  - step: Configuring Rancher
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-k8s-cluster-rancher#-Configuring-Rancher
  - step: Creating a Cluster
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-k8s-cluster-rancher#-Creating-a-Cluster
  - step: Deploying a Cluster Workload
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-k8s-cluster-rancher#-Deploying-a-Cluster-Workload
  - step: Scaling a Cluster Workload
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/setup-k8s-cluster-rancher#-Scaling-a-Cluster-Workload
categories:
  - compute
  - instances
image: /assets/images/docs/scaleway.png
keywords: cloud tutorials, cloud documentations, cloud server, cloud hosting, ubuntu server, debian server, linux server, IAAS, baremetal, scalability, ARM, physical server, dedicated server, physicalization, server, kubernetes, k8s, cluster, rancher
dates:
...
2. Click **Instances** in the menu on the left:
  ...
  <Lightbox src="scaleway-rancher_instances.png" alt="" />

3. To deploy instances with [Docker](https://www.docker.com/) pre-installed, click **+ Create an instance**:

  <Lightbox src="scaleway-rancher_create_instance.png" alt="" />

4. Click the **InstantApps** tab, and choose the **Docker** image:

  <Lightbox src="scaleway-rancher_instant_apps.png" alt="" />

  <Lightbox src="scaleway-rancher_docker.png" alt="" />
...
1. Once Rancher is installed, open a web browser and point it to your rancher domain (i.e. `https://rancher.example.com`). The following page displays:
  ...
  <Lightbox src="scaleway-rancher_password.png" alt="" />
...
3. Enter the domain name for Rancher. Normally this value is already pre-filled with the domain name configured when launching the docker container. Check that the value is correct and click **Save URL** to continue with the configuration:
  ...
  <Lightbox src="scaleway-rancher_domain.png" alt="" />

4. The (empty) Rancher Dashboard displays:

  <Lightbox src="scaleway-rancher_dashboard_empty.png" alt="" />

### Creating a cluster

1. Click **Add Cluster** to configure a new Kubernetes cluster.

2. The cluster creation page displays. Click **Custom** to deploy the Cluster on the already launched Scaleway compute instances.

  <Lightbox src="scaleway-rancher_add_cluster.png" alt="" />
...
3. Enter a name for the cluster, choose the desired Kubernetes version and network provider and select **None** as cloud provider.
  ...
  <Lightbox src="scaleway-rancher_cluster_options.png" alt="" />
...
2. The clusters dashboard displays. Click **Deploy**:
  ...
  <Lightbox src="scaleway-rancher_menu.png" alt="" />
...
3. Enter a the details of the workload:
  ...
  <Lightbox src="scaleway-deploy_workload.png" alt="" />
...
4. Once deployed, open a web browser and point it to `http://<rancher.example.com>:30000/`. The Nginx demo application displays:
  ...
  <Lightbox src="scaleway-nginx_demo_app.png" alt="" />
...
1. From the cluster dashboard, click **...** then on **Edit** in the pop-up menu:
  ...
  <Lightbox src="scaleway-workload_edit.png" alt="" />

2. Edit the Workload type and set the number of scalable deployments to **3**:

  <Lightbox src="scaleway-upgrade_service.png" alt="" />
...
3. Click **Save**. Rancher will now send instructions to Kubernetes to update the workload configuration and to deploy 3 pods running the Nginx demo application parallel.
  ...
  <Lightbox src="scaleway-workload_updated.png" alt="" />
