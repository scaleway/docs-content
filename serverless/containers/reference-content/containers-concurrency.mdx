...
A higher number of instances processing requests at the same time implies a greater usage of memory and [vCPU](/containers/concepts/#vcpu), and consequently a higher cost.
...
When [deploying a container](/containers/how-to/deploy-container/), Scaleway Serverless Containers allows you to configure the [concurrency](/containers/concepts/#concurrency), which is the maximum number of requests a single container instance can handle at the same time. The concurrency setting ranges from 1 to 1,000 simultaneous instances.
