...
During this tutorial you will learn how to install the latest version of open-source version of Zimbra on an [Elastic Metal server](https://www.scaleway.com/en/elastic-metal/) running on Ubuntu Bionic Beaver server.
...
9. Confirm that you want to use the repository of Zimbra, by pressing on `Enter`:
    ...
    ```
10. Zimbra will ask you which packages to install. You can keep the default values and confirm their installation by typing `y`:
    ```code
...
17. You will be asked if you want to report your installation to Zimbra, you can type `no` if you do not want to do this:
    ```code
...
The installation of Zimbra Open Source is now complete, and you can continue with the configuration of your collaboration suite.

## Configuring Zimbra Open Source Edition

1. You can now log in to the admin console at `https://YOUR_SERVER_IP:7071/zimbraAdmin/`. Use the user `admin@domin.tld` and the password that you have set during the installation to log in:
...
The user interface of your Zimbra collaboration suite is available at `https://YOUR_SERVER_IP`. Use your complete email address and password to log in:
...
4. Exit from the Zimbra user:
    ...
    exit
    ```
5. Launch the following command to use the `certonly` feature of certbot:
    ```
...
6. The certificate has been issued once the following message appears:
    ...
    Let's Encrypt creates the `chain.pem` file without the root CA. Copy the [IdenTrust root Certificate](https://www.identrust.com/certificates/trustid/root-download-x3.html) and merge it after the `chain.pem`. The root certificate is available for download [here](https://letsencrypt.org/certs/trustid-x3-root.pem.txt):
