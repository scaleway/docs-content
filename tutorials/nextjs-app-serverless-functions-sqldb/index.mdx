...
<Message type="tip">
If you want to avoid doing these modifications manually, you can also retreive the final code from this tutorial from our [serverless-examples](https://github.com/scaleway/serverless-examples/tree/main/projects/tutorial-sdb-nextjs-terraform) repository with the command below:
```
...
4. Build your application container with the updated code:
    ...
    ```
    This time, the `build` command requires environment variables, because Next.js pre-rendering will access your database while building time to load pages content.
...
4. Copy the files from the [serverless-examples](https://github.com/scaleway/serverless-examples/tree/main/projects/tutorial-sdb-nextjs-terraform) repository to the folder.
      <Message type="note">
        These files extend the blog demo application from `npx create-next-app --example blog-starter my-nextjs-blog`, and adds the required code to fetch data from a database instead of static markdown files.
      </Message>
