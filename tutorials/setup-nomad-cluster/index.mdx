...
We have created a [GitHub repository](https://github.com/scaleway/learn-nomad-cluster-setup.git) that contains all the code you need to deploy a Nomad cluster on Scaleway. Clone the repository to your local machine using the following command:
...
Open the `variables.hcl` file in your text editor of choice and update the following variables:
...
These variables need to be UUIDs. You can generate an UUID using the following command:

```shell
uuidgen | tr "[:upper:]" "[:lower:]"
```

You can also generate UUIDs through Terraform console's `uuid()` function:
...
Run `nano variables.hcl` to open the `variables.hcl` file.

Paste the image name next to the `instance_image` variable to configure your image and press `CTRL O` and `CTRL X` to save and exit.
...
Type `yes` when asked if you want to perform these actions, then click enter.
...
Run the `post-setup.sh` script to setup your local environment to use the Nomad CLI:
...
Delete the `nomad.token` file and run `./post-setup.sh` again.
...
Run the following commands to setup the environment variables:
