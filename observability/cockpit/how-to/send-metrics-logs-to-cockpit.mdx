...
## Example: send Python logs to Scaleway Cockpit on a new logs dashboard

First we need [logging_loki](https://pypi.org/project/python-logging-loki/), you can install with `pip install python-logging-loki`
...
```
    ...
    <Message type="tip">
On Loki Scaleway API, ou can POST with bearer token authorization, or POST directly https://api_key:YOUR_COCKPIT_API_KEY@logs.prd.obs.fr-par.scw.cloud/loki/api/v1/push
    </Message>

Python logs are send to Cockpit from Scaleway Loki API. Now, we need to retrieve them on Scaleway Cockpit with grafana :

* We choose a [simple logs dashboard](https://grafana.com/grafana/dashboards/13639-logs-app/) for this exemple on Grafana Dashoards. Grafana Dashboard id 13639
