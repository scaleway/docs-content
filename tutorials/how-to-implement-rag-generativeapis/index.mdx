...
3. Create a `rag.py` file and add the following code to it:
   ```python
   # rag.py
...
7. Edit the beginning of `rag.py` to import `boto3`, `S3FileLoader` and `RecursiveCharacterTextSplitter`:
...
9. Edit `rag.py` to load all files in your bucket using `S3DirectoryLoader`, split them into chunks of 500 characters using `RecursiveCharacterTextSplitter` and embed them and store them into your PostgreSQL database using `PGVector`.
   ...
   The chunk size of 500 characters is chosen to fit within the context size limit of the embedding model used in this tutorial, but could be raised up to 4096 characters (or slightly more as context size is counted in tokens). Keeping chunks small also optimize performance during inference.
