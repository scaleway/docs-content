...
Several operating systems are available for [automatic installation from the console](/elastic-metal/how-to/install-server/).
...
Yes, Elastic Metal servers are delivered with a pre-configured IPv6 address (/128).

## Pricing and billing

### Which Elastic Metal commitment plan should I choose?

Scaleway provides a variety of options to best suit your needs:
  - **No commitment** (hourly): Best if you need temporary compute power, sporadic peak-capacity support, or complete flexibility to release resources at any time.
  - **Monthly**: Ideal for ongoing workloads where you want a lower rate than hourly billing with month-to-month flexibility. Note that monthly plans require a setup fee and auto-renew every month until the server is deleted.
  - **Long-term commitments** (12, 24, or 36 months): Best for predictable, long-term production workloads. These plans offer the deepest discounts, waive setup fees, and unlock exclusive features like additional disk.
...
Your billing depends on the commitment plan you selected when creating a new server. You can upgrade your plan at any time and the new plan starts immediately.

Detailed modalities are as follows.

| Offer modalities | No commitment | Monthly commitment | Long-term commitment |
| :--- | :--- | :--- | :--- |
| **Periodicity** | Hourly | Monthly | 12, 24, or 36 months |
| **Setup fee** | None | Yes | Waived |
| **Discount** | No | No | Fixed discount applied to server base price, excluding options |
| **Commitment term** | None | Sliding 1-month cycle (e.g., May 15 – June 15) | Sliding 1, 2, or 3-year term (e.g., May 15, 2026 – May 15, 2027) |
| **Billing proration** | Billed hourly for actual allocation | First/last months prorated; current active month fully due | First/last months prorated; monthly billing over commitment term |
| **Auto-renewal** | N/A | Mandatory (cannot be deactivated) | Enabled by default (can be deactivated) |
| **Expiration** | N/A | Auto-renews monthly until server is deleted | Reverts automatically to monthly commitment if auto-renewal is disabled |
| **Server deletion** | Immediate | Effective automatically at the end of the monthly period | Effective automatically on the anniversary date |
| **Additional disks** | Not available | Not available | Available |

### What is the loyalty reward, and how do I use it?

<Message type="important">
  The loyalty voucher, which helps offset the setup fee, is no longer available for new subscriptions as of April 1, 2026.
...
The loyalty reward is a cloud service voucher in the sum of the setup fees paid at the time of your Elastic Metal server purchase.

You are entitled to it if you have a monthly Elastic Metal server that is up and running for 12 consecutive months without termination. You have the right to one voucher per physical server, which can be used for all Scaleway Cloud products, except Elastic Metal servers and their associated options.

The loyalty reward voucher is valid for 6 months and automatically added to your Scaleway account. [Learn more about how to get and use a loyalty reward](/elastic-metal/how-to/get-use-loyalty-reward/).

### Are setup fees refundable?

It is not possible to get a cash refund of the engagement fees, even after 12 months of keeping the server.
...
An Elastic Metal server is delivered to your account within a few minutes. You can then launch the server installation, which can take up to 1 hour.
...
The first level is to back up your Elastic Metal server's data and configuration. This can be done by exporting it to an [Object Storage bucket](/object-storage/quickstart/). You can check our tutorial on [backing up servers to Object Storage](/tutorials/backup-dedicated-server-s3-duplicity/) for more information.
...
You can have up to 64 [flexible IPs](/elastic-metal/how-to/order-flexible-ip/) per Elastic Metal server.
...
A Windows Server license is available in 2 versions, Standard and Datacenter, each at a different price. The availability of certain features, such as roles, depends on the version you choose.
...
*Refer to the [Pricing page](https://www.scaleway.com/en/pricing/elastic-metal-pricing/) to learn about the current costs for Windows licenses.
...
Each Windows installation implies the invoicing of a new license, except when reinstalling an Elastic Metal server.
