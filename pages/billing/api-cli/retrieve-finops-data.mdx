...
The Consumption API allows you to get the data used in your invoice, whereas the FinOps API enables you to ??? {/* Need a definition here. */}
...
The `resource_name` is only available if the related product had the transfer of this information enabled when creating the resource.
{/* Who enables this and where? */}
...
Follow the procedure below to download your FinOps data using the Scaleway [Billing API](https://www.scaleway.com/en/developers/api/billing/finops) and `curl`.
