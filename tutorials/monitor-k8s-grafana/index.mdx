...
excerpt: |
  ...
  This tutorial will explain you how to monitor your Kubernetes Kapsule cluster.
totalTime: PT10M
steps:
  - step: Preparing the Kubernetes Kapsule Cluster
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/monitor-k8s-grafana/#-Preparing-the-Kubernetes-Kapsule-Cluster
  - step: Deploying Prometheus
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/monitor-k8s-grafana/#-Deploying-Prometheus
  - step: Deploying Grafana
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/monitor-k8s-grafana/#-Deploying Grafana
categories:
  - compute
  - kubernetes
tags: compute kubernetes kapsule prometheus monitoring
image: /assets/scaleway_grafana.png
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, ubuntu server, debian server, linux server, IAAS, baremetal, scalability, ARM, physical server, dedicated server, physicalization, server, compute, kubernetes, kapsule, prometheus, monitoring
dates:
...
4. Access the `Prometheus` dashboard using the following URL: <http://localhost:9090>
  ...
  <Lightbox src="scaleway-prom1.png" alt="" />
...
5. Verify that both `node-exporter` and `kube-state-metrics` metrics are correctly scrapped by `Prometheus`:
  ...
  <Lightbox src="scaleway-prom2.png" alt="" />

    - The `kube-state-metrics` begin with "kube\_"

  <Lightbox src="scaleway-prom3.png" alt="" />
...
6. Prometheus is capable of generating graphs on its own. You can test the feature by chosing metrics to analyse directly in the application:
  ...
  <Lightbox src="scaleway-prom4.png" alt="" />
...
5. The welcome screen displays and invites you to complete the configuration of Grafana. Click **Add data source** to configure a new data source:
  ...
  <Lightbox src="scaleway-grafana_home.png" alt="" />
...
8. Click the **+** sign and then **Import** to import a ready-to-use dashboard from the Grafana website. To create a dashboard that uses _kube-state-metrics_, import the dashboard number [8588](https://grafana.com/grafana/dashboards/8588) and get information about your Deployement, Statefulset and Daemonset:
  ...
  <Lightbox src="scaleway-grafana_import.png" alt="" />

9. Choose `Prometheus` as data source and click **Import**.

  <Lightbox src="scaleway-grafana_dash_datasource.png" alt="" />
...
11. You can also configure additional dashobards, for example the node exporter full dashboard ([1860](https://grafana.com/grafana/dashboards/1860)) to display a dashboard with system metrics for each Kubernetes node:
  ...
  <Lightbox src="scaleway-monito3.png" alt="" />
