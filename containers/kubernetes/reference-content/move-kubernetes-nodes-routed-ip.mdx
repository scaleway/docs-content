...
content:
  ...
  paragraph: Safely Migrating Kapsule clusters to routed IPs
...
<Message type="note">
  ...
  However, it is important to note that while routed IPs offer this capability, IPv6 support for Kapsule is not yet available due to compatibility limitations with existing Container Network Interfaces (CNIs).
</Message>
...
5. Confirm the move of your worker nodes to routed IPs by typing **MOVE**, then click **Move to routed IPs** to launch the process.
    <Message type="important">
      * This operation is similar to a clster upgrade. Each node in your cluster will reboot. Ensure having more than one node ready in order to avoid downtime.
