...
8. Enter a **next hop** for the route. The VPC will route traffic for the destination IP to the resource designated as next hop.
    - Select a resource type: **Instance**, **Public Gateway** or **Elastic Metal**. Routing is not yet compatible with Managed Databases, nor with other types of Scaleway resource which are not integrated with VPC. 
    ...
    If the selected resource is attached to multiple Private Networks within the VPC, you are prompted to select the Private Network that should be used for the route.
