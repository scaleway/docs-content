...
- Automatically, using a bash script based on the Scaleway CLIv2. This could be a good option if you have multiple databases to migrate, and/or are concerned about 
errors during a manual restore.
...
This solution lets you migrate your database(s) automatically, with a bash script using the Scaleway CLI v2. The script will walk you through the migration process.
...
1. Copy and paste the following script into a text file, and save it as `db_mig_script.sh`
    ...
    # This script is designed to migrate a Scaleway RDB instance to a new Postgres major version
    # It requires Scaleway CLI v2 as well as Postgres binaries
    ...
    get_region () {
        ...
        while true; do
            ...
            case $region in
                [fr-par|nl-ams|pl-waw|fr-srr]* ) REGION=$region; break;;
                '') break;;
    ...
    if [ "$(instance_major_verion "$old_instance")" -ge "$(instance_major_verion "$new_instance")" ]; then
...
3. Run the script from your terminal by typing `./db_mig_script.sh`
    ...
    The script will walk you through the migration process, asking you to complete the necessary information for your migration as required.
