...
The methodsq listed below are best-suited for database sizes below 100 GB (approx.). Above this size, we recomment you use dedicated tooling to perform the operation, as your local storage size and bandwidth will be create botteneck and the operation may last several hours.
</Message>
...
2. Fill in the required information below by entering the numer associated to each parameter:
...
6. Click **Create new service**, select **DBCPConnectionPool**, click **OK**, then click the arrow to the right to access DBCPConnectionPool configuration.
...
3. Create the table structure corresponding to your file columns types:
