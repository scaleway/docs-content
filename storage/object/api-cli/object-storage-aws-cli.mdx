...
To interact with Object Storage, `aws-cli` and `awscli-plugin-endpoint` need to be installed. The `awscli-plugin-endpoint` is a great plugin to easily access third-party S3 providers.
