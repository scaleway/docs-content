...
A single-node server installation is a fully-functional Kubernetes cluster. It is not necessary to add additional server or agents nodes, but you may want to do so to add additional capacity or redundancy to your cluster (see. adding k3s node)
...
4. Create the instance
    ...
    ``` console
    $ scw instance server create name=master type=AMP2-C4 image=ubuntu-jammy routed-ip-enabled=true ip=none tags.0="k3s"
...
7. Create an environment variable named `$master` with the public IPv6 address assigned to the instance for later use.

    ``` console
    $ export master=$(scw instance server get a2c29016-690c-40bc-a35f-898646d958d4 | awk '$1 == "PublicIP.Address" {print $2}')
    ```
...
1. On your local machine, run the following command to upload the K3s binary to the `/usr/local/bin` folder on `$master`.

    ``` console
    $ wget https://github.com/k3s-io/k3s/releases/download/v1.28.2%2Bk3s1/k3s-arm64
    $ scp -6 ./k3s-arm64 root@[$master]:/usr/local/bin/k3s
    $ ssh root@[$master] chmod +x /usr/local/bin/k3s
    ```
  
    The K3s binary for each architecture can be found in the  [releases page](https://github.com/k3s-io/k3s/releases).

2. On your local machine, run the following command to upload the Cilium binary to the `/usr/local/bin` folder on `$master`:
    ...
    $ scp -6 ./cilium-linux-${CLI_ARCH}.tar.gz root@[$master]:/tmp    
    $ ssh root@[$master] tar xzvfC /tmp/cilium-linux-${CLI_ARCH}.tar.gz /usr/local/bin
    $ ssh root@[$master] rm /tmp/cilium-linux-${CLI_ARCH}.tar.gz
...
2. Connect to the Scaleway instance with `ssh root@[$master]`
...
3. Verify that your application is deployed and accessible `http://[$master]/`
...
4. Run the installation script with the `K3S_URL` and `K3S_TOKEN` environment variables. Here is an example showing how to join an agent:
    ...
    $ curl -sfL https://get.k3s.io | INSTALL_K3S_SKIP_DOWNLOAD=true K3S_URL=https://master:6443 K3S_TOKEN=$K3S_TOKEN INSTALL_K3S_EXEC="--node-external-ip=$public_ip --node-ip=$private_ip" sh -
    ```

    The K3s agent will register with the K3s server listening at the supplied URL. The value to use for `K3S_TOKEN` is stored at `/var/lib/rancher/k3s/server/node-token` on your Scaleway `master` instance.

5. Connect to the `master` node and verify the status of the Kubernetes nodes.

    ``` console
    $ kubectl get nodes

    NAME      STATUS     ROLES                  AGE   VERSION
    master    Ready      control-plane,master   36m   v1.28.2+k3s1
