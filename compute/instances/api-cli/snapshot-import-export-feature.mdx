...
Any snapshot (a valid QCOW2 image) imported to a Scaleway Object Storage bucket, can be imported into Instances in the same Region as the snapshot. This allows you to create Instances in other Avilable Zones within the same region, for example you can deploy Instances in `fr-par-2` based on the snapshot exported from an Instance in `fr-par-1`.
