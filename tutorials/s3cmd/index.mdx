...
In this tutorial, you will learn how to use [s3cmd](https://github.com/s3tools/s3cmd) as a client for [Scaleway Object Storage](/object/concepts/#object-storage). `s3cmd` is a tool that allows you to create, list and delete buckets from the command line, as well as to download, upload, and delete objects to/from your buckets.
...
<Message type="important">
  It is currently not possible to get the CORS configuration of a bucket with `s3cmd`. It is recommended to use [aws-cli](/object/api-cli/bucket-operations/#getbucketcors) to get the CORS settings of a bucket.
</Message>
