...
content:
  h1: Setting up web analytics with Matomo on a Scaleway Instance
  paragraph: This page shows you how to install and configure Matomo web analytics on a Scaleway Instance running Ubuntu Focal Fossa (20.04)
categories:
  - instances
...
5. Reload Nginx to activate the new configuration.
    ...
    ```
6. [Run Certbot](/tutorials/installation-lemp-ubuntu-bionic/#configuring-tlsssl-with-lets-encrypt) with the `--nginx` flag to request a Let's Encrypt TLS/SSL certificate for the newly created server block.
    ```
    certbot --nginx
    ```
