...
6. Paste the following Python code sample into your `inference.py` file:
    <Message type="tip">
    This script takes an example of a conversational task with a request sent to an [LLM through the Chat Completions API](/managed-inference/reference-content/openai-compatibility/#chat-completions-api-or-responses-api).
    </Message>
    ```py
    import requests
