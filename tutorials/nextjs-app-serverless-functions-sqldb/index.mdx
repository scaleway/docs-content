...
3. Run the following command to clone the template source code from the repository into a new folder and access it:
  ...
  ```
    <Message type="note">
      This repository extends the blog demo application from `npx create-next-app --example blog-starter my-nextjs-blog`, and adds required code to fetch data from a database instead of static markdown files.
    </Message>
...
1. Run the follwing command to initialize the Terraform working directory:
  ```hcl
  terraform init
  ```
...
4. Deploy your application by executing the actions listed in your Terraform plan:
  ...
  ```
    ...
    The output will provide the urls to which you can access your application and the connection string for your database. Since they contain sensitive value, they are hidden by default.
...
6. Copy the container URL from the output and paste it in a browser to access your application. However, no post will display because your database in still empty.
...
- You have installed and configured the [Scaleway CLI](https://www.scaleway.com/en/docs/developer-tools/scaleway-cli/quickstart/)
