...
These variables need to be UUIDs. You can generate an UUID using `uuidgen`:

```shell
uuidgen | tr "[:upper:]" "[:lower:]"
```

Or through Terraform console's `uuid()` function:
...
Build the Nomad instance image using Packer by running the following command:

```shell
packer build -var-file=variables.hcl image.pkr.hcl
```

At the end of the build process, the following output should display:
...
At this step your `variables.hcl` file should look like the following (with your own Consul token ID and secret):
...
Once the deployment process finishes, the following output should display:
...
Execute the following commands to setup the environment variables:
