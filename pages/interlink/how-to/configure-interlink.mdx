...
The VPC and the external infrastructure's routers both announce their routes to each other via [BGP](/interlink/concepts/#border-gateway-protocol-bgp) when route propagation is activated. Still, all of these routes are blocked by default. Routing policies let you allow ranges of route announcements, so that traffic can flow along these routes across the InterLink.
