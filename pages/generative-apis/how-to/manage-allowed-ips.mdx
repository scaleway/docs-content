...
When [authentication by API key](#how-to-enable-or-disable-authentication-by-api-key) is enabled, a valid [Scaleway API key](/iam/concepts/glossary/#api-key) must accompany all requests sent to your deployment's endpoint.

An API key is considered valid to access a deployment when:

- It belongs to the [Owner](/iam/concepts/glossary/#owner) of the Organization which owns the deployment, or
- It belongs to a [Member](/iam/concepts/glossary/#member) or [Application](/iam/concepts/glossary/#application) of the Organization which owns the deployment, and the Member/Application has appropriate [IAM permissions](/iam/policies-permissions/permission-sets/).
...
Permissions are attributed via [policies](/iam/concepts/glossary/#policy), which are then attached to a Member or Application. 
...
<Message type="note">
If you only want to access the deployment yourself, and you are the Owner of the Organization that created the deployment, simply [generate an API key](/iam/credentials/create-api-keys/) for yourself, and it will automatically have full rights to access and manage the deployment.

Read on if you want to manage access to your deployment for others.
</Message>

1. [Invite Members](/iam/principals/manage-members/) (other humans) to your Organization, or [create Applications](/iam/principals/create-application/) (non-human users).
2. Create and attach a [policy](/iam/policies-permissions/create-policy/) to the Member or Application, defining the permissions they should have in your Organization by selecting permission sets (e.g., `InferenceFullAccess`). If desired, define [conditions](/iam/concepts/glossary/#conditions) as part of the policy, to further restrict access based on user agent type, date/time or IP address.
...
You can revoke access to a deployment at any time by [modifying or deleting the policy](/iam/policies-permissions/manage-policies/) attached to the Member or Application in question.
...
1. Log into the [Scaleway console](https://console.scaleway.com) and [generate an API key for yourself](/iam/credentials/create-api-keys/).
...
4. (Optional) Ensure that API key authentication is enabled, and use [policies](/iam/policies-permissions/create-policy/) to define IAM-based rules and conditions for access.
