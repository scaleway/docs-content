...
[Git](https://git-scm.com) is unarguably the most popular and powerful version control system to store your code, and can handle files up to 5 GB thanks to Git [LFS](https://git-lfs.com/), but what about large data sets?

[DVC](https://dvc.org) is a version control tool designed to handle large data sets. It works alongside Git and allows you to store your data in the remote storage of your choice (such as a Scaleway S3-enabled bucket) while storing only the metadata in a Git repository.
...
4. Run the following command to stage new DVC-related files:
    ```bash
    git add
    ```

5. Run the following command to commit the initial configuration:
    ```bash
...
3. Run the following command to copy the example data file to your repository:
    ```bash
    dvc get https://github.com/iterative/dataset-registry \
    get-started/data.xml -o data/data.xml
    ```

4. run the following command to stage files you want to store in your bucket:
    ```bash
    dvc add data/data.xml
    ```

5. Run the following command to track the DVC metadata file and .gitignore file with git:
    ```bash
...
6. Run the following command to commit the metadata file:
    ```bash
    git commit -m "Add raw data"
    ```

7. Run the following command to push your changes to your Git repository:
   ```bash
   git push
   ```
...
7. Run the following command to add your bucket as a remote storage for your data:
   ```bash
   dvc remote add -d  myremote s3://mybucket/path
   ```

8. Run the following command to set your bucket's endpoint:
    ```bash
    dvc remote modify myremote \
    endpointurl https://my-bucket.s3.fr-par.scw.cloud
    ```

9. Run the following command to push your data to your bucket:
    ```bash
    dvc push
    ```

10. Run the following command to list the content of your bucket:
    ```bash
    ...
    <Message type="note">
      The name of the files are generated automatically based on the content of the .dvc created upon tracking the data.
    </Message>
