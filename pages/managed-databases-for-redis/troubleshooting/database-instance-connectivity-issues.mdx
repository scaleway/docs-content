...
if ! [ -x "$(command -v uname)" ]; then
  echo 'Skipped: uname command is not available.'
else
  uname -a
fi
...
if ! [ -x "$(command -v ping)" ]; then
  echo 'Skipped: ping command is not available.'
else
...
if ! [ -x "$(command -v telnet)" ]; then
  ...
  if ! [ -x "$(command -v curl)" ]; then
    echo "Skipped: neither telnet nor curl command are available."
  else
    echo "(using curl)"
...
if ! [ -x "$(command -v ip)" ]; then
  echo 'Skipped: ip command is not available.'
else
  echo -e "Interfaces:\n- \n"
  ip a
  echo -e "\nNeighbour:\n- \n"
