...
excerpt: |
  Scaleway Dedibox allows you to order additional hard disks. You can use these disks for example as additional Datastore in ESXi to store your virtual machines.
tags: compute server esxi data storage harddisk disk additonal
totalTime: PT30M
steps:
...
keywords: physical server, dedicated server, physicalization, blockstorage, cloud storage, additional disk, dedibox, esxi, vmware
dates:
  validation: 2020-01-21
  posted: 2020-01-21
---

## Additional disks with ESXi hypervisor - Overview

On [Scaleway Dedibox](https://www.scaleway.com/en/dedibox/), you can order as many [additional disks](https://www.scaleway.com/en/dedibox/storage/local-disks/) as you need. For instance, they can be used as additional Datastore in [ESXi](https://www.vmware.com/products/esxi-and-esx.html) to store your virtual machines.

A range of different SATA, SAS, and SSD disks is proposed. Depending on the server chassis, the number of available slots may differ.
...
6. Choose the payment method for the installation fees and validate your order:
  ...
  The disk will be installed installed within approximately one hour from the moment you placed the order.

  </Message>

8. Connect to the ESXi web interface at `https://your_scaleway_dedibox_ip/` and log into it:
...
Depending ob the configuration of your Dedibox, it may come with a [hardware RAID](https://en.wikipedia.org/wiki/Disk_array_controller). On this type of server you can not configure the datastore directly from ESXi. Instead, you must create a virtual disk from the KVM-over-IP interface of the server before adding the disk to ESXi by following the next steps.
...
4. If all disks are **OK**, click **Virtual Disks** in the side-menu.

5. The list of all existing virtual disks displays. Click **Create** in the menu bar to create a new virtual disk.
...
5. Choose the partitioning for the disk. To use all use all available space of the disk, go with the default settings and click **Next**:
...
6. The summary displays. If all settings are correct, click **Finish**:

  <Lightbox src="scaleway-add_disk_ready_to_extend.png" width="800" alt="" />

The disk is added to the datastore and the total storage capacity of the datastore is being increased.
