...
Certain older legacy Instance types (e.g. **VC1-x**, **X64-x**, **Start1-x**) offered the default option to use a bootscript (a preconfigured boot configuration) to start your Instance. This boot method is also available on **DEV1-x**, **GP1-x**, and **STARDUST1** Instances. The bootscript option has become deprecated and will no longer be supported.
