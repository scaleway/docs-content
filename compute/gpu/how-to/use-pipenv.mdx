...
Pipenv is pre-installed on all of Scaleway's AI Docker images for GPU Instances. Launching one of these Docker images puts you in an activated pipenv virtual environment, with all your favorite packages and tools pre-installed. You can also use pipenv to create your own virtual environments, if you wish to go beyond the pre-installed Python environments already provided with the various Docker images.
