...
3. Enter a file path to save the key to when prompted. Alternatively, press **Enter** to leave this at the default setting (the key will be saved in a file called `id_ed25519` in the users `~/.ssh/` directory).
  ```
...
5. Enter the passphrase again for confirmation when prompted, and press **Enter**:
  ...
  The key pair is generated in the filepath you specified. The key pair consists of:
    ...
    - The private (identification) key, named `id_ed25519.pub`
