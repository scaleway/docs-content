...
```
<Message type="important">
 - If you have not configured your environment in your local machine, make sure you replace `\<project_id\>` with your [Project ID](console/my-project/concepts.mdx), `$REGION` with `fr-par` and `$TOKEN` with your secret key. If you don't replace the Project ID, the API will reply with a "permission denied" error message.
