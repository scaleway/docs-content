...
<Message type="note">
  Scratch storage does not prevent your data from being lost when stopping the server. Doing a full stop/start cycle erases scratch data. However, rebooting or using the stop in place function will keep the data.
</Message>

## What can I use scratch storage for?

For example, the GPU Render-S requires several minutes (typically ranging from 6 to 12 minutes) to fetch a 400 GB image, a delay that impedes the user's ability to promptly initiate model training. 
