...
description: This page shows how to create a Zulip Server on a Scaleway Elements Virtual Instance.
tags: zulip community unbuntu slack
image: /assets/scaleway_zulip.png
excerpt: |
  Zulip is an open-source communications platform, similar to Mattermost, Slack or Rocket.Chat, but providing unique features by combining the immediacy of real-time chat with an email threading model. In this tutorial you learn how to setup Zulip on a Scaleway Elements Virtual Instance.
totalTime: PT20M
steps:
  - step: Installing Zulip
    type: HowToStep
    url: https://www.scaleway.com/en/docs//deploy-zulip-open-source-communication-platform/#-Installing-Zulip
  - step: Configuring outgoing eMails
    type: HowToStep
    url: https://www.scaleway.com/en/docs/deploy-zulip-open-source-communication-platform/#-Configuring-outgoing-emails
  - step: Configuring Zulip
    type: HowToStep
    url: https://www.scaleway.com/en/docs/deploy-zulip-open-source-communication-platform/#-Configuring-Zulip
categories:
  - compute
brands:
  - elements
permalink: docs/deploy-zulip-open-source-communication-platform/
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, ubuntu server, debian server, linux server, zulip, community, slack, slack alternative
---
...
- You have a FQDN (Fully Qualified Domain Name) pointing to your compute instance's IP address
...
2. Update the `apt` package manager and upgrade the software already installed on the instance to the lastest version available in Ubuntu's repositories:
...
4. Run the automated Zulip installer:
  ...
  The following flags are available when running the installer:
    ...
    - `--hostname=YOUR_HOSTNAME`: A valid domain or subdomain for your Zulip server (e.g. `zulip.yourdomain.com`). Make sure that it points to the IP address of your virtual instance, as it is required for the hostname validation done by certbot.
...
6. Open your web browser and point it to your Zulip activation link. Enter your email address to create new organization. Confirm by clicking **Create Organization**:
  ...
  <Lightbox src="scaleway-zulip_new_orga.png" alt="" />
...
7. Complete the registration by providing your organization's name and additional information, such as a secure password. Once completed, you see Zulip's main screen:
  ...
  <Lightbox src="scaleway-zulip_dashboard.png" alt="" />
...
4. Test if everything works by running the following command (replace `your@email.address` with your valid email address):
  ...
  <Lightbox src="scaleway-sendinblue_config.png" alt="" />
...
Your Zulip instance is now ready for basic use. However there are many additional features you can configure for your organization. To do so, click the menu button and then on **Manage Organization**:
  ...
  <Lightbox src="scaleway-zulip_manage_orga.png" alt="" />
