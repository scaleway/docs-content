...
<Message type="tip">
  We recommend [migrating any existing clusters](#how-can-i-migrate-my-existing-clusters-to-private-networks) to a Private Network via the Scaleway console before Q1/2024, as **public-only endpoints will become deprecated.**
</Message>
...
Even though these nodes have public IP addresses for specific maintenance and operational purposes, your cluster's security remains uncompromised. See below for more information. Optionally you can configure your nodes inside a 100% private network using [full isolation](#what-is-the-difference-between-controlled-isolation-and-full-isolation)
...
You can use the Scaleway console to both create new Private Networks, and add clusters to existing Private Networks. Each cluster is autoconfigured using a /22 IP subnet thanks to Scaleway’s IPAM.

## Can I use a Public Gateway with my Private Network to exit all outgoing traffic from the nodes?

Yes, you can use a Public Gateway to exit outgoing traffic from your Kapsule nodes having private IPs. For more information, how to configure a Public Gateway, refer to our [Public Gateway documentation](/network/public-gateways/how-to/configure-a-public-gateway/).
...
| Visibility  | Controlled isolation (default)                                                                                                                  | Full isolation (option)                                                                       | Public (deprecated)                          |
|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|----------------------------------------------|
| Description | Worker nodes are assigned both private IPs and public IPs. All inbound traffic on public interface is dropped by default using Security Groups. | Worker nodes are set without public IPs (100% private network). A [Public Gateway](/network/public-gateways/how-to/configure-a-public-gateway/) is required. | Clusters without a Private Network attached. |
...
## Can I have clusters with pools using both, controlled isolation and full isolation?

Yes, you can configure clusters with Kapsule pools using controlled isolation and full isolation. Note, that pools using full isolation require a [Public Gateway](/network/public-gateways/how-to/configure-a-public-gateway/) to reach the Kubernetes control plane of the cluster.

<Message type="important">
  Removing or detaching the Public Gateway from a Kapsule pool with full isolation can lead to a single point of failure in the cluster, as nodes will no longer be able to reach their control plane.
</Message>
...
In the future, additional features are planned, such as:
- **Control Plane in isolation** with nodes and API communicating in the same isolated network. The CNI’s network policies will restrict/allow a range of IPs or ports to control who can access to the API server.
- **End of support for legacy public clusters.** All clusters will be required to [migrate to controlled isolation (Private Networks)](#how-can-i-migrate-my-existing-clusters-to-private-networks) by Q1/2024.
...
Since July 2023, the automatic allocation of IP addresses via IPAM is available for Managed Databases. These IP addresses are compatible with Scaleway's VPC, which is now in General Availability and can be used with, not only Managed Databases, but all products in the Scaleway ecosystem.
