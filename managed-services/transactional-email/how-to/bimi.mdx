...
BIMI is a DNS TXT record that you set up to indicate what brand's logo. The record contains the URL to the brand's logo in SVG (Scalable Vector Graphics) format.
...
5. Fill in the required information for the record.
    ...
    | ------| ------- | ----- |
    | Type  | TXT           |      |
    | Name  | `default._bimi.<example.com>`   | At Scaleway your domain name is added automatically, so you need only add `default._bimi`. Other DNS clients might require you to add the entire address.  |
