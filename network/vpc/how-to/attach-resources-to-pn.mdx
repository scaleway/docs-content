...
The instructions below show how to attach a resource to a Private Network using either the Scaleway console, or Terraform.

<Tabs id="attach-to-pn">
  ...
  <TabsTab label="Terraform">
    The code to attach a resource to a Private Network via Terraform will vary from product to product. See the [Scaleway Terraform documentation](https://registry.terraform.io/providers/scaleway/scaleway/latest/docs/resources/) for full information.

    The steps below show you the basic process for creating an Instance and attaching it to a Private Network.
    ...
    3. Add a block to create a Private Network:
        ...
        The Private Network will be created in the [default VPC](/network/vpc/concepts/#default-vpc) for the specified region.
    
    4. Add a block to create an Instance and attach it to the Private Network:

        ```tf
        resource "scaleway_instance_server" "instance01" {
          type = "DEV1-S"
          image = "ubuntu_jammy"
        ...
        A `private_nic` resource will automatically be created for the interface between the Instance and the Private Network.

        For a full list of available arguments when creating an Instance and attaching it to a Private Network, see our Terraform provider documentation, particularly for the [instance_server](https://registry.terraform.io/providers/scaleway/scaleway/latest/docs/resources/instance_server) and |private_nic](https://registry.terraform.io/providers/scaleway/scaleway/latest/docs/resources/instance_private_nic) resources.

    5. Run `terraform plan` to preview the changes that will be made, and then `terraform apply` to execute the necessary actions to create the resources with the Instance attached to the Private Network.
  </TabsTab>
</Tabs>
...
In the Scaleway console, you can view a resource's IPv4 or v6 address in the **Attached Resources** tab of the Private Network itself (follow steps 1 - 5 above)
