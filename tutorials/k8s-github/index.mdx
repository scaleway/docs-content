...
excerpt: |
  ...
  In this tutorial you will learn how to use the gitlab Kubernetes integration using a Scaleway Virtual Instance.
  The instance will be deployed using the Scaleway gitlab InstantApp.
totalTime: PT20M
steps:
  - step: Configuring GitLab Runner using Helm Charts
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/k8s-github/#-Configuring-GitLab-Runner-using-Helm-Charts
  - step: Running a pipepline in the runner using a test app
    type: HowToStep
    url: https://www.scaleway.com/en/docs/tutorials/k8s-github/#-Running-a-pipepline-in-the-runner-using-a-test-app
categories:
  - compute
  - kubernetes
  - instances
image: /assets/images/docs/scaleway.png
keywords: cloud tutorials, cloud documentations, cloud instance, cloud hosting, ubuntu server, debian server, linux server, IAAS, baremetal, scalability, ARM, physical server, dedicated server, kubernetes, kapsule, gitlab, git
dates:
...
In this tutorial you will learn how to use the `gitlab` Kubernetes integration using a [Scaleway Virtual Instance](https://www.scaleway.com/en/virtual-instances/).
The instance will be deployed using the Scaleway `gitlab` InstantApp.
...
5. Use the `helm` command to install the runner (note that you specify in this command line the `values.yaml` file) :
  ...
  <Lightbox src="scaleway-runnerok.png" alt="" />

## Running a pipepline in the runner using a test app

To demonstrate if the runner is working, we [create a repository](https://docs.gitlab.com/ee/user/project/repository/#create-a-repository) with a "hello world" piece of code written in python.
...
5. Push the repository and an ubuntu container is launched. At first `apt` is updated and `python3` is installed, and then the “hello, world” script is launched in the terminal.
  ...
  <Lightbox src="scaleway-job1.png" alt="" />
  <Lightbox src="scaleway-job2.png" alt="" />
