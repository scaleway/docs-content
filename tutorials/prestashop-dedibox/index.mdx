...
1. Update and upgrade the system. Then, install the `apt-transport-https` package, which allows the use of repositories accessed via the HTTP Secure protocol (HTTPS).
    ...
    ```
2. Download the pre-required packages and repositories to install PHP. Then, update the APT package cache.
    ```
    add-apt-repository ppa:ondrej/php
    apt-get update
    ```
3. Install the `nginx` web server, PHP and `unzip` using `apt`. Install the `php8.1-curl` package separately.
    ```
    apt install nginx php8.1-fpm php8.1-xml php8.1-zip php8.1-gd php8.1-mysql php8.1-intl unzip wget
