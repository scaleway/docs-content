...
2. Create a file and instanciate a Scaleway connection. Replace `$SCW_PROJECT_ID` and `$SCW_SECRET_KEY` with your Scaleway Project ID and secret key (or set them as environment variables).
    ...
    # Retrieve all QPU devices (emulated or real) and select the one you desire (see the [Pasqal processors information page](/quantum-computing/additional-content/pasqal-qpus) for more information)
