...
2. Click **Kubernetes** in the menu on the left:
...
4. The cluster creation page displays. Enter the following information:
    ...
    - **Choose your cluster type**: Choose between Scaleway Kubernetes Kapsule or Kubernetes Kosmos.
    - **Choose a region**: A region refers to the geographical location in which your cluster will be created.
    ...
    - **Enter a name for the cluster**: Set a name for the cluster, for example `Kubeflow`.
    ...
    - **Select a node type**: We will use `GP1-M` Instances for this tutorial (16 cores, 64 GB RAM)
    - **Configure node options**: Choose the number of nodes for your default pool
