...
<Message type="tip">
  Refer to the [How to create a policy](/iam/policies-permissions/create-policy/) and [How to manage policies](/iam/policies-permissions/create-policy/) documentation pages to learn where and how to specify a condition.
</Message>
...
- **[Request-level conditions](/iam/policies-permissions/understanding-request-level-conditions/)** that control access based on the IP address, user agent, or time of the request.
- **[Resource-level conditions](/iam/policies-permissions/understanding-resource-level-conditions/)** that control access based on attributes of the resource being accessed, such as its name, ID, or locality. These are only available for a limited set of products — see [Products supporting resource-level conditions](/iam/policies-permissions/supported-products-resource-level/) for details.
...
- Request-level variables (`request.ip`, `request.time`, `request.user_agent`) are documented in [Request-level conditions](/iam/policies-permissions/understanding-request-level-conditions/).
- Resource-level variables (`resource.name`, `resource.id`, `resource.locality`) are documented in [Resource-level conditions](/iam/policies-permissions/understanding-resource-level-conditions/).
