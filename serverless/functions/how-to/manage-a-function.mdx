---
meta:
  title: How to manage a function
  description: This page explains how to manage a function
content:
  h1: How to manage a function
  paragraph: This page explains how to manage a function
tags: functions 
dates:
  validation: 2023-01-02
  posted: 2021-05-26
categories:
  - serverless
---

This page shows you how to manage a function, should you need to edit the function or verify/change its deployment parameters after creation.

<Macro id="iam-requirements" />

<Message type="requirement">
  - You have an account and are logged into the [Scaleway console](https://console.scaleway.com)
  - You have [created a function](/serverless/functions/how-to/create-a-function/)
</Message>


1. Click **Functions** in the **Serverless** section of the side menu. The functions page displays.
2. Click the relevant functions namespace.
3. Click the name of the function you want to manage.
4. Check or edit any of the information as required, in the **Enter function code** section:
    - **Handler**: The functions handler.
      <Message type="note">
        The module-name.export value in your function. For example, in node: "index.handler" calls exports.handler in index.js. In Golang, indicate the path to your main package (example: mypackage calls mypackage/main.go)
      </Message>
    - **Function runtime**: The runtime and version to use for your function.
    - **Function code**: The code of your function. You can either enter it directly using the inline code editor or import it from a ZIP file.
5. Click **Deploy function** to save any changes you have made.

<Navigation>
  <PreviousButton to="/serverless/functions/how-to/package-function-dependencies-in-zip/">How to package a function in a zip file</PreviousButton>
  <NextButton to="/serverless/functions/how-to/add-trigger-to-a-function/">How to add a trigger to a function</NextButton>
</Navigation>
