...
You can create one or more Edge Services pipelines for each of your Load Balancers. If creating multiple pipelines for the same origin Load Balancer, each one must have a different [origin host](#origin-host). TODO DOUBLE CHECK THIS.
...
The protocol (HTTP or HTTPS) that the Edge Services pipeline should use when sending requests to the origin. HTTPS is recommended. TODO ADD INFORMATION ON HOW TO CHOOSE THE CORRECT PROTOCOL
...
You can create one or more Edge Services pipelines for each of your Load Balancers. Each pipeline has its own [origin](#origin), [endpoint](#endpoint) and [cache](#cache). If you create multiple Edge Services pipelines for the same origin Load Balancer, each one must have a different [origin host](#origin-host).
...
4. Configure the [origin](#origin) for this pipeline:
        ...
        <Message type="tip">
        For Kubernetes Load Balancers, see our [dedicated documentation](TODO) for help selecting the correct Load Balancer and frontend.
        </Message>

5. Define the protocol and origin host for this pipeline:
        - Select the protocol that Edge Services should use when making requests to the origin, either `HTTP` or `HTTPS` (recommended).
...
- **Protocol**: Ensure you did not select the wrong protocol, e.g. HTTP selected whlie the frontend chosen is configured to receive HTTPS, or the opposite.
...
2. 
Port number so he can retrieve the related Frontend/ssl info
...
5. Enter the path of each object you want to purge. You can purge a maximum of 5 objects at a time.
      ...
      You **cannot** purge entire directories by simply specifying the path to the directory, e.g. `/videos`. Purging objects must done strictly object-by-object, so `/videos/my-video1.mp4`, `/videos/my-video2.mp4`, `/videos/my-video3.mp4` etc. Watch this space for updates to this feature in the future. TODO CHECK THIS IS RIGHT FOR LOAD BALANCER
      </Message>
...
TODO WILL THIS BE THE SAME? NEED TO REPLACE BUCKET

The Grafana dashboard presents a number of different metrics. Use the `Bucket name` TODO drop-down in the top left to select which pipeline to view Edge Services metrics for, and the time range drop-down in the top right to modify the time period to apply to the metrics.
