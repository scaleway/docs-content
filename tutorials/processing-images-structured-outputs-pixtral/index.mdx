...
- Access to a Scaleway [Generative APIs - Dedicated Deployment](/generative-apis/reference-content/supported-models/#pixtral-12b-2409) endpoint with Pixtral deployed or to Scaleway [Generative APIs](/generative-apis/quickstart/) service
...
Make sure to set the `SCALEWAY_API_KEY` and `SCALEWAY_INFERENCE_ENDPOINT_URL` environment variables with your actual API key from Scaleway IAM, and the appropriate endpoint URL for Scaleway Generative APIs - Dedicated Deployment or Generative APIs - Serverless service.
