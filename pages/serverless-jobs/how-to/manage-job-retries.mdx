...
The retry option lets you define how many times a Serverless Job automatically retries on failure. A retry triggers when the Job Run returns an [Exit Code](/serverless-jobs/concepts/#exit-code) different than `0`. Once the job reaches the maximum number of failed retries, it is permanently marked as `failed`.
