...
<Message type="requirement">
  ...
  - You have generated your [Scaleway API keys](/identity-and-access-management/iam/how-to/create-api-keys/)
  - **(Recommended)** You have installed the [Serverless Framework](https://serverless.com/) with [Scaleway's Serverless Plugin](https://github.com/scaleway/serverless-scaleway-functions)
</Message>

## Project set up (for Serverless Framework)

1. Using the `serverless` command import the project from our Github example repository:
  ```bash
  serverless create --path nodetransform --template-url https://github.com/scaleway/serverless-examples/tree/main/functions/image-transform-node
  ```
2. Install the npm module using
  ```npm install```
3. Fill the `serverless.yml` file with your Scaleway credentials
4. Set the following environment variables at the namespace level (provider section in `serverless.yml`):
...
1. Create the `BucketScan.js` file:
    * Import dependencies:
    ```js
    ...
    * Connect to Object Storage:
    ...
    ```
    * Initiate your function, the `handle` function will be the one called when your function is invoked
    ```js
...
3. Create the `ImageTransformation.js` file:
    * Import dependencies
    ```js
    const AWS = require("aws-sdk");
...
Read our documentation for more information on [ways to deploy functions](/serverless/functions/reference-content/deploy-function) to deploy this project. 

For the full project and deployment using Serverless Framework refer to our [serverless-examples project](https://github.com/scaleway/serverless-examples/tree/main/functions/image-transform-node)
