...
content:
  h1: Getting started with Kubernetes Part 2 - Load Balancers
...
<Message type="requirement">
  ...
  - You have followed the [first](/tutorials/get-started-containers-docker/) and [second](/tutorials/get-started-deploy-kapsul/) in this series, and have a Kubernetes Kapsule cluster running with the `whoami` application deployed on it.
...
By default, Kubernetes clusters are not exposed to the Internet. These means that external users cannot access the application deployed in our cluster.

In the previous tutorial, we solved this problem by creating a **NodePort** Service for our cluster. NodePort is a Kubernetes service which opens a port on each node in a cluster that contains pods for the specified deployment. It then forwards any external traffic received on that port to the right pods. This allowed us to go to our node’s external IP address in a browser, specifying the open port, and be served  our web application.
...
## What are the steps to create a Load Balancer?
...
1. Open a terminal, and run the following command to check the state of your cluster:
  ...
  You should see two pods, the NodePport Service created during the previous tutorial (along with the basic default ClusterIP Service), the deployment and the two replicasets.
