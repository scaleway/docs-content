...
## Accessing the cluster
    ...
    <Tabs id="connect-cluster">
      <TabsTab label="Using the Scaleway console">
         ...
         4. In the **Getting Started** section of the page click **2 Download kubeconfig**, then click **Generate kubeconfig**.
         5. Enter your existing Scaleway API key or click **Generate API key** to generate a new one.
         6. (Optional) Enter API key specifics by select the API key bearer, enter a description and expiration date for the key. Then click **Generate API key**.
