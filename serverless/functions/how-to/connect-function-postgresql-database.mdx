...
<Message type="tip">
  If you do not know how to configure your package, refer to our documentation about [packaging functions in zip files](/serverless/functions/how-to/package-function-dependencies-in-zip/).
</Message>
...
3. Copy the following code into the file to connect to the database. You can pass the required information using `environment variables`:
    ...
    * Finally, add the `handle` function to the file:
    ...
    ```
4. [Create the function](/serverless/functions/how-to/create-a-function/) in the Scaleway console, then [upload your zipped package](/serverless/functions/how-to/package-function-dependencies-in-zip/#how-to-upload-your-zip-file-to-the-scaleway-console) and add the environment variables in the **Advanced** section of the function.
    <Message type="tip">
...
<Message type="note">
  The Python libraries to connect to PostgreSQL require the postgresql Linux library. [You will need to use Docker to generate the dependencies.](/serverless/functions/how-to/package-function-dependencies-in-zip/#specific-libraries-(with-needs-for-specific-c-compiled-code))
</Message>
...
2. Create a `handler.py` file at the root of your folder using `touch`, then open it in a text editor, for example `nano`:
    ...
    * Create a cursor in the handle function to submit queries and fetch the result:
    ...
    ```
3. [Create the function](/serverless/functions/how-to/create-a-function/) on the Scaleway console, then [upload your zipped package](/serverless/functions/how-to/package-function-dependencies-in-zip/#how-to-upload-your-zip-file-to-the-scaleway-console) and add the environment variable in the **advanced settings** of the function.
    <Message type="tip">
      Alternatively use the [Serverless Framework](/serverless/functions/api-cli/fun-uploading-with-serverless-framework/) to create the function and upload your code automatically.
    </Message>
