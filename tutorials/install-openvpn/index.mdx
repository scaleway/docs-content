...
2. Create the client configuration file:
   ...
   ```
   Add the following configuration, replacing `your_server_ip_or_domain` with your server's IP address or domain name:
   ```
   client
   dev tun
   proto udp
...
3. Transfer the client configuration file to the client device:
...
4. Install OpenVPN on the client device:
...
```sh
echo "0 0 1 */2 * certbot renew --quiet" | sudo tee -a /etc/crontab
```
