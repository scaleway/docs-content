...
Each Load Balancer is configured with one or several backends. Each backend consists of one or several servers, which receive forwarded requests from the frontend. You can [add and manage backends via the console](/network/load-balancer/how-to/manage-frontends-and-backends/).
...
A protocol a standard format for communication over a network. When you configure your Load Balancer's backend, you choose a protocol (HTTP, HTTPs or TCP) which it uses to send and receive data.
