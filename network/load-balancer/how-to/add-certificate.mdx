...
<Message type="note">
Let's Encrypt certificates are not compatible with Load Balancers which have a frontend listening on port 80 attached to a TCP backend. Frontends listening on port 80 attached to an HTTP backend, or frontends listening on port other than 80 and attached to a TCP backend, are both fine.
</Message>
