...
The changes outlined in this document only concern Serverless Containers. The Serverless Functions and Serverless Jobs APIs remain the same.

<Message type="note">Be mindful of the `Breaking changes` mentions.</Message>
...
New field `timeout` allows to customize the duration before the check times out.
