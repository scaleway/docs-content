...
Scaleway Load Balancers can use the **Host** header field to define [route rules](#route-rules) from a frontend to a specified HTTP backend. HTTP headers can also be used in [ACLs](#acl) to **allow**, **deny** or **redirect** traffic based on the specified header field and value.
...
Scaleway Load Balancers can use SNI to define [route rules](#route-rules) from a frontend to a specified TCP backend.
