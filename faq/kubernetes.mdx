...
Please refer to the [API documentation](https://developers.scaleway.com/en/products/k8s/api/#post-498231) for more information.
...
`kapsule-agent` is a software developed internally at Scaleway. It is used to assist the managing of nodes lifecycles. Its primary function is to set up the network during the initial boot, and toggle the SSH server on or off based on a k8s label of each node.

## How do I connect to my nodes using SSH?

By default, an SSH server is installed and configured on the nodes of your Kubernetes Kapsule cluster. To ensure the correct behavior of your cluster, it is not recommended to perform actions directly on the nodes. All actions and configurations should be done using Kubernetes (from the Scaleway Console, `kubectl` command, or Scaleway Kapsule `APIs`).
To improve the security of your Kapsule cluster, you can disable this SSH server on your nodes by following the documentation [how to enable or disable SSH on Kubernetes Kapsule nodes](/containers/kubernetes/how-to/enable-disable-ssh/).