...
<Lightbox src="scaleway-routing.webp" alt="A diagram shows two Private Networks inside a VPC. An Elastic Metal server on Private Network A is able to send a packet to an Instance on Private Network B, thanks to the automatic route created and held in the VPC's routing table. An Instance on Private Network A is able to send a packet to the public internet via the automatic route created and held in the VPC's routing table that points to a Public Gateway also on Private Network A." />
...
- When creating a new VPC via Terraform, check that routing will be activated by default
- Can we provide a TF template for the three-tier architecture above?

## Limitations

TODO CHECK IF THESE STILL APPLY FOR GA:
