...
### Integrating the database to your application
  ...
  ```
  - Secondly, edit the file located at `src/app/page.tsx` by replacing its content with the following code. This will update the main application home page which displays the posts list.
  ```typescript
  ...
  ```
  - Finally, edit the file located at `src/app/posts/[slug]/page.tsx` by replacing its content with the following code. This will update pages that display a single post.
  ```typescript
...
6. Go to [http://localhost:3000](http://localhost:3000) in your browser. The blog template should display with the updated data from your database (images and titles of the first blog post have changed!)
  ...
  **Congrats!** You could already deploy your application by:
  - Building a new container version with `docker build -t my-nextjs-blog .`, as long as you also provide the right environment variables as build arguments
  - Pushing the new container to the **Container Registry** with `docker tag my-nextjs-blog:latest $REGISTRY_ENDPOINT/my-nextjs-blog:latest`, and `docker push $REGISTRY_ENDPOINT/my-nextjs-blog:latest`
  - Deploying your container to **Serverless Containers** with `scw container container deploy`, as long as you provide the right environment variables to your container (as in step 2.). 
  However, your application would then connect with your [user credentials](https://www.scaleway.com/en/docs/identity-and-access-management/iam/concepts/#user), which is not a recommended security practice. To secure your deployment, you will now add a dedicated [IAM application](https://www.scaleway.com/en/docs/identity-and-access-management/iam/concepts/#application), give it the minimum required permissions, and provide its credentials to your application.
...
4. Build your application container with the updated code:
  ...
  ```
  Go to http://localhost:8080 in your browser. The blog template displays.
5. Push your containerized application to the Container Registry:
  ```bash
  docker tag my-nextjs-blog:latest $REGISTRY_ENDPOINT/my-nextjs-blog:latest
  docker push $REGISTRY_ENDPOINT/my-nextjs-blog:latest
  ```
6. Retrieve your container ID:
  ```bash
  export CONTAINER_ID=$(scw container container list name="my-nextjs-blog" -o json | jq -r '.[0].id')
  ```
6. Redeploy your Serverless Container:
  ```bash
  scw container container deploy $CONTAINER_ID
  ```
7. Refresh your browser page displaying the blog. An updated version  displays. 
...
5. Confirm you can run the application locally with the following commands:
  ...
  npm run dev
  ```
6. Go to [http://localhost:3000](http://localhost:3000) in a browser. The blog template displays.
...
2. Run the following command to check that your container runs locally:
  ...
  docker run -it -p 8080:8080 my-nextjs-blog
  ```
3. Go to http://localhost:8080 in your browser. The message below displays because your application is not connected to the database yet.
  ```
...
6. Run the following command to stop all your resources:
  ```hcl
  terraform destroy
  ```
