...
An endpoint is the point of entry in a communication channel when two systems are interacting. The endpoint is the means from which the API can access the resources they need from a server to perform their task. An endpoint can include a URL of a server or service. The Observability Cockpit provides 6 endpoints:
...
- Three Tempo-compatible endpoints responsible for dealing with traces
...
Tempo is [Grafana's open source](https://grafana.com/docs/tempo/latest/) tracing tool that allows you to search for traces, generate metrics from them, and link your tracing data with logs and metrics. You can use Tempo with open-source tracing protocols such as Jaeger, Zipkin, and OpenTelemetry.
...
Scaleway supports the following agents for pushing traces:
