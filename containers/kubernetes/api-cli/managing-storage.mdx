...
<Message type="note">
  ...
  `PersistentVolumes` created without the `csi.storage.k8s.io/node-stage-secret` annotations need to be patched manually if expansion is needed. Make sure you are extra careful when doing so, as the needed fields are immutable and the patch will need to be forced. We recommend taking preventative measures such as backing up your data and switching the `reclaimPolicy` of the volume to `Retain`.
</Message>
