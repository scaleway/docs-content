...
Prior to [TODO DATE], Scaleway GPU Instances were pre-installed with [Anaconda](https://www.anaconda.com) for managing python dependencies and virtual environments with Python. From [TO DO DATE], instead of a package or environment manager being installed directly on your GPU Instance OS:
- You will need to launch one of our Docker containers to access a pre-installed environment. See our documentation on [how to use your GPU Instance with Docker](for more help with this).
- Pipenv will replace Anaconda on our Docker images as the pre-installed package manager.
...
3. TODO: fill this part in once it's decided whether the user with be automatically cd-ed into the ai folder with the environment pre-activated, or whether they will need to do this manually.

  You are now in an activated `pipenv` virtual environment, with a number of tools and packages pre-installed, depending on the [Docker image you chose].
...
2. On your local computer, open a browser window and enter the following URL. Replace `<ip-address>` with the IP address of your Scaleway GPU Instance. The IP address can be found in the Scaleway console, on the Instance's Overview tab.
...
5. Activate the virtual environment:
  ...
  Carrying out a `pipenv graph` now will show that there are currently no packages or dependencies installed. Follow the instructions for [managing and installing packages](TODO LINK) and [launching applications in your local browser](TODO LINK) to start using your new environment.

  You can repeat these instructions to create as many virtual environments as you want for your different projects.

## Exiting the virtual environment and the container

When you are in an activated virtual environment, your command line prompt will normally be prefixed by the name of the environment. Here, for example, `(ai-NTxBFBeG)` we see that we are in the activated `ai` environment, and `/ai#` shows that we are in the `\ai` directory of our container:
