...
Cryptographic operations are executed in general-purpose processors, which can be slower and less optimized than dedicated hardware accelerators.

If your use cases requires that your organization comply with security regulations and standards (e.g., PCI‑DSS, FIPS, HIPAA, ISO 27001), proving to auditors that keys have never been compromised might be difficult.
...
HSM-based hardware isolation is often required for regulatory and certification purposes (e.g., PCI‑DSS, FIPS, HIPAA, ISO 27001) in industries such as Financial Services and Health, so if compliance is a requirement, choose HSM-backed keys.
...
- Symmetric and asymmetric key storage is billed €0.06 per key version monthly.
- Symmetric and asymmetric key operations are billed €0.03 per 10,000 requests.
